...
MODULE TIMEROUTINES
  ...
  !Copyright 2011-2013,2015-2017,2019,2021 SMHI
  !
...
!  !>
!  !>\b Consequences Module modvar variables current_time is set. 
!  !---------------------------------------------------------------------
!  SUBROUTINE calculate_time_for_model(idt,date)
!
!    USE MODVAR, ONLY : dayno,dayofweek,month,prevdoy,currentdate,tsofday, &    !OUT
!                       timesteps_per_day
!    USE WORLDVAR, ONLY : idtlag, &   !number of timesteps between 00:00 and first timestep
!                         dtmonday    !number of timesteps between first timestep and first Monday
!
!    !Argument declarations
...
!    INTEGER dos   !day of simulation, order (for time step shorter than one day)
!    INTEGER shift !days to add to dos for 0 to be a Monday
!
!    month = date%month
!    dayno = doy(date)
!
!    dos = (idt-1+idtlag)/timesteps_per_day   !heltalsdivision --> (part of) first day 0, second 1, ...
!    shift = (7*timesteps_per_day-dtmonday)/timesteps_per_day  !days to add to current day (dos) for day 0 to be a Monday, add 7 to avoid negative weekdays
!    dayofweek = MOD(dos+shift,7)   !Remainder of division by 7 of dos shifted to Monday --> Monday=0
!    dayofweek = dayofweek + 1      !First day of week does not start at 0.
!
...
!
  ...
  SUBROUTINE calculate_time_for_model(idt,date)
    ...
    USE MODVAR, ONLY : current_time, & !OUT
                       timesteps_per_day
    USE WORLDVAR, ONLY : idtlag, &   !number of timesteps between 00:00 and first timestep
                         dtmonday    !number of timesteps between first timestep and first Monday
    ...
    current_time%dayno = doy(date)

    dos = (idt-1+idtlag)/timesteps_per_day   !heltalsdivision --> (part of) first day 0, second 1, ...
    shift = (7*timesteps_per_day-dtmonday)/timesteps_per_day  !days to add to current day (dos) for day 0 to be a Monday, add 7 to avoid negative weekdays
    dayofweek = MOD(dos+shift,7)   !Remainder of division by 7 of dos shifted to Monday --> Monday=0
    current_time%dayofweek = dayofweek + 1      !First day of week does not start at 0.

    yearago = datetype(date%year-1,date%month,date%day,date%hour,date%minute)
    current_time%prevdoy = numdays_of_year(yearago)

    tsofday = MOD(idt+idtlag,timesteps_per_day)
    IF(tsofday==0)THEN
      tsofday = timesteps_per_day 
    ENDIF
    current_time%tsofday = tsofday
  ...
  !>Set Monday time step variable. 
  !>The first Monday of the simulation will be idt=1+tstep (tstep>0).
  !>For negative tstep; the first Monday of the simulation will be idt=1+tstep+timesteps_per_week.
  !>
  !>\b Consequences Module worldvar variable dtmonday is set in calling routine
  !-------------------------------------------------------------------
    SUBROUTINE calculate_timesteps_to_monday(fdate,tstep)

    USE MODVAR, ONLY : timesteps_per_day

    !Argument declarations
    TYPE(DateType), INTENT(IN) :: fdate         !<first date of simulation
    INTEGER, INTENT(OUT)  :: tstep              !<time steps to first monday
    ...
    !Initialisations
    tstep = 0
    amonday = DateType(2000,5,1,0,0)  !1 May 2000 is a Monday
    
    period = period_length(fdate,amonday) !timesteps
    tstep = MOD(period,7*timesteps_per_day) !timesteps to Monday at 00:00

    END SUBROUTINE calculate_timesteps_to_monday

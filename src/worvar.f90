...
!> \{
  ...
  INTEGER, PARAMETER :: maxcharpath = 400    !<Number of characters in path variables
...
!> \{
  ...
  INTEGER, PARAMETER :: fileunit_qobs  = 104   !<Fileunit for discharge observation data
  INTEGER :: fileunit_wobs   !<Fileunit for water level observation data
...
!> \{
  ...
  INTEGER,PARAMETER :: maxsubass = 18 !<Number of subbasin assessment performance criteria (+ number of data points)
...
!> \brief Type for holding information about forcing data
  TYPE FORCINGDATATYPE
    ...
    INTEGER          :: nskip           !<number of comment rows in the beginning of file
...
!> \brief Types for holding information about simulation settings; state files and submodel  
!> \{
!> \brief Types for settings related to simulating a submodel
  TYPE BASEMODELTYPE
    INTEGER :: nsub        !<Number of subbasin in base model (earlier nsub_basemodel)
    INTEGER,ALLOCATABLE :: iindex(:)    !<Base model subbasin index of submodel subbasins
...
!> \brief Type for settings related to writing of state files
  TYPE OUTSTATETYPE
    ...
    LOGICAL :: compress           !<Status for saving compressed state files
...
!> \brief Type for settings related to reading/using initial state file
  TYPE INSTATETYPE
    LOGICAL :: fromfile = .FALSE.  !<Status to read instate from file
    LOGICAL :: compress = .FALSE.  !<Status for compressed state file
  END TYPE INSTATETYPE
!> \}
...
!> \brief Type for variables for accumulated load to be used for source apportionment
  TYPE SOURCEAPPTYPE
    INTEGER :: dtskip = 0                  !<Number of timesteps skipped in print load calculation/output
    TYPE(DateType) :: begindate = DateType(2000,1,1,0,0) !<Period begin, usually 1 January
    TYPE(DateType) :: enddate = DateType(2000,12,31,0,0) !<Period end, usually 31 December
    REAL, ALLOCATABLE :: slcclass(:,:,:,:) !<Accumulated data for yearly loads (class dependent), prev. accdata_classload
    REAL, ALLOCATABLE :: basin(:,:,:)      !<Accumulated data for yearly loads (not class dependent), prev. accdata_basinload
  END TYPE SOURCEAPPTYPE
...
!> \{
  ...
  TYPE(DateType) :: readadddate(10)  !<Dates for new AtmdepData file, max 10 files
...
!  LOGICAL :: atmdepvegreset      !<Flag for atmospheric deposition vegetation type is missing in GeoClass and set to 1
  ...
  TYPE(BASEMODELTYPE) :: basemodel !<Information on base model used for simulating a submodel
  TYPE(INSTATETYPE) :: instate   !<Information on state file to be read
  TYPE(OUTSTATETYPE) :: outstate !<Information on state files to be written
  TYPE(OUTSTATETYPE) :: outstateensemble !<Information on data assimilation ensemble state files to be written
!> \}  
...
!> \{
  ...
  INTEGER,ALLOCATABLE :: winxobsindex(:)    !<Index to find wstr in xobs-array
  REAL,ALLOCATABLE :: qobs(:,:)             !<Matrix with all observed runoff data (time,stn)
  REAL,ALLOCATABLE :: wobs(:,:)             !<Matrix with all observed lake water level data (alternative to xobs)
  ...
  INTEGER :: numwobsstn                     !<Number of wobs station
  INTEGER :: ncomrowsofqobs                 !<Number of Qobs.txt comment rows
  INTEGER :: ncomrowsofwobs                 !<Number of Wobs.txt comment rows
  INTEGER :: ncomrowsofxobs                 !<Number of Xobs.txt comment rows
  INTEGER :: ncomrowsofxoregobs             !<Number of Xoregobs.txt comment rows
  ...
  TYPE(DateType) :: bwdate                  !<Begin date for W observations within simulation period
  TYPE(DateType) :: ewdate                  !<End date for W observations within simulation period
  ...
  LOGICAL :: readwobs  = .FALSE.            !<Flag for reading wobs daily
  LOGICAL :: conductwobs = .FALSE.          !<Flag for reading wobs from Wobs
  LOGICAL :: conductwinx = .FALSE.          !<Flag for reading wobs from xobs
...
!> \{
  ...
  TYPE(SOURCEAPPTYPE) :: accload                  !<Variable for accumulation of load for later source apportionment
...
CONTAINS
  ...
  SUBROUTINE allocate_accumulation(n,nj,ns,nc,nb)
    ...
    !Allocate space for accumulation of substance load output
    IF(writeload)THEN
      IF(ns>0)THEN
        IF(.NOT.ALLOCATED(accload%slcclass)) ALLOCATE(accload%slcclass(nj,nc,ns,n))
        IF(.NOT.ALLOCATED(accload%basin)) ALLOCATE(accload%basin(ns,nb,n))
      ENDIF
    ENDIF
  ...
  !Argument declarations
  TYPE(OUTSTATETYPE), INTENT(INOUT) :: localoutstate   !<Variable holding infirmation on output of state

    !>Default is no writing of states
    localoutstate%doall     = .FALSE.
    localoutstate%doperiod  = .FALSE.
    localoutstate%compress  = .FALSE.
    localoutstate%numdates  = 0
  ...
  INTEGER i
    ...
    IF(localoutstate%doperiod)THEN
      IF(.NOT.ALLOCATED(localoutstate%predate)) ALLOCATE(localoutstate%predate(2))
      localoutstate%predate(1)=SubtractDates(dates(1),steplen)
      localoutstate%predate(2)=SubtractDates(dates(2),steplen)
    ELSEIF(localoutstate%numdates>0)THEN
      IF(.NOT.ALLOCATED(localoutstate%predate)) ALLOCATE(localoutstate%predate(localoutstate%numdates))
      DO i = 1,localoutstate%numdates     
        localoutstate%predate(i)=SubtractDates(dates(i),steplen)
      ENDDO
    ENDIF
  
  END SUBROUTINE set_outstate_dates

  !>Check if state file should be written. Works for both ordinary state_save
  !> and data assimilation ensemble state bin-files.
  !-----------------------------------------
  LOGICAL FUNCTION status_write_outstate(d,localoutstate)
  
    TYPE(DateType), INTENT(IN) :: d   !<current date
    TYPE(OUTSTATETYPE), INTENT(IN) :: localoutstate   !<Variable holding information on output of state
    ...
    IF(localoutstate%doall)THEN
      status_write_outstate = .TRUE.
      RETURN
    ENDIF
    !>or if states should be written for a period
    IF(localoutstate%doperiod)THEN
      IF(d.GE.localoutstate%predate(1) .AND. d.LE.localoutstate%predate(2))THEN
        status_write_outstate = .TRUE.
        RETURN
      ENDIF
    ENDIF
    ...
    DO i = 1,localoutstate%numdates
      IF(d.EQ.localoutstate%predate(i))THEN
        status_write_outstate = .TRUE.
        RETURN
      ENDIF
    ENDDO
  ...
  !--------------------------------------------------------
  FUNCTION get_current_wobs(i,n)

    INTEGER, INTENT(IN) :: i    !<current time step
    INTEGER, INTENT(IN) :: n    !<number of wobs columns
    REAL :: get_current_wobs(n) !< \retval get_current_wobs current water level
    
    !Local variables
    REAL x(n)

    x = missing_value
    IF(ALLOCATED(wobs))THEN
      x = wobs(i,:)
    ENDIF
    get_current_wobs = x

  END FUNCTION get_current_wobs
  ...
  !>\brief Add date to string with a underscore in between
  !---------------------------------------------------------------------
  FUNCTION add_date_to_filename(date,fnlen,filename) RESULT(newfilename)

    !Argument declarations
    TYPE(DateType), INTENT(IN) :: date !<date to be added to string
    INTEGER,INTENT(IN) :: fnlen         !<length of string
    CHARACTER(LEN=fnlen),INTENT(IN) :: filename  !< the filename before and after addition of bdate
    CHARACTER(LEN=fnlen) :: newfilename  !< the filename before and after addition of bdate

    !Local variables
    INTEGER cflen
    CHARACTER(LEN=4) fileending
    CHARACTER(LEN=16) datestr 
    CHARACTER(LEN=fnlen) :: cutfilename
    
    !>Set the file name 
    IF(seconds_per_timestep==seconds_per_day)THEN
      CALL format_date(date,'yyyymmdd',datestr)
    ELSE
      CALL format_date(date,'yyyymmddHHMM',datestr)
    ENDIF
    cflen=LEN(TRIM(filename))
    fileending = filename(cflen-3:cflen)
    cutfilename = filename(1:cflen-4)
    newfilename = TRIM(cutfilename)//'_'//TRIM(ADJUSTL(datestr))//TRIM(fileending)                

  END FUNCTION add_date_to_filename
  ...
  SUBROUTINE set_directory(idir,dir,ddir)
    ...
    !Argument declarations
    CHARACTER(LEN=maxcharpath),INTENT(IN)          :: idir !<Info file directory
    CHARACTER(LEN=maxcharpath),INTENT(INOUT)       :: dir  !<File directory to be checked and set
    CHARACTER(LEN=maxcharpath),OPTIONAL,INTENT(IN) :: ddir !<Default file directory (other than idir)
  
    !Local variable
    CHARACTER(LEN=maxcharpath) :: temp_dir, defaultdir
    
    !>Initialise default directory
    defaultdir = idir
    IF(PRESENT(ddir)) defaultdir = ddir
    
    !>Check for missing or relative path
    IF(dir=='')THEN   !missing path
      dir = defaultdir
    ELSEIF(dir(1:1)=='.')THEN  !relative path
      IF(dir(2:2)=='.')THEN
        temp_dir = TRIM(idir)//dir(1:LEN(dir))
        dir = ''
        dir = temp_dir
      ELSE
        temp_dir = TRIM(idir)//dir(3:LEN(dir))
        dir = ''
        dir = temp_dir
      ENDIF
    ENDIF
...
SUBROUTINE count_optim_par(dim1,dim2)
  ...
  !>\brief Get the step length of output, aka the accumulation period for output.
  !---------------------------------------------------------------------
  FUNCTION get_steplength_string(iperiod) RESULT(length)

    !Argument declarations
    INTEGER,INTENT(IN) :: iperiod    !<time step of file to be written
    CHARACTER(LEN=20)  :: length  !< \retval the step length
    ...
    CASE(i_t)
      !Check steplen
      IF(steplen==DateType(0,0,1,0,0))THEN
        length='day'
      ELSEIF(steplen==DateType(0,0,0,12,0))THEN
        length='twelve-hour'
      ELSEIF(steplen==DateType(0,0,0,8,0))THEN
        length='eight-hour'
      ELSEIF(steplen==DateType(0,0,0,6,0))THEN
        length='six-hour'
      ELSEIF(steplen==DateType(0,0,0,4,0))THEN
        length='four-hour'
      ELSEIF(steplen==DateType(0,0,0,3,0))THEN
        length='three-hour'
      ELSEIF(steplen==DateType(0,0,0,2,0))THEN
        length='two-hour'
      ELSEIF(steplen==DateType(0,0,0,1,0))THEN
        length='hour'
      ENDIF          
    CASE(i_d)
      length='day'
  ...
  END FUNCTION get_steplength_string

  !>\brief Set variables for accumulation of loads to zero to (re)start accumulation
  !---------------------------------------------------------------------
  SUBROUTINE set_accload_to_zero()
  
    accload%slcclass = 0.E0
    accload%basin = 0.E0
        
  END SUBROUTINE set_accload_to_zero

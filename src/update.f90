      ...
      IF(useinupdate(i_war)%station(i)) THEN
        ...
        IF(ALLOCATED(wobsi))THEN
          IF(wobsindex(i)>0)THEN
            wstobs = wobsi(wobsindex(i))  !Get current wst observation, m in wref-system
          ELSE
          ...
          IF(wstobs/=missing_value)THEN
            ...
            IF(wstobs<=w0ref-thresholddepth(2,i)) wstobs = missing_value
          ENDIF
          IF(wstobs/=missing_value)THEN

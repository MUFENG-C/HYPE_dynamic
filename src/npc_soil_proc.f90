...
CONTAINS
  ...
  !>to snow or soil on land classes. Calculate the load for output.
  !!
  !> \b Reference ModelDescription Processes above ground (Atmospheric deposition of nitrogen and phosphorus)
  !---------------------------------------------------------------------
  SUBROUTINE add_dry_deposition_to_landclass(i,j,iluse,veg, &
                                  areaij,source,frozenstate,soilstate)
    ...
    REAL fakeflow
    REAL sourceN
    REAL, DIMENSION(1) :: source1D
    REAL :: sourceDD(numsubstances) !,sourceD2(numsubstances)

    IF(numsubstances==0) RETURN

    !Get load of deposition
    CALL set_class_deposition(i,numsubstances,iluse,veg,sourceDD)   !Prepare for dry deposition
    
    !Calculate atmospheric dry deposition loads (kg/timestep), OUTPUT
    source(:) = sourceDD*areaij

    !Add dry deposition to snow or soil
    IF(frozenstate%snow(j,i)>0)THEN
      CALL add_source_to_water(frozenstate%snow(j,i),numsubstances,frozenstate%csnow(:,j,i),sourceDD)
    ELSE 
      !Inorganic nitrogen may partly be added to fastN
      IF(simulate%substance(i_in))THEN
        ...
        sourceDD(i_in) = sourceN    !change to remainder as load to water
      ENDIF
      !Particulate phosphorus is added to partP
      IF(simulate%substance(i_pp))THEN
        source1D = sourceDD(i_pp)
        CALL production_pool(1,soilstate%partP(1,j,i),source1D)
        sourceDD(i_pp) = 0.
      ENDIF
      !The remainder of deposition is added to soil water
      CALL add_source_to_water(soilstate%water(1,j,i),numsubstances,soilstate%conc(:,1,j,i),sourceDD)
    ENDIF
    ...
    DO kcrop = 1, 2                             !main and secondary crop
      ...
      IF(part>0)THEN
        ...
        IF(modeloption(p_growthstart)==1)THEN
          IF(current_time%dayno == cropdata(k)%firstday .OR. (cropdata(k)%firstday==0.AND.current_time%dayno==1)) THEN   !reset day before start of accumulation or beginning of year (default)
            miscstate%gdd(kcrop,j,i) = 0.
            miscstate%gsbegin(kcrop,j,i) = 0
          ENDIF
          IF(daylength >= cropdata(k)%daylength .AND. current_time%dayno >= cropdata(k)%firstday)THEN        !criteria met, accumulate GDD
          ...
          IF(miscstate%gdd(kcrop,j,i) > cropdata(k)%gddsow)THEN
            IF(miscstate%gsbegin(kcrop,j,i) == 0)THEN 
              miscstate%gsbegin(kcrop,j,i) = current_time%dayno
            ENDIF
          ENDIF
          IF(miscstate%gsbegin(kcrop,j,i) > 0)THEN 
        ...
        IF(in_season_end(NINT(bd2),NINT(bd3)))THEN
          IF(current_time%dayno>=bd2)THEN
          !  uptakeN = cropdata(k)%uptake1/ (bd3-bd2)
            uptakeN = plant_growth(cropdata(k)%uptake1,cropdata(k)%uptake2,cropdata(k)%uptake3,REAL(current_time%dayno-bd2))
          ELSE
          !  uptakeN = cropdata(k)%uptake1/ (bd3-bd2)
            uptakeN = plant_growth(cropdata(k)%uptake1,cropdata(k)%uptake2,cropdata(k)%uptake3,REAL(current_time%dayno+current_time%prevdoy-bd2))
          ENDIF
        ENDIF

        !>Calculate nitrogen uptake for autumn sowed crop before winter set in
        IF(bd5>0)THEN
          IF(NINT(bd5)>=181.AND.in_season_end(NINT(bd5+1),numdays_of_year(current_time%date)))THEN  !to end of year
            IF(temp<5.) THEN       
              tmpfcn=0.
            ELSE
              tmpfcn = MIN(1., (1./20.)*temp-(5./20.)) 
            ENDIF
            uptakeN = tmpfcn * plant_growth(cropdata(k)%uptake1,cropdata(k)%uptake2,cropdata(k)%uptake3,REAL(current_time%dayno-(bd5+25)))
          ELSEIF(NINT(bd5)<181.AND.in_season_end(NINT(bd5+1),numdays_of_year(current_time%date)-184))THEN !to 30/6
            IF(temp<5.) THEN       
              tmpfcn=0.
            ELSE
              tmpfcn = MIN(1., (1./20.)*temp-(5./20.)) 
  ...
  SUBROUTINE crop_sources(i,j,calcN,calcP,calcC,ndays,ldays,areaij,thickness,soilstate,sources)
    ...
    IF(current_time%tsofday /= 1) RETURN     !apply crop sources once per day (first time step)
  ...
  !---------------------------------------------------------------------------------------
  SUBROUTINE particle_processes_for_runoff(i,j,isoil,iluse,pdayno,prec,surfacerunoff,macroporflow,    &
                          tilerunoff,totflow,csurface,cmacro,ctile,crunoff1,crunoff2,  &
    ...
    USE MODVAR, ONLY : soilthick, &
                       ...
                       numsubstances, &
                       i_pp,i_ss, &
                       genpar,landpar,regpar,soilpar
    USE HYPEVARIABLES, ONLY : m_soilcoh,m_soilerod,m_sreroexp,m_phoscorr,           &
    ...
    INTEGER, INTENT(IN) :: pdayno    !<pseudo day number of the year
    ...
    CASE(0)
      ...
      ELSE
        phoscorr = 1.
      ENDIF
      CALL calculate_erosion(i,j,pdayno,prec,surfacerunoff, &   !calculate eroded sediment
             soilpar(m_soilcoh,isoil),phoscorr*soilpar(m_soilerod,isoil), &
             snow,genpar(m_sreroexp),erodingflow,erodedsed)
    CASE(1)
      CALL calculate_hbvsed_erosion(i,prec,landpar(m_erodluse,iluse),soilpar(m_erodsoil,isoil), &   !calculate eroded sediment based on HBV-sed
                      genpar(m_erodslope),genpar(m_erodexp),genpar(m_erodindex),snow,frostdepth,erodedsed)
    END SELECT
    !>Step 2 Transport the eroded sediment of the field
    ...
    !>Step 3 Suspended sediment and pariculate phosphorus reaching the local stream
    ...
    IF(totflow>0.)THEN
      IF(simulate%substance(i_pp))THEN
        ...
        IF((ctile(i_pp) * tilerunoff + csurface(i_pp) * surfacerunoff) == 0.) THEN
          soilstate%PPrelpool(j,i) = soilstate%PPrelpool(j,i) * EXP(-1. * genpar(m_eroddecay))
        ENDIF
      ...
      IF(simulate%substance(i_ss))THEN
        ...
        IF((ctile(i_ss) * tilerunoff + csurface(i_ss) * surfacerunoff) == 0.) THEN
          soilstate%Srelpool(j,i) = soilstate%Srelpool(j,i) * EXP(-1. * genpar(m_eroddecay))
        ENDIF
    ...
    ELSE
      !>Remove/decay sediment from temporary pool if no erosion occurred so that sediment doesn't remain detached forever
      IF(simulate%substance(i_ss)) &
        soilstate%Srelpool(j,i) = soilstate%Srelpool(j,i) * EXP(-1. * genpar(m_eroddecay))
      IF(simulate%substance(i_pp)) &
        soilstate%PPrelpool(j,i) = soilstate%PPrelpool(j,i) * EXP(-1. * genpar(m_eroddecay))
    ENDIF
  ...
  !------------------------------------------------------------------
  SUBROUTINE calculate_erosion(i,j,pdayno,prec,surfacerunoff,cohesion, &
    ...
    DO kcrop = 1, 2
      ...
      IF(part>0)THEN
        ...
        ELSE                                           !spring, winter and row crops
          ...
          IF(bd2>bd3)  gperiod = REAL(bd3 + current_time%prevdoy - bd2)/2.
          ...
          IF(maxday1>current_time%prevdoy) maxday1 = maxday1 - current_time%prevdoy
          IF(bd5>=181)THEN
            maxday2 = INT(bd5 + REAL(current_time%prevdoy-bd5)/2.)   !day of maximum crop and ground cover for winter crops in autumn (northern hemispere)
          ELSE
          ...
          IF(bd5 > 0)THEN                              !winter crop
            ...
            ELSEIF(in_season_period(bd2,NINT(gperiod)))THEN
              IF(current_time%dayno>=bd2)THEN
                cropcover   = cropdata(k)%ccmax2 + (cropdata(k)%ccmax1-cropdata(k)%ccmax2)*((current_time%dayno - REAL(bd2)) / gperiod)
                groundcover = cropdata(k)%gcmax2 + (cropdata(k)%gcmax1-cropdata(k)%gcmax2)*((current_time%dayno - REAL(bd2)) / gperiod)
              ELSE    !bd2 and dayno on different sides of new year
                cropcover   = cropdata(k)%ccmax2 + (cropdata(k)%ccmax1-cropdata(k)%ccmax2)*((current_time%dayno + current_time%prevdoy - REAL(bd2)) / gperiod) 
                groundcover = cropdata(k)%gcmax2 + (cropdata(k)%gcmax1-cropdata(k)%gcmax2)*((current_time%dayno + current_time%prevdoy - REAL(bd2)) / gperiod)
              ENDIF
            ...
            ELSEIF(in_season_period(bd5,NINT(gperiod2)))THEN
              IF(current_time%dayno>=bd5)THEN
                cropcover   = cropdata(k)%ccmax2 * ((current_time%dayno - REAL(bd5)) / gperiod2)
                groundcover = cropdata(k)%gcmax2 * ((current_time%dayno - REAL(bd5)) / gperiod2)
              ELSE
                cropcover   = cropdata(k)%ccmax2 * ((current_time%dayno + current_time%prevdoy - REAL(bd5)) / gperiod2)
                groundcover = cropdata(k)%gcmax2 * ((current_time%dayno + current_time%prevdoy - REAL(bd5)) / gperiod2)
              ENDIF
            ENDIF
          ELSE  !bd4>0                                       !spring crop (or row crop)
            ...
            ELSEIF(in_season_period(bd2,NINT(gperiod)))THEN
              IF(current_time%dayno>=bd2)THEN
                cropcover   = cropdata(k)%ccmax1 * ((current_time%dayno - REAL(bd2)) / gperiod) 
                groundcover = cropdata(k)%gcmax1 * ((current_time%dayno - REAL(bd2)) / gperiod)
              ELSE
                cropcover   = cropdata(k)%ccmax1 * ((current_time%dayno + current_time%prevdoy - REAL(bd2)) / gperiod)
                groundcover = cropdata(k)%gcmax1 * ((current_time%dayno + current_time%prevdoy - REAL(bd2)) / gperiod)
              ENDIF
            ELSEIF(maxday1/=bd3.AND.in_season_end(maxday1,bd3-1))THEN
              cropcover   = cropdata(k)%ccmax1
              groundcover = cropdata(k)%gcmax1
  ...
  SUBROUTINE calculate_hbvsed_erosion(i,prec,lusepar,soilpar,slopepar, &
    ...
    REAL erodmonth      !monthly erosion factor (-)
  ...
  !------------------------------------------------------------------------------
  SUBROUTINE soil_substance_processes(i,j,iluse,isoil,config,area,wp,fc,pw, &
                      common_uptake,thickness,nitrification,  &
                      denitrification,cropuptake,cropsources, &
                       soilstate,glaciersoil)       

    USE HYPEVARIABLES, ONLY : m_fertdays,m_littdays,m_denitrlu, &
                              m_freuc,m_freuexp,m_freurate, &
                              m_incorr,m_oncorr,m_phoscorr, &
                              m_dissolfN,m_dissolfP,m_dissolhN,m_dissolhP, &
                              m_minerfn,m_minerfp,m_degradhn,m_degradhp, &
                              m_crate1,m_crate2,m_crate3,m_crate9, &
                              m_crate10,m_minc,m_ocsoim,m_ocsmslp
    ...
    TYPE(soilstatetype),INTENT(INOUT)  :: soilstate !<Soil states
    LOGICAL, INTENT(IN), OPTIONAL :: glaciersoil  !<flag for glacier soil model

    !Local variables
    REAL sink(numsubstances),source(numsubstances)
    REAL :: incorr,oncorr,phoscorr   !superparameter positive correction
    REAL :: pardisfN            !model parameter mineralisation fastN to dissolved ON
    REAL :: pardisfP            !model parameter mineralisation fastP to dissolved OP
    REAL :: pardishN            !model parameter mineralisation humusN to dissolved ON
    REAL :: pardishP            !model parameter mineralisation humusP to dissolved OP
    REAL :: parminfN            !model parameter mineralisation fastN
    REAL :: parminfP            !model parameter mineralisation fastP
    REAL :: pardegrhN           !model parameter degradation humusN
    REAL :: denpar1             !general denitrification rate
    REAL :: pardegrhP           !model parameter degradation humusP      
    ...
    ELSE
      phoscorr = 1.
    ENDIF
    pardisfN = landpar(m_dissolfN,iluse)
    pardisfP = landpar(m_dissolfP,iluse)
    pardishN = oncorr * landpar(m_dissolhN,iluse)
    pardishP = phoscorr * landpar(m_dissolhP,iluse)
    parminfN = landpar(m_minerfn,iluse)
    parminfP = landpar(m_minerfp,iluse)
    pardegrhN = incorr * landpar(m_degradhn,iluse)
    pardegrhP = landpar(m_degradhp,iluse)
    denpar1 = (2.-incorr) * landpar(m_denitrlu,iluse)   !denitrification opposite correction, aka 1-incorrpar=2-(1+incorrpar)
    IF(PRESENT(glaciersoil))THEN
      IF(glaciersoil)THEN     !no corrections applied for glacier soil
        pardishN = landpar(m_dissolhN,iluse)
        pardishP = landpar(m_dissolhP,iluse)
        pardegrhN = landpar(m_degradhn,iluse)
        denpar1 = landpar(m_denitrlu,iluse)
      ENDIF
    ENDIF

    !Calculate the nutrient processes; sources
    CALL crop_sources(i,j,config%simN,config%simP,config%simC,genpar(m_fertdays), &
                      genpar(m_littdays),area,thickness,soilstate,cropsources)  !fertilizer and residues
    CALL weathering(config%simSi,i,j,isoil,thickness,soilstate)
    
    !Calculate the nutrient processes
    CALL soil_pool_transformations(i,j,config%simN,config%simP,wp,pw,thickness,source,pardisfN,  &
    ...
    CALL plant_uptake(config%simN,config%simP,common_uptake,wp,thickness,soilstate%water(:,j,i),soilstate%conc(:,:,j,i),sink)    
    IF(simulate%substance(i_in)) cropuptake = sink(i_in)
    CALL calculate_soil_profile_denitrification(config%simN,i,j,iluse,pw,thickness,denpar1,soilstate,denitrification)
    IF(config%simC) CALL soil_carbon_pool_transformations(i,j,wp,fc,pw,thickness, &
             genpar(m_crate1),genpar(m_crate2),genpar(m_crate3),genpar(m_crate9), &
             genpar(m_crate10),genpar(m_minc),landpar(m_ocsoim,iluse), &
             landpar(m_ocsmslp,iluse),soilstate)
    IF(config%simP) CALL balance_spsoil(i,j,thickness,soilpar(m_freuc,isoil),soilpar(m_freuexp,isoil),soilpar(m_freurate,isoil),soilstate)

  END SUBROUTINE soil_substance_processes
  ...
  SUBROUTINE plant_uptake(calcN,calcP,common_uptake,wp,thickness,soil,csoil,sink)
    ...
    uptakeP = 0.
    IF(calcN)THEN
      uptakeN(1) = MAX(MIN(common_uptake(1,1), maxpooluptake(1)*inorgNpool(1)),0.)
      IF(thickness(2)>0)THEN
        uptakeN(2) = MAX(MIN(common_uptake(1,2), maxpooluptake(2)*inorgNpool(2)),0.)
      ENDIF
      IF(thickness(2)>0)THEN
    ...
    ENDIF
    IF(calcP)THEN    
      uptakeP(1) = MAX(MIN(common_uptake(2,1), maxpooluptake(1)*SRPpool(1)),0.)
      IF(thickness(2)>0)THEN
        uptakeP(2) = MAX(MIN(common_uptake(2,2), maxpooluptake(2)*SRPpool(2)),0.)
      ENDIF
      IF(thickness(2)>0)THEN
  ...
  SUBROUTINE  freundlich(iin,jin,lin,PoolPP,SRP_Conc,Vol,LayerThick,Kfr,Nfr,Kadsdes)
    ...
    !Liten s�kerhets�tg�rd ifall negativa SRP_conc skulle dyka upp igen. S� det m�rks.
    !Ta bort om ett �r (idag 140506) ifall inte anv�nd innan dess.
    IF(SRP_conc<0.) THEN
  ...
  SUBROUTINE local_diffuse_source(i,j,pw,classarea,soilstate,ruralaload,upwardflow,ruralflow,cruralflow,alldissolved)
    ...
    IF(addruralflow)THEN
      ...
      IF(qin>0)THEN
        ...
        cin = load(i)%locconc
        ruralaload = cin * qin
        IF(simulate%substance(i_pp) .AND. .NOT.ppdissolved)THEN
  ...
  !>
  !>\b Reference ModelDescription Chapter Silica
  !-----------------------------------------------------------------
  SUBROUTINE weathering(calcSi,i,j,isoil,thickness,soilstate)

    USE MODVAR, ONLY : i_dsi, &
                       genpar,soilpar, &
                       maxsoillayers, &
                       basin
    
    USE HYPEVARIABLES, ONLY : m_weathrate,m_weathkt,m_weathdep

    !Argument declarations
    LOGICAL, INTENT(IN) :: calcSi                   !<Status of silica simulation
    INTEGER, INTENT(IN) :: i                        !<index of subbasin 
    INTEGER, INTENT(IN) :: j                        !<index of class
    INTEGER, INTENT(IN) :: isoil                    !<index of soil type   
    REAL, INTENT(IN)    :: thickness(maxsoillayers) !<thickness of soil layers (m)
    ...
    !Local variables
    INTEGER k
    REAL a, weath, tmpfnc, vol, temp
    REAL wfactor
    
    IF(.NOT.calcSi) RETURN
    
    DO k=1,3
      IF(thickness(k)>0)THEN
        !Calculate factor based on weathering depth
        IF (k==1) THEN
          wfactor = EXP(-LOG(2.)*(thickness(1)/2.)/genpar(m_weathdep))
        ELSEIF (k==2) THEN
          wfactor = EXP(-LOG(2.)*(thickness(1)+thickness(2)/2.)/genpar(m_weathdep))
        ELSE
          wfactor = EXP(-LOG(2.)*(thickness(1)+thickness(2)+thickness(3)/2.)/genpar(m_weathdep))
        ENDIF
        vol = soilstate%water(k,j,i)    !mm
        !Calculate temperature function
        temp = soilstate%temp(k,j,i)
        a = 1.0 / (temp + 238.15) - 1.0 / 245.15 
        IF(temp > 0.)THEN
          tmpfnc = EXP((-genpar(m_weathkt)/8.3145) * a)
        ELSE
          tmpfnc = EXP((-genpar(m_weathkt)/8.3145) * a) * (1. - temp / (temp - 1.))
        ENDIF
        !Calculate weathering and add to dissolved silica in soil water
        IF(soilpar(m_weathrate,isoil)>0 .AND. vol > 0)THEN
          weath = basin(i)%weatheringfactor * soilpar(m_weathrate,isoil) * tmpfnc * thickness(k) * wfactor  !mg/m2/d
          soilstate%conc(i_dsi,k,j,i) = (soilstate%conc(i_dsi,k,j,i) * vol + weath) / vol
        ELSE
          !Other weathering equation
        ENDIF
      ENDIF
    ENDDO
    
  END SUBROUTINE weathering  

...
MODULE DATAMODULE
  ...
  USE LIBDATE
  USE READWRITE_ROUTINES, ONLY : count_comment_rows, &
                                 count_data_cols, &
                                 count_data_rows, &
                                 read_basindata5, &
                                 read_basindata6, &
...
CONTAINS
  ...
  SUBROUTINE load_one_forcing_variabel(ns,ndt,bdate,edate,forcobs,status) 
    ...
    IF(forcobs%readfile)THEN
      ...
      !Check comment rows
      CALL count_comment_rows(fileunit_temp,forcobs%filepath,forcobs%nskip,status) 
      IF(status.NE.0) RETURN
      
      !Get and check stations
      IF(.NOT.ALLOCATED(forcobs%basinindex))ALLOCATE(forcobs%basinindex(ns))
      CALL check_file_station_id_order(fileunit_temp,forcobs%filepath,ns,forcobs%nskip,forcobs%stationid,forcobs%basinindex,forcobs%ncols,status)
      IF(status.NE.0) RETURN
      IF(ALLOCATED(forcobs%stationid)) DEALLOCATE(forcobs%stationid)

      !Check time period
      CALL check_obs_timeperiod(fileunit_temp,forcobs%filepath,forcobs%nskip+1,i_t,bdate,   &
      ...
      CALL prepare_read_matrix(forcobs%fileunit,forcobs%filepath,forcobs%nskip+1,bdate,status)
      IF(status.NE.0) RETURN
      IF(readdaily)THEN
      ...
      ELSE
        ...
        CALL READ_MATRIX(forcobs%fileunit,ndt,forcobs%ncols,edate,nt,   &
             dates,forcobs%allvalues,missing_value,readmatlab,status) 
        IF(status.NE.0)THEN
          WRITE(6,*) 'ERROR: reading '//TRIM(forcobs%filepath)
          RETURN
        ENDIF
  ...
  !>Reads Qobs, Wobs and Xobs data values in file
  !!
  !>\b Consequences Module worldvar variables qobs, wobs, xobs, xoregobs, xcol, xorcol, readqobs,
  !!qobsindex, wobsindex, wobsindex, numqobsstn, numwobsstn, bqdate, eqdate, bwdate, ewdate, bxdate, exdate, bxrdate and exrdate may change.
  !!Module modvar variables qobsi, xobsi, wobsi and xoregobsi may be allocated, and 
  ...
  SUBROUTINE load_ascii_qx_observations(dir,ns,ndt,bdate,edate,status) 
    ...
    INTEGER,ALLOCATABLE :: subnr(:),temp_index(:),temp2_index(:)       !Help variables
    ...
    IF(qex)THEN
      ncomrowsofqobs = 0
      CALL count_comment_rows(fileunit_temp,TRIM(dir)//filename_Qobs,ncomrowsofqobs,status) 
      IF(.NOT.ALLOCATED(qobsindex)) ALLOCATE(qobsindex(ns))
      CALL check_station(fileunit_temp,TRIM(dir)//filename_Qobs,ns,ncomrowsofqobs,subnr,numqobsstn,qobsindex,status)
      IF(status==3)THEN
        qex = .FALSE.
      ...
      ENDIF
    ENDIF
    INQUIRE(FILE=TRIM(dir)//filename_Wobs,EXIST=wex)
    IF(wex)THEN
      CALL count_comment_rows(fileunit_temp,TRIM(dir)//filename_Wobs,ncomrowsofwobs,status) 
      IF(status/=0)RETURN
      IF(.NOT.ALLOCATED(wobsindex)) ALLOCATE(wobsindex(ns))
      CALL check_station(fileunit_temp,TRIM(dir)//filename_Wobs,ns,ncomrowsofwobs,subnr,numwobsstn,wobsindex,status)
      IF(status==3)THEN
        wex = .FALSE.
        IF(ALLOCATED(wobsindex)) DEALLOCATE(wobsindex)
      ELSEIF(status.NE.0)THEN
        RETURN
      ELSE
        fileunit_wobs = fileunit_get()
      ENDIF
    ENDIF
    INQUIRE(FILE=TRIM(dir)//filename_Xobs,EXIST=xex)
    IF(xex)THEN
      CALL count_comment_rows(fileunit_temp,TRIM(dir)//filename_Xobs,ncomrowsofxobs,status) 
!      IF(ncomrowsofxobs<1) ncomrowsofxobs = 1   !To read old files without '!!'. File without comment row will give error later in check_xobs.
      IF(ncomrowsofxobs<1)THEN
        WRITE(6,*) 'WARNING: No comment row found in file ',TRIM(dir)//filename_Xobs   !To read old files without '!!'. File without comment row will give error later in check_xobs.
        WRITE(6,*) 'WARNING: Check if older HYPE version comment row is present in'
        WRITE(6,*) 'WARNING: file, i.e. a row not starting with !!. If so add !!.'
        ncomrowsofxobs = 1
        WRITE(6,*) 'WARNING: For now one row with comments are assumed (as in old files),'
        WRITE(6,*) 'WARNING: and a file without comment will stop HYPE with error.'
!        WRITE(6,*) 'WARNING: file, i.e. a row not starting with !!. If such a comment'
!        WRITE(6,*) 'WARNING: row is present, HYPE will stop with error.'
      ENDIF
      CALL count_data_cols(fileunit_temp,TRIM(dir)//filename_Xobs,ncomrowsofxobs,xcol,status)
!      CALL count_data_cols(fileunit_temp,TRIM(dir)//filename_Xobs,1, xcol,status)
      IF(status.NE.0) RETURN
      xcol = xcol-1
      IF(xcol>0)THEN
        IF(.NOT.ALLOCATED(xvar)) ALLOCATE(xvar(xcol,2))
        CALL check_xobs(fileunit_temp,TRIM(dir)//filename_Xobs,ncomrowsofxobs,xcol,    &
             xvar,status) 
        IF(status.NE.0) RETURN
      ENDIF
    ENDIF
    IF(readoutregion)THEN
      ...
      IF(xorex)THEN
        CALL count_comment_rows(fileunit_temp,TRIM(dir)//filename_Xoreg,ncomrowsofxoregobs,status) 
!        IF(ncomrowsofxoregobs<1) ncomrowsofxoregobs = 1   !To read old files without '!!'. File without comment row will give error later in check_xobs.
        IF(ncomrowsofxoregobs<1)THEN
          WRITE(6,*) 'WARNING: No comment row found in file ',TRIM(dir)//filename_Xoreg   !To read old files without '!!'. File without comment row will give error later in check_xobs.
          WRITE(6,*) 'WARNING: Check if older HYPE version comment row is present in'
          WRITE(6,*) 'WARNING: file, i.e. a row not starting with !!. If so add !!.'
          ncomrowsofxoregobs = 1
          WRITE(6,*) 'WARNING: For now one row with comments are assumed (as in old files),'
          WRITE(6,*) 'WARNING: and a file without comment will stop HYPE with error.'
        ENDIF
        CALL count_data_cols(fileunit_temp,TRIM(dir)//filename_Xoreg,ncomrowsofxoregobs,xorcol,status)
        IF(status.NE.0) RETURN
        xorcol = xorcol-1
        IF(xorcol>0)THEN
          IF(.NOT.ALLOCATED(xorvar)) ALLOCATE(xorvar(xorcol,2))
          CALL check_xobs(fileunit_temp,TRIM(dir)//filename_Xoreg,ncomrowsofxoregobs,xorcol,    &
                 xorvar,status) 
          IF(status.NE.0) RETURN
        ENDIF
      ENDIF      
    ...
    IF(qex)THEN
      CALL check_obs_timeperiod(fileunit_qobs,TRIM(dir)//filename_Qobs,ncomrowsofqobs+1,i_t,   &
      ...
      IF(status==2)THEN  !Shorter time period
        ...
        IF(bdate.GT.fedate .OR. edate.LT.fbdate)THEN    !no overlap
          qex = .FALSE.
        !IF(ALLOCATED(qobsindex)) DEALLOCATE(qobsindex)  !add?
        ELSE                                      !some overlap
    ...
    ENDIF
    IF(wex)THEN
      CALL check_obs_timeperiod(fileunit_wobs,TRIM(dir)//filename_Wobs,ncomrowsofwobs+1,i_t,   &
            bdate,edate,fbdate,fedate,notimefound,status)
      bwdate = bdate
      ewdate = edate
      IF(status==2)THEN  !Shorter time period
    ...
    IF(xex .AND. xcol>0)THEN
      CALL check_obs_timeperiod(fileunit_xobs,TRIM(dir)//filename_Xobs,ncomrowsofxobs+2,i_t,   &
            bdate,edate,fbdate,fedate,notimefound,status)
      bxdate = bdate
      exdate = edate
      IF(status==2)THEN  !Shorter time period
    ...
    IF(readoutregion)THEN
      IF(xorex .AND. xorcol>0)THEN
        CALL check_obs_timeperiod(fileunit_xoreg,TRIM(dir)//filename_Xoreg,ncomrowsofxoregobs+2,i_t,   &
              bdate,edate,fbdate,fedate,notimefound,status)
        bxrdate = bdate
        exrdate = edate
        IF(status==2)THEN  !Shorter time period
    ...
    !Load discharge data
    IF(qex)THEN
      CALL prepare_read_matrix(fileunit_qobs,TRIM(dir)//filename_Qobs,ncomrowsofqobs+1,bqdate,status)
      IF(status.NE.0) RETURN
      ...
      ELSE
        ...
        CALL READ_MATRIX(fileunit_qobs,ndt,numqobsstn,eqdate,nt,   &
             localdates,x2(:,:),missing_value,readmatlab,status) 
        IF(status.NE.0)THEN
          WRITE(6,*) 'ERROR: reading '//TRIM(dir)//filename_Qobs
          RETURN
        ENDIF
        CLOSE(fileunit_qobs)
    ...
    IF(wex)THEN
      CALL prepare_read_matrix(fileunit_wobs,TRIM(dir)//filename_Wobs,ncomrowsofwobs+1,bwdate,status)
      IF(status.NE.0) RETURN
      IF(.NOT.ALLOCATED(wobsi)) ALLOCATE(wobsi(numwobsstn))
      IF(readdaily)THEN
        WRITE(6,*) 'File ready: ', TRIM(dir)//filename_Wobs
        readwobs = .TRUE.
      ELSE
        IF(.NOT.ALLOCATED(x2)) ALLOCATE(x2(ndt,numwobsstn))
        CALL READ_MATRIX(fileunit_wobs,ndt,numwobsstn,ewdate,nt,   &
             localdates,x2(:,:),missing_value,readmatlab,status) 
        IF(status.NE.0)THEN
          WRITE(6,*) 'ERROR: reading '//TRIM(dir)//filename_Wobs
          RETURN
        ENDIF
        CLOSE(fileunit_wobs)
        CALL fileunit_free(fileunit_wobs)
        nskip = period_length(bdate,bwdate)
        ALLOCATE (wobs(nskip+1:nskip+nt,numwobsstn))
        wobs(:,:)=x2(1:nt,1:numwobsstn)
        DEALLOCATE(x2)
        WRITE(6,*) 'File loaded: ', TRIM(dir)//filename_Wobs
      ENDIF
    ENDIF

    !Load other observation data
    IF(xex .AND. xcol>0)THEN
      CALL prepare_read_matrix(fileunit_xobs,TRIM(dir)//filename_Xobs,ncomrowsofxobs+2,bxdate,status)
      IF(status.NE.0) RETURN
      ...
      ELSE
        ...
        CALL READ_MATRIX(fileunit_xobs,ndt,xcol,exdate,nt,localdates,    &
             x3(:,:),missing_value,readmatlab,status) 
        IF(status.NE.0)THEN
          WRITE(6,*) 'ERROR: reading '//TRIM(dir)//filename_Xobs
          RETURN
        ENDIF
        CLOSE(fileunit_xobs)
    ...
    IF(readoutregion)THEN
      IF(xorex .AND. xorcol>0)THEN
        CALL prepare_read_matrix(fileunit_xoreg,TRIM(dir)//filename_Xoreg,ncomrowsofxoregobs+2,bxrdate,status)
        IF(status.NE.0) RETURN
        ...
        ELSE
          ...
          CALL READ_MATRIX(fileunit_xoreg,ndt,xorcol,exrdate,nt,localdates,    &
                 x3(:,:),missing_value,readmatlab,status) 
          IF(status.NE.0)THEN
            WRITE(6,*) 'ERROR: reading '//TRIM(dir)//filename_Xoreg
            RETURN
          ENDIF
    ...
    !Prepare to use wobs from xobs
    conductwinx = .FALSE.
    IF(.NOT.wex .AND. xex .AND. xcol>0)THEN
      !Check if any wstr in xobs
      nwinx = 0
      ALLOCATE(temp_index(ns))
      ALLOCATE(temp2_index(ns))
      temp_index = 0
      temp2_index = 0
      DO i = 1,ns
      ...
      IF(nwinx>0)THEN
        ...
        numwobsstn = nwinx
        IF(.NOT.ALLOCATED(wobsindex)) ALLOCATE(wobsindex(ns))
        IF(.NOT.ALLOCATED(winxobsindex)) ALLOCATE(winxobsindex(numwobsstn))
        wobsindex = temp_index
        winxobsindex = temp2_index(1:nwinx)
        IF(.NOT.ALLOCATED(wobsi)) ALLOCATE(wobsi(numwobsstn))
      ENDIF
      DEALLOCATE(temp_index)
      DEALLOCATE(temp2_index)
    ENDIF

    !Finalise the subroutine, deallocate in opposite order
    ...
    IF(readdaily)THEN
      ...
      IF(readformat==0)THEN
        DO iforc = 1, max_forcingdata   !Mandatory and optional forcing observations
          IF(forcingdata(iforc)%readfile)THEN
            CALL prepare_read_matrix(forcingdata(iforc)%fileunit,forcingdata(iforc)%filepath,forcingdata(iforc)%nskip+1,bdate,status)
            IF(status.NE.0) RETURN
          ENDIF
        ENDDO
        IF(readwobs)THEN
  ...
  !> may change.
  !-----------------------------------------------------------------------------------------
  SUBROUTINE prepare_for_update(dir,locupall,locupnone,nsfile,ns,status) 
    ...
    USE WORLDVAR, ONLY : fileunit_temp, &
                         filename_upd, &
                         qobsindex
    USE MODEL_TEST_ROUTINES, ONLY : propagate_external_msg,e_update, &
    ...
    INTEGER, INTENT(IN)  :: nsfile              !<Number of subbasins of basemodel
    ...
    INTEGER wendnum 
    INTEGER filenum 
    INTEGER qusetemp(nsfile)
    INTEGER cusetemp(nsfile)
    INTEGER qartemp(nsfile)
    INTEGER wartemp(nsfile)
    REAL    qarfac(nsfile)
    REAL    warfac(nsfile)
    INTEGER wendtemp(nsfile)
    REAL    tptemp(nsfile,2)
    REAL    tntemp(nsfile,2)
    REAL    tploctemp(nsfile,2)
    REAL    tnloctemp(nsfile,2)
    LOGICAL fileexist
    ...
    IF(fileexist) CALL read_update_data(fileunit_temp,TRIM(dir)//filename_upd,    & 
         nsfile,qusenum,qusetemp,qarnum,warnum,qartemp,wartemp,qarfac,warfac,wendnum,  &
    ...
    IF(doupdate(i_quseobs))THEN
      ...
      ELSEIF(locupall(i_quseobs) .OR. qusenum>0) THEN
        ...
        ENDIF
      ELSE
        IF(conductwarning) WRITE(6,*) 'WARNING: missing stations in update.txt, no quseobs updating'
        doupdate(i_quseobs) = .FALSE.
      ENDIF
    ENDIF
    ...
    IF(doupdate(i_qar))THEN
      ...
      ELSEIF(qarnum>0) THEN
        ...
        ENDDO
      ELSE
        IF(conductwarning) WRITE(6,*) 'WARNING: missing stations in update.txt, no qar updating'
        doupdate(i_quseobs) = .FALSE.
      ENDIF
    ENDIF
    ...
    IF(doupdate(i_war))THEN
      ...
      ELSEIF(warnum>0) THEN
        ...
        ENDDO
      ELSE
        IF(conductwarning) WRITE(6,*) 'WARNING: missing stations in update.txt, no war updating'
        doupdate(i_war) = .FALSE.
      ENDIF
    ENDIF
    ...
    IF(doupdate(i_tpcorr).OR.doupdate(i_tploccorr))THEN
      IF(i_sp==0.OR.i_pp==0)THEN
        ...
        CALL propagate_external_msg(e_update,e_error,propagate_str)
        status = 1
        RETURN
      ENDIF
      IF(filenum>0)THEN
        IF(doupdate(i_tpcorr))    CALL set_update_nutrientcorr(nsfile,ns,filenum,i_tpcorr,tptemp)
        IF(doupdate(i_tploccorr)) CALL set_update_nutrientcorr(nsfile,ns,filenum,i_tploccorr,tploctemp)
      ELSEIF(.NOT.fileexist)THEN
        IF(conductwarning) WRITE(6,*) 'WARNING: missing update.txt file, necessary for update of phosphourus'
        doupdate(i_tpcorr) = .FALSE.
        doupdate(i_tploccorr) = .FALSE.
      ELSE
        IF(conductwarning) WRITE(6,*) 'WARNING: missing data in update.txt file, no phosphourus updating'
        doupdate(i_tpcorr) = .FALSE.
        doupdate(i_tploccorr) = .FALSE.
      ENDIF
    ENDIF
    ...
    IF(doupdate(i_tncorr).OR.doupdate(i_tnloccorr))THEN
      ...
      ENDIF
      IF(filenum>0)THEN
        IF(doupdate(i_tncorr))    CALL set_update_nutrientcorr(nsfile,ns,filenum,i_tncorr,tntemp)
        IF(doupdate(i_tnloccorr)) CALL set_update_nutrientcorr(nsfile,ns,filenum,i_tnloccorr,tnloctemp)
      ELSEIF(.NOT.fileexist)THEN
        IF(conductwarning) WRITE(6,*) 'WARNING: missing update.txt file, necessary for update nitrogen'
        doupdate(i_tncorr) = .FALSE.
        doupdate(i_tnloccorr) = .FALSE.
      ELSE
        IF(conductwarning) WRITE(6,*) 'WARNING: missing data in update.txt file, no nitrogen updating'
        doupdate(i_tncorr) = .FALSE.
        doupdate(i_tnloccorr) = .FALSE.
      ENDIF
    ENDIF
    ...
    IF(doupdate(i_cuseobs))THEN
      ...
      ELSEIF(locupall(i_cuseobs) .OR. cusenum>0) THEN
        ...
        ENDIF
      ELSE
        IF(conductwarning) WRITE(6,*) 'WARNING: missing stations in update.txt, no cuseobs updating'
        doupdate(i_cuseobs) = .FALSE.
      ENDIF
    ENDIF
    ...
    IF(doupdate(i_wendupd))THEN
      ...
      ELSEIF(locupall(i_wendupd) .OR. wendnum>0) THEN
        ...
        ELSEIF(wendnum>0) THEN
          ...
          ENDDO
        ENDIF
        !Check for available observations. No good test: observations can be in Wobs instead
        !DO i = 1,ns
        !  IF(useinupdate(i_wendupd)%station(i).AND. xobsindex(wobsvarindex,i)==0) useinupdate(i_wendupd)%station(i)=.FALSE.
        !ENDDO
      ELSE
        IF(conductwarning) WRITE(6,*) 'WARNING: missing stations in update.txt, no wendupd updating'
        doupdate(i_wendupd) = .FALSE. 
      ENDIF
    ENDIF
    ...
    IF(doupdate(i_war).AND.doupdate(i_qar))THEN
      DO i = 1,ns
        IF(useinupdate(i_war)%station(i).AND.useinupdate(i_qar)%station(i))THEN
          ...
          IF(conductwarning) WRITE(6,*)'WARNING: Update with q-AR and w-AR at the same subbasin is not allowed. w-AR turned off for subbasin',basin(i)%subid
          warnum = warnum - 1
        ENDIF
      ENDDO
    ...
    IF(doupdate(i_war).AND.doupdate(i_wendupd))THEN
      DO i = 1,ns
        IF(useinupdate(i_war)%station(i).AND.useinupdate(i_wendupd)%station(i))THEN
          useinupdate(i_war)%station(i)=.FALSE.
          IF(conductwarning) WRITE(6,*)'WARNING: Update with wendupd and w-AR at the same subbasin is not allowed. w-AR turned off for subbasin',basin(i)%subid
          warnum = warnum - 1
        ENDIF
      ENDDO
      IF(warnum == 0) doupdate(i_war)=.FALSE.
    ...
    IF(doupdate(i_cuseobs))THEN
      IF(numsubstances==0)THEN
        doupdate(i_cuseobs) = .FALSE. 
        IF(conductwarning) WRITE(6,*) 'WARNING: Not possible to update concentrations when no substances are simulated'
      ELSEIF(.NOT.ALLOCATED(xobsindex))THEN
        doupdate(i_cuseobs) = .FALSE. 
        IF(conductwarning) WRITE(6,*) 'WARNING: Not possible to update concentrations when no observations are given'
      ENDIF
    ENDIF
  ...
  SUBROUTINE get_current_forcing(idt,ns,current_date)
    ...
    USE WORLDVAR, ONLY : forcingdata, &
                         ...
                         i_swobs,i_uobs, &
                         i_uwobs,i_vwobs, &
                         numqobsstn,numwobsstn, &
                         conductwinx,conductwobs
    USE MODVAR, ONLY :   preci,tempi, & !OUT
                         snowfraci,shortwavei, & !OUT
                         windi,humidi, & !OUT
                         tmini,tmaxi, & !OUT
                         uwindi,vwindi, & !OUT
                         qobsi,wobsi,xobsi,xoregobsi, &  !OUT
                         psload,absload !OUT
    ...
    TYPE(DateType), INTENT(OUT) :: current_date !<current date
      ...
      IF(conductwobs)THEN
        wobsi(:) = get_current_wst(idt,current_date,numwobsstn)
      ELSEIF(conductwinx)THEN  !conductwinx
        !wstr may be in xobsi
        wobsi(:) = get_current_wst_from_xobs(xobsi,numwobsstn)
      ENDIF
  ...
  !---------------------------------------------------------
  FUNCTION get_current_wst(i,cd,no) RESULT(current_wst)
    ...
    TYPE(DateType), INTENT(IN) :: cd  !<current date
    INTEGER, INTENT(IN) :: no         !<number of observation stations
    REAL current_wst(no)               !<current water stage of lake
    
    !Local variables
    TYPE(DateType) d
    REAL x(no)

    IF(cd.LT.bwdate .OR. cd.GT.ewdate)THEN
      current_wst = missing_value
      RETURN
    ENDIF
    ...
    IF(readformat==0.OR.readformat==4)THEN   !ASCII
      IF(readdaily)THEN
        x = missing_value
        CALL read_matrix_line(fileunit_wobs,no,d,x,missing_value,readmatlab) 
        current_wst = x
        IF(d.NE.cd) WRITE(*,*) 'ERROR read Wobs, date not equal to Wobs date'   !should never happen
      ELSE
        current_wst = get_current_wobs(i,no)
      ENDIF
    ENDIF

  END FUNCTION get_current_wst

  !>Get current water level from xobsi array
  !---------------------------------------------------------
  FUNCTION get_current_wst_from_xobs(readxobs,no) RESULT(current_wst)

    USE WORLDVAR, ONLY : xcol, &
                         winxobsindex
    USE MODVAR, ONLY : wobsvarindex, &
                       !wobsindex, &
                       xobsindex

    !Argument declarations
    REAL, INTENT(IN) :: readxobs(xcol)   !<current xobs observations
    INTEGER, INTENT(IN) :: no        !<number of observation stations
    REAL current_wst(no)             !<current water stage of lake
    ...
    DO k=1,no
      !winxobsindex(k) give i, subbasin index of observation
      !xobsindex(wobsvarindex,i)>0 give column in xobs for variable wstr and subbasin i
      x(k) = readxobs(xobsindex(wobsvarindex,winxobsindex(k)))
    ENDDO
    current_wst = x

  END FUNCTION get_current_wst_from_xobs
  ...
  SUBROUTINE close_observations(dir)
    ...
    IF(readdaily)THEN
      ...
      IF(readwobs) CLOSE(fileunit_wobs)
      INQUIRE(FILE=TRIM(dir)//filename_Xobs,EXIST=xex)
  ...
  !!modeloption, conductdamconstruction, conduct%riverwetland and numrotations are set. 
  !----------------------------------------------------------------
...
!                         atmdepvegreset       !OUT
    ...
    ENDIF
    IF(.NOT.ALLOCATED(classattributedata)) ALLOCATE(classattributedata(nclass))
    classattributedata%luse  = classdata%luse
    classattributedata%soil  = classdata%soil
    classattributedata%crop  = classdata%crop
    classattributedata%crop2 = classdata%crop2
    classattributedata%rotation = classdata%rotation
    classattributedata%vegtype = classdata%vegtype
    classattributedata%tiledepth = classdata%tiledepth
    classattributedata%streamdepth = classdata%streamdepth
    DO ss = 1,maxsoillayers
      classattributedata%soildepth(ss) = classdata%soildepth(ss)
    ENDDO
    ...
    CALL read_and_calc_basindata(fileunit_temp,TRIM(dir)//infile,n,lakedataid,ncols)

    !Check if lakesection is present
    IF(.NOT.ALLOCATED(lakesectiondata) .AND. modeloption(p_connectivity)>0)THEN
        IF(conductwarning) WRITE(6,*) 'WARNING: No input data for connectivity found (lakesectiondata), calculations will be turned off.'
    ...
    IF(conduct%riverwetland)THEN
      IF(SUM(wetland(1:n,1:2)%area)==0)THEN
        IF(conductwarning) WRITE(6,*) 'Warning: No input data for river wetlands found, calculations will be turned off.'
    ...
    CALL load_rrcurvedata(fileunit_temp,dir,n,status)
    IF(status/=0)RETURN
 ...
 ! Reload subbasin data according to the year. 
 ! Reload SLC info and SLC part in each subbasin. new class = old class%soil + new class%landuse
 ! Some related info also be reloaded. 
  SUBROUTINE reload_basindata(dir,n,year,status) 
    ...
    !IF(status/=0)THEN
    !  status = 0
      CALL reload_geoclass(TRIM(dir)//lfile,n,classdata,nclass)
    !ENDIF
    ...
    IF(.NOT.ALLOCATED(lakedataid)) ALLOCATE(lakedataid(n))
    lakedataid = 0
    CALL read_only_slc(fileunit_temp,TRIM(dir)//infile,n,ncols,lakedataid)
...
204 CONTINUE
    ...
    DO i = 1,mcols
      ...
      IF(str(i)(1:nstr)=='atmdepveg  ') slcdata(6,:) = xr(i,:)  !TODO: error this is for snw also!!!
      ...
      IF(str(i)(1:nstr)=='numlayers  ') slcdata(10,:) = xr(i,:)  !on�dig?
...
202 CONTINUE
  ...
  !------------------------------------------------------------------
  SUBROUTINE  load_geoclass(fname,geoclassdata,numslc,maxluse,maxsoil,maxrotation)
    ...
    ! IF(ALLOCATED(geoclassdata)) DEALLOCATE(geoclassdata)              ! clean previous data for reload
  ...
  SUBROUTINE reload_geoclass(fname,n,geoclassdata,numslc)
    ...
    !Local parameters
    INTEGER,PARAMETER :: maxgccol = 4     !Number of data columns in GeoClass: combination	PREVIOUS_CLASS(for soil) NEW_CLASS(for luse) special_code
    ...
    !IF(ALLOCATED(tempsoildepth)) DEALLOCATE(tempsoildepth)
    ...
    IF(.NOT.ALLOCATED(soildepth)) ALLOCATE(soildepth(maxsoillayers,numslc))
    IF(ALLOCATED(soilthick)) DEALLOCATE(soilthick)
    IF(.NOT.ALLOCATED(soilthick)) ALLOCATE(soilthick(maxsoillayers,numslc))
    DO j = 1, numslc
        geoclassdata(j)%luse  = classattributedata(NINT(slcdata(3,j)))%luse
        geoclassdata(j)%crop  = classattributedata(NINT(slcdata(3,j)))%crop
        geoclassdata(j)%crop2 = classattributedata(NINT(slcdata(3,j)))%crop2
        geoclassdata(j)%rotation = classattributedata(NINT(slcdata(3,j)))%rotation
        geoclassdata(j)%vegtype = classattributedata(NINT(slcdata(3,j)))%vegtype
        geoclassdata(j)%soil  = classattributedata(NINT(slcdata(2,j)))%soil 
        geoclassdata(j)%tiledepth = classattributedata(NINT(slcdata(2,j)))%tiledepth 
        geoclassdata(j)%streamdepth = classattributedata(NINT(slcdata(2,j)))%streamdepth
        DO ss = 1,maxsoillayers
            geoclassdata(j)%soildepth(ss) = classattributedata(NINT(slcdata(2,j)))%soildepth(ss)
        ENDDO
       ! soildepth(:,j) = tempsoildepth(:,NINT(slcdata(2,j)))
       ! soilthick(:,j) = tempsoilthick(:,NINT(slcdata(2,j)))
        soildepth(1:maxsoillayers,j) = geoclassdata(j)%soildepth(:)
        soilthick(:,j) = 0.
        DO layer = 1,maxsoillayers
          IF(layer==1)THEN
            soilthick(layer,j) = soildepth(layer,j)
          ELSE
            soilthick(layer,j) = soildepth(layer,j)-soildepth(layer-1,j)
          ENDIF
        ENDDO
    ENDDO

    !>Find coded classes
    CALL set_coded_classes(numslc,slcdata(4,1:numslc))
    ...
    !CALL inherit_state_to_new_class(n,numslc,slcdata(1,1:numslc),frozenstate,soilstate,miscstate)
  ...
  ! Read SLC info in new (year) GeoData.txt
  SUBROUTINE read_only_slc(funit,infile,n,maxcol,lakedataid) 
...
SUBROUTINE inherit_state_to_new_class(dir,year,n,dim,frozenstate,soilstate,miscstate)
  ...
  INTEGER, INTENT(IN)  :: year             !<current year for change geo 
  INTEGER , INTENT(IN) :: n                 !<Number of subbasin
  INTEGER , INTENT(IN) :: dim            !<Number of class
  ...
  !Local variables
  !TYPE(snowicestatetype), POINTER :: pfrozen => NULL() !Temporary snow and ice states
  !TYPE(soilstatetype), POINTER    :: psoil => NULL()  !Temporary soil states
  !TYPE(miscstatetype) , POINTER   :: pmisc => NULL()  !Temporary misc states
  ...
  CHARACTER(LEN=12) :: curyear            !Year for file name
  CHARACTER(LEN=17) :: file              !Name of GeoClass file
  INTEGER :: i,j,numslc
  REAL, ALLOCATABLE :: slcdata(:,:)   !data from Geoclass-file
  !REAL :: array(dim)
  INTEGER,PARAMETER :: maxgccol = 4

  !Read GeoClass
  write(curyear,"(I4)") year
  file='GeoClass_'//trim(curyear)//'.txt'   !Name of GeoClass file 
  CALL reread_geoclass(fileunit_temp,TRIM(dir)//file,maxgccol,slcdata,numslc)

  !>temp store current state value
  CALL partly_allocate_model_states(n,statesize,conduct,frozenstate2,soilstate2,miscstate2)
  ...
  ! Set temp state
    !frozenstate2
    frozenstate2%snow(:,:) = frozenstate%snow(:,:)
    frozenstate2%snowage(:,:) = frozenstate%snowage(:,:)
    frozenstate2%snowdepth(:,:) = frozenstate%snowdepth(:,:)
    frozenstate2%snowcov(:,:) = frozenstate%snowcov(:,:)
    frozenstate2%snowmax(:,:) = frozenstate%snowmax(:,:)
    IF(statesize%substance>0)THEN
      frozenstate2%csnow(:,:,:) = frozenstate%csnow(:,:,:)
    ENDIF
    IF(conduct%snowheat)THEN
      frozenstate2%snowheat(:,:) = frozenstate2%snowheat(:,:)
    ENDIF
    frozenstate2%snowliq(:,:) = frozenstate2%snowliq(:,:)
    !soilstate2
    soilstate2%water(:,:,:)  = soilstate%water(:,:,:)
    soilstate2%temp(:,:,:)   = soilstate%temp(:,:,:)
    soilstate2%deeptemp(:,:) = soilstate%deeptemp(:,:)
    IF(statesize%substance>0)THEN
      soilstate2%conc(:,:,:,:) = soilstate%conc(:,:,:,:)
      IF(ALLOCATED(soilstate%humusN))THEN
        soilstate2%humusN(:,:,:) = soilstate%humusN(:,:,:)
        soilstate2%fastN(:,:,:)  = soilstate%fastN(:,:,:)
      ENDIF  
      IF(ALLOCATED(soilstate%humusP))THEN
        soilstate2%humusP(:,:,:) = soilstate%humusP(:,:,:)
        soilstate2%fastP(:,:,:)  = soilstate%fastP(:,:,:)
        soilstate2%partP(:,:,:)  = soilstate%partP(:,:,:)
        soilstate2%PPrelpool(:,:) = soilstate%PPrelpool(:,:)
      ENDIF  
      IF(ALLOCATED(soilstate%humusC))THEN
        soilstate2%humusC(:,:,:) = soilstate%humusC(:,:,:)
        soilstate2%fastC(:,:,:)  = soilstate%fastC(:,:,:)
        soilstate2%oldgrw(:,:)   = soilstate%oldgrw(:,:)
      ENDIF  
      IF(ALLOCATED(soilstate%Srelpool))THEN
        soilstate2%Srelpool(:,:) = soilstate%Srelpool(:,:)
      ENDIF  
      IF(ALLOCATED(soilstate%partT1))THEN
        soilstate2%partT1(:,:,:) = soilstate%partT1(:,:,:)
      ENDIF  
      IF(ALLOCATED(soilstate%surface))THEN
        soilstate2%surface(:,:,:) = soilstate%surface(:,:,:)
      ENDIF  
    ENDIF
    IF(ALLOCATED(soilstate%icelens)) soilstate2%icelens(:,:) = soilstate%icelens(:,:)
    !miscstate2
    IF(conduct%growthdegreeday)THEN
      miscstate2%gdd(:,:,:) = miscstate%gdd(:,:,:)
      miscstate2%gsbegin(:,:,:) = miscstate%gsbegin(:,:,:)
    ENDIF
    IF(conduct%irrigation)THEN
      miscstate2%nextirrigation(:,:) = miscstate%nextirrigation(:,:)
      IF(numsubstances>0)THEN
        miscstate2%cnextirrigation(:,:,:) = miscstate%cnextirrigation(:,:,:)
      ENDIF
    ENDIF
    IF(conduct%simT1)THEN
      miscstate2%partT1sf(:,:) = miscstate%partT1sf(:,:)
    ENDIF


  !RENEW STATESIZE & CONDUCT NEED CHANGE
  !renew statesize
  statesize%slcclass = dim
  CALL partly_deallocate_model_states(frozenstate,soilstate,miscstate)
  CALL partly_allocate_model_states(n,statesize,conduct,frozenstate,soilstate,miscstate)

  !array(:) = slcdata(1,1:dim)
  !> Inherit
  DO i = 1,n
      DO j = 1,dim 
          !>frozenstate
          frozenstate%snow(j,i) = frozenstate2%snow(NINT(slcdata(1,j)),i)
          frozenstate%snowage(j,i) = frozenstate2%snowage(NINT(slcdata(1,j)),i)
          frozenstate%snowdepth(j,i) = frozenstate2%snowdepth(NINT(slcdata(1,j)),i)
          frozenstate%snowcov(j,i) = frozenstate2%snowcov(NINT(slcdata(1,j)),i)
          frozenstate%snowmax(j,i) = frozenstate2%snowmax(NINT(slcdata(1,j)),i)
          IF(statesize%substance>0)THEN
            frozenstate%csnow(:,j,i) = frozenstate2%csnow(:,NINT(slcdata(1,j)),i)
          ENDIF
          IF(conduct%snowheat)THEN
            frozenstate%snowheat(j,i) = frozenstate2%snowheat(NINT(slcdata(1,j)),i)
          ENDIF
          frozenstate%snowliq(j,i) = frozenstate2%snowliq(NINT(slcdata(1,j)),i)

          !>soilstate
          soilstate%water(:,j,i) = soilstate2%water(:,NINT(slcdata(1,j)),i)
          soilstate%temp(:,j,i) = soilstate2%temp(:,NINT(slcdata(1,j)),i)
          soilstate%deeptemp(j,i) = soilstate2%deeptemp(NINT(slcdata(1,j)),i)
          IF(statesize%substance>0)THEN
              soilstate%conc(:,:,j,i) = soilstate2%conc(:,:,NINT(slcdata(1,j)),i)
              IF(ALLOCATED(soilstate%humusN))THEN
                  soilstate%humusN(:,j,i) = soilstate2%humusN(:,NINT(slcdata(1,j)),i)
                  soilstate%fastN(:,j,i) = soilstate2%fastN(:,NINT(slcdata(1,j)),i)
              ENDIF
              IF(ALLOCATED(soilstate%humusP))THEN
                  soilstate%partP(:,j,i) = soilstate2%partP(:,NINT(slcdata(1,j)),i)
                  soilstate%fastP(:,j,i) = soilstate2%fastP(:,NINT(slcdata(1,j)),i)
                  soilstate%humusP(:,j,i) = soilstate2%humusP(:,NINT(slcdata(1,j)),i)
                  soilstate%PPrelpool(j,i) = soilstate2%PPrelpool(NINT(slcdata(1,j)),i)
              ENDIF
              IF(ALLOCATED(soilstate%humusC))THEN
                  soilstate%fastC(:,j,i) = soilstate2%fastC(:,NINT(slcdata(1,j)),i)
                  soilstate%humusC(:,j,i) = soilstate2%humusC(:,NINT(slcdata(1,j)),i)
                  soilstate%oldgrw(j,i) = soilstate2%oldgrw(NINT(slcdata(1,j)),i)
              ENDIF
              IF(ALLOCATED(soilstate%Srelpool)) THEN
                  soilstate%Srelpool(j,i) = soilstate2%Srelpool(NINT(slcdata(1,j)),i)
              ENDIF
              IF(ALLOCATED(soilstate%partT1)) THEN
                   soilstate%partT1(:,j,i) = soilstate2%partT1(:,NINT(slcdata(1,j)),i)
              ENDIF
              IF(ALLOCATED(soilstate%surface)) THEN
                   soilstate%surface(:,j,i) = soilstate2%surface(:,NINT(slcdata(1,j)),i)  
              ENDIF
          ENDIF
          IF(ALLOCATED(soilstate%icelens))  soilstate%icelens(j,i) = soilstate2%icelens(NINT(slcdata(1,j)),i)
          !>aquiferstate no
          !>riverstate no
          !>lakestate no
          !>miscstate
          IF(conduct%growthdegreeday)THEN
              miscstate%gdd(:,j,i) = miscstate2%gdd(:,NINT(slcdata(1,j)),i)
              miscstate%gsbegin(:,j,i) = miscstate2%gsbegin(:,NINT(slcdata(1,j)),i)
          ENDIF
          IF(conduct%irrigation)THEN
              miscstate%nextirrigation(j,i) = miscstate2%nextirrigation(NINT(slcdata(1,j)),i)
              IF(numsubstances>0)THEN
                miscstate%cnextirrigation(:,j,i) = miscstate2%cnextirrigation(:,NINT(slcdata(1,j)),i)
              ENDIF
          ENDIF
          IF(conduct%simT1)THEN
              miscstate%partT1sf(j,i) = miscstate2%partT1sf(NINT(slcdata(1,j)),i)
          ENDIF
          
      ENDDO !j
  ENDDO !i

  CALL partly_deallocate_model_states(frozenstate2,soilstate2,miscstate2)
  IF(ALLOCATED(slcdata)) DEALLOCATE(slcdata)
...
END SUBROUTINE inherit_state_to_new_class
  ...
  !>may be changed.
  !------------------------------------------------------------------
  SUBROUTINE set_model_base_configuration(basemodel,idir,idir2,dim,status)
  
  USE WORLDVAR, ONLY : doens,doopt,instate,&
                       doAssimilation, &
                       BASEMODELTYPE
  USE MODVAR, ONLY : conductregest, &
                     def_min,def_max, &
                     i_in,i_on,i_sp,i_pp,i_oc,i_ss,i_ae,i_dsi,i_asi,i_t1,i_t2, &
                     STATEDIMENSIONTYPE, &
                     nclass, &
  ...
  INTEGER helparray(basemodel%nsub)
  INTEGER dimrivlag
  INTEGER nparens   !number of par_nnn.txt files
  INTEGER nsbase  !number of subbasin
  ...
  !>Initialisations
  status = 0
  nsbase = basemodel%nsub
  helparray = 0   !Will not be used since nsbase is both nsub arguments of load_modeldefined_input
  ...
  IF(doAssimilation)THEN
    IF(.NOT.ALLOCATED(dim%substancelimits)) ALLOCATE(dim%substancelimits(numsubstances,2))
    DO i = 1,numsubstances
      IF(i==i_t2)THEN 
        dim%substancelimits(i,:) = (/def_min,def_max/)
      ELSEIF(i==i_t1)THEN
        dim%substancelimits(i,:) = (/def_min,def_max/)  !Unlimited, unnecessary?
      ELSE  !i_in,i_on,i_sp,i_pp,i_oc,i_ss,i_ae,i_dsi,i_asi
        dim%substancelimits(i,:) = (/0.,def_max/)  !Positive
      ENDIF
    ENDDO
  ENDIF

  !>Check if river queue variables is compatible with initial state and calibration
  IF(doopt.AND.instate%fromfile.AND.(optrivvel<9999. .OR. optdamp<9999.))THEN
  ...
  SUBROUTINE load_branchdata(dir,status) 
    ...
    USE MODEL_TEST_ROUTINES, ONLY : propagate_external_msg,e_branchdata, &
                                    e_error,e_warning,e_info
    ...
    IF(status.NE.0) THEN
      ...
      CALL propagate_external_msg(e_branchdata,e_error,propagate_str)
      RETURN
    ENDIF
    ...
    !Deallocate local arrays (for gfortran), in opposite order
    IF(ALLOCATED(xr)) DEALLOCATE(xr)
  ...
  SUBROUTINE load_aquiferdata(dir,nsub,numaqu,status) 
    ...
    USE MODVAR, ONLY : basin,     &
                       ...
                       modeloption, &
                       p_deepgroundwater
    USE MODEL_TEST_ROUTINES, ONLY : propagate_external_msg,e_aquiferdata, &
                                    e_error,e_warning,e_info
    ...
    IF(fex)THEN
      IF(modeloption(p_deepgroundwater)/=2)THEN
        IF(conductwarning) WRITE(6,*) 'Warning: Existing AquiferData.txt will not be used, since modeloption not set.'
        RETURN
      ENDIF
    ELSE
      IF(modeloption(p_deepgroundwater)==2)THEN
    ...
    IF(status.NE.0) THEN
      WRITE(6,*) 'ERROR reading heading in file: ',TRIM(filename)
      propagate_str = 'reading heading in file: '//TRIM(filename)
      CALL propagate_external_msg(e_aquiferdata,e_error,propagate_str)
      RETURN
    ENDIF
    ...
    DO i = 1,ncols
      ...
      ENDIF
    ENDDO
    CALL propagate_external_msg(e_aquiferdata,e_info,TRIM(propagate_str),code)
    ...
    DO i = 1,nrows
      IF(xi(i,subid_i)>0)THEN   !Row with aquifer subbasin coupling information
        ...
        ELSE
          IF(conductwarning) WRITE(6,*) 'WARNING: subid in AquiferData not in GeoData', xi(i,subid_i)
        ENDIF
  ...
  SUBROUTINE load_glacierdata(dir,nsb,status) 
    ...
    USE MODVAR, ONLY : basin, &
                       ...
                       glacierindex, &  !OUT
                       missing_value
    USE MODEL_TEST_ROUTINES, ONLY : propagate_external_msg,e_glacierdata, &
                                    e_error,e_warning,e_info
    ...
    IF(status.NE.0) THEN
      WRITE(6,*) 'ERROR reading heading in file: ',TRIM(filename)
      propagate_str = 'reading heading in file: '//TRIM(filename)
      CALL propagate_external_msg(e_glacierdata,e_error,TRIM(propagate_str))
      RETURN
    ENDIF
    ...
    CALL propagate_external_msg(e_glacierdata,e_info,TRIM(propagate_str),code)
  ...
  !>Module modvar variables basin and conductdamconstruction may be changed.
  !------------------------------------------------------------------
  SUBROUTINE load_lakedata(funit,dir,n,lakedataid,status) 
    ...
    USE MODEL_TEST_ROUTINES, ONLY : propagate_external_msg,e_lakedata, &
                                    e_error,e_warning,e_info
...
!    INTEGER,PARAMETER :: i_llbasin = 4           !Lakedata type; last lake basin
    ...
    LOGICAL fex
    LOGICAL areaok
    LOGICAL lddamconstruction,lddamconstruction2
    INTEGER maxcol
    ...
    INTEGER,ALLOCATABLE :: xi(:,:)            !Integer data read from file
    CHARACTER(LEN=100),ALLOCATABLE :: xs(:,:)  !String data read from file (dates)
    REAL,ALLOCATABLE :: xr(:,:)               !Real data read from file
    ...
    REAL,ALLOCATABLE :: flushbeg(:),eflushbeg(:)           !Flushing begin fraction/dayno
    REAL,ALLOCATABLE :: flushfreq(:),eflushfreq(:)         !Flushing frequency
    REAL,ALLOCATABLE :: flushvol(:),eflushvol(:)           !Flushing volume fraction to restore
    REAL,ALLOCATABLE :: flushper(:),eflushper(:)           !Flushing period (days)
    ...
    IF(.NOT.fex)THEN
      IF(ANY(lakedataid(:)>0))THEN
        ...
        CALL propagate_external_msg(e_lakedata,e_error,propagate_str)
      ELSE
        propagate_str = 'LakeData.txt not found, but not required'
        CALL propagate_external_msg(e_lakedata,e_info,propagate_str)
      ENDIF
      RETURN
    ENDIF
    IF(slc_olake==0)THEN
    ...
    IF(status/=0)RETURN
    IF(nrows==0)THEN
      IF(conductwarning) WRITE(6,*) 'WARNING: No lakes in LakeData.txt ',TRIM(filename)
      propagate_str = 'No lakes in LakeData.txt '//TRIM(filename)
      CALL propagate_external_msg(e_lakedata,e_warning,propagate_str)
      RETURN
    ENDIF
    ...
    IF(.NOT.ALLOCATED(sindex)) ALLOCATE(sindex(maxcol))
    IF(.NOT.ALLOCATED(xi)) ALLOCATE(xi(nrows,maxcol))
    IF(.NOT.ALLOCATED(xr)) ALLOCATE(xr(nrows,maxcol))
    IF(.NOT.ALLOCATED(xs)) ALLOCATE(xs(nrows,maxcol))
    ...
    DO i = 1,ncols
      ...
      IF(str(i)(1:letters)=='builddam  ') code(i) = i_str   !dam_builddam
      IF(str(i)(1:letters)=='removedam ') code(i) = i_str   !dam_removedam
      IF(str(i)(1:letters)=='sm_resop  ') code(i) = i_intg   !reservoir operation model
      IF(str(i)(1:letters)=='sm_mode   ') code(i) = i_intg   !flush parameters
      IF(str(i)(1:letters)=='sm_start  ') code(i) = i_real
      IF(str(i)(1:letters)=='sm_yfreq  ') code(i) = i_real
      IF(str(i)(1:letters)=='sm_caprest') code(i) = i_real
      IF(str(i)(1:letters)=='sm_duraday') code(i) = i_real
      IF(str(i)(1:letters)=='sm_dest   ') code(i) = i_intg
      ...
      ENDDO
      IF(code(i) == i_str .AND. (.NOT.(str(i)(1:letters)=='removedam '.OR.str(i)(1:letters)=='builddam  '))) THEN
!      IF(code(i) == i_str) THEN
        propagate_str_extra=propagate_str_extra//','//TRIM(str(i)(1:letters))
      ENDIF
    ENDDO
    CALL propagate_external_msg(e_lakedata,e_info,propagate_str_extra,code)
    IF(ALLOCATED(propagate_str_extra)) DEALLOCATE(propagate_str_extra)
...
!    CALL read_basindata5(funit,filename,maxcol,nrows,ncols,code,rindex,iindex,xi,xr)
    CALL read_basindata6(funit,filename,maxcol,nrows,ncols,100,code,rindex,iindex,sindex,xi,xr,xs) 
    CLOSE(UNIT=funit)
    ...
    IF(.NOT.ALLOCATED(flushbeg)) ALLOCATE(flushbeg(nlake))
    IF(.NOT.ALLOCATED(flushfreq)) ALLOCATE(flushfreq(nlake))
    IF(.NOT.ALLOCATED(flushvol)) ALLOCATE(flushvol(nlake))
    IF(.NOT.ALLOCATED(flushper)) ALLOCATE(flushper(nlake))
      ...
      DO j = 1,nrows
        IF(lakedataid(i)==xi(j,iindex(iclakedataid)))THEN
          ...
          IF(.NOT.(xi(j,iindex(ictype)) == i_simple .OR. (xi(j,iindex(ictype)) >= i_ldout1 .AND. xi(j,iindex(ictype)) <= i_ebasin))) THEN
            ...
            CALL propagate_external_msg(e_lakedata,e_error,TRIM(propagate_str))
          ENDIF
    ...
    IF(nlakeeb>0)THEN
      ...
      ALLOCATE(eflushbeg(nlakeeb))
      ALLOCATE(eflushfreq(nlakeeb))
      ALLOCATE(eflushvol(nlakeeb))
      ALLOCATE(eflushper(nlakeeb))
      noutlet = 0
      outletsofelake = 0
        ...
        DO ioutlet = 1,elake(ilakee)%noutlet
          DO i = 1,ncols
            IF(ioutlet==1)THEN
              ...
              IF(str(i)=='sm_resop  ')   elake(ilakee)%resmode     = xi(outletsofelake(ilakee,ioutlet),iindex(i))   !reservoir operation model
              IF(str(i)=='sm_mode   ')   elake(ilakee)%sedmgmt%mode = xi(outletsofelake(ilakee,ioutlet),iindex(i))
              IF(str(i)=='sm_dest   ')   elake(ilakee)%sedmgmt%dest = xi(outletsofelake(ilakee,ioutlet),iindex(i))
              IF(str(i)=='sm_start  ')   eflushbeg(ilakee) = xr(outletsofelake(ilakee,ioutlet),rindex(i))
              IF(str(i)=='sm_yfreq  ')   eflushfreq(ilakee) = xr(outletsofelake(ilakee,ioutlet),rindex(i))
              IF(str(i)=='sm_caprest')   eflushvol(ilakee) = xr(outletsofelake(ilakee,ioutlet),rindex(i))
              IF(str(i)=='sm_duraday')   eflushper(ilakee) = xr(outletsofelake(ilakee,ioutlet),rindex(i))
            ENDIF
            ...
            IF(str(i)=='limqprod  ')   elake(ilakee)%outlet(ioutlet)%limprod   = xr(outletsofelake(ilakee,ioutlet),rindex(i))
             IF(str(i)=='builddam ')THEN
              IF(xs(j,sindex(i))(1:5)=='-9999')THEN
                !WRITE(6,*) i,xs(j,sindex(i))(1:5)
              ELSE
                CALL string_convert_to_datetype(xs(j,sindex(i)),elake(ilakee)%btdate(1))
                lddamconstruction = .TRUE.
              ENDIF
            ENDIF
            IF(str(i)=='removedam')THEN
              IF(xs(j,sindex(i))(1:5)=='-9999')THEN
                !WRITE(6,*) i,xs(j,sindex(i))(1:5)
              ELSE
                CALL string_convert_to_datetype(xs(j,sindex(i)),elake(ilakee)%btdate(2))
                lddamconstruction = .TRUE.
              ENDIF
            ENDIF
         ENDDO
      ...
      DO ilakee = 1,nlakeeb
        ...
        ENDDO
        IF(elake(ilakee)%sedmgmt%mode == 1)THEN
          elake(ilakee)%sedmgmt%start = eflushbeg(ilakee) 
          elake(ilakee)%sedmgmt%sstop = eflushbeg(ilakee) + eflushvol(ilakee)
          elake(ilakee)%sedmgmt%rate = eflushvol(ilakee)/eflushper(ilakee) 
        ELSEIF(elake(ilakee)%sedmgmt%mode == 2)THEN
          elake(ilakee)%sedmgmt%start = eflushbeg(ilakee) 
          elake(ilakee)%sedmgmt%sstop = eflushbeg(ilakee) + eflushper(ilakee)  !Check if >365
          IF(elake(ilakee)%sedmgmt%sstop > 365) elake(ilakee)%sedmgmt%sstop = elake(ilakee)%sedmgmt%sstop - 365
          elake(ilakee)%sedmgmt%rate = eflushvol(ilakee)/eflushper(ilakee) 
          elake(ilakee)%sedmgmt%frequency = NINT(eflushfreq(ilakee))
        ENDIF
      ENDDO
    ...
    DO j = 1,nrows
      ...
      IF(xi(j,iindex(ictype))==i_simple)THEN     !single lake
        ...
        IF(ica>0)THEN
          ...
          IF(lakearea>0.)THEN
            parea = ABS(xr(j,rindex(ica))-lakearea)/lakearea
          ELSEIF(xr(j,rindex(ica))>0.)THEN
            parea = 1.
          ENDIF
          areaok = areaok .AND. (parea <= 0.005)
        ENDIF
        IF(icdepth>0)THEN
          IF(xr(j,rindex(icdepth)).NE.missing_value) basin(lakeindex2(j))%lakedepth(2) = xr(j,rindex(icdepth))
        ENDIF
        DO i = 1,ncols
          ...
          IF(str(i)=='sm_resop  ')   lake(ilake)%resmode  = xi(j,iindex(i))   !reservoir operation model
          IF(str(i)=='sm_mode   ')   lake(ilake)%sedmgmt%mode     = xi(j,iindex(i))   !flush parameters
          IF(str(i)=='sm_dest   ')   lake(ilake)%sedmgmt%dest     = xi(j,iindex(i))
          IF(str(i)=='sm_start  ')   flushbeg(ilake)     = xr(j,rindex(i))
          IF(str(i)=='sm_yfreq  ')   flushfreq(ilake)    = xr(j,rindex(i))
          IF(str(i)=='sm_caprest')   flushvol(ilake)     = xr(j,rindex(i))
          IF(str(i)=='sm_duraday')   flushper(ilake)     = xr(j,rindex(i))
          ...
          IF(str(i)=='builddam ')THEN
            IF(xs(j,sindex(i))(1:5)=='-9999')THEN
              !WRITE(6,*) i,xs(j,sindex(i))(1:5)
            ELSE
              CALL string_convert_to_datetype(xs(j,sindex(i)),lake(ilake)%btdate(1))
              lddamconstruction = .TRUE.
            ENDIF
          ENDIF
          IF(str(i)=='removedam')THEN
            IF(xs(j,sindex(i))(1:5)=='-9999')THEN
              !WRITE(6,*) i,xs(j,sindex(i))(1:5)
            ELSE
              CALL string_convert_to_datetype(xs(j,sindex(i)),lake(ilake)%btdate(2))
              lddamconstruction = .TRUE.
            ENDIF
          ENDIF
        ENDDO
...
!              WRITE(6,*) i,xs(j,sindex(i))(1:5)
      ...
      ENDIF
    ENDDO
    IF(ilake<nlake .AND. conductwarning)THEN
    ...
    ENDIF
    IF(.NOT.areaok)THEN
      IF(conductwarning) WRITE(6,*) 'WARNING: Some lakes in LakeData has an area that differ more than 0.5% from the one in GeoData.'
    ENDIF
    ...
    DO ilake = 1,nlake 
      ...
      IF(lake(ilake)%sedmgmt%mode == 1)THEN
        lake(ilake)%sedmgmt%start = flushbeg(ilake) 
        lake(ilake)%sedmgmt%sstop = flushbeg(ilake) + flushvol(ilake)
        lake(ilake)%sedmgmt%rate = flushvol(ilake)/flushper(ilake) 
      ELSEIF(lake(ilake)%sedmgmt%mode == 2)THEN
        lake(ilake)%sedmgmt%start = flushbeg(ilake) 
        lake(ilake)%sedmgmt%sstop = flushbeg(ilake) + flushper(ilake)  !Check if>365!
        IF(lake(ilake)%sedmgmt%sstop > 365) lake(ilake)%sedmgmt%sstop = lake(ilake)%sedmgmt%sstop - 365
        lake(ilake)%sedmgmt%rate = flushvol(ilake)/flushper(ilake) 
        lake(ilake)%sedmgmt%frequency = NINT(flushfreq(ilake))
      ENDIF
    ENDDO

    !Check for dam construction in simulation period
    lddamconstruction2 = .FALSE.
    IF(lddamconstruction)THEN
      DO ilake = 1,nlake
        IF(lake(ilake)%btdate(1).NE.DateType(0,0,0,0,0))THEN
          lddamconstruction2 = .TRUE.
          !Recalculate lakedepth if dam is build later than bdate
          IF(lake(ilake)%btdate(1).GT.bdate )THEN
            DO j = 1,nrows
              IF(lakeindex3(j)==ilake)EXIT
            ENDDO
            i = lakeindex2(j)
            basin(i)%lakedepth(2) = basin(i)%lakedepth(2) + lake(ilake)%wmin
            lake(ilake)%w0ref = lake(ilake)%w0ref + lake(ilake)%wmin
          ENDIF
        ENDIF
        IF(lake(ilake)%btdate(2).NE.DateType(0,0,0,0,0))THEN
          !Check dates in sim period
          lddamconstruction2 = .TRUE.
          !Set lakedepth to lakedepth-wmin if dam is removed before bdate
          IF(lake(ilake)%btdate(2).LE.bdate) THEN
            DO j = 1,nrows
              IF(lakeindex3(j)==ilake)EXIT
            ENDDO
            i = lakeindex2(j)
      ...
      ENDDO
      DO ilakee =  1,nlakeeb
        IF(elake(ilakee)%btdate(1).NE.DateType(0,0,0,0,0))THEN
          lddamconstruction2 = .TRUE.
          IF(elake(ilakee)%btdate(1).GT.bdate )THEN
            !Recalculate lakedepth if dam is build later than bdate
            DO iebasin = 1,nebasin
              IF(ilakee == lakebasin(iebasin)%ilk)THEN
                i = lakeindex2(lakeindex7(iebasin)) !isb
                basin(i)%lakedepth(2) = basin(i)%lakedepth(2) + elake(ilakee)%wmin
              ENDIF
            ENDDO
            elake(ilakee)%w0ref = elake(ilakee)%w0ref + elake(ilakee)%wmin
          ENDIF
        ENDIF
        IF(elake(ilakee)%btdate(2).NE.DateType(0,0,0,0,0))THEN
          lddamconstruction2 = .TRUE.
          !Set lakedepth to lakedepth-wmin if dam is removed before bdate
          IF(elake(ilakee)%btdate(2).LT.bdate)THEN
            DO iebasin = 1,nebasin
              IF(ilakee == lakebasin(iebasin)%ilk)THEN
                i = lakeindex2(lakeindex7(iebasin)) !isb
                basin(i)%lakedepth(2) = basin(i)%lakedepth(2) + elake(ilakee)%wmin
      ...
      ENDDO
   ENDIF
    IF(lddamconstruction2) conductdamconstruction = .TRUE. !Set global variable
    ...
    !Deallocate local variables, in opposite order
    IF(ALLOCATED(eflushper)) DEALLOCATE(eflushper)
    IF(ALLOCATED(eflushvol)) DEALLOCATE(eflushvol)
    IF(ALLOCATED(eflushfreq)) DEALLOCATE(eflushfreq)
    IF(ALLOCATED(eflushbeg)) DEALLOCATE(eflushbeg)
    ...
    IF(ALLOCATED(flushper)) DEALLOCATE(flushper)
    IF(ALLOCATED(flushvol)) DEALLOCATE(flushvol)
    IF(ALLOCATED(flushfreq)) DEALLOCATE(flushfreq)
    IF(ALLOCATED(flushbeg)) DEALLOCATE(flushbeg)
  ...
  !>Module modvar variable basin and conductdamconstruction may be changed (lake_depth).
  !------------------------------------------------------------------
  SUBROUTINE load_damdata(funit,dir,n,status)
    ...
    !Local variables
    LOGICAL fex
    LOGICAL dddamconstruction,dddamconstruction2
    INTEGER i,idam
    ...
    REAL :: damheight(n)           !Dam height from regvol (m)
    ...
    IF(ALLOCATED(damindex)) DEALLOCATE(damindex)
    IF(.NOT.ALLOCATED(damindex)) ALLOCATE(damindex(n))
    damindex = 0
    ...
    IF(status/=0)RETURN
    IF(nrows==0)THEN
      IF(conductwarning) WRITE(6,*) 'WARNING: No dam in DamData.txt ',TRIM(filename)
    ...
    CALL propagate_external_msg(e_damdata,e_info,propagate_str_extra,code)
    IF(ALLOCATED(propagate_str_extra)) DEALLOCATE(propagate_str_extra)
...
!    CALL read_basindata5(funit,filename,maxcol,nrows,ncols,code,rindex,iindex,xi,xr)
    ...
    DO idam = 1,nrows
      DO i = 1,ncols
        ...
        IF(str(i)=='builddam ')THEN
          IF(xs(idam,sindex(i))(1:5)=='-9999')THEN
!            WRITE(6,*) i,xs(idam,sindex(i))(1:5)
          ELSE
            CALL string_convert_to_datetype(xs(idam,sindex(i)),dam(idam)%btdate(1))
            dddamconstruction = .TRUE.
          ENDIF
        ENDIF
        IF(str(i)=='removedam')THEN
          IF(xs(idam,sindex(i))(1:5)=='-9999')THEN
!            WRITE(6,*) i,xs(idam,sindex(i))(1:5)
          ELSE
            CALL string_convert_to_datetype(xs(idam,sindex(i)),dam(idam)%btdate(2))
            dddamconstruction = .TRUE.
          ENDIF
        ENDIF
      ENDDO
    ...
    DO i = 1,n
      IF(damindex(i)>0)THEN
        ...
        IF(damarea>0)THEN
          damheight(i) = dam(damindex(i))%regvol * 1000000. / damarea
          dam(damindex(i))%wmin = 0. - damheight(i)
!          dam(damindex(i))%wmin = 0. - dam(damindex(i))%regvol * 1000000. / damarea
          IF(wamp(damindex(i))/=missing_value) &
              dam(damindex(i))%wampcoeff = wamp(damindex(i))/damheight(i)
!              dam(damindex(i))%wampcoeff = wamp(damindex(i))/(-1.*dam(damindex(i))%wmin)
    ...
    !Check for dam construction witin simulation period
    dddamconstruction2 = .FALSE.
    IF(dddamconstruction)THEN
      DO i = 1,n
        IF(damindex(i)>0)THEN
          idam = damindex(i)
          IF(dam(idam)%btdate(1)/=DateType(0,0,0,0,0))THEN
            dddamconstruction2 = .TRUE.   !Needed for wmin handling if build later than sdate
            !Recalculate lakedepth if dam is build later than bdate
            IF(dam(idam)%btdate(1).GT.bdate)THEN
              IF(damheight(i)/=missing_value)THEN
                dam(idam)%w0ref = dam(idam)%w0ref - damheight(i)
                basin(i)%lakedepth(2) = basin(i)%lakedepth(2) - damheight(i)
              ENDIF
            ENDIF
          ENDIF
          IF(dam(idam)%btdate(2)/=DateType(0,0,0,0,0))THEN
            dddamconstruction2 = .TRUE.
            !Set lakedepth to lake depth if dam is removed before bdate
            IF(dam(idam)%btdate(2).LE.bdate)THEN
              IF(damheight(i)/=missing_value)THEN
                dam(idam)%w0ref = dam(idam)%w0ref - damheight(i)
                basin(i)%lakedepth(2) = basin(i)%lakedepth(2) - damheight(i)
                dam(idam)%wmin = missing_value
                dam(idam)%regvol = 0.
                !set wmin to missing? later
              ENDIF
            ENDIF
          ENDIF
        ENDIF
      ENDDO
    ENDIF
    IF(dddamconstruction2) conductdamconstruction = .TRUE. !Set global variable

    !Deallocate local variables, in opposite order
    IF(ALLOCATED(damindex2)) DEALLOCATE(damindex2)
    IF(ALLOCATED(qinflow)) DEALLOCATE(qinflow)
    IF(ALLOCATED(wamp)) DEALLOCATE(wamp)
    IF(ALLOCATED(qprod2date)) DEALLOCATE(qprod2date)
    IF(ALLOCATED(qprod1date)) DEALLOCATE(qprod1date) 
    IF(ALLOCATED(xs)) DEALLOCATE(xs)
  ...
  SUBROUTINE load_flooddata(funit,dir,n,status)
    ...
    IF(modeloption(p_floodplain)==0)THEN
      IF(conductwarning) WRITE(6,*) 'WARNING: floodmodel not set in info.txt. Simulation will not simulate floodplains or use FloodData.txt'
      RETURN
    ENDIF
    ...
    IF(status/=0)RETURN
    IF(nrows==0)THEN
      IF(conductwarning) WRITE(6,*) 'WARNING: No flood areas in FloodData.txt ',TRIM(filename)
      RETURN
    ENDIF
    ...
    CALL propagate_external_msg(e_flooddata,e_info,propagate_str_extra,code)
    IF(ALLOCATED(propagate_str_extra)) DEALLOCATE(propagate_str_extra)
  ...
  !>\b Consequences Module variables will be allocated and set
  !------------------------------------------------------------------------------
  SUBROUTINE read_and_calc_basindata(funit,infile,n,lakedataid,maxcol) 
...
!    INTEGER, INTENT(OUT) :: mcols                  !<Actual number of columns
    ...
    DO i = 1,mcols
      ...
      IF(str(i)(1:4)=='scr_') THEN    !area part with secondary crop
        ...
        ENDIF          
      ENDIF
      IF(str(i)(1:7)=='sloslc_') THEN      !slope, not used in HYPE, other model?
        idslc = 0
        ...
        ELSE
          IF(conductwarning) WRITE(6,*) 'WARNING: sloslc larger than nclass skipped: ', TRIM(str(i))
          propagate_str = 'sloslc larger than nclass skipped: '//TRIM(str(i))
          CALL propagate_external_msg(e_geodata,e_warning,propagate_str)
        ENDIF          
      ENDIF
      IF(str(i)(1:7)=='aspslc_') THEN    !aspect, not used in HYPE, other model?
        idslc = 0
        IF(ICHAR(str(i)(8:8))>=49 .AND. ICHAR(str(i)(8:8))<=57)THEN
          idslc = ICHAR(str(i)(8:8)) - 48
        ENDIF
        ...
        ELSE
          IF(conductwarning) WRITE(6,*) 'WARNING: aspslc larger than nclass skipped: ', TRIM(str(i))
          propagate_str = 'aspslc larger than nclass skipped: '//TRIM(str(i))
          CALL propagate_external_msg(e_geodata,e_warning,propagate_str)
        ENDIF          
      ENDIF
      IF(str(i)(1:3)=='ws_') THEN   !wsf
  ...
  !>\brief Gets the information about atmospheric deposition from AtmdepData.
  !!
  !>\b Consequences Module variables deposition,deposition2 may be allocated and set
  !------------------------------------------------------------------------------
  SUBROUTINE load_deposition(funit,dir,n,status) 
    ...
    USE WORLDVAR, ONLY : i_str, &
                         ...
                         maxcharpath
    USE MODVAR, ONLY : timesteps_per_day, &
                       deposition, &     !may be deallocated
                       deposition2, &     !OUT
                       i_in,i_sp,i_pp, & !i_t1,i_oc,  &
                       basin,        &  
                       nluse, &
                       numsubstances, &
                       substance_name
    ...
    USE MODEL_TEST_ROUTINES, ONLY : propagate_external_msg,e_atmdepdata,e_error,e_info
    ...
    !Local parameters
    INTEGER, PARAMETER :: letters = 15  !Max number of letters (read) in heading; IN_DD_D365_L999
    CHARACTER(LEN=2),DIMENSION(2),PARAMETER :: depcode = (/'dd','wd'/)  !column heading code; drydep and wetdep

    !Local variables
    LOGICAL fexist,icolfound
    INTEGER i,j,isubst,idep,itime,iveg
    INTEGER currentsubst
    INTEGER subidcol
    INTEGER maxcol
    INTEGER mcols,nrows                !Actual number of columns/rows
    INTEGER filebasin(n),fileindex(n)  !subid of file, order of subid in file
    INTEGER timesize(numsubstances,2),vegsize(numsubstances,2)  !present 
    INTEGER, ALLOCATABLE :: xi(:,:)    !Integer data read from file
    INTEGER, ALLOCATABLE :: iindex(:)  !Index for column integer variables
    INTEGER, ALLOCATABLE :: rindex(:)  !Index for column real variables
    INTEGER, ALLOCATABLE :: code(:)    !Code for column variable
    INTEGER, ALLOCATABLE :: varcode(:,:,:)    !Code for vegetation dependence and time dependence (veg/time,ns,dry/wet)
    REAL, ALLOCATABLE :: xr(:,:)       !Real data read from file
    REAL    onetspday                  !Reciprocal of timesteps per day
    CHARACTER(LEN=letters),ALLOCATABLE :: str(:)      !Content string
    CHARACTER(LEN=maxcharpath) infile
    CHARACTER(LEN=150) :: propagate_str                  !Propagate string
    CHARACTER(LEN=:),ALLOCATABLE :: propagate_str_extra  !Propagate string extra
    ...
    !>Check for file 
    infile = TRIM(dir)//'AtmdepData.txt'
    INQUIRE(FILE = TRIM(infile), EXIST = fexist) 

    !Use GeoData/par deposition if no file found
    IF(.NOT.fexist)THEN
      IF(ALLOCATED(deposition%inwetconc).OR.&
         ALLOCATED(deposition%indryload).OR.&
         ALLOCATED(deposition%inloadwater))THEN

        !Allocate deposition structure, set size/dependence and initialise values
        ALLOCATE(deposition2(numsubstances))
        DO isubst = 1,numsubstances
          ALLOCATE(deposition2(isubst)%depcode(2,2)) !(veg/time,dry/wet)
          deposition2(isubst)%depcode(:,:) = 0
          !Set values from GeoData
          IF(isubst==i_in)THEN
            IF(ALLOCATED(deposition%inwetconc))THEN
              deposition2(isubst)%depcode(1,2) = 1  !constant for veg and time
              deposition2(isubst)%depcode(2,2) = 1
              ALLOCATE(deposition2(isubst)%wetconc(1,n,1))
              deposition2(isubst)%wetconc(1,:,1) = deposition%inwetconc    !mg/L
              DEALLOCATE(deposition%inwetconc)
            ENDIF
            IF(ALLOCATED(deposition%indryload))THEN
              deposition2(isubst)%depcode(1,1) = 2  !use three veg type and constant for time  
              deposition2(isubst)%depcode(2,1) = 1
              ALLOCATE(deposition2(isubst)%dryload(3,n,1))
              DO iveg = 1,3
                deposition2(isubst)%dryload(iveg,:,1) = deposition%indryload(:,iveg)  !kg/km2/ts
              ENDDO
              DEALLOCATE(deposition%indryload)
            ENDIF
            IF(ALLOCATED(deposition%inloadwater))THEN
              deposition2(isubst)%depcode(1,1) = 2  !use three veg type and monthly for time
              deposition2(isubst)%depcode(2,1) = 2
              ALLOCATE(deposition2(isubst)%dryload(3,n,12))
              DO iveg = 1,2
                deposition2(isubst)%dryload(iveg,:,:) = 0.  !no load on land classes
              ENDDO
              deposition2(isubst)%dryload(3,:,:) = deposition%inloadwater(:,:)
              DEALLOCATE(deposition%inloadwater)
            ENDIF
          ENDIF
          !Prepare for setting values from par.txt
          IF(isubst==i_pp)THEN
            deposition2(isubst)%depcode(1,1) = 3  !land use and constant for time
            deposition2(isubst)%depcode(2,1) = 1
            IF(ALLOCATED(deposition2)) DEALLOCATE(deposition2)
            IF(.NOT. ALLOCATED(deposition2)) ALLOCATE(deposition2(isubst)%dryload(nluse,n,1))
            deposition2(isubst)%dryload = 0.
            !par.txt not read yet, drydeppp, set later in finish_atmdep_file_deposition
          ENDIF
          IF(isubst==i_sp)THEN
            !wetsp as load and conc: wetdepsp, wetdepspl, general parameters
            deposition2(isubst)%depcode(1,2) = 1  !constant for veg and time
            deposition2(isubst)%depcode(2,2) = 1
            ALLOCATE(deposition2(isubst)%wetconc(1,n,1))
            deposition2(isubst)%wetconc = 0.
            deposition2(isubst)%depcode(1,1) = 2  !veg type water and constant for time
            deposition2(isubst)%depcode(2,1) = 1
            ALLOCATE(deposition2(isubst)%dryload(3,n,1))
            deposition2(isubst)%dryload = 0.
          ENDIF
    ...
    !Count rows
    CALL count_data_rows(funit,infile,1,nrows,status)
    IF(status/=0) RETURN
    IF(n/=nrows)THEN
      WRITE(6,*) 'ERROR: Not all subbasins found in AtmdepData.txt'
      status = 1
      RETURN
    ENDIF
    
    OPEN(UNIT = funit,FILE = infile, STATUS = 'old', ACTION='read')     !Open AtmdepData-file
    WRITE(6,*) 'File opened: ', TRIM(infile)
    ...
    IF(status.NE.0) THEN
      WRITE(6,*) 'ERROR reading file: ',TRIM(infile)
      propagate_str = 'reading file: '//TRIM(infile)
      CALL propagate_external_msg(e_atmdepdata,e_error,propagate_str)
      RETURN
    ENDIF
    ...
    ALLOCATE(xi(n,mcols))
    ALLOCATE(xr(n,mcols))
    ALLOCATE(iindex(mcols))
    ALLOCATE(rindex(mcols))
    ALLOCATE(varcode(2,2,numsubstances))  ! (veg/time,ns,dry/wet)

    IF(.NOT.ALLOCATED(propagate_str_extra)) ALLOCATE(CHARACTER(LEN=(13*maxcol)) :: propagate_str_extra)
    propagate_str_extra = 'columns ignored: -'

    !Code variables for easy finding of variable type
    !Set variable size depending on column name time and vegetation code
    code = i_str    !string ignore
    timesize = 0; vegsize = 0
    varcode = 0
    DO i = 1,mcols
      IF(str(i)(1:letters)=='subid          ')THEN
        subidcol = i
      ...
      ENDIF
      DO isubst = 1,numsubstances
        IF(str(i)(1:3)==get_lower_case(2,substance_name(isubst))//'_')THEN
          DO idep = 1,2
            IF(str(i)(4:5)==depcode(idep))THEN
              code(i) = i_real
              IF(str(i)(6:6)=='_')THEN
                IF(str(i)(7:7)=='m')THEN
                  timesize(isubst,idep) = 12
                  varcode(2,idep,isubst) = 2  !month
                  IF(str(i)(11:11)=='v')THEN
                    vegsize(isubst,idep) = 3
                    varcode(1,idep,isubst) = 2  !vegtype
                  ELSEIF(str(i)(11:11)=='l')THEN
                    vegsize(isubst,idep) = nluse
                    varcode(1,idep,isubst) = 3  !luse
                  ELSE
                    vegsize(isubst,idep) = 1
                    varcode(1,idep,isubst) = 1  !constant
                  ENDIF
                !ELSEIF(str(i)(7:7)=='d')THEN  !dayno variation not implemeted
                !  timesize(isubst,idep) = 366
                !   IF(str(i)(12:12)=='v')THEN
                !    vegsize(isubst,idep) = 3
                !  ELSEIF(str(i)(12:12)=='l')THEN
                !    vegsize(isubst,idep) = nluse
                !  ELSE
                !    vegsize(isubst,idep) = 1
                !  ENDIF
                ELSEIF(str(i)(7:7)=='v')THEN
                  vegsize(isubst,idep) = 3
                  timesize(isubst,idep) = 1
                  varcode(1,idep,isubst) = 2  !vegtype
                  varcode(2,idep,isubst) = 1  !constant
                ELSEIF(str(i)(7:7)=='l')THEN
                  vegsize(isubst,idep) = nluse
                  timesize(isubst,idep) = 1
                  varcode(1,idep,isubst) = 3  !luse
                  varcode(2,idep,isubst) = 1  !constant
                !ELSE
                !  WRITE(6,*) 'ERROR: column string unknown',str(i)  
                ENDIF
              ELSE
                vegsize(isubst,idep) = 1
                timesize(isubst,idep) = 1
    ...
    !Read all data
    CALL read_basindata5(funit,infile,mcols,n,mcols,code,rindex,iindex,xi,xr)
    CLOSE(UNIT=funit)

    !Check subid column and find its order
    IF(subidcol>0)THEN
      filebasin(1:nrows) = xi(1:nrows,iindex(subidcol))
    ELSE
      WRITE(6,*) 'ERROR: No subid column in AtmdepData.txt'
      status = 1
      RETURN
    ENDIF
    DO i = 1,n
      DO j = 1,nrows
        IF(basin(i)%subid==filebasin(j))THEN
          fileindex(i)=j  !correct order?
          EXIT
        ENDIF
      ENDDO
    ENDDO
    ...
    ALLOCATE(deposition2(numsubstances))
    DO isubst = 1,numsubstances
      ALLOCATE(deposition2(isubst)%depcode(2,2)) !(veg/time,wd)
      deposition2(isubst)%depcode(:,:) = varcode(:,:,isubst)
      IF(vegsize(isubst,1)>0 .AND. timesize(isubst,1)>0)THEN
        ALLOCATE(deposition2(isubst)%dryload(vegsize(isubst,1),n,timesize(isubst,1)))
        deposition2(isubst)%dryload = 0.
      ENDIF
      IF(vegsize(isubst,2)>0 .AND. timesize(isubst,2)>0)THEN
        ALLOCATE(deposition2(isubst)%wetconc(vegsize(isubst,2),n,timesize(isubst,2)))
        deposition2(isubst)%wetconc = 0.
      ENDIF
    ENDDO

    !Set deposition values
    DO i = 1,mcols
      icolfound = .FALSE.
      IF(i==subidcol) CYCLE
      DO isubst = 1,numsubstances
          ...
          IF(str(i)(1:3)==get_lower_case(2,substance_name(isubst))//'_')THEN
            IF(str(i)(4:5)==depcode(idep))THEN
              ...
              IF(str(i)(6:6)=='_')THEN
                !Decode heading
                IF(str(i)(7:7)=='m')THEN
                  CALL convert_unclean_string_to_integer(str(i)(8:9),itime,status) 
                  IF(status/=0)RETURN
                  IF(str(i)(11:11)=='v')THEN
                    CALL convert_unclean_string_to_integer(str(i)(12:12),iveg,status) 
                    IF(status/=0)RETURN
                  ELSEIF(str(i)(11:11)=='l')THEN
                    CALL convert_unclean_string_to_integer(str(i)(12:14),iveg,status) 
                    IF(status/=0)RETURN
                  ELSE
                    iveg = 1
                  ENDIF
                !ELSEIF(str(i)(7:7)=='d')THEN
                !  timesize(isubst,idep) = 366
                !  IF(str(i)(12:12)=='v')THEN
                !  ELSEIF(str(i)(12:12)=='l')THEN
                !  ELSE
                ...
                ELSEIF(str(i)(7:7)=='v')THEN
                  itime = 1
                  CALL convert_unclean_string_to_integer(str(i)(8:8),iveg,status) 
                  IF(status/=0)RETURN
                ELSEIF(str(i)(7:7)=='l')THEN
                  itime = 1
                  CALL convert_unclean_string_to_integer(str(i)(8:10),iveg,status) 
                  IF(status/=0)RETURN
                ENDIF
              ELSE
                itime = 1
        ...
        IF(icolfound) EXIT
      ENDDO
      !Set read data to deposition structure
      IF(icolfound)THEN
        IF(idep == 2) deposition2(currentsubst)%wetconc(iveg,1:n,itime) = xr(fileindex(1:n),rindex(i))
        IF(idep == 1) deposition2(currentsubst)%dryload(iveg,1:n,itime) = xr(fileindex(1:n),rindex(i))
      ELSE
        propagate_str_extra = propagate_str_extra//','//TRIM(str(i))
      ENDIF
    ENDDO
    
    CALL propagate_external_msg(e_atmdepdata,e_info,propagate_str_extra)
    IF(ALLOCATED(propagate_str_extra)) DEALLOCATE(propagate_str_extra)

    !Deallocate local arrays in opposite order
    DEALLOCATE(varcode)
    DEALLOCATE(rindex)
    DEALLOCATE(iindex)
    DEALLOCATE(xr)
    ...
    WRITE(6,*) 'Atmosperic deposition loaded'


  END SUBROUTINE load_deposition

  !>\brief Gets the information about atmospheric deposition from AtmdepData.
  !!
  !>\b Consequences Module variables deposition2 may be reallocated and reset
  !------------------------------------------------------------------------------
  SUBROUTINE reload_deposition(funit,dir,basemodel,n,status,filedate) 

    USE WORLDVAR, ONLY : BASEMODELTYPE, &
                         i_str, &
                         i_intg, &
                         i_real, &
                         maxcharpath, &
    ...
    USE CONVERT, ONLY : get_lower_case, &
                        convert_unclean_string_to_integer
    USE LIBDATE, ONLY : DateType, format_date

    !Argument declarations
    INTEGER, INTENT(IN)  :: funit           !<Unit for file
    CHARACTER (LEN=*), INTENT(IN) :: dir    !<File directory
    TYPE(BASEMODELTYPE), INTENT(INOUT) :: basemodel  !<Information on basemodel
    INTEGER, INTENT(IN)  :: n               !<Number of subbasins
    INTEGER, INTENT(OUT) :: status          !<error status
    TYPE(DateType),OPTIONAL,INTENT(IN) :: filedate  !<date of start of this file
    ...
    !Local variables
    LOGICAL fexist,icolfound
    INTEGER i,j,isubst,idep,itime,iveg,pos
    INTEGER currentsubst
    INTEGER subidcol
    INTEGER maxcol
    INTEGER mcols,nrows                !Actual number of columns/rows
    INTEGER timesize(numsubstances,2),vegsize(numsubstances,2)  !present 
    INTEGER, ALLOCATABLE :: filebasin(:),fileindex(:)  !subid of file, order of subid in file
    INTEGER, ALLOCATABLE :: xi(:,:)    !Integer data read from file
    INTEGER, ALLOCATABLE :: iindex(:)  !Index for column integer variables
    INTEGER, ALLOCATABLE :: rindex(:)  !Index for column real variables
    INTEGER, ALLOCATABLE :: code(:)    !Code for column variable
    ...
    infile = TRIM(dir)//'AtmdepData.txt'
    IF(PRESENT(filedate))THEN
      CALL format_date(filedate,'yyyymmdd',datestr)
      infile = TRIM(dir)//'AtmdepData_'//datestr//'.txt'
    ELSE
      !? read ordinary file, again?
    ENDIF
      
    INQUIRE(FILE = TRIM(infile), EXIST = fexist) 
    IF(.NOT.fexist)THEN
      WRITE(6,*) 'Asked for file does not exist:', TRIM(infile)
      RETURN
    ENDIF

    !Count rows, should be nsub_basemodel but for submodel
    CALL count_data_rows(funit,infile,1,nrows,status)
    IF(status/=0) RETURN
    IF(n>nrows)THEN
      WRITE(6,*) 'ERROR: Not all subbasins found in ',TRIM(infile)
      status = 1
      RETURN
    ELSEIF(n<nrows)THEN
      IF(.NOT.(simsubmodel.AND.nrows==basemodel%nsub))THEN
        WRITE(6,*) 'ERROR: Wrong number of subbasins found in ',TRIM(infile)
        status = 1
        RETURN
      ENDIF
    ENDIF
    ...
    !Allocate and initialise
    IF(.NOT.ALLOCATED(code)) ALLOCATE(code(mcols))
    IF(.NOT.ALLOCATED(xi)) ALLOCATE(xi(nrows,mcols))
    IF(.NOT.ALLOCATED(xr)) ALLOCATE(xr(nrows,mcols))
    IF(.NOT.ALLOCATED(iindex)) ALLOCATE(iindex(mcols))
    IF(.NOT.ALLOCATED(rindex)) ALLOCATE(rindex(mcols))
    IF(.NOT.ALLOCATED(varcode)) ALLOCATE(varcode(2,2,numsubstances))  ! (veg/time,ns,dry/wet)
            ...
            IF(str(i)(4:5)==depcode(idep))THEN
              ...
              IF(str(i)(6:6)=='_')THEN
                IF(str(i)(7:7)=='m')THEN
                  timesize(isubst,idep) = 12
                  varcode(2,idep,isubst) = 2  !month
                  IF(str(i)(8:9)=='10'.OR.str(i)(8:9)=='11'.OR.str(i)(8:9)=='12')THEN
                    pos = 11
                  ELSE
                    pos = 10
                  ENDIF
                  IF(str(i)(pos:pos)=='v')THEN
                    vegsize(isubst,idep) = 3
                    varcode(1,idep,isubst) = 2  !vegtype
                  ELSEIF(str(i)(pos:pos)=='l')THEN
                    vegsize(isubst,idep) = nluse
                    varcode(1,idep,isubst) = 3  !luse
                  ELSE
                    vegsize(isubst,idep) = 1
    ...
    !Read all data
    CALL read_basindata5(funit,infile,mcols,nrows,mcols,code,rindex,iindex,xi,xr)
    CLOSE(UNIT=funit)

    !Check subid column and find its order
    ALLOCATE(filebasin(basemodel%nsub))
    ALLOCATE(fileindex(n))
    IF(subidcol>0)THEN
      filebasin(1:nrows) = xi(1:nrows,iindex(subidcol))
    ELSE
      WRITE(6,*) 'ERROR: No subid column in ',TRIM(infile)
      status = 1
      RETURN
    ENDIF
    DO i = 1,n
    ...
    !Allocate deposition structure, set size/deepndence and initialise values
    IF(.NOT.ALLOCATED(deposition2)) ALLOCATE(deposition2(numsubstances))
    DO isubst = 1,numsubstances
      IF(.NOT.ALLOCATED(deposition2(isubst)%depcode)) ALLOCATE(deposition2(isubst)%depcode(2,2)) !(veg/time,wd)
      deposition2(isubst)%depcode(:,:) = varcode(:,:,isubst)
      IF(vegsize(isubst,1)>0 .AND. timesize(isubst,1)>0)THEN
        IF(ALLOCATED(deposition2(isubst)%dryload))THEN
          IF(vegsize(isubst,1)==SIZE(deposition2(isubst)%dryload,1).AND.timesize(isubst,1)==SIZE(deposition2(isubst)%dryload,3))THEN
            !ok, reuse
          ELSE  !reallocate
            DEALLOCATE(deposition2(isubst)%dryload)
            ALLOCATE(deposition2(isubst)%dryload(vegsize(isubst,1),n,timesize(isubst,1)))
          ENDIF
        ELSE  !new allocate
          ALLOCATE(deposition2(isubst)%dryload(vegsize(isubst,1),n,timesize(isubst,1)))
        ...
        deposition2(isubst)%dryload = 0.
      ELSE  !deallocate
        IF(ALLOCATED(deposition2(isubst)%dryload)) DEALLOCATE(deposition2(isubst)%dryload)
      ENDIF
      IF(vegsize(isubst,2)>0 .AND. timesize(isubst,2)>0)THEN
        IF(ALLOCATED(deposition2(isubst)%wetconc))THEN
          IF(vegsize(isubst,2)==SIZE(deposition2(isubst)%wetconc,1).AND.timesize(isubst,2)==SIZE(deposition2(isubst)%wetconc,3))THEN
            !ok, reuse
          ELSE  !reallocate
            DEALLOCATE(deposition2(isubst)%wetconc)
            ALLOCATE(deposition2(isubst)%wetconc(vegsize(isubst,2),n,timesize(isubst,2)))
          ENDIF
        ELSE  !new allocate
          ALLOCATE(deposition2(isubst)%wetconc(vegsize(isubst,2),n,timesize(isubst,2)))
        ...
        deposition2(isubst)%wetconc = 0.
      ELSE  !deallocate
        IF(ALLOCATED(deposition2(isubst)%wetconc)) DEALLOCATE(deposition2(isubst)%wetconc)
      ENDIF
    ENDDO
            ...
            IF(str(i)(4:5)==depcode(idep))THEN
              ...
              IF(str(i)(6:6)=='_')THEN
                ...
                IF(str(i)(7:7)=='m')THEN
                  ...
                  ENDIF
                  IF(str(i)(pos:pos)=='v')THEN
                    CALL convert_unclean_string_to_integer(str(i)(pos+1:pos+1),iveg,status) 
                    IF(status/=0)RETURN
                  ELSEIF(str(i)(pos:pos)=='l')THEN
                    CALL convert_unclean_string_to_integer(str(i)(pos+1:pos+3),iveg,status) 
                    IF(status/=0)RETURN
                  ELSE
                    iveg = 1
                  ENDIF
    ...
    !Deallocate local arrays in opposite order
    IF(ALLOCATED(fileindex)) DEALLOCATE(fileindex)
    IF(ALLOCATED(filebasin)) DEALLOCATE(filebasin)
    IF(ALLOCATED(varcode)) DEALLOCATE(varcode)
    IF(ALLOCATED(rindex)) DEALLOCATE(rindex)
    IF(ALLOCATED(iindex)) DEALLOCATE(iindex)
    IF(ALLOCATED(xr)) DEALLOCATE(xr)
    IF(ALLOCATED(xi)) DEALLOCATE(xi)
    ...
    WRITE(6,*) 'Atmosperic deposition reloaded'

  END SUBROUTINE reload_deposition

  !>Set deposition to model specified values if no AtmdepData.txt file is used
  !------------------------------------------------------------------
  SUBROUTINE finish_atmdep_file_deposition(dir)

    USE WORLDVAR, ONLY : maxcharpath
    USE MODELMODULE, ONLY : model_specified_atmospheric_deposition

    !Argument declaration
    CHARACTER (LEN=*), INTENT(IN) :: dir  !<File directory
    
    !Local variables
    LOGICAL fexist        !status of AtmdepData file
    CHARACTER(LEN=maxcharpath) infile
    ...
    infile = TRIM(dir)//'AtmdepData.txt'
    INQUIRE(FILE = TRIM(infile), EXIST = fexist) 
    !What if only have dated files??
    !Then these will be written over later! Ok
    
    IF(.NOT.fexist)THEN

      CALL model_specified_atmospheric_deposition()
    ENDIF

  END SUBROUTINE finish_atmdep_file_deposition

  !>\brief Gets the information about parameters to use for calculating main 
  !!river water level from RiverRatingCurveData.txt
  !!
  !>\b Consequences Module variables will be allocated and set
  !------------------------------------------------------------------------------
  SUBROUTINE load_rrcurvedata(funit,dir,n,status) 
    ...
    USE MODVAR, ONLY : basin, &
                       ...
                       missing_value, &
                       rrcdata  !OUT
    USE MODEL_TEST_ROUTINES, ONLY : propagate_external_msg,e_rrcdata,e_error,e_info
    ...
    !Local parameters
    INTEGER, PARAMETER :: letters = 10  !Max number of letters (that are read) in heading
    INTEGER, PARAMETER :: maxcol = 22   !Enough?
    INTEGER, PARAMETER :: maxsec = 100  !Enough?

    !Local variables
    LOGICAL fexist,foundtype2(3)
    INTEGER i,j,nrow
    INTEGER nsector,isec,j2,j2_lowest
    INTEGER, ALLOCATABLE :: xi(:,:)    !Integer data read from file
    INTEGER code(maxcol)               !Code for column variable
    INTEGER rindex(maxcol)             !Index for column real variables
    INTEGER iindex(maxcol)             !Index for column integer variables
    INTEGER mcols
    INTEGER,ALLOCATABLE :: subid(:),typeid(:),iceseason(:)
    INTEGER :: rowofrrcs(maxsec)   !Hold row number of sectors in current subbasin
    REAL qmintemp(maxsec),qmaxtemp(maxsec)   !Hold borders of sectors in current subbasin recalculated to flow
    REAL wmin_lowest,last_wmax
    REAL,ALLOCATABLE :: rrck(:),rrcp(:)    !Rating curve parameters for river (no ice): q=k(w-w0)^p   
    REAL,ALLOCATABLE :: wmin(:),wmax(:)    
    REAL,ALLOCATABLE :: rrcw0(:)
    REAL,ALLOCATABLE :: gauge0(:),unittrans(:)
    REAL,ALLOCATABLE :: xr(:,:)               !Real data read from file
    CHARACTER(LEN=maxcharpath) infile
    CHARACTER(LEN=letters) str(maxcol)      !Content string
    CHARACTER(LEN=150) :: propagate_str                  !Propagate string
    ...
    infile = TRIM(dir)//'RiverRatingCurveData.txt'
    INQUIRE(FILE = TRIM(infile), EXIST = fexist) 
    IF(.NOT.fexist)  RETURN
      
    ALLOCATE(rrcdata(n))
    !Count rows
    CALL count_data_rows(funit,infile,1,nrow,status)
    IF(status/=0) RETURN
    ...
    IF(status.NE.0) THEN
      WRITE(6,*) 'ERROR reading file: ',TRIM(infile)
      propagate_str = 'reading file: '//TRIM(infile)
      CALL propagate_external_msg(e_rrcdata,e_error,propagate_str)
      RETURN
    ENDIF
    ...
    DO i = 1,mcols
      IF(str(i)(1:letters)=='subid     ') code(i) = i_intg
      IF(str(i)(1:letters)=='typeofrrc ') code(i) = i_intg
      IF(str(i)(1:letters)=='iceseason ') code(i) = i_intg
      IF(str(i)(1:letters)=='k         ') code(i) = i_real
      IF(str(i)(1:letters)=='p         ') code(i) = i_real
      IF(str(i)(1:letters)=='wmin      ') code(i) = i_real
      IF(str(i)(1:letters)=='wmax      ') code(i) = i_real
      IF(str(i)(1:letters)=='w0ref     ') code(i) = i_real
      IF(str(i)(1:letters)=='gaugezero ') code(i) = i_real
      IF(str(i)(1:letters)=='unitconv  ') code(i) = i_real
      IF(code(i) == i_str) THEN
        propagate_str_extra = propagate_str_extra//','//TRIM(str(i)(1:letters))
      ENDIF
    ENDDO

    CALL propagate_external_msg(e_rrcdata,e_info,propagate_str_extra,code)
    IF(ALLOCATED(propagate_str_extra)) DEALLOCATE(propagate_str_extra)

    !Allocate and initialise arrays for holding read data
    ALLOCATE(subid(nrow),typeid(nrow),iceseason(nrow))
    ALLOCATE(rrck(nrow),rrcp(nrow))    !Rating curve parameters for river (no ice): q=k(w-w0)^p   !Dessa b�r vara st�rre
    ALLOCATE(rrcw0(nrow)) !rrcw0 is w0 in reference system
    ALLOCATE(gauge0(nrow),unittrans(nrow)) !gaugezero is zero in local gauge system, transformation to unit of local water level
    ALLOCATE(wmin(nrow),wmax(nrow))
    rrck = 0.; rrcp = 0.
    rrcw0 = 0.; 
    gauge0 = missing_value; unittrans = 1.    !Default value; unit in m and no local reference scale
    subid = 0; typeid = 0; iceseason = 0
    wmin = 1000.; wmax = 1000.
    ...
    DO i = 1,mcols
      IF(str(i)(1:letters)=='subid     ')  subid(1:nrow)     = xi(1:nrow,iindex(i))
      IF(str(i)(1:letters)=='typeofrrc ')  typeid(1:nrow)    = xi(1:nrow,iindex(i))
      IF(str(i)(1:letters)=='iceseason ')  iceseason(1:nrow) = xi(1:nrow,iindex(i))
      IF(str(i)(1:letters)=='k         ')  rrck(1:nrow)      = xr(1:nrow,rindex(i))
      IF(str(i)(1:letters)=='p         ')  rrcp(1:nrow)      = xr(1:nrow,rindex(i))
      IF(str(i)(1:letters)=='wmin      ')  wmin(1:nrow)      = xr(1:nrow,rindex(i))
      IF(str(i)(1:letters)=='wmax      ')  wmax(1:nrow)      = xr(1:nrow,rindex(i))
      IF(str(i)(1:letters)=='w0ref     ')  rrcw0(1:nrow)     = xr(1:nrow,rindex(i))
      IF(str(i)(1:letters)=='gaugezero ')  gauge0(1:nrow)    = xr(1:nrow,rindex(i))
      IF(str(i)(1:letters)=='unitconv  ')  unittrans(1:nrow) = xr(1:nrow,rindex(i))
    ENDDO
    ...
    DO i = 1,n
      ...
      !q=k*(w-w0)**p -> w=a*q**b+w0,a=(1/k)**b,b=1/p
        IF(basin(i)%subid==subid(j))THEN
          rrcdata(i)%typeid = typeid(j)
          IF(typeid(j)==1)THEN  !single curve
            IF(.NOT.ALLOCATED(rrcdata(i)%single)) ALLOCATE(rrcdata(i)%single(1))
            IF(rrck(j)>0.)THEN
              rrcdata(i)%w0ref = rrcw0(j)
              rrcdata(i)%gauge0 = gauge0(j)
              rrcdata(i)%gunitconv = unittrans(j)
              IF(rrcp(j)>0.) rrcdata(i)%single(1)%b = 1./rrcp(j)  
              rrcdata(i)%single(1)%a = (1./rrck(j))**rrcdata(i)%single(1)%b
              EXIT
            !ELSE catch with test
            ENDIF
          ELSEIF(typeid(j)==2)THEN
            IF(.NOT.ALLOCATED(rrcdata(i)%ice))THEN   !First row with this subid
              ALLOCATE(rrcdata(i)%ice(1))
              foundtype2(1)=.TRUE.
            ENDIF
            IF(iceseason(j)==0)THEN !summer curve
              IF(rrck(j)>0.)THEN
                rrcdata(i)%w0ref = rrcw0(j)
                rrcdata(i)%gauge0 = gauge0(j)
                rrcdata(i)%gunitconv = unittrans(j)
                rrcdata(i)%ice(1)%qpar(1)%p = rrcp(j)
                rrcdata(i)%ice(1)%qpar(1)%k = rrck(j)
                IF(rrcp(j)>0.) rrcdata(i)%ice(1)%wpar(1)%b = 1./rrcp(j)
                rrcdata(i)%ice(1)%wpar(1)%a = (1./rrck(j))**rrcdata(i)%ice(1)%wpar(1)%b
                foundtype2(2) = .TRUE.
              !ELSE test
              ENDIF
            ELSE  !ice season
              IF(rrck(j)>0.)THEN
                rrcdata(i)%ice(1)%qpar(2)%p = rrcp(j)
                rrcdata(i)%ice(1)%qpar(2)%k = rrck(j)
                IF(rrcp(j)>0.) rrcdata(i)%ice(1)%wpar(2)%b = 1./rrcp(j)
                rrcdata(i)%ice(1)%wpar(2)%a = (1./rrck(j))**rrcdata(i)%ice(1)%wpar(2)%b
              ENDIF
              foundtype2(3) = .TRUE.
            ENDIF
          ELSEIF(typeid(j)==3)THEN  !sectorial curve
            !Count sectors and remember their rows in the file
            nsector = 0
            rowofrrcs = 0
            DO j2=j,nrow
              IF(basin(i)%subid==subid(j2))THEN
                IF(typeid(j2)==3)THEN
                  nsector = nsector + 1
                  rowofrrcs(nsector) = j2
                ENDIF
              ENDIF
            ENDDO
            !Allocate memory for data
            IF(.NOT.ALLOCATED(rrcdata(i)%sector)) ALLOCATE(rrcdata(i)%sector(nsector))
            DO isec = 1,nsector
              IF(.NOT.ALLOCATED(rrcdata(i)%sector(isec)%wpar)) ALLOCATE(rrcdata(i)%sector(isec)%wpar(1))
            ENDDO
            isec = 0
            j2_lowest = 1
            DO isec = 1,nsector
              !Find lowest wmin
              wmin_lowest = wmin(rowofrrcs(1))
              DO j2 = 2, nsector
                IF(wmin(rowofrrcs(j2))<wmin_lowest)THEN
                  wmin_lowest = wmin(rowofrrcs(j2))
                  j2_lowest = j2
                ENDIF
              ENDDO
              IF(isec==1)THEN
                rrcdata(i)%w0ref = rrcw0(rowofrrcs(j2_lowest))   !Can I demand this?
                rrcdata(i)%gauge0 = gauge0(rowofrrcs(j2_lowest))
                rrcdata(i)%gunitconv = unittrans(rowofrrcs(j2_lowest))
              ENDIF
              !Check wmin against w0
              IF(wmin(rowofrrcs(j2_lowest))<rrcw0(rowofrrcs(j2_lowest)))THEN
                WRITE(6,*) 'ERROR: sectorial river rating curve in: ',TRIM(infile)
                propagate_str = 'sectorial river rating curve wmin < w0'
                CALL propagate_external_msg(e_rrcdata,e_error,propagate_str)
                status = 1
                RETURN
              ENDIF
              !Check wmin against last wmax
              IF(isec>1)THEN
                IF(wmin(rowofrrcs(j2_lowest))/=last_wmax)THEN
                  WRITE(6,*) 'ERROR: sectorial river rating curve in: ',TRIM(infile)
                  WRITE(6,*) 'ERROR: thresholds do not match: ',last_wmax,wmin(rowofrrcs(j2_lowest))
                  WRITE(6,*) 'ERROR: problematic row in file: ',rowofrrcs(j2_lowest)-1
                  propagate_str = 'sectorial river rating curve w treshold intervals do not match'
                  CALL propagate_external_msg(e_rrcdata,e_error,propagate_str)
                  status = 1
                  RETURN
                ENDIF
              ENDIF
              !Set the found rrc data to memory
              IF(rrck(rowofrrcs(j2_lowest))>0.)THEN
                rrcdata(i)%sector(isec)%wpar(1)%w0 = rrcw0(rowofrrcs(j2_lowest)) - rrcdata(i)%w0ref   !w0 above threshold (q=0)
                IF(rrcp(rowofrrcs(j2_lowest))>0.) rrcdata(i)%sector(isec)%wpar(1)%b = 1./rrcp(rowofrrcs(j2_lowest))  
                rrcdata(i)%sector(isec)%wpar(1)%a = (1./rrck(rowofrrcs(j2_lowest)))**rrcdata(i)%sector(isec)%wpar(1)%b
              !ELSE catch with test?
              ENDIF
              qmintemp(isec) = rrck(rowofrrcs(j2_lowest)) * (wmin(rowofrrcs(j2_lowest)) - rrcw0(rowofrrcs(j2_lowest)))**rrcp(rowofrrcs(j2_lowest))
              qmaxtemp(isec) = rrck(rowofrrcs(j2_lowest)) * (wmax(rowofrrcs(j2_lowest)) - rrcw0(rowofrrcs(j2_lowest)))**rrcp(rowofrrcs(j2_lowest))
              last_wmax = wmax(rowofrrcs(j2_lowest)) !Save last wmax to check against next wmin
              !Put lowest wmin out of the play
              wmin(rowofrrcs(j2_lowest)) = 100000.   !high enough?
            ENDDO
            !Set maximum flow borders of curve, compromise between sectors
            DO isec = 1, nsector - 1
              rrcdata(i)%sector(isec)%qmax = (qmaxtemp(isec) + qmintemp(isec+1))/2.
            ENDDO
            rrcdata(i)%sector(nsector)%qmax = qmaxtemp(nsector)
            EXIT
          ELSE
            IF(conductwarning) WRITE(6,*) 'WARNING: Unkown typeid for river rating curve skipped: ', typeid(j)
          ENDIF
        ENDIF
        IF(foundtype2(1).AND.foundtype2(2).AND.foundtype2(3)) EXIT    !Two curves found for typeofRRC=2
      ENDDO
      IF(foundtype2(1).AND.(.NOT.foundtype2(2).OR..NOT.foundtype2(3)))THEN
        IF(foundtype2(2).AND..NOT.foundtype2(3))THEN
          IF(rrcdata(i)%ice(1)%qpar(1)%k>0.)THEN
            rrcdata(i)%ice(1)%qpar(2)%p = 1.
            rrcdata(i)%ice(1)%qpar(2)%k = rrcdata(i)%ice(1)%qpar(1)%k
            rrcdata(i)%ice(1)%wpar(2)%b = 1.
            rrcdata(i)%ice(1)%wpar(2)%a = 1./rrcdata(i)%ice(1)%qpar(2)%k  !Skipped ^1 here
          ENDIF
          WRITE(6,*) 'Set default ice curve for ',basin(i)%subid !Only one curve given for this subid. Set default ice curve parameters
        ELSE
          WRITE(6,*) 'ERROR: RiverRatingCurveData.txt',basin(i)%subid
          STOP 1
        ENDIF
      ENDIF
    ENDDO

    WRITE(6,*) 'File loaded: ', TRIM(infile)
    
    IF(ALLOCATED(xr)) DEALLOCATE(xr)
    IF(ALLOCATED(xi)) DEALLOCATE(xi)
    IF(ALLOCATED(wmax)) DEALLOCATE(wmax)
    IF(ALLOCATED(wmin)) DEALLOCATE(wmin)
    IF(ALLOCATED(unittrans)) DEALLOCATE(unittrans)
    IF(ALLOCATED(gauge0)) DEALLOCATE(gauge0)
    IF(ALLOCATED(rrcw0)) DEALLOCATE(rrcw0)
    IF(ALLOCATED(rrcp)) DEALLOCATE(rrcp)
    IF(ALLOCATED(rrck)) DEALLOCATE(rrck)
    IF(ALLOCATED(iceseason)) DEALLOCATE(iceseason)
    IF(ALLOCATED(typeid)) DEALLOCATE(typeid)
    IF(ALLOCATED(subid)) DEALLOCATE(subid)

  END SUBROUTINE load_rrcurvedata
  ...
  SUBROUTINE load_pointsourcedata(dir,infile,n,status) 
    ...
    USE MODVAR, ONLY : basin,       &
                       ...
                       i_t1,i_t2,i_ss,i_ae,i_dsi,i_asi, &
                       max_pstype, &
                       numsubstances, &
                       simulate, &
                       substance_name, &
              ...
              ELSEIF(pointsource(j,5)>0.)THEN
                ...
                IF(k>0 .AND. (readlocation(j)>4.OR.readlocation(j)<=0))THEN   !Try to avoid T1 ps !This means old PS-files may not work automatically.
                  ...
                  IF(simulate%substance(i_oc))THEN
                    help = pointsource(j,10)*pointsource(j,5)*onetspday*1.E-3    !kg OC/timestep
                    load(i)%psload(k,i_oc) = load(i)%psload(k,i_oc) + help
                  ENDIF
                  IF(simulate%substance(i_dsi))THEN
                    help = pointsource(j,11)*pointsource(j,5)*onetspday*1.E-3    !kg dSi/timestep
                    load(i)%psload(k,i_dsi) = load(i)%psload(k,i_dsi) + help
                    help = pointsource(j,12)*pointsource(j,5)*onetspday*1.E-3    !kg aSi/timestep
                    load(i)%psload(k,i_asi) = load(i)%psload(k,i_asi) + help
                  ENDIF
                ELSEIF(k==-1)THEN !Negative point source, abstraction
                  load(i)%abstrvol = ABS(pointsource(j,5)/86400.)  !m3/d->m3/s
                  load(i)%abstrind = readlocation(j)
                ELSEIF(conductwarning)THEN 
    ...
    ELSEIF(readpstime)THEN
      ...
      ELSE
        ...
        ELSE
          ...
          ELSE
            ...
            propagate_str = 'no PSTimeSeries.txt file found'
      ...
      IF(npsused>0.OR.nabsused>0)THEN
        ...
        DO j=1,nrows
          IF(aindex(j)>0)THEN
            ...
            ELSE
              ...
              ENDIF
              WRITE(propagate_str,*) 'psid:',readpsid(j),' non-valid ps_type'
              CALL propagate_external_msg(e_pointsourcedata,e_warning,propagate_str)
            ENDIF
          ENDIF
        ENDDO
        ...
        IF(pstscol<=0)THEN
          ...
          propagate_str = 'no data in '//TRIM(pstsfilename)
          CALL propagate_external_msg(e_pointsourcedata,e_error,propagate_str)
          status = 1
          RETURN
        ENDIF
        ...
        IF(dailypsfile)THEN
          ...
          IF(status==2)THEN  !Shorter time period
            ...
            ENDIF
          ELSEIF(status.NE.0)THEN
            propagate_str = 'timeperiod of: '//TRIM(pstsfilename)
            CALL propagate_external_msg(e_pointsourcedata,e_error,propagate_str)
            RETURN
          ENDIF
      ...
      ELSE
        IF(conductwarning) WRITE(6,*) 'WARNING: No point sources or abstractions found in PointSourceData.txt.'
        propagate_str = 'no point sources/abstractions in PointSourceData.txt'
        CALL propagate_external_msg(e_pointsourcedata,e_warning,propagate_str)
      ENDIF
    ENDIF
  ...
  SUBROUTINE read_pointsourcedata(dir,infile,nrows,status,fexist,readsubid,readpsid,readpstype,readlocation,pointsource,readdate1,readdate2) 
    ...
    IF(.NOT.ALLOCATED(pointsource)) ALLOCATE(pointsource(nrows,12))
    ...
    DO i = 1,ncols
      ...
      IF(str(i)(1:letters)=='ps_occonc ') code(i) = i_real
      IF(str(i)(1:letters)=='ps_dsiconc') code(i) = i_real
      IF(str(i)(1:letters)=='ps_asiconc') code(i) = i_real
      IF(code(i) == i_str.AND.(str(i)(1:letters)/='fromdate  '.AND.str(i)(1:letters)/='todate    ')) THEN
        propagate_str_extra = propagate_str_extra//','//TRIM(str(i)(1:letters))
      ENDIF
    ENDDO

    CALL propagate_external_msg(e_pointsourcedata,e_info,propagate_str_extra,code)
    IF(ALLOCATED(propagate_str_extra)) DEALLOCATE(propagate_str_extra)
    ...
    DO i = 1,ncols
      ...
      IF(str(i)(1:letters)=='ps_occonc ') pointsource(1:nrows,10) = xr(1:nrows,rindex(i))
      IF(str(i)(1:letters)=='ps_dsiconc') pointsource(1:nrows,11) = xr(1:nrows,rindex(i))
      IF(str(i)(1:letters)=='ps_asiconc') pointsource(1:nrows,12) = xr(1:nrows,rindex(i))
    ENDDO
  ...
  SUBROUTINE get_current_pointsources(dir,infile,ns,time,status) 
    ...
    CHARACTER(LEN=10) :: datestr  !Date string
    CHARACTER(LEN=50),ALLOCATABLE :: readdate1(:)  !String date read from file
    CHARACTER(LEN=50),ALLOCATABLE :: readdate2(:)  !String date read from file
    TYPE(DateType),ALLOCATABLE :: psdateslocal(:,:)  !Array of dates from file
...
!        WRITE(6,*) 'Point source in subid',readsubid(j),'ignored, not in model domain.'
    ...
    IF(ALLOCATED(readpsid)) DEALLOCATE(readpsid)
    IF(ALLOCATED(readsubid)) DEALLOCATE(readsubid)
    CALL format_date(time,'yyyy-mm-dd',datestr) 
    WRITE(6,*) 'PointSourceData information loaded at ',datestr
  ...
  SUBROUTINE load_permanent_soilleakage(dir,n,bd,status) 
    ...
    USE WORLDVAR, ONLY : maxcharpath
    USE MODVAR, ONLY : basin,classmodel, &
                       modeloption, &
                       nclass, &
                       numsubstances,i_t2, &
                       p_soilleakage, &
    ...
    IF(modeloption(p_soilleakage)==1)THEN
      IF(numsubstances==0)THEN
        WRITE(6,*) 'INFO: No substances are simulated. Turning off soil leakage concentration model.'
    ...
    ELSEIF(modeloption(p_soilleakage)==2)THEN
      IF(numsubstances==0)THEN
        WRITE(6,*) 'INFO: No substances are simulated. Turning off soil leakage load model.'
        modeloption(p_soilleakage)=0
        RETURN
      ENDIF
  ...
  SUBROUTINE get_current_soilleakage(dir,n,time,status) 
    ...
    IF(modeloption(p_soilleakage)==3)THEN
      IF(numsubstances==0)THEN
        WRITE(6,*) 'INFO: No substances are simulated. Turning off soil leakage model.'
        modeloption(p_soilleakage)=0
        RETURN
      ENDIF
      ...
      !>\li Allocate and initiate variables 
      IF(ALLOCATED(soilleak%load)) DEALLOCATE(soilleak%load)
      IF(.NOT.ALLOCATED(soilleak%load)) ALLOCATE(soilleak%load(numsubstances,nclass,12,n))
      soilleak%load = 0.
      IF(.NOT.ALLOCATED(geoid)) ALLOCATE(geoid(n))
      geoid = basin%subid
  ...
  SUBROUTINE read_monthly_leakage(dir,infile,n,geoid,status,leakage) 
    ...
    IF(MINVAL(readleakage)<0. .AND. conductwarning) WRITE(6,*) 'WARNING: Soil leakage less than zero found in file.'
  ...
  SUBROUTINE read_first_monthly_leakage(dir,infile,bd,n,geoid,status,leakage) 
    ...
    DO 
      ...
      IF(date%Year>bd%Year)THEN
        IF(date%Month==1.AND.date%day==1 .AND. conductwarning) WRITE(6,*) 'WARNING: Load for later year from ',TRIM(infile),' used for', bd%Year
      ENDIF
      month = date%Month
...
100 CLOSE(UNIT=fileunit_temp)
    ...
    IF(MINVAL(readleakage)<0. .AND. conductwarning) WRITE(6,*) 'WARNING: Values less than zero found in file.'
    IF(.NOT.ALLOCATED(mixindex)) ALLOCATE(mixindex(n))
    ...
    !Finish the subroutine, in opposite order
    IF(ALLOCATED(mixindex)) DEALLOCATE(mixindex)
  ...
  SUBROUTINE read_one_year_monthly_leakage(dir,infile,cd,n,geoid,status,leakage) 
    ...
    DO 
      ...
      IF(date%Year>cd%Year)THEN
        IF(date%Month==1.AND.date%day==1 .AND. conductwarning) WRITE(6,*) 'WARNING: Load for later year from ',TRIM(infile),' used for', cd%Year
      ENDIF
      month = date%Month
...
102 WRITE(6,*) 'Error: reading file',TRIM(infile)
  ...
  SUBROUTINE load_management_data(funit,dir,n,status) 
    ...
    IF(status/=0)RETURN
    IF(nrows==0)THEN
      WRITE(6,*) 'INFO: No data in MgmtData.txt file found.'
      RETURN
    ENDIF
    ...
    CALL propagate_external_msg(e_mgmtdata,e_info,propagate_str_extra,code)
    IF(ALLOCATED(propagate_str_extra)) DEALLOCATE(propagate_str_extra)
          ...
          ELSEIF(xi(j,iindex(typecolumn))==2)THEN !Water transfer
            ...
            IF(str(i)(1:letters)=='receiver  ')THEN
              ...
              IF(.NOT.receiverfound)THEN
                IF(conductwarning) WRITE(6,*) 'WARNING: No reciever of the water transfer is found in MgmtData.txt: row: ',j
...
!--------------------------------------------------------------------
  ...
  SUBROUTINE load_forcing_information_data(funit,dir,ns,status) 
    ...
    INTEGER mcols
    INTEGER ncomrows    !number of comment rows (with !!) in the beginning of file
    INTEGER i,j,iforc
    ...
    IF(.NOT.fileex)THEN
      IF(readobsid)THEN
        ...
        WRITE(6,*) 'ERROR: Missing file:', TRIM(infile)
        STOP 1
      ENDIF
      RETURN
    ELSEIF(fileex.AND..NOT.readobsid)THEN
      IF(conductwarning) WRITE(6,*) 'WARNING: Existing ForcKey-file will not be read, because readobsid turned off in info.txt'
      RETURN
    ENDIF
    WRITE(6,*) 'File opened: ', TRIM(infile)

    !Check comment rows
    CALL count_comment_rows(funit,TRIM(infile),ncomrows,status) 
    IF(status.NE.0) RETURN
    
    !Count number of columns in file
    CALL count_data_cols(funit,TRIM(infile),ncomrows,ncols,status)
    IF(status/=0)RETURN
    ...
    IF(.NOT.ALLOCATED(rindex)) ALLOCATE(rindex(ncols))
    IF(.NOT.ALLOCATED(iindex)) ALLOCATE(iindex(ncols))
    IF(.NOT.ALLOCATED(xr)) ALLOCATE(xr(ns,ncols))
    IF(.NOT.ALLOCATED(xi)) ALLOCATE(xi(ns,ncols))  
    ...
    CALL propagate_external_msg(e_forckey,e_info,propagate_str_extra,code)
    IF(ALLOCATED(propagate_str_extra)) DEALLOCATE(propagate_str_extra)
    ...
    CLOSE(funit) 
    IF(itobselev==0.AND.conductwarning) WRITE(6,*) 'WARNING: no elevation for tobs found in file:', TRIM(infile)
...
900 WRITE(6,*) 'Error open file: ', TRIM(infile)
  ...
  SUBROUTINE read_update_data(funit,infile,ns,nqsub,qupdsub,nqarsub,nwarsub, &
    ...
    CALL propagate_external_msg(e_update,e_info,propagate_str_extra,code)
    IF(ALLOCATED(propagate_str_extra)) DEALLOCATE(propagate_str_extra)
  ...
  !-------------------------------------------------------------------
  SUBROUTINE reform_inputdata_for_submodel(basemodel,ns)
    ...
    !Local variables 
    INTEGER i,isub,isubst,iforc,k,s,nbasinpar,temp_isb,isec,nsec
    INTEGER nsfile  !number of subbasins in file
    INTEGER indexarray(ns)   !index for basemodel
    ...
    TYPE(DEPOSITIONTYPE),ALLOCATABLE :: temp_deposition2(:)
    TYPE(CLASSBASINTYPE),ALLOCATABLE :: temp_classbasin(:,:)
    TYPE(WETLANDTYPE), ALLOCATABLE :: temp_wetland(:,:)
    TYPE(RIVERRATINGTYPE), ALLOCATABLE :: temp_rrcdata(:)
    TYPE(LAKESECTIONTYPE),ALLOCATABLE :: temp_lakesectiondata(:,:) 
    ...
    INTEGER,ALLOCATABLE :: temp_petmodel(:)

    !Initialisation
    nsfile = basemodel%nsub
    indexarray = basemodel%iindex
    ...
    IF(ALLOCATED(petmodel))THEN
      IF(.NOT.ALLOCATED(temp_petmodel)) ALLOCATE(temp_petmodel(nsfile))
      temp_petmodel = petmodel
      DEALLOCATE(petmodel)
    ENDIF
    IF(ALLOCATED(lakesectiondata))THEN
      IF(.NOT.ALLOCATED(temp_lakesectiondata)) ALLOCATE(temp_lakesectiondata(nsfile,maxlakesections))
      temp_lakesectiondata = lakesectiondata
      DEALLOCATE(lakesectiondata)
    ENDIF
    IF(.NOT.ALLOCATED(temp_subweightcrit)) ALLOCATE(temp_subweightcrit(nsfile)) !in GeoData but not allocate_basinvariables
    ...
    IF(ALLOCATED(temp_petmodel))THEN
      petmodel(:) = temp_petmodel(indexarray(:))
      DEALLOCATE(temp_petmodel)
    ELSE
      IF(ALLOCATED(petmodel)) DEALLOCATE(petmodel)
    ENDIF
    IF(ALLOCATED(temp_lakesectiondata))THEN
      lakesectiondata(:,:) = temp_lakesectiondata(indexarray(:),:)
      DEALLOCATE(temp_lakesectiondata)
    ENDIF
    ...
    !Reform river rating curve data
    IF(ALLOCATED(rrcdata))THEN
      IF(.NOT.ALLOCATED(temp_rrcdata)) ALLOCATE(temp_rrcdata(nsfile))
      temp_rrcdata%typeid = rrcdata%typeid
      temp_rrcdata%w0ref = rrcdata%w0ref
      temp_rrcdata%gauge0 = rrcdata%gauge0
      temp_rrcdata%gunitconv = rrcdata%gunitconv
      DO i = 1,nsfile
        IF(ALLOCATED(rrcdata(i)%single))THEN
          ALLOCATE(temp_rrcdata(i)%single(1))
          temp_rrcdata(i)%single = rrcdata(i)%single
          DEALLOCATE(rrcdata(i)%single)
        ENDIF
        IF(ALLOCATED(rrcdata(i)%ice))THEN
          ALLOCATE(temp_rrcdata(i)%ice(1))
          temp_rrcdata(i)%ice(1) = rrcdata(i)%ice(1)
          DEALLOCATE(rrcdata(i)%ice)
        ENDIF
        IF(ALLOCATED(rrcdata(i)%sector))THEN
          nsec = SIZE(rrcdata(i)%sector)
          ALLOCATE(temp_rrcdata(i)%sector(nsec))
          DO isec = 1,nsec
            ALLOCATE(temp_rrcdata(i)%sector(isec)%wpar(1))
            temp_rrcdata(i)%sector(isec)%wpar = rrcdata(i)%sector(isec)%wpar
            DEALLOCATE(rrcdata(i)%sector(isec)%wpar)
            temp_rrcdata(i)%sector(isec)%qmax = rrcdata(i)%sector(isec)%qmax
          ENDDO
          DEALLOCATE(rrcdata(i)%sector)
        ENDIF
      ENDDO
      DEALLOCATE(rrcdata)
    ENDIF
    IF(ALLOCATED(temp_rrcdata))THEN
      IF(.NOT.ALLOCATED(rrcdata)) ALLOCATE(rrcdata(ns))
      rrcdata%typeid = temp_rrcdata(indexarray(:))%typeid
      rrcdata%w0ref = temp_rrcdata(indexarray(:))%w0ref
      rrcdata%gauge0 = temp_rrcdata(indexarray(:))%gauge0
      rrcdata%gunitconv = temp_rrcdata(indexarray(:))%gunitconv
      DO i = 1,ns
        IF(ALLOCATED(temp_rrcdata(indexarray(i))%single))THEN
          ALLOCATE(rrcdata(i)%single(1))
          rrcdata(i)%single = temp_rrcdata(indexarray(i))%single
          DEALLOCATE(temp_rrcdata(indexarray(i))%single)
        ENDIF
        IF(ALLOCATED(temp_rrcdata(indexarray(i))%ice))THEN
          ALLOCATE(rrcdata(i)%ice(1))
          rrcdata(i)%ice(1) = temp_rrcdata(indexarray(i))%ice(1)
          DEALLOCATE(temp_rrcdata(indexarray(i))%ice)
        ENDIF
        IF(ALLOCATED(temp_rrcdata(indexarray(i))%sector))THEN
          nsec = SIZE(temp_rrcdata(indexarray(i))%sector)
          ALLOCATE(rrcdata(i)%sector(nsec))
          DO isec = 1,nsec
            ALLOCATE(rrcdata(i)%sector(isec)%wpar(1))
            rrcdata(i)%sector(isec)%wpar = temp_rrcdata(indexarray(i))%sector(isec)%wpar
            DEALLOCATE(temp_rrcdata(indexarray(i))%sector(isec)%wpar)
            rrcdata(i)%sector(isec)%qmax = temp_rrcdata(indexarray(i))%sector(isec)%qmax
          ENDDO
          DEALLOCATE(temp_rrcdata(indexarray(i))%sector)
        ENDIF
      ENDDO
      IF(ALLOCATED(temp_rrcdata)) DEALLOCATE(temp_rrcdata)
    ENDIF
    
    !!Reform atmdep structure
    IF(ALLOCATED(deposition2))THEN
      ALLOCATE(temp_deposition2(numsubstances))
      DO isubst = 1, numsubstances
        IF(ALLOCATED(deposition2(isubst)%depcode))THEN
          !ALLOCATE(temp_deposition2(isubst)%depcode(2,2)) !(veg/time,w/d)
          !temp_deposition2(isubst)%depcode = deposition2(isubst)%depcode
          !DEALLOCATE(deposition2(isubst)%depcode)
          IF(ALLOCATED(deposition2(isubst)%dryload))THEN
            !dsize = SIZE(deposition2(isubst)%dryload,1)
            ALLOCATE(temp_deposition2(isubst)%dryload(SIZE(deposition2(isubst)%dryload,1),SIZE(deposition2(isubst)%dryload,2),SIZE(deposition2(isubst)%dryload,3)))    !movealloc?
            temp_deposition2(isubst)%dryload = deposition2(isubst)%dryload
            DEALLOCATE(deposition2(isubst)%dryload)
            ALLOCATE(deposition2(isubst)%dryload(SIZE(deposition2(isubst)%dryload,1),ns,SIZE(deposition2(isubst)%dryload,3)))
            deposition2(isubst)%dryload(:,:,:) = temp_deposition2(isubst)%dryload(:,indexarray(:),:)
            DEALLOCATE(temp_deposition2(isubst)%dryload)
          ENDIF
          IF(ALLOCATED(deposition2(isubst)%wetconc))THEN
            !dsize = SIZE(deposition2(isubst)%dryload,1)
            ALLOCATE(temp_deposition2(isubst)%wetconc(SIZE(deposition2(isubst)%wetconc,1),SIZE(deposition2(isubst)%wetconc,2),SIZE(deposition2(isubst)%wetconc,3)))    
            temp_deposition2(isubst)%wetconc = deposition2(isubst)%wetconc
            DEALLOCATE(deposition2(isubst)%wetconc)
            ALLOCATE(deposition2(isubst)%wetconc(SIZE(deposition2(isubst)%wetconc,1),ns,SIZE(deposition2(isubst)%wetconc,3)))    
            deposition2(isubst)%wetconc(:,:,:) = temp_deposition2(isubst)%wetconc(:,indexarray(:),:)
            DEALLOCATE(temp_deposition2(isubst)%wetconc)
          ENDIF
        ENDIF
      ENDDO
    ENDIF

    !Reform pointsource and abstraction information subbasin indices
    ...
    DO i=1,noutreg
      DO isub=1,outregion(i)%nsubbasin
        ...
        ENDDO
        IF(k>ns)THEN
          IF(conductwarning) WRITE(6,*) 'WARNING: outregion',i,'is not (fully) included in submodel.'
    ...
    ENDIF
    IF(ALLOCATED(wobsindex))THEN
      IF(.NOT.ALLOCATED(tmp)) ALLOCATE(tmp(ns))
      tmp = wobsindex(indexarray(:))
      DEALLOCATE(wobsindex)
      CALL MOVE_ALLOC(tmp,wobsindex)
    ENDIF
    ...
    !Reform subbasin information on glaciers, floodplains, watertransfer
  ...
  SUBROUTINE load_cropdata(dir,n,status) 
    ...
    IF(.NOT.fileexist)THEN
      IF(calcNPC)THEN
        ...
        IF(modeloption(p_soilleakage)==0)THEN !Jag tror det r�cker
    ...
    CALL propagate_external_msg(e_cropdata,e_info,propagate_str_extra,code)
    IF(ALLOCATED(propagate_str_extra)) DEALLOCATE(propagate_str_extra)
    ...
    !Deallocate local arrays, in opposite order
    IF(ALLOCATED(cropid)) DEALLOCATE(cropid)
    IF(ALLOCATED(cropregion)) DEALLOCATE(cropregion)
    IF(ALLOCATED(rindex)) DEALLOCATE(rindex)
    IF(ALLOCATED(iindex)) DEALLOCATE(iindex)
    IF(ALLOCATED(xi)) DEALLOCATE(xi)
    IF(ALLOCATED(xr)) DEALLOCATE(xr)
...
300 WRITE(*,*) 'ERROR reading filedir.txt' 
  ...
  !> readobsid, readpstime, writeload, accload, readmatlab, writematlab, weightsub,  
  !> usestop84, ncrit, readformat, readoutregion, simsubmodel, resultseq, steplen, 
  !> forcingdata, indtacheckonoff, indatachecklevel, doassimilation, resetstate, 
  !> outstate, instate. 
  !> The following modvar module variables is set: index for substances, 
  !> simulate, simulatesubstances, conduct, conductxoms, conductwb, conductload, 
  !> conductregest, conductwarnon, classgroup2, soiliniwet and modeloption.
  !--------------------------------------------------------------------
  SUBROUTINE load_coded_info(dir,status,date1,date2,date3,skip,step,nsubst,     &
                             rsmax,rsnum,readstatedate, &
                             mdir,rdir,fdir,ldir,conductinibin,                 &
    ...
    USE WORLDVAR, ONLY : doens,doopt, &
                         ...
                         doassimilation, &       !OUT
                         resetstate, &  !OUT
                         instate, outstate, & !OUT
                         outstateensemble, & !OUT
                         initialize_outstate, &
                         set_outstate_dates, &
                         set_directory, &
                         dtmonday !OUT
  ...
  USE READWRITE_ROUTINES, ONLY : read_next_date_on_line, &
                                 print_output_information_to_logfile, &
                                 get_number_of_classgroups
  USE CONVERT, ONLY : convert_string_to_integer, &
  ...
  USE MODEL_TEST_ROUTINES, ONLY : propagate_external_msg,e_info_file, &
    ...
    INTEGER, PARAMETER :: maxoutstates = 50     !Max number of dates for saving state
    ...
    TYPE(DateType) :: helpdate
    TYPE(DateType) :: bstateoutdate(maxoutstates)     !Read dates for saving state
    TYPE(DateType) :: daensoutdate(maxoutstates)     !Read dates for saving ensemble state
    ...
    conductwarning    = .TRUE.
    conductinibin = .FALSE.
    CALL initialize_outstate(outstate)
    CALL initialize_outstate(outstateensemble)
    bstateoutdate  = DateType(0,0,0,0,0)
    daensoutdate   = DateType(0,0,0,0,0)
    date3          = DateType(0,0,0,0,0)
    readstatedate  = DateType(0,0,0,0,0)
    nextiaddate = 1
    DO i = 1,10
      readadddate(i) = DateType(0,0,0,0,0)
    ENDDO
    ncrit          = 0
    readformat     = 0         !ascii
    nsubst         = 0
    i_in=0;i_on=0;i_sp=0;i_pp=0;i_t1=0;i_t2=0;i_oc=0;i_ss=0;i_ae=0;i_dsi=0;i_asi=0
    mdir = ''
    rdir = ''
    fdir = ''
    ldir = ''
    ...
    DO
      ...
      IF(code(1:5)=='bdate' .OR. &
         ...
         code(1:9)=='loadbdate' .OR. &
         code(1:9)=='loadedate' .OR. &
         code(1:12)=='outdaensdate' .OR. &
         code(1:11)=='readadddate' .OR. &
        ...
        CASE ('edate')
          ...
          CYCLE
        CASE ('loadbdate')
          CALL string_convert_to_DateType(strdate,accload%begindate)
          CYCLE
        CASE ('loadedate')
          CALL string_convert_to_DateType(strdate,accload%enddate)
          CYCLE
        CASE ('readadddate')
          IF(nextiaddate>10)THEN
            WRITE(6,*) 'ERROR: maximum 10 AtmdepData.txt files'
            STOP 1
          ENDIF
          CALL string_convert_to_DateType(strdate,readadddate(nextiaddate))
          nextiaddate = nextiaddate + 1
          CYCLE
        CASE ('outstatedate')
        ...
        CASE ('outdaensdate')
          CALL scalar_lower_case(strdate)
          IF(strdate(1:3)=='all')THEN
            outstateensemble%doall = .TRUE.
            CYCLE
          ELSEIF(strdate(1:6)=='period')THEN
            outstateensemble%doperiod = .TRUE.
            IF(strdate(9:9)<=CHAR(57) .AND. strdate(9:9)>=CHAR(48))THEN   !0-9 !Reading "period" with date-read can give to long strdate return extra letters to line
              linepos=linepos-6
            ELSE
              linepos = linepos-2
            ...
            CALL read_next_date_on_line(linelen,16,linepos,line,strdate,nostrfound,notimefound,errstr)
            IF(nostrfound.OR.strdate(1:2)==errstr)THEN
              WRITE(6,*) 'ERROR: No dates given for outdaensdate period'
              propagate_str = 'Not correct date for outdaensdate period in info.txt'
              CALL propagate_external_msg(e_info_file,e_error,propagate_str)
              status = 1
              RETURN
            ENDIF
            CALL string_convert_to_DateType(strdate,daensoutdate(1))
            CALL read_next_date_on_line(linelen,16,linepos,line,strdate,nostrfound,notimefound,errstr)
            IF(nostrfound.OR.strdate(1:2)==errstr)THEN
              WRITE(6,*) 'ERROR: No dates given for outdaensdate period'
              propagate_str = 'Not correct date for outdaensdate period in info.txt'
              ...
              RETURN
            ENDIF
            CALL string_convert_to_DateType(strdate,daensoutdate(2))
            CYCLE
          ELSE    !given dates
            DO
              outstateensemble%numdates = outstateensemble%numdates + 1
              IF(outstateensemble%numdates>maxoutstates) THEN
                WRITE(6,*) 'ERROR: Too many outdaensdate in info.txt'
                propagate_str = 'Too many outdaensdate in info.txt'
                CALL propagate_external_msg(e_info_file,e_error,propagate_str)
                status = 1
                RETURN
              ENDIF
              CALL string_convert_to_DateType(strdate,daensoutdate(outstateensemble%numdates))
              CALL read_next_date_on_line(linelen,16,linepos,line,strdate,nostrfound,notimefound,errstr)
              IF(nostrfound.OR.strdate(1:2)==errstr)THEN
                EXIT
              ENDIF
      ...
      IF(code(1:10)=='steplength')THEN
        ...
        IF(nostrfound)THEN   ! No error here if not defined, only using default values
          IF(conductwarning) WRITE(6,'(A60,I2,A3)') &
        ...
        ELSE
          ...
          ELSE
            IF(conductwarning) WRITE(6,'(A65,I2,A3)') &
      ...
      IF(code(1:12)=='assimilation'.OR. &
         ...
         code(1:12)=='useicecurves' .OR. &
         code(1:12)=='usefixedseed')THEN
        ...
        CASE ('submodel')
          simsubmodel = onoff
        CASE ('instate')
          instate%fromfile = onoff
        CASE ('instatecompress')
          instate%compress = onoff
        CASE ('outstatecompress')
          outstate%compress = onoff
        CASE ('indaensstate')
          conductinibin = onoff
        CASE ('resseqnr')
          resultseq = onoff
      ...
      IF(code(1:9) =='substance')THEN
        DO
          ...
          CASE('T','t')
            ...
            END SELECT
          CASE('S','s')
            IF(strvalue(2:2)=='i'.OR.strvalue(2:2)=='I')THEN
              i_dsi = nsubst+1
              i_asi = nsubst+2
              nsubst=nsubst+2
              conduct%simSi = .TRUE.
...
25      CONTINUE
        IF(code2(1:11)=='definegroup')THEN
          ...
          DO
            ...
            IF(ndefgroup(idefgroup)>maxclassingroup.AND.conductwarning)THEN
        ...
        IF(code2(1:8)=='allbasin')THEN
          ...
          ELSEIF(code(1:12)=='regionoutput'.OR.code(1:9)=='mapoutput'.OR.code(1:10)=='timeoutput')THEN
            IF(conductwarning) WRITE(6,*) 'WARNING: Code allbasin not used for mapoutput, timeoutput or regiooutput. Check info.txt'
          ENDIF
          CYCLE
        ENDIF
        ...
        IF(code2(1:11)=='signfigures')THEN
          ...
          ENDIF
          READ(strvalue,*,ERR=801) intvalue
!          IF(intvalue<4.AND.conductwarning) WRITE(6,*) 'WARNING: Less than 4 signfigures of '//TRIM(code(1:12))//' will destroy missing value.'
          IF(intvalue<4)THEN 
            IF(conductwarning)THEN
              WRITE(6,*) 'WARNING: Less than 4 signfigures of '//TRIM(code(1:12))//' will destroy missing value.'
              WRITE(6,*) 'WARNING: Signfigures will be set to 4.'
            ENDIF
            intvalue = 4
          ENDIF
          IF(code(1:11)=='basinoutput')THEN
        ...
        IF(code2(1:8)=='subbasin')THEN
          DO
            ...
            IF(code(1:11)=='basinoutput')THEN
              ...
              IF(ioutbasin(icheck)>maxoutbasins.AND.conductwarning)THEN
            ...
            ELSEIF(code(1:11)=='classoutput')THEN            
              ...
              IF(icoutbasin(icheck)>maxoutbasins.AND.conductwarning)THEN
        ...
        IF(code2(1:9)=='outregion')THEN
          DO
            ...
            IF(ioutregion(icheck)>maxoutbasins.AND.conductwarning)THEN
      ...
      IF(code(1:4)=='crit')THEN
        ...
        ELSEIF(code2(1:8)=='subbasin')THEN
          DO
            ...
            IF(icritbasin>maxcritbasins.AND.conductwarning)THEN
      ...
      IF(conductwarning) WRITE(6,*) 'WARNING: Unknown code on info.txt line: ',TRIM(line)
    ENDDO
...
200 CONTINUE
    ...
    IF((modeloption(p_lakeriverice)>=1.AND.i_t2==0) .OR. (modeloption(p_lakeriverice)==0.AND.i_t2>0))THEN
      WRITE(6,*) 'ERROR: lakerivericemodel requires simulation of water temperature (T2), and vice versa'
      propagate_str = 'lakerivericemodel requires simulation of water temperature (T2), and vice versa'
      CALL propagate_external_msg(e_info_file,e_error,propagate_str)
      status = 1
      RETURN
    ENDIF
    ...
    IF(modeloption(p_floodplain)==1.AND.nsubst>0)THEN
      IF(conductwarning) WRITE(6,*) 'WARNING: floodmodel 1 is not tested for substance simulations'
    ENDIF
    IF(modeloption(p_swtemperature)==1.AND.i_t2==0)THEN
      ...
      RETURN
    ENDIF
    IF(simulate%soildepthstretch.AND.(instate%fromfile.OR.outstate%numdates>0.OR.outstate%doall.OR.outstate%doperiod).AND.conductwarning)THEN
    ...
    IF(modeloption(p_wetland)==2.AND.i_t1>0)THEN
      IF(conductwarning) WRITE(6,*) 'WARNING: wetland model is not developed for general substance simulations'
    ENDIF
    IF(doassimilation)THEN
      IF(calibration)THEN
        IF(conductwarning) WRITE(6,*) 'WARNING: calibration and assimilation are both ON, which is not allowed -> assimilation is kept ON and calibration is set OFF.'
        calibration = .FALSE.
      ENDIF
      IF(doupdate(i_quseobs) .OR. & 
        ...
        IF(conductwarning) WRITE(6,*) 'WARNING: assimilation and updating are both on - make sure you dont unintentially try to assimilate the same data twice with different methods!'
      ENDIF
    ENDIF
    ...
    skip = period_length(date1,date3)         !Calculate length of warmup period for criteria and output
    step = period_length(date1,AddDates(date2,steplen)) !Calculate length of simulation period
    CALL calculate_timesteps_to_monday(date1,dtmonday)  !Set dtmonday
    IF(writeload)THEN
      helpdate = accload%begindate              !Calculate length of skipped warmup period for load output
      helpdate%Year = date3%Year
      IF(helpdate.LT.date3) helpdate%Year = helpdate%Year + 1
      accload%dtskip = period_length(date1,helpdate)
    ENDIF
    !CALL handle_relative_directories(dir,mdir,rdir,fdir,ldir)
    CALL set_directory(dir,mdir)
    CALL set_directory(dir,rdir)
    CALL set_directory(dir,fdir,mdir)
    CALL set_directory(dir,ldir)
    IF(.NOT.readdaily)THEN   
      IF(step>26*366)THEN
        WRITE(6,*)
    ...
    IF(modeloption(p_siltation)>=1 .AND. conduct%simS) conduct%siltation = .TRUE.
    ...
    DO
      ...
      i=i+1
    ENDDO
    IF(SUM(ibasinoutputvar(i:max_typeofperiods))>0.)THEN
      WRITE(6,*) 'ERROR: Basinoutput have gaps in group definitions.'
      WRITE(6,*) 'ERROR: This is not allowed.'
      STOP 1
    ENDIF
    i=1
    DO
      ...
      i=i+1
    ENDDO
    IF(SUM(imapoutputvar(i:max_typeofperiods))>0.)THEN
      WRITE(6,*) 'ERROR: Mapoutput have gaps in group definitions.'
      WRITE(6,*) 'ERROR: This is not allowed.'
      STOP 1
    ENDIF
    i=1
    DO
      ...
      i=i+1
    ENDDO
    IF(SUM(itimeoutputvar(i:max_typeofperiods))>0.)THEN
      WRITE(6,*) 'ERROR: Timeoutput have gaps in group definitions.'
      WRITE(6,*) 'ERROR: This is not allowed.'
      STOP 1
    ENDIF
    i=1
    DO
      ...
      i=i+1
    ENDDO
    IF(SUM(iregionoutputvar(i:max_typeofperiods))>0.)THEN
      WRITE(6,*) 'ERROR: Regionoutput have gaps in group definitions.'
      WRITE(6,*) 'ERROR: This is not allowed.'
      STOP 1
    ENDIF
    ...
    DO
      ...
      IF(classoutvarallbasin(i))THEN
        IF(ioutgroups(i)>0)THEN
          ...
          ENDDO
        ELSE
          IF(conductwarning) WRITE(6,*) 'WARNING: No class group given for output. Skipped!'
          io = io - 1
        ENDIF
      ELSE
        IF(icoutbasin(i)==0)THEN
          IF(conductwarning) WRITE(6,*) 'WARNING: No subbasins given for classoutput', i,'. Skipped!'
          io = io - 1
    ...
    IF (status /= 0) THEN
      ...
      RETURN
    END IF
    CALL set_outstate_dates(maxoutstates,bstateoutdate,outstate)    !Calculate previous time step mm for output of state
    CALL set_outstate_dates(maxoutstates,daensoutdate,outstateensemble)    !Calculate previous time step mm for output of ensemble state
    ...
    WRITE(6,*)'          Lake siltation',modeloption(p_siltation)    
    WRITE(6,*)'        River flow model',modeloption(p_rivermodel)
    WRITE(6,*)' River sediment & resusp',modeloption(p_sedresusp)
    WRITE(6,*)
    CALL print_output_information_to_logfile(6)
    IF(writeload.AND.(conduct%simN.OR.conduct%simP.OR.conduct%simC.OR.conduct%simS.OR.conduct%simT1))THEN
      WRITE(6,*) 'Yearly loads will be saved'
      conductload = .TRUE.
    ELSEIF(writeload)THEN
      WRITE(6,*) 'Yearly loads will not be saved. Need to simulate substances.'
      writeload = .FALSE.
    ENDIF
    IF(instate%fromfile) WRITE(6,*)'Initial state from file will be used.'
    DO i=1,rsnum
    ...
    ENDIF
    IF(usefixedseed)THEN
      !Initialise random number generator in case deterministic random is asked for
      CALL RANDOM_SEED(size=seeddim)
      ALLOCATE(randseed(seeddim))
      randseed = 5
      CALL RANDOM_SEED(put=randseed)
      DEALLOCATE(randseed)
      WRITE(6,*) 'Random number generator seeded with deterministic seed.'
    ELSE
      CALL RANDOM_SEED()
    ENDIF
    WRITE(6,*)'-------------------------------------------------'
    ...
    IF(ALLOCATED(critcgroup)) DEALLOCATE(critcgroup)
...
801 CONTINUE
  ...
  !  CHARACTER(LEN=maxcharpath),INTENT(INOUT) :: fdir !<File directory forcing (possibly relative to info file directory), default is mdir
  ...
  USE MODVAR, ONLY : conductwarning,outvarid
    ...
    DO i = 1,n
      IF(aggarea(i)==1)THEN   !upstream variables checked
        IF(outvarid(varid(i))%basearea==0)THEN
          ...
          nnew = nnew - 1
          skipped(i)=.TRUE.
          IF(conductwarning) WRITE(6,*) 'WARNING: Erronous output variable up'//outvarid(varid(i))%shortname//' removed from output'
        ENDIF
      ENDIF
    ENDDO
  ...
  !--------------------------------------------------------------------
  SUBROUTINE load_submodel_information(dir,submodelrun,basemodel,msub,status)
    ...
    IF(.NOT.ALLOCATED(basemodel%iindex)) ALLOCATE(basemodel%iindex(msub))
    basemodel%iindex(:)=-9999
    IF(submodelrun)THEN
      DO i = 1,basemodel%nsub
        DO j = 1,msub
          IF(basin(i)%subid==selectedaro(j))THEN
            basemodel%iindex(j)=i
            EXIT
          ENDIF
        ENDDO
      ENDDO
      ...
      j=0 
      DO i=1,msub 
        IF(basemodel%iindex(i)==-9999) THEN
          IF(j==0) THEN
            WRITE(6,*) ' '
            WRITE(6,*) '---------------------------------------------------'
...
200 CONTINUE
  ...
  END SUBROUTINE load_submodel_information
    ...
    DO io=1,noutput
        ...
        IF(output(io)%fileformat==4)THEN
          ...
          DO ia = 1,na
            ...
            IF(ireg>noutreg)THEN
              IF(conductwarning) WRITE(6,*) 'WARNING: regionoutput region', tempoutareas(ia), 'not found in model'
            ENDIF
          ENDDO
      ...
      ELSE  !na=0
        IF(output(io)%fileformat==1)THEN
          IF(conductwarning) WRITE(6,*) 'WARNING: no output areas found for some subbasinoutput'
        ELSEIF(output(io)%fileformat==4)THEN
          IF(conductwarning) WRITE(6,*) 'WARNING: no output areas found for some regionoutput'
        ENDIF
      ENDIF
    ENDDO
    IF(ALLOCATED(tempoutareas)) DEALLOCATE(tempoutareas)
  ...
  SUBROUTINE load_output_regions(dir,nreg,status)
    ...
    USE WORLDVAR, ONLY : noutreg,  &    !OUT
                         ...
                         fileunit_temp
    USE CONVERT, ONLY : real_to_str
    USE MODEL_TEST_ROUTINES, ONLY : propagate_external_msg,e_outregions, &
                                    e_error,e_warning,e_info
    ...
    IF(.NOT.fileex)THEN
      ...
      CALL propagate_external_msg(e_outregions,e_error,propagate_str)
      status = 1
      RETURN
    ENDIF
    WRITE(6,*) 'File opened: ', TRIM(infile)
    ...
    CALL propagate_external_msg(e_outregions,e_info,propagate_str_extra,code)
    IF(ALLOCATED(propagate_str_extra)) DEALLOCATE(propagate_str_extra)
    ...
    DO io = 1,noutput
      IF(output(io)%fileformat==2)THEN
        DO ivar = 1,output(io)%nvar
          ...
          comment = 'Table with '//TRIM(var2)//' in '//TRIM(unit)//' for map drawing'
          var = get_variable_string(output(io)%variable(ivar))  !short variable name
          slen = get_steplength_string(output(io)%period)  !output frequency
          OPEN(file=TRIM(dir)//filename,unit=fileunit_temp,status='unknown',form='formatted',ACTION='write',ERR=900)
          ...
          IF(.FALSE.)THEN
            ...
            ELSE
              comment = '!!'//TRIM(comment)
            ENDIF
            CALL write_commentline_to_file(fileunit_temp,comment)
          ELSE
            !New format of comment with coded metadata, not introduced yet
            CALL write_comment_with_metadata(commentline,model=model_version_string,variable=var,timestep=slen,unit=unit,comment=comment)
            IF(writematlab) commentline = '%'//TRIM(commentline)
            CALL write_commentline_to_file(fileunit_temp,commentline)
          ENDIF
          outtxt = ''
          IF(writematlab)THEN
            IF(output(io)%variable(ivar)%areaagg==2)THEN    !region output
...
900 CONTINUE
  ...
  SUBROUTINE write_subbasin_assessment(filedir,numsub,na,sas2,iens,runens)
    ...
    DO ia = 1, na
      ...
      ENDIF
601   FORMAT(I3.3)
      OPEN(file=TRIM(filedir)//filename,unit=fileunit_temp,status='unknown',form='formatted',ACTION='write',ERR=900)
      ...
      IF(acccalvar(ia)%areaagg==0)THEN
        IF(writematlab)THEN
          ...
          ELSE
            WRITE(fileunit_temp,'(a56,a2,a9,a6,a12,a4,a2,a4,a8,a6)')    &
              '%Subbasin assessment. Criteria is calculated for period ',outperiodname(calvarper),'. Group: ',acccalvar(ia)%cgname, ' Variables: ',outvarid(acccalvar(ia)%rec)%shortname,', ',outvarid(acccalvar(ia)%comp)%shortname,' Unit: ',outvarid(acccalvar(ia)%comp)%shortunit
          ENDIF
          WRITE(fileunit_temp,'(a)') '%SUBID'//CHAR(9)//'NSE'//CHAR(9)//'CC'//CHAR(9)//'RE(%)'//CHAR(9)//'RSDE(%)'//CHAR(9)//'Sim'//CHAR(9)//'Rec'//CHAR(9)//'SDSim'//CHAR(9)//'SDRec'//CHAR(9)//'MAE'//CHAR(9)//'RMSE'//CHAR(9)//'Bias'//CHAR(9)//'SDE'//CHAR(9)//'KGE'//CHAR(9)//'KGESD'//CHAR(9)//'KGEM'//CHAR(9)//'NRMSE'//CHAR(9)//'NSEW'//CHAR(9)//'Nrec'
        ELSE
          IF(acccalvar(ia)%cgname=='')THEN
            WRITE(fileunit_temp,'(a57,a2,a13,a4,a2,a4,a8,a6)')    &
                '!!Subbasin assessment. Criteria is calculated for period ',outperiodname(calvarper),'. Variables: ',outvarid(acccalvar(ia)%rec)%shortname,', ',outvarid(acccalvar(ia)%comp)%shortname,' Unit: ',outvarid(acccalvar(ia)%comp)%shortunit
          ELSE
            WRITE(fileunit_temp,'(a57,a2,a9,a6,a12,a4,a2,a4,a8,a6)')    &
              '!!Subbasin assessment. Criteria is calculated for period ',outperiodname(calvarper),'. Group: ',acccalvar(ia)%cgname, ' Variables: ',outvarid(acccalvar(ia)%rec)%shortname,', ',outvarid(acccalvar(ia)%comp)%shortname,' Unit: ',outvarid(acccalvar(ia)%comp)%shortunit
          ENDIF
          WRITE(fileunit_temp,'(a)') 'SUBID'//CHAR(9)//'NSE'//CHAR(9)//'CC'//CHAR(9)//'RE(%)'//CHAR(9)//'RSDE(%)'//CHAR(9)//'Sim'//CHAR(9)//'Rec'//CHAR(9)//'SDSim'//CHAR(9)//'SDRec'//CHAR(9)//'MAE'//CHAR(9)//'RMSE'//CHAR(9)//'Bias'//CHAR(9)//'SDE'//CHAR(9)//'KGE'//CHAR(9)//'KGESD'//CHAR(9)//'KGEM'//CHAR(9)//'NRMSE'//CHAR(9)//'NSEW'//CHAR(9)//'Nrec'
        ENDIF
      ELSEIF(acccalvar(ia)%areaagg==2)THEN
        IF(writematlab)THEN
          ...
          WRITE(fileunit_temp,'(a)') '%OUTREGID'//CHAR(9)//'NSE'//CHAR(9)//'CC'//CHAR(9)//'RE(%)'//CHAR(9)//'RSDE(%)'//CHAR(9)//'Sim'//CHAR(9)//'Rec'//CHAR(9)//'SDSim'//CHAR(9)//'SDRec'//CHAR(9)//'MAE'//CHAR(9)//'RMSE'//CHAR(9)//'Bias'//CHAR(9)//'SDE'//CHAR(9)//'KGE'//CHAR(9)//'KGESD'//CHAR(9)//'KGEM'//CHAR(9)//'NRMSE'//CHAR(9)//'NSEW'//CHAR(9)//'Nrec'
        ELSE
    ...
    !Error handling
900 CONTINUE
    WRITE(6,*) 'ERROR: Open file: '//TRIM(filedir)//TRIM(filename)//' for writing.'
    STOP 1
...
900 CONTINUE
  ...
  SUBROUTINE open_subbasinfiles(dir,n,iens,runens,allens,ensstat)
    ...
    USE MODVAR, ONLY : outvarid,     &
                       ...
                       nsub, &
                       get_classgroup_classes
    USE READWRITE_ROUTINES, ONLY : write_commentline_to_file
    ...
    DO io=1,noutput
      IF(output(io)%fileformat==1)THEN
        ...
        DO i = 1,nselected
          ...
          OPEN(UNIT=funit,FILE=TRIM(dir)//TRIM(str),status='unknown',form='formatted',ACTION='write',ERR=900)
    ...
    DO io=1,noutput
      IF(output(io)%fileformat==6)THEN
        ...
        CALL write_integer_array_to_string(nslc,classindex(1:nslc),classes)
        ...
        DO i = 1,nselected
          ...
          OPEN(UNIT=funit,FILE=TRIM(dir)//TRIM(str),status='unknown',form='formatted',ERR=900)
          IF(writematlab)THEN
            commentheading = '%This is a file with variables grouped for classes ('//TRIM(classes)//')'
          ELSE
            commentheading = '!!This is a file with variables grouped for classes ('//TRIM(classes)//')'
          ENDIF
          CALL write_commentline_to_file(funit,commentheading)
          WRITE(funit,'(a)') outtxt(1:lout1)
          WRITE(funit,'(a)') outtxt2(1:lout2)
        ENDDO
      ENDIF
...
900 CONTINUE
    WRITE(6,*) 'ERROR: Open file: '//TRIM(dir)//TRIM(str)
    STOP 1
  ...
  !--------------------------------------------------------------------
  SUBROUTINE write_subbasinfiles(io,idt,ndt,ct)

    USE WORLDVAR, ONLY : output, &
                         writematlab, &
                         outstartdate
    USE MODVAR, ONLY : nsub,TIMEINFORMATIONTYPE
    ...
    IF(output(io)%nvar>0)THEN  !Write period mean for selected subbasins
      ...
      DO ia = 1,n
        CALL compute_basinoutput(ct,io,ia,output(io)%nvar,x,pwrite,idt,ndt,1)
        IF(pwrite) CALL write_dataline(output(io)%fileunit(ia),output(io)%nvar,x,output(io)%decimal,  &
                        output(io)%signfig,output(io)%period,CHAR(9),0,writematlab,d=ct%date,odate=outstartdate)
      ENDDO
    ENDIF
  ...
  !--------------------------------------------------------------------
  SUBROUTINE write_subbasinfiles_in_parallel(io,idt,ndt,iens,ct)
    ...
    IF(output(io)%nvar>0)THEN  !Write period mean for selected subbasins
      ALLOCATE(x(output(io)%nvar))
      DO ia = 1,n
        CALL compute_basinoutput(ct,io,ia,output(io)%nvar,x,pwrite,idt,ndt,iens)
  ...
  !--------------------------------------------------------------------
  SUBROUTINE write_regionfiles(io,idt,ndt,ct)

    USE MODVAR, ONLY : TIMEINFORMATIONTYPE
    USE WORLDVAR, ONLY : output, &
                         noutreg, &
                         writematlab, &
                         outstartdate
    ...
    DO i = 1,nselected
      ...
      CALL compute_regionoutput(ct,io,ir,i,output(io)%nvar,idt,ndt,1,pwrite,x)
  ...
  !>Calculate and write output region files
  !--------------------------------------------------------------------
  SUBROUTINE write_regionfiles_in_parallel(io,idt,ndt,iens,ct)
    ...
    INTEGER, INTENT(IN) :: ndt          !<Maximum simulation time steps
    INTEGER, INTENT(IN) :: iens         !<Current simulation
    TYPE(TIMEINFORMATIONTYPE), INTENT(IN) :: ct    !<Current date-time
    ...
    DO i = 1,nselected
      ir = i
      IF(output(io)%narea>0) ir = output(io)%areaindex(i)
      CALL compute_regionoutput(ct,io,ir,i,output(io)%nvar,idt,ndt,iens,pwrite,x)
  ...
  !>Calculate and write subbasin files
  !--------------------------------------------------------------------
  SUBROUTINE write_subbasinfiles_class(io,idt,ndt,ct)
  ...
  !>Writes sequences' files in parallel
  !--------------------------------------------------------------------
  SUBROUTINE write_subbasinfiles_class_in_parallel(io,idt,ndt,iens,ct)
    ...
    DO io = 1, noutput
      IF(output(io)%fileformat==3)THEN
        ...
        DO ivar = 1,output(io)%nvar
          ...
          !Write headings
          IF(.FALSE.)THEN
            comment = 'This is a file with timeseries of '//TRIM(var2)//' in '//TRIM(unit)
            IF(writematlab)THEN
              comment = '%'//TRIM(comment)
            ...
            ELSE
              ...
              lout = 5
            ENDIF
            CALL write_commentline_to_file(funit,comment)
            IF(output(io)%variable(ivar)%areaagg==2)THEN    !region output
          ...
          ELSE
            !New format of comment with coded metadata, not introduced yet
            comment = 'Timeseries of '//TRIM(var2)//' in '//TRIM(unit)
            CALL write_comment_with_metadata(commentline,model=model_version_string,variable=var,timestep=slen,unit=unit,comment=comment)
            IF(writematlab) commentline = '%'//TRIM(commentline)
            CALL write_commentline_to_file(funit,commentline)
            IF(writematlab)THEN
              outtxt(1:6) = '%DATE'//CHAR(9)
              lout = 6
            ELSE
...
900 WRITE(6,*) 'ERROR: Open file '//TRIM(dir)//TRIM(filename)//' for writing.'
  ...
  SUBROUTINE open_timefiles_classes(dir,n,iens,runens,allens,ensstat)
    ...
    INTEGER, ALLOCATABLE :: classindex(:)
    CHARACTER(LEN=29) filename
    CHARACTER(LEN=20) var,fmtstr,slen
    CHARACTER(LEN=40) var2
    CHARACTER(LEN=30) unit
    INTEGER lout
    CHARACTER (LEN=150) classes
    ...
    DO io = 1, noutput
      IF(output(io)%fileformat==5)THEN
        ...
        DO ivar = 1,output(io)%nvar
          ...
          !Write headings
          IF(.FALSE.)THEN
            comment = 'This is a file with timeseries of '//TRIM(var2)//' in classes ('//TRIM(classes)//') in '//TRIM(unit)
            IF(writematlab)THEN
              comment = '%'//TRIM(comment)
              outtxt(1:6) = '%DATE'//CHAR(9)
              lout = 6
          ...
          ELSE
            !New format of comment with coded metadata, not introduced yet
            comment = 'Timeseries of '//TRIM(var2)//' in classes ('//TRIM(ADJUSTL(classes))//') in '//TRIM(unit)
            CALL write_comment_with_metadata(commentline,model=model_version_string,variable=var,classes=ADJUSTL(classes),timestep=slen,unit=unit,comment=comment)
            IF(writematlab) commentline = '%'//TRIM(commentline)
            CALL write_commentline_to_file(funit,commentline)
            IF(writematlab)THEN
              outtxt(1:6) = '%DATE'//CHAR(9)
...
603    FORMAT(I3.3)
  ...
  !--------------------------------------------------------------------
  SUBROUTINE write_timefiles_class(io,idt,ndt,ct)
    ...
    DO ivar = 1, output(io)%nvar
      ...
      ELSE
        n = nsub
      ENDIF
      CALL compute_timeoutput(ct,io,ivar,n,y,pwrite,idt,ndt,1)
      IF(pwrite) CALL write_dataline(output(io)%fileunit(ivar),n,y,output(io)%decimal,  &
                      output(io)%signfig,output(io)%period,CHAR(9),0,writematlab,d=ct%date,  &
                      odate=outstartdate)
    ENDDO 
  ...
  !>Calculate time aggregate of time-output and writes to file
  !--------------------------------------------------------------------
  SUBROUTINE write_timefiles(io,idt,ndt,ct)
  ...
  !--------------------------------------------------------------------
  SUBROUTINE write_timefiles_class_in_parallel(io,idt,ndt,iens,ct)
    ...
    DO ivar = 1, output(io)%nvar
      ...
      ELSE
        n = nsub
      ENDIF
      CALL compute_timeoutput(ct,io,ivar,n,y,pwrite,idt,ndt,iens)
  ...
  !>for data assimilation)
  !--------------------------------------------------------------------
  SUBROUTINE write_timefiles_in_parallel(io,idt,ndt,iens,ct)
...
110   CONTINUE
  ...
  !--------------------------------------------------------------------
  SUBROUTINE load_parameters(dir,basemodel,status,seqflag,iens)
    ...
    USE WORLDVAR, ONLY : fileunit_temp, &
                         ...
                         BASEMODELTYPE
    USE MODVAR, ONLY : conductwarning, &
                       soilpar,    &   !OUT, parameters depending on soil type
    ...
    dim = MAX(1,basemodel%nsub,nsoil,nluse,MAXVAL(nregions),12)
    nbasinpar = SIZE(basinpar,1)
    IF(.NOT.ALLOCATED(values)) ALLOCATE(values(dim))
    IF(.NOT.ALLOCATED(temp_basinpar)) ALLOCATE(temp_basinpar(nbasinpar,basemodel%nsub))
    ...
    DO 
      ...
      ENDDO
      IF(varindex==0)THEN
        IF(conductwarning) WRITE(6,*) 'WARNING: unknown parameter name on line. Parameter ',TRIM(varstr),' ignored.'
        propagate_str=TRIM(propagate_str)//','//TRIM(varstr)
      ELSE
        ...
        CASE(m_gpar)
          dimcheck = 1                   !genpar
        CASE(m_bpar)
          dimcheck = basemodel%nsub              !basinpar
        CASE(m_spar)
          dimcheck = nsoil
        CASE(m_lpar)
          dimcheck = nluse
        ...
        IF(dimcheck /= nvalues)THEN
          IF(dimcheck<nvalues)then
            ...
            IF(conductwarning) WRITE(6,*) 'WARNING: '//TRIM(propagate_str2)
          ELSE
...
100 CONTINUE
    ...
    IF(SIZE(basinpar,1)>0)THEN  !any basinpar defined?
      ...
      ELSE    !simsubmodel, call in ensemble loop
        basinpar = temp_basinpar(:,basemodel%iindex)
      ENDIF
    ENDIF
    ...
    IF(ALLOCATED(propagate_str)) DEALLOCATE(propagate_str)
    IF(ALLOCATED(propagate_str2)) DEALLOCATE(propagate_str2)
    IF(ALLOCATED(temp_basinpar)) DEALLOCATE(temp_basinpar)
...
101 CONTINUE
  ...
  SUBROUTINE load_optpar(dir) 
    ...
    USE WORLDVAR, ONLY : doopt,doens, &
                         ...
                         numoptimpar, &
                         fileunit_temp
    USE MODVAR, ONLY : conductwarning,modparid,max_par, &
...
133   CONTINUE
    ...
    IF(doopt)THEN
      ...
      IF(optim%task_DEMC.AND.optim%DEMC_npop<3)THEN
        ...
        WRITE(6,*) 'ERROR: check file ',TRIM(filename)
        STOP 1
      ENDIF
...
198 WRITE(6,*) 'ERROR: open optpar.txt'
    WRITE(6,*) 'ERROR: File ',TRIM(filename)
    CLOSE(fileunit_temp)
    STOP 1
...
199 WRITE(6,*) 'ERROR: reading ',TRIM(filename),' for river dimension, io=',io
  ...
  SUBROUTINE save_respar(dir,numpar,n) 
    ...
    OPEN(UNIT = fileunit_temp,FILE = filename,STATUS = 'unknown',ACTION='write',ERR=900)
...
900 CONTINUE
  ...
  !>
  !>\b Consequences Module worldvar variables maptime,  
  !> tmap, output, and accload are set.
  !------------------------------------------------------------------
  SUBROUTINE initiate_output_routines() 

    USE WORLDVAR, ONLY : maptime, &  !OUT
                         output, & !OUT
                         tmap, &  !OUT
                         set_accload_to_zero, &
                         writeload, &
  ...
  !>\brief Get the name of a variable from output.
  !---------------------------------------------------------------------
  FUNCTION get_variable_string(outputvariable) RESULT(name)

    USE WORLDVAR, ONLY : OUTPUTVARIABLETYPE
    USE MODVAR, ONLY : outvarid
    
    !Argument declarations
    TYPE(OUTPUTVARIABLETYPE) :: outputvariable !<variable from output; output(io)%variable(ivar)
    CHARACTER(LEN=20)  :: name     !< \retval the variable name
    
    INTEGER iout
    
    !< Get whole shortname of variable
    name=''
    iout=0
    IF(outputvariable%areaagg==2)THEN
      name(iout+1:iout+2) = 'rg'
      iout=iout+2
    ELSEIF(outputvariable%areaagg==1)THEN
      name(iout+1:iout+2) = 'up'
      iout=iout+2
    ENDIF
    name(iout+1:iout+4) = outvarid(outputvariable%idindex)%shortname

  END FUNCTION get_variable_string
  ...
  SUBROUTINE save_ensemble_simulations(dir,numpar,nperf,numcrit,&
    ...
    !Open files
    OPEN(file=TRIM(dir)//filename_best,unit=fileunit_temp,status='unknown',form='formatted',ACTION='write',ERR=900)
    !Write heading
    ...
    !Error handling
900 CONTINUE
    WRITE(6,*) 'ERROR: Open file: '//TRIM(dir)//TRIM(filename_best)//' for writing.'
    STOP 1
  ...
  SUBROUTINE prepare_save_all_simulations(dir,filename,funit,numpar,nperf,numcrit)
    ...
    !Open file
    OPEN(file=TRIM(dir)//filename,unit=funit,status='unknown',form='formatted',ACTION='write',ERR=900)
    !Write heading
  ...
  SUBROUTINE load_ensemble_simulations(dir,fname,numpar,nperf,numcrit,&
    ...
    DO i = 1,nsim
      CALL read_simulation_perf_and_par(fileunit_temp,i,numpar,numpar,nperf,numcrit,&
            optcrit(i),performance(i,:,:),parameters(i,:),status)
      IF(status/=0)THEN
        WRITE(6,*) 'ERROR: Reading file: ',TRIM(dir)//TRIM(fname)
        STOP 1
      ENDIF
    ENDDO
...
100 WRITE(6,*) 'ERROR: Open parameter file: ',TRIM(dir)//TRIM(fname)
  ...
  SUBROUTINE read_simulation_perf_and_par(funit,i,mpar,numpar,nperf,&
       numcrit,optcrit,performance,parameters,status)
    ...
    INTEGER,INTENT(OUT) :: status        !<flag for subroutine status
  ...
  !>Could be the load of a part (period) of the last year.
  !>
  !>\b Consequences Module worldvar variable accload are zeroed.
  !--------------------------------------------------------------------
  SUBROUTINE save_loadfiles(dir,year)

    USE WORLDVAR, ONLY : accload, & !OUT
                         fileunit_temp, &
                         set_accload_to_zero
    USE MODVAR, ONLY : basin,               &
                       numsubstances,       &
    ...
    DO l=2,max_classoutvar+1
      DO j=1,nclass
        ...
        WRITE(headings(k),'(a6,a1,I3.3,a)') loadheadings(l),'_',j,CHAR(9)     !class dependent loads
      ENDDO
    ENDDO
    DO l=max_classoutvar+2,max_classoutvar+max_basinoutvar+1 
      k=max_classoutvar*nclass+(l-max_classoutvar)
      WRITE(headings(k),'(a6,a)') loadheadings(l),CHAR(9)                   !subbasin dependent loads
    ENDDO
    ...
    DO i = 1,numsubstances
      IF(i/=i_t2)THEN
        !>\li Open file
        IF(period_length(accload%begindate,accload%enddate)==365.OR.period_length(accload%begindate,accload%enddate)==366)THEN
          WRITE(str,'(I4,a,a2)') year,'_',substance_name(i)
        ELSE
          WRITE(str,'(I4,a2,a2)') year,'X_',substance_name(i)
        ENDIF
        OPEN(UNIT=ffunit,FILE=TRIM(dir)//TRIM(ADJUSTL(str))//'.txt',status='unknown',form='formatted',ACTION='write',ERR=900)
        !>\li Write heading prepared above
        WRITE(ffunit,'(8014a11)') headings
        !>\li Write data for each subbasin on rows
        DO isb = 1,nsub
          !Write data to a row array
          DO k=1,max_classoutvar
            xload(((k-1)*nclass+1):(k*nclass))=accload%slcclass(1:nclass,k,i,isb)
          ENDDO
          xload((max_classoutvar*nclass+1):(max_classoutvar*nclass)+max_basinoutvar)=accload%basin(i,:,isb)
          CALL write_dataline(ffunit,nvalues,xload,3,0,0,CHAR(9),0,.FALSE.,id=basin(isb)%subid) 
        ENDDO ! end subbasin loop (s)
        CLOSE(ffunit)
      ENDIF
    ...
    !Error handling
900 CONTINUE
    WRITE(6,*) 'ERROR: Open file: '//TRIM(dir)//TRIM(ADJUSTL(str))//'.txt for writing.'
    STOP 1

...
!Copyright 2012,2014,2016-2022 SMHI
!
...
CONTAINS
  ...
  REAL FUNCTION moisturefactor(sm,wp,pw,thickm,satsmf,exp,thetalow,thetaupp)
    ...
    ! Local variables
    REAL thickmm  !thickness of soil layer (mm)
    REAL smfcn    !soil moisture dependence factor
    
    !Initiations    
    smfcn = 0.
    thickmm = thickm * 1000.      !mm
    ...
    IF(thickmm>0)THEN
      ...
      ELSEIF(sm <= wp) THEN
        smfcn=0.
      ELSE 
        smfcn = min(1., (1-satsmf)*((pw-sm)/((thetaupp/100.)*thickmm))**exp + satsmf, ((sm-wp)/((thetalow/100.)*thickmm))**exp)    
      ENDIF
    ENDIF
  ...
  !-----------------------------------------------------------------------------
  REAL FUNCTION exponential_decay(timesteps,halflife)
      
    !Argument declarations
    REAL, INTENT(IN) :: timesteps     !<in unit of half time
    REAL, INTENT(IN) :: halflife      !<half-life of decay
    ...
    !Calculate discharge
    IF(halflife>0)THEN
      factor = EXP(-LOG(2.)*timesteps/halflife)
    ELSE
      WRITE(6,*) 'ERROR: half-life parameter is zero. Check parameter file.'
      STOP 1
    ENDIF
  ...
  !>\brief Calculate sedimentation effect on concentration from sinking velocity
  !-----------------------------------------------------------------
  SUBROUTINE sedimentation(depth,conc,vel)
    ...
    !pool = depth * conc
    !!  
    !!!Calculate sedimentation      !Change to simpler code with less transformations
    !sedloss = vel * conc
    !!
  ...
  REAL FUNCTION linear_response(x,xs,xe,ys,ye)
    ...
    ELSEIF(xt.GE.xet)THEN
      linear_response = 1.
    ELSE
!      linear_response = 1./(xe-xs) !CP220103 bug!
      linear_response = xt/(xe-xs)
    ENDIF

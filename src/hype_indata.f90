...
MODULE HYPE_INDATA
  ...
  !Copyright 2014-2015,2019-2021 SMHI
  !
...
! !>These files use sequence number.
  ...
  INTEGER            :: max_nstn          !<Maximum number of stations (columns) in any file
  INTEGER ncom(max_xoms,2)                !<Number of comment rows in Xoms files (starting with !!)
  INTEGER, ALLOCATABLE :: xoms_funit(:,:) !<File unit of files with observations (nvar,2)
  INTEGER, ALLOCATABLE :: xoms_nstn(:,:)  !<Number of stations (columns in file) (nvar,2)
...
!    INTEGER ncom(max_xoms,2)    !number of comment rows (was one)
    ...
    DO i = 1,max_xoms
      DO j = 1,2
        ...
        IF(fex(i,j))THEN
          ...
          funit = fileunit_get()
          CALL count_comment_rows(funit,TRIM(dir)//TRIM(xoms_filename(i,j)),ncom(i,j),status)   !number of comment rows, starting with !!
          IF(ncom(i,j)<1)THEN
            WRITE(6,*) 'WARNING: No comment row found in file ',TRIM(dir)//TRIM(xoms_filename(i,j))   !To read old files without '!!'. File without comment row will give error later in check_stations.
          ...
          ENDIF
          IF(status/=0) RETURN
          CALL check_station(funit,TRIM(dir)//TRIM(xoms_filename(i,j)),ns,ncom(i,j),subnr,   &   !Skip ncom rows
        ...
        IF(fex(i,j))THEN
          ...
          funit = fileunit_get()
          CALL check_obs_timeperiod(funit,TRIM(dir)//TRIM(xoms_filename(i,j)),ncom(i,j)+1,0,   &    !i_t=0 timesteply file
    ...
    DO i = 1,num_xomsfiles !Number of files
      DO j = 1,2
        IF(fex(i,j))THEN
          ...
          max_nstn = MAX(max_nstn,numobsstn(i,j))
          IF(j==1) xomindex(i,:) = localindex(:,i,j)
          IF(j==2) xosindex(i,:) = localindex(:,i,j)
          CALL prepare_read_matrix(xoms_funit(i,j),TRIM(dir)//TRIM(xoms_filename(i,j)),ncom(i,j)+1,xoms_fbdate(i,j),status)
  ...
  SUBROUTINE get_current_xoms(cd,n)
    ...
    DO i=1,num_xoms(j)
      IF(xoms_funit(i,j)>0)THEN
        ...
        CALL READ_MATRIX_LINE(xoms_funit(i,j),xoms_nstn(i,j),d,x(1:xoms_nstn(i,j)),missing_value,.TRUE.,status)  !This do not manage files with time in date.
        IF(status==1)THEN
          WRITE(6,*) 'ERROR: reading XOM-file'
          STOP 1
        ENDIF
        DO k=1,n
    ...
    DO i=1,num_xoms(j)
      IF(xoms_funit(i,j)>0)THEN
        IF(cd.LT.xoms_fbdate(i,j) .OR. cd.GT.xoms_fedate(i,j)) CYCLE
        CALL READ_MATRIX_LINE(xoms_funit(i,j),xoms_nstn(i,j),d,x(1:xoms_nstn(i,j)),missing_value,.TRUE.,status) 
        IF(status==1)THEN
          WRITE(6,*) 'ERROR: reading XOS-file'
          STOP 1
        ENDIF
        DO k=1,n

...
!-----------------------------------------------------------------------------------------
  ...
  CHARACTER(LEN=24),PARAMETER :: model_version_string = '5.18.0' !<String with model version numbers, e.g. 1.1.0
...
!> \{
  ...
  REAL, PARAMETER    :: def_min = -1.1E38       !<default minimum value for state variables
  REAL, PARAMETER    :: def_max =  1.1E38       !<default maximum value for state variables
  ...
  REAL, PARAMETER    :: Lfreezing = 335.        !<Latent heat of freezing, kJ/kg (for ice and snow)
  REAL, PARAMETER    :: Lfusion = 334000.          !<Latent heat of fusion, J/kg (for liqfrac of soil)
...
!> \brief Type for holding data about simulation configuration for states
  TYPE STATECONFIGURATIONTYPE
    ...
    LOGICAL :: simSi          !<status of silica simulation
    ...
    LOGICAL :: siltation      !<Status of siltation affecting hydrology
...
!> \brief Type for holding dimension data about simulation configuration for states
  TYPE STATEDIMENSIONTYPE
    ...
    REAL,ALLOCATABLE :: substancelimits(:,:)  !<Minimum and maximum value of substance concentrations (numsubstance,2=min/max)
...
!> \brief Type to hold information on current time  
  TYPE TIMEINFORMATIONTYPE
    TYPE(DateType) date      !<Current date in DateType format Year yyyy, Month 01-12, Day 01-31, Hour 00-23, Minute 00-59
    INTEGER dayno            !<Current day number of year (Julian day)
    INTEGER prevdoy          !<Number of days the previous year
    INTEGER tsofday          !<Current time step of day (e.g. second ts of 24 this day with hourly timesteps)
    INTEGER dayofweek        !<Current day of the week (1=Monday, .., 7=Sunday)
...
!> \brief Type for holding data about subbasin characteristics
  TYPE BASINTYPE
    ...
    REAL    :: rivlen(2) = -1.  !<length of local (1) and main (2) river (m)
    REAL    :: ilakecatch = 0. !<part of catchment/runoff that runs to ilake
    REAL    :: iwetcatch = 0.  !<part of catchment/runoff that runs to iwet
    REAL    :: lakedepth(2) = 0.   !<lake water depth at threshold (m) (ilake and olake), no siltation?
    ...
    REAL    :: eroindex = 1.   !<erosion index, used for soil erosion model 1
    REAL    :: clay = 0.       !<soil clay fraction, used for siltation model 2&3
    REAL    :: sand = 0.       !<soil sand fraction
    REAL    :: silt = 0.       !<soil silt fraction
    REAL    :: channelfactor = -1.    !<river channel erodability/cover modification factor of resuspension (0-1), used for sedresp model 2 [-]
    REAL    :: weatheringfactor = 1. !<weathering of silica adjustment, e.g. lithology factor
  ...
  END TYPE BASINTYPE
!> \brief Type for holding rating curve parameters; q=k*(w-w0)**p
  TYPE RRCQTYPE
    REAL    :: k = 0.  !<coefficients for river rating curve 
    REAL    :: p = 0.  !<coefficients for river rating curve 
  END TYPE RRCQTYPE
!> \brief Type for holding rating curve parameters; w=a*q**b
  TYPE RRCWTYPE
    REAL    :: a = 0.  !<coefficients for inverse river rating curve 
    REAL    :: b = 0.  !<coefficients for inverse river rating curve 
  END TYPE RRCWTYPE
!> \brief Type for holding rating curve parameters; w=w0+a*q**b
  TYPE RRCW0TYPE
    REAL    :: a = 0.  !<coefficients for inverse river rating curve 
    REAL    :: b = 0.  !<coefficients for inverse river rating curve 
    REAL    :: w0 = 0. !<coefficients for inverse river rating curve 
  END TYPE RRCW0TYPE
!> \brief Type for holding data about different ice season river rating curves
  TYPE ICESEASONRATINGCURVETYPE
    TYPE(RRCQTYPE) :: qpar(2)   !<coefficients for two rating curves
    TYPE(RRCWTYPE) :: wpar(2)   !<coefficients for two inverse rating curves
  END TYPE ICESEASONRATINGCURVETYPE
!> \brief Type for holding information about river rating curves divided into sections
  TYPE SECTORRATINGCURVETYPE
    REAL :: qmax  !<upper border flow of sector inverse rating curve (lower border is given by previous sector or zero)
    TYPE(RRCW0TYPE),ALLOCATABLE :: wpar(:)
  END TYPE SECTORRATINGCURVETYPE
!> \brief Type for holding information about river rating curves for calculation main river water level
  TYPE RIVERRATINGTYPE
    INTEGER :: typeid = 0     !<type of river rating curve
    REAL    :: w0ref  = 0.    !<w0 in reference system, water level of zero flow
    REAL    :: gauge0 = 0.    !<0 of local gauge scale in w0ref's reference system
    REAL    :: gunitconv = 0. !<unit transformation factor for local gauge scale, e.g. 100 for m->cm
    TYPE(RRCWTYPE),ALLOCATABLE :: single(:)    !used for typeid=1
    TYPE(ICESEASONRATINGCURVETYPE),ALLOCATABLE :: ice(:)    !used for typeid=2
    TYPE(SECTORRATINGCURVETYPE),ALLOCATABLE :: sector(:)    !used for typeid=3
  END TYPE RIVERRATINGTYPE
!> \brief Type for holding information about dams
  TYPE DAMTYPE
    ...
    INTEGER :: datum1  = 0   !
    INTEGER :: datum2  = 0   !
    TYPE(DateType) :: btdate(2) = DateType(0,0,0,0,0) !<start and end date of regulation for dam (build,tear down)
  END TYPE DAMTYPE
!> \brief Type for holding information about sediment management of lakes/dams
  TYPE SEDIMENTMANAGEMENTTYPE
    INTEGER :: mode = 0     !<sediment management mode (0=no flushing, 1=flushing by capacity, 2=flushing by dayno)
    INTEGER :: dest = 0     !<sediment flush destination (0=remove the sediments (dredging?), 1=flush to downstream subbasin)
    REAL    :: start = 0.   !<sediment flush start (mode 1:fraction of reservoir storage capacity, mode 2:day of year)
    REAL    :: sstop = 0.   !<sediment flush end (mode 1:fraction of reservoir storage capacity, mode 2:day of year)
    REAL    :: rate = 0.    !<sediment flush rate (fraction of reservoir storage capacity removed per day) (mode 1 and 2)
    INTEGER :: frequency = 1 !<sediment flush frequency (mode 2:years)
  END TYPE SEDIMENTMANAGEMENTTYPE
!> \brief Type for holding information about lakes/lakeoutflow
  TYPE LAKETYPE
    ...
    REAL    :: wampcoeff     !<adjustment factor for waterstage to "real" regulation amplitude
    REAL    :: limprod = missing_value  !<fractional level when production is starting to be limited (-)
    INTEGER :: resmode = 0   !<Reservoir operation mode (1=sediment always submerged or nearly submerged, 2=normally moderate to considerable reservoir drawdown, 3=reservoir normally empty, 4=riverbed sediments)
...
!> \brief Type for holding information about lakes/lakeoutflow
  TYPE OUTLETTYPE
    ...
    REAL    :: qamp = 0.     !<regulated lake, amplitude of variation of average discharge between wmin and w0
    REAL    :: qpha = 102.   !<regulated lake, phase of variation of average discharge (dayno)
    REAL    :: rate = 0.     !<rate parameter of outflow equation (m2/s)
    REAL    :: exp = 0.      !<exponent parameter of outflow equation
    REAL    :: deltaw0 = 0.  !<difference in threshold for lake rating curve production period 2, waterstage parameter of outflow equation (m)
...
!> \brief Type for holding applied nutrient load data
  TYPE NPCLOADTYPE  
    ...
    INTEGER :: abstrind = 0    !<Index for abstraction source; 1=main river,2=olake,3=main river inflow,4=aquifer
...
!> \brief Type for holding deposition data
  TYPE DEPOSITIONTYPE
    ...
    INTEGER,ALLOCATABLE :: depcode(:,:) !(veg/time,dry/wet)
    REAL,ALLOCATABLE :: dryload(:,:,:)  !<Load of (dry) deposition (kg/km2/ts) [veg/luse,nsub,12/366]
    REAL,ALLOCATABLE :: wetconc(:,:,:)  !<Concentration in precipitation (ug/L?) [veg/luse,nsub,12/366]
...
!> \brief Type for holding information about classes
  ...
  END TYPE CLASSTYPE
  TYPE CLASSATTRIBUTETYPE  !Hold initial infomation for all class (for each siol type and landuse)
  INTEGER :: luse          !<landuse code
  INTEGER :: soil          !<soil type code
  INTEGER :: crop          !<main crop id number
  INTEGER :: crop2         !<secondary crop id number
...
!> \brief Type for holding information about output variables
  TYPE OUTVARIDTYPE
    ...
    REAL              :: min = 0.       !<minimum value of variable
    REAL              :: max = def_max  !<maximum value of variable
...
!> \{
  ...
  INTEGER :: i_dsi    !<index of variable, dissolved silica
  INTEGER :: i_asi    !<index of variable, (suspended) algae (silica)
  INTEGER,PARAMETER :: max_substances = 11     !<maximum number of substances that can be simulated
  ...
  LOGICAL :: conducticecurve      !<Status for using river water level ice curves, default is TRUE
  LOGICAL :: conductdamconstruction      !<Status for building or removing dams during simulation period
  ...
  INTEGER, PARAMETER :: p_frozensoil     = 17 !Options for frozen/unfrozen soil water content (0) no frozen water in soil (current HYPE), (1) unfrozen water as function of temperature, (2) unfrozen water as function of three temperatures
  ...
  INTEGER, PARAMETER :: p_siltation      = 21 !Lake/reservoir sedimentation affecting hydrology, (0) off (lake/reservoir does not affect hydrology), (1) no compaction + general density, (2) no compaction + density from soil fraction & reservoir operation, (3) compaction + density from soil fraction & reservoir operation
  INTEGER, PARAMETER :: p_sedresusp      = 22 !River sedimentation/resuspension, (0) original HYPE method, (1) flow dependent (2) Simplified Bagnold Equation
  INTEGER, PARAMETER :: p_rivermodel     = 23 !River flow model (0) original with corrected dampq, (1) original
  INTEGER, PARAMETER :: num_modelprocess = 23 !Number of processes with options in HYPE
  CHARACTER(LEN=16),ALLOCATABLE :: modeloptionname(:)
  INTEGER,ALLOCATABLE :: modeloption(:)
  
  INTEGER :: maxmodelsinoption(num_modelprocess)
  PARAMETER(maxmodelsinoption = [2,3,3,6,3,2,2,2,3,2,2,4,2,3,4,3,3,2,5,2,4,3,2])
  ...
  CHARACTER(LEN=64) :: siltationmodelnames(0:maxmodelsinoption(p_siltation)-1)   !<siltation names
  PARAMETER(siltationmodelnames = [CHARACTER(LEN=64) :: 'off (lake/reservoir does not affect hydrology)', 'no compaction + general density','no compaction + density from soil fraction & reservoir operation', 'compaction + density from soil fraction & reservoir operation']) 
  CHARACTER(LEN=64) :: sedresuspmodelnames(0:maxmodelsinoption(p_sedresusp)-1)   !<sedimentation/resuspension names
  PARAMETER(sedresuspmodelnames = [CHARACTER(LEN=64) :: 'original flow dependent', 'flow dependent with parameter', 'simplified Bagnold equation'])
  CHARACTER(LEN=64) :: rivermodelnames(0:maxmodelsinoption(p_rivermodel)-1)   !<river flow model names
  PARAMETER(rivermodelnames = [CHARACTER(LEN=64) :: 'original delay&attenuation (fixed)', 'original simpler dampq'])
!> \}
...
!> \{
  ...
  REAL,ALLOCATABLE :: wobsi(:)             !<Lake water level data for current time step
  ...
  INTEGER, ALLOCATABLE :: wobsindex(:)     !<Index for finding variables in wobsi
  ...
  TYPE(DEPOSITIONTYPE),ALLOCATABLE :: deposition2(:) !<Atmospheric deposition of substances (numsubstances)
  ...
  TYPE(CLASSTYPE),ALLOCATABLE :: classdata(:)     !<Class characteristics for current simulation period
  TYPE(CLASSATTRIBUTETYPE),ALLOCATABLE :: classattributedata(:)  !<Basic class attributes, set at the very first beginning and will not change
  ...
  TYPE(RIVERRATINGTYPE),ALLOCATABLE :: rrcdata(:)   !River ratingcurve data
...
!> \{
  ...
  INTEGER, PARAMETER :: i_open   = 1    !<Vegetation type code for open land classes
  INTEGER, PARAMETER :: i_forest = 2    !<Vegetation type code for forested land classes
  INTEGER, PARAMETER :: i_water  = 3    !<Vegetation type code for water classes
...
!> \{
  TYPE(TIMEINFORMATIONTYPE) current_time   !<Current date and other information on current time
...
!> \{
  LOGICAL          :: conductload     !<flag for calculation of NP etc. load for source apportionment
  ...
  REAL,ALLOCATABLE :: outvar_classload(:,:,:,:)  !<variable to save class and substance dependent load outputs for yearly (period) accumulation
  REAL,ALLOCATABLE :: outvar_basinload(:,:,:)    !<variable to save substance dependent load outputs for yearly (period) accumulation
...
!> \{
  ...
  REAL,ALLOCATABLE,TARGET :: monthpar(:,:)  !<parameter values for each month
...
!>!> \}
  ...
  SUBROUTINE allocate_initialize_modeloptions()
    ...
    modeloptionname(p_siltation) = 'siltation       '
    modeloptionname(p_sedresusp) = 'sedresuspmodel  '
    modeloptionname(p_rivermodel) = 'riverflowmodel  '
  ...
  FUNCTION substance_name(i) RESULT (shortname)
    ...
    IF(i==i_dsi) shortname = 'DS'
    IF(i==i_asi) shortname = 'AS'
  ...
  SUBROUTINE set_coded_classes(dim,array,ilakearg,olakearg,lriverarg,mriverarg,iwetarg,owetarg)
    ...
    IF(ALLOCATED(classmodel)) DEALLOCATE(classmodel)    ! for refresh
  ...
  FUNCTION get_current_soilleakage_memory(icl,isub,nsubst) RESULT(output)
    ...
    IF(modeloption(p_soilleakage)==1)THEN
      output = soilleak%concentration(:,current_time%date%month,isub)
    !>Or monthly soilleakage load
    ELSEIF(modeloption(p_soilleakage)==2 .OR. modeloption(p_soilleakage)==3)THEN
      output = soilleak%load(:,icl,current_time%date%month,isub)
    ENDIF
  ...
  LOGICAL FUNCTION endofday(ts)
    ...
    !Argument declarations
    INTEGER, INTENT(IN) :: ts  !<timestep of day (tsofday)
    
    endofday = .FALSE.
    IF(ts==timesteps_per_day) endofday = .TRUE.

  END FUNCTION endofday
  ...
  LOGICAL FUNCTION in_season_end(begjd,endjd)
    ...
    status = .FALSE.
    IF(endday > 365)THEN
      IF(current_time%dayno >= begjd) status = .TRUE.
      IF(current_time%dayno <= endday - 365) status = .TRUE.
    ELSE
      IF(current_time%dayno >= begjd .AND. current_time%dayno <= endday) status = .TRUE.
    ENDIF
  ...
  LOGICAL FUNCTION in_season_period(begjd,period)
    ...
    IF(endjd > 365)THEN
      IF(current_time%dayno >= begjd) status = .TRUE.
      IF(current_time%dayno <= endjd - current_time%prevdoy) status = .TRUE.
    ELSE
      IF(current_time%dayno >= begjd .AND. current_time%dayno <= endjd) status = .TRUE.
    ENDIF
  ...
  SUBROUTINE get_class_wet_deposition(i,ns,iluse,veg,conc)
    ...
    INTEGER, INTENT(IN) :: veg          !<vegetation type: 1=open,2=forest,3=water
    REAL, INTENT(OUT)   :: conc(ns)     !<wet deposition concentration (mg/L for IN)
    
    !Local variables
    INTEGER isubst,iveg,itime

    conc = 0.
    !Prepare for wet deposition 
    IF(ALLOCATED(deposition2))THEN
      DO isubst = 1, ns
        IF(ALLOCATED(deposition2(isubst)%wetconc))THEN   !mg/L for IN
          iveg = get_deposition_vegindex(isubst,2,veg,iluse)
          itime = get_deposition_timeindex(isubst,2)
          IF(iveg>0) conc(isubst) = deposition2(isubst)%wetconc(iveg,i,itime)
        ENDIF
      ENDDO
    ENDIF

  END SUBROUTINE get_class_wet_deposition
  ...
  SUBROUTINE set_class_deposition(i,ns,iluse,veg,source)
    ...
    INTEGER, INTENT(IN) :: iluse        !<index landuse
    INTEGER, INTENT(IN) :: veg          !<vegetation type: 1=open,2=forest,3=water
    REAL, INTENT(OUT)   :: source(ns)   !<dry deposition (kg/km2/timestep)
    ...
    IF(ALLOCATED(deposition2))THEN
      DO isubst = 1, ns
        IF(ALLOCATED(deposition2(isubst)%dryload))THEN   !kg/km2/timestep for IN
          iveg = get_deposition_vegindex(isubst,1,veg,iluse)
          itime = get_deposition_timeindex(isubst,1)
          IF(iveg>0) source(isubst) = deposition2(isubst)%dryload(iveg,i,itime)
        ENDIF
      ENDDO
    ENDIF

  END SUBROUTINE set_class_deposition
  ...
  INTEGER FUNCTION get_deposition_vegindex(isubst,idep,veg,iluse)
    ...
    get_deposition_vegindex = 0
    IF(isubst==0) RETURN
  
    IF(deposition2(isubst)%depcode(1,idep)==1)THEN
      get_deposition_vegindex = 1
    ELSEIF(deposition2(isubst)%depcode(1,idep)==2)THEN
      get_deposition_vegindex = veg
    ELSEIF(deposition2(isubst)%depcode(1,idep)==3)THEN
      get_deposition_vegindex = iluse
    ENDIF
      
  END FUNCTION get_deposition_vegindex
  ...
  INTEGER FUNCTION get_deposition_timeindex(isubst,idep)
    ...
    get_deposition_timeindex = 0
    IF(isubst==0) RETURN
  
    IF(deposition2(isubst)%depcode(2,idep)==1)THEN
      get_deposition_timeindex = 1
    ELSEIF(deposition2(isubst)%depcode(2,idep)==2)THEN
      get_deposition_timeindex = current_time%date%month
!      ELSEIF(deposition2%depcode(2,isubst,idep)==3)THEN
!        itime = currentdate%dayno
    ENDIF
      
  END FUNCTION get_deposition_timeindex
  ...
  SUBROUTINE check_connected_subbasins(i2,i3,isource,ibranch)
    ...
    ELSEIF(ALLOCATED(branchindex))THEN
      !IF(branchindex(i2)>0)THEN    !Bugg: This does not catch the branch in i3->i2 if there is a branch from i2 to someplace!

...
CONTAINS
  ...
  SUBROUTINE initiate_soil_water()
    ...
    IF(ALLOCATED(epotdist)) DEALLOCATE(epotdist)
    ...
    IF(ALLOCATED(soilrc)) DEALLOCATE(soilrc)
  ...
  SUBROUTINE calculate_snow(i,j,subid,iluse,snowfall,csnowfall,snow,csnow,temp,melt, &
    ...
    !!>Calculate ablation and check against snow pack   Simplified to reduce differences between compilers CP 21-11-03
    ...
    IF(abla0>0.)THEN
      IF(snow<abla0)THEN
        melt = melt * (snow/abla0)   !melt is already limited to frozen part of snow
        evap = snow - melt           !so the distribution between melt and evap should still be correct like this
      ENDIF
    ELSE
      melt = 0.
      evap = 0.
    ...
    IF(evap>0.)THEN
      ...
      IF(evap<snow .AND. snow>0.)THEN
        ...
        IF(snowliq>0.) snowliq = AMAX1(0.,snowliq - evap * (snowliq / snow))
    ...
    !Remove snow less than 1nm for more consistent behaviour for snow/nosnow threshold for calculation of snow cover fraction.
    !Scf has different equations for snow build up and at the end of the snow season. Scf affect evaporation from soil (and snow).
    !This check replaced by resetting snowmax when new snow fall at start of new season, in calculate_fractional_ snowcover.
    !IF(snow>0.)THEN
    !  IF(snow<1.E-6)THEN
    !    evap = evap + snow
    !    snow = 0.
    ...
    !>Calculate degree of snowcover and update maximum snow pach during winter
    CALL calculate_fractional_snowcover(iluse,0.,snow,snowage,snowmax,snowcover)
...
!------------------------------------------------------------------------
  ...
  !>The maximum snow of the winter season is also updated
  !>
  !Based on Lindstr�m&Gardelin(1999;2000) following the implementation in 
  ...
  SUBROUTINE calculate_fractional_snowcover(iluse,elevstd,snow,snowage,snowmax,fsc)
    ...
    USE MODVAR, ONLY : landpar, genpar, seconds_per_timestep, timesteps_per_day
    ...
    REAL, INTENT(IN)    :: snow        !<snow pack (mm)
    REAL, INTENT(IN)    :: snowage     !<snow pack age (timesteps)
    REAL, INTENT(INOUT) :: snowmax     !<maximum snow pack during winter (mm)
    REAL, INTENT(OUT)   :: fsc         !<fractional snowcover area (-)

    !Local parameter
    REAL, PARAMETER :: snowage_reset_snowmax = 1. !day

    !Local variables
    REAL timestep_seconds,snowage_threshold,fscdist
    timestep_seconds = REAL(seconds_per_timestep)
    snowage_threshold = snowage_reset_snowmax*REAL(timesteps_per_day)
    ...
    IF(snow.gt.0.)THEN   !Snow present
      ...
      ELSE
        ...
        IF(snowage<snowage_threshold) snowmax = 0.
        ...
        ELSE
          ...
          ELSE
            ...
            IF(snow.LT.genpar(m_fsck1)*snowmax)THEN
              snowmax = snowmax - (snowmax - snow/genpar(m_fsck1))*(1.-EXP(-genpar(m_fsckexp) * timestep_seconds)) 
            ENDIF 
          ENDIF
  ...
  SUBROUTINE calculate_snowdepth(iluse,snow,oldsnow,snowfall,temp,snowdensdt,snowage,snowdepth)
    ...
    ELSE  
      ...
      CASE(1)
        ...
        ELSE
          !Data assimilation could have affected snow&depth separately? so snow>0, while (old) snowdepth (&snowfall)=0
          snowdens = snowdens0  !restart snowdens
        ENDIF
        !snowdepth = snowdepth + snowfall/10./snowdens0  !add snowfall to old snow
        !IF(snowdepth<=0)THEN
        !  !Data assimilation could have affected snow&depth separately? so snow>0, while (old) snowdepth &snowfall=0?
 ...
 END SUBROUTINE calculate_snowheat_processes
  ...
  SUBROUTINE calculate_tile_drainage(i,j,isoil,subid,water,ep_org,&
    ...
    !Local parameter
    REAL, PARAMETER :: mindiff = 0.000002    !10^-6 verkar vara en onogrannhetsgr�ns
...
!        IF(soilstate%water(1,j,i)>=wp(1)+fc(1)+ep(1))THEN
  ...
  !
  !From Cosby et al 1984 (WRR 20(6):682-690): PSI = matric potential. logsatmp ~1.5 ger 
  !PSI=10**logsatmp i cm H2O. We want m, thus divided by 100.
  !----------------------------------------------------------------------
  SUBROUTINE calculate_unfrozen_soil_water(i,j,isoil,airtemp,wp,fc,ep,soilstate,frozenvol,liqfrac)
    
    USE MODVAR, ONLY : Lfusion, modeloption, soilpar, maxsoillayers, p_frozensoil, realzero
    ...
    !Local parameter
    INTEGER, PARAMETER :: nsublayer = 3 !number of layers for soil temperature within each soillayer (of water)
    
    !Local variables
    INTEGER k,il
    REAL liqmax, porosity, numerator, denominator
    !REAL threetemp(3),temp, liqfrac1,liqfrac2,liqfrac3,HELPVAR
    REAL subtemp(nsublayer),liqfracl(nsublayer),helpvar
      ...
      DO k=1,maxsoillayers
        ...
        ELSE
          IF(soilstate%water(k,j,i).GT.realzero)THEN
            ...
            numerator = -Lfusion*soilstate%temp(k,j,i)
    ...
    CASE(2)  !Liquid water fraction as a function of three soil temperature
      DO k=1,maxsoillayers
        IF(soilstate%water(k,j,i).GT.realzero)THEN
          ...
          CALL calculate_linearly_interp_soil_temperature(i,j,k,nsublayer,airtemp,soilstate,subtemp)
          DO il = 1,nsublayer
            IF(subtemp(il) >= - realzero) THEN
              liqfracl(il) = 1.
            ELSE
              numerator = -Lfusion*subtemp(il)
              denominator = 9.81*(subtemp(il)+273.16)*(10**soilpar(m_logsatmp,isoil)/100.)
              liqfracl(il) = MIN(1. , helpvar*(numerator/denominator)**(-1./soilpar(m_bcosby,isoil)))
            ENDIF
          ENDDO
          liqfrac(k) = SUM(liqfracl(:))/REAL(nsublayer)
        ELSE
          liqfrac(k) = 0.
        ENDIF
      ENDDO
  ...
  !>Calculates odd number of soil teperatures representing one soil layer. 
  !>The temperatures represent middle of 1/nth part of the soillayer, assuming 
  !>linear interpolation between soilstate%temp representing middle of soillayers.
  !----------------------------------------------------------------------
  SUBROUTINE calculate_linearly_interp_soil_temperature(i,j,k,n,airtemp,soilstate,temp)
  
    USE MODVAR, ONLY : soildepth,soilthick,maxsoillayers
    ...
    INTEGER, INTENT(IN) :: n     !<number of points within layer
    REAL, INTENT(IN)    :: airtemp  !<class airtemp
    TYPE(soilstatetype),INTENT(INOUT)  :: soilstate   !<Soil states
    REAL, INTENT(OUT)   :: temp(n) !<linearly interpolated soil temperature of soil layer k
  
    !Local variables
    INTEGER il
    REAL x(0:4),y(0:4),t(maxsoillayers)
    REAL depths(n)
    ...
    !Calculate n depths in soil layer, each in the middle of a nth part of the soil layer 
    DO il = 1,n
      depths(il) = x(k) + REAL(2*il-n-1)/2.*t(k)/REAL(n)
    ENDDO
    
    !Calculate temperature of the n depths by linear interpolation
    DO il = 1,n/2
      temp(il) = linear_response(depths(il),x(k-1),x(k),y(k-1),y(k))
    ENDDO
    temp(n/2+1) = y(k)
    DO il = n/2+2,n
      temp(il) = linear_response(depths(il),x(k),x(k+1),y(k),y(k+1))
    ENDDO
    
  END SUBROUTINE calculate_linearly_interp_soil_temperature

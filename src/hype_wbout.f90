...
!> \{
  ...
  INTEGER,PARAMETER :: w_abstaq    = 87  !<Index for abstraction from aquifer
  ...
  INTEGER,PARAMETER :: num_wbflows   = 87  !<Number of water balance flows
!>\}
...
! !Variables for file names and file units, and for which subbasins has irrigation.
  ...
  CHARACTER(LEN=50) :: fname_wbflows(num_wbflows) = (/ &
      ...
      'WBf_evaporation_owet_.txt                       ', &
      'WBf_abstraction_aquifer_.txt                    '/) !<filenames waterbalance flows
...
!      lout3 = 5
  ...
  !    CALL get_seq_filename(filename)    
      OPEN(FILE=TRIM(dir)//TRIM(filename),UNIT=funit_wbflows(i),STATUS='unknown',FORM='formatted',ACTION='write',ERR=900)

      !Write headings
      IF((na>0.AND.i==w_rgrwtoos).OR.i==w_abstaq)THEN
!      IF(i==w_rgrwtoos.AND.na>0)THEN
    ...
    DO i = 1,num_wbstores
      ...
      IF(i==w_snow.OR.i==w_glacier.OR.i==w_soil1.OR.i==w_soil2.OR.i==w_soil3.OR.  &
        ...
        OPEN(FILE=TRIM(dir)//TRIM(filename),UNIT=funit_wbstores(i),STATUS='unknown',FORM='formatted',ACTION='write',ERR=900)
      ELSE
        CYCLE
      ENDIF
    ...
    !    CALL get_seq_filename(filename)
        OPEN(FILE=TRIM(dir)//TRIM(filename),UNIT=funit_wbirrflows(i),STATUS='unknown',FORM='formatted',ACTION='write',ERR=900)
    ...
    IF(nwbfpbasin>0)THEN
      DO i = 1,num_wbfpflows
        ...
        OPEN(FILE=TRIM(dir)//TRIM(filename),UNIT=funit_wbfpflows(i),STATUS='unknown',FORM='formatted',ACTION='write',ERR=900)
...
!      IF(i==w_rgrwtoos.AND.na>0)THEN
      ...
      ELSEIF(i==w_abstaq)THEN !na=0
        WRITE(funit_wbflows(i),*) TRIM(textin)    !funkar?
      ELSE

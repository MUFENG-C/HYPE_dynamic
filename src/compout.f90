...
!with HYPE. If not, see <http://www.gnu.org/licenses/>.
  ...
  SUBROUTINE compute_load_accumulation_period_status(date,loadvar,nper,pstart,pend)
    ...
    USE WORLDVAR, ONLY : SOURCEAPPTYPE
    USE TIMEROUTINES
    USE LIBDATE

    !Argument declaration
    TYPE(DateType), INTENT(IN)  :: date  !<Current date
    TYPE(SOURCEAPPTYPE), INTENT(IN) :: loadvar  !<Accload structure
    INTEGER, INTENT(OUT) :: nper         !<Number of timesteps in current period (calculated only if pend is true)
    LOGICAL, INTENT(OUT) :: pstart       !<Flag for started period
    LOGICAL, INTENT(OUT) :: pend         !<Flag for end of period
    
    !Variable declarations
    TYPE(DateType) :: datearray(1)  !Current date
    TYPE(DateType) :: begindate,enddate  !Period
    ...
    datearray(1) = date
    begindate = loadvar%begindate
    enddate = loadvar%enddate
    begindate%Year = date%Year
    enddate%Year = date%Year
    IF(begindate>enddate)THEN  !Handle periods over new year by changing the year of the period begin or end.
      IF(date<=enddate)THEN 
        begindate%Year = date%Year - 1
      ELSE
        enddate%Year = date%Year + 1
      ENDIF
    ENDIF
    CALL check_if_date_in_period(1,1,datearray,begindate,enddate,pstart)
    
    !>Check if end of period
    IF(pstart)THEN
      IF(date==enddate)THEN
        pend=.TRUE.
        nper=period_length(begindate,enddate)
      ENDIF
    ENDIF
    
  END SUBROUTINE compute_load_accumulation_period_status
  ...
  SUBROUTINE compute_period_timesteplength_and_end_status(ct,pcode,nper,pend,pfull,idt,ndt)
    ...
    USE WORLDVAR, ONLY : i_t,i_h,i_d,i_w,i_m,i_y,i_s,dtskip,seconds_per_day
    USE MODVAR, ONLY : seconds_per_timestep,timesteps_per_day,TIMEINFORMATIONTYPE
    USE TIMEROUTINES
    USE LIBDATE
    ...
    LOGICAL, INTENT(OUT) :: pfull        !<Flag for full summation period at end
    ...
    dayofew = MOD(idt,7*timesteps_per_day) !This give 7-day periods starting at bdate
!    dayofew = MOD(idt-dtskip,7*timesteps_per_day) !This will give 7-day periods starting at cdate
    endweek = (dayofew==0)
!    endweek = (ct%dayofweek==7 .AND. endday)  !This will give weeks starting Monday. TODO: Change to this one later? 
    ...
    CASE(i_d) !daily output
      IF(endday)THEN
        nper=timesteps_per_day  !Number of time steps per day
        pend=.TRUE.
        pfull = .TRUE.
!        IF(idt-dtskip<nper) pfull = .FALSE.   !skip the short first day... This will remove halfvalues from output. TODO: Add the funktionality?
      ENDIF
    CASE(i_w) !weekly output
      IF(endweek)THEN
        nper=7*timesteps_per_day
        pend=.TRUE.
        pfull=.TRUE.
!        IF(idt-dtskip<nper) pfull = .FALSE.   !skip the short first week
      ENDIF
    CASE(i_m) !monthly output
      IF(endmonth)THEN
        nper=d*timesteps_per_day
        pend=.TRUE.
        pfull=.TRUE.
!        IF(idt-dtskip<nper) pfull = .FALSE.   !skip the short first month
      ENDIF
    CASE(i_y) !yearly output
      IF(endyear)THEN
        nper=numdays_of_year(date)*timesteps_per_day
        pend=.TRUE.
        pfull=.TRUE.
!        IF(idt-dtskip<nper) pfull = .FALSE.   !skip the short first year
      ENDIF
  ...
  !----------------------------------------------------------------------------------------------
  SUBROUTINE compute_basinoutput(ct,io,iad,dim,x,pwrite,idt,ndt,iens)
    ...
    LOGICAL, INTENT(OUT) :: pwrite      !<Flag for writing data to file, end of full summation period
    ...
    LOGICAL pfull       !Flag for full summation period at end
    ...
    CALL compute_period_timesteplength_and_end_status(ct,output(io)%period,nper,pend,pfull,idt,ndt)
    pwrite = pend.AND.pfull
  ...
  !--------------------------------------------------------------------
  SUBROUTINE compute_mapoutput(ct,io,idt,ndt,intformat,nmapperiod)
...
!    IF(pend)THEN
    IF(pend.AND.pfull)THEN
      IF(intformat)THEN
        ...
        CASE(i_t)
          CALL format_date(ct%date,'yyyymmddHHMM',maptime(tmap)) 
        CASE(i_d)
          CALL format_date(ct%date,'yyyymmdd',maptime(tmap)) 
        CASE(i_w)
          aweek = DateType(0,0,6,0,0)
          CALL format_date(ct%date-aweek,'yyyymmdd',maptime(tmap))  !beginning of week
        CASE(i_m)
          WRITE(maptime(tmap),'(I4,I2.2)') ct%date%Year,ct%date%Month
        CASE(i_y)
          WRITE(maptime(tmap),'(I4)') ct%date%Year
        CASE(i_s)
          WRITE(maptime(tmap),'(I4,I4)') outstartdate%Year,ct%date%Year  !eg. 19601999
        END SELECT
      ELSE
        SELECT CASE(output(io)%period)
        CASE(i_t)
          CALL format_date(ct%date,'yyyy-mm-dd HH:MM',maptime(tmap)) 
        CASE(i_d)
          CALL format_date(ct%date,'yyyy-mm-dd',maptime(tmap)) 
        CASE(i_w)
          aweek = DateType(0,0,6,0,0)
          CALL format_date(ct%date-aweek,'yyyy-mm-dd',maptime(tmap))  !beginning of week
        CASE(i_m)
          WRITE(maptime(tmap),'(I4,A,I2.2)') ct%date%Year,'-',ct%date%Month
        CASE(i_y)
          WRITE(maptime(tmap),'(I4)') ct%date%Year
        CASE(i_s)
          WRITE(maptime(tmap),'(I4,A,I4)') outstartdate%Year,'-',ct%date%Year  !eg. 1960-1999
        END SELECT
      ENDIF
      tmap=tmap+1
      IF(tmap<=maxmap)THEN
  ...
  !--------------------------------------------------------------------
  SUBROUTINE compute_timeoutput(ct,io,ivar,dim,x,pwrite,idt,ndt,iens)
    ...
    LOGICAL, INTENT(OUT) :: pwrite       !<Flag for end of period to be written
    INTEGER, INTENT(IN)  :: idt          !<Current time step number
    INTEGER, INTENT(IN)  :: ndt          !<Number of time steps in run
    INTEGER, INTENT(IN)  :: iens         !<Current simulation
  ...
  !----------------------------------------------------------------------------------------------
  SUBROUTINE compute_regionoutput(ct,io,ir,iad,dim,idt,ndt,iens,pwrite,x)
    ...
    TYPE(TIMEINFORMATIONTYPE), INTENT(IN) :: ct    !<Current time
    INTEGER, INTENT(IN)  :: io          !<Current output
    INTEGER, INTENT(IN)  :: ir          !<Current region (region order)
    INTEGER, INTENT(IN)  :: iad         !<Index of accdata (of current region)
    INTEGER, INTENT(IN)  :: dim         !<Data dimension, number of variables for print out
    INTEGER, INTENT(IN)  :: idt         !<Current time step number
    INTEGER, INTENT(IN)  :: ndt         !<Number of time steps in simulation
    INTEGER, INTENT(IN)  :: iens        !<Current simulation
    LOGICAL, INTENT(OUT) :: pwrite      !<Flag for end of period
  ...
  !>Accumulate yearly totals of nutrient loads from daily values. Can also 
  !>accumulate for a shorter period if asked for.
  !>
  !>\b Consequences Module variables accload may change.
  !--------------------------------------------------------------------
  SUBROUTINE compute_outloads(d,pend,idt)   
    ...
    LOGICAL, INTENT(OUT) :: pend        !<Flag for writing load files, end of summation period
    INTEGER, INTENT(IN)  :: idt         !<Current time step number
    
    !Local parameter
!    REAL, PARAMETER :: eps = 5.9604E-8                    ! Distance from 1.0 to next largest real number, i.e. eps = 1/2^24
!    REAL, PARAMETER :: eps = 2.2204E-16                   ! Distance from 1.0 to next largest dp number, i.e. eps = 1/2^52

    !Variable declarations
    INTEGER nper                    !Number of values in current period
    INTEGER k           !substance
    INTEGER i,imd,ibd   !subbasin index of current subbasin, subbasin maindown and branchdown
    LOGICAL pstarted    !flag for accumulation period started and is on-going
    REAL mainload(numsubstances)  !load of maindown outflow
    
    !>\b Algorithm \n
    !>Skip warmup period
    IF(idt<=accload%dtskip)THEN
      pend=.FALSE.
      RETURN
    ENDIF

    !>Check for period start and end
    CALL compute_load_accumulation_period_status(d,accload,nper,pstarted,pend) 
    
    IF(pstarted)THEN
      
      !>Accumulate data
      accload%slcclass(:,:,:,:) = accload%slcclass(:,:,:,:) + outvar_classload(:,:,:,:)
      accload%basin(:,:,:)   = accload%basin(:,:,:) + outvar_basinload(:,:,:)  
      ...
      IF(pend)THEN
        ...
        DO i=1,nsub
          mainload = accload%basin(:,max_basinoutvar-1,i) - accload%basin(:,max_basinoutvar,i)  !tot-branch
          DO k=1,numsubstances
            IF(k==i_t2) CYCLE      !T2 cannot be source apportioned
            IF(mainload(k)<0.0)THEN   !main negative
              !This ought to be a lakebasin to lakebasin but better check
              IF(path(i)%uplakebasin)THEN
                !Upstream lakebasin
                imd = path(i)%main
                accload%basin(k,max_basinoutvar-2,imd) = accload%basin(k,max_basinoutvar-2,imd) - mainload(k)  !update inflow to lb
                IF(accload%basin(k,max_basinoutvar,i)<0.)THEN  !branch negative also
                  !This ought to be a lakebasin to lakebasin but better check
                  ...
                  accload%basin(k,max_basinoutvar-2,ibd) = accload%basin(k,max_basinoutvar-2,ibd) - accload%basin(k,max_basinoutvar,i)  !update inflow to lb
                  accload%basin(k,max_basinoutvar,i) = 0.   !update branchflow
                ENDIF
                accload%basin(k,max_basinoutvar-1,i) = accload%basin(k,max_basinoutvar,i) !update totflow, tot=branch
!              ELSE
!                !Negative load out of subbasin, not allowed.
!                !Could check, but had numerical differences, so removed 211014.
...
!    INTEGER :: irgC206, irgC208, irgC211, irgC214
  ...
  !--------------------------------------------------------------------
  SUBROUTINE prepare_to_compute_crit(ct,idt,ndt)
    ...
    p=calvarper
    CALL compute_period_timesteplength_and_end_status(ct,p,nper,pend,pfull,idt,ndt)
  ...
  SUBROUTINE calculate_criteria(crit0,basincrit,simcrit,crit1,thres1)
    ...
    INTEGER locnrec(nsubCrit)           !number of observations used
    ...
    DO iac = 1, nacrit
      ...
      IF(calcsubass.OR.calcsimass)THEN
        ...
        CALL calculate_mean_and_errors(iac,nsubCrit,locqr,locqc,locmae,locrmse,locnrmse,locnrec) !MAE, RMSE, and NRMSE
      ENDIF
    ...
    DO i = 1, ncrit
      ...
      SELECT CASE (calvar(i)%crit)
        ...
        CASE('MNW')
          ...
          CALL add_max_criterion_to_objective(i,critadd,objcrit,condcrit,condthres)
        CASE DEFAULT
          IF(conductwarning) WRITE(6,*) 'Warning: Unknown criterion found', calvar(i)%crit
      END SELECT
    ENDDO
  ...
  REAL FUNCTION compute_yearfact(vartype,per)
    ...
    USE WORLDVAR, ONLY : i_t,i_d,i_w,i_m,i_y, &
                         meandaysofyear,seconds_per_day
  ...
  SUBROUTINE calculate_mean_and_errors(iac,dim,rec,sim,rmae,rmse,nrmse,n)
    ...
    INTEGER,INTENT(OUT) :: n(dim)       !<number of observations used
    ...
    DO isb = 1,dim
      IF(critvec(1,isb,iac)>=calvarlim) THEN
        n(isb) = NINT(critvec(1,isb,iac))   !Number of observations for subbasin

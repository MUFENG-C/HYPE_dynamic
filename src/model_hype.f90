...
!IRRIGATION_MODULE, REGIONAL_GROUNDWATER_MODULE
  ...
  SUBROUTINE model_version_information(funit)
    ...
    IF(funit==0)THEN
      WRITE(*,601) '---------------------------------------------'
      WRITE(*,*)   '   HYPE version '//model_version_stringz 
      WRITE(*,601) '---------------------------------------------'
    ...
    ELSE
      WRITE(funit,600) '--------------------------'
      WRITE(funit,*)   ' HYPE version '//model_version_string
      WRITE(funit,600) '--------------------------'
    ENDIF
600 FORMAT(A26)
601 FORMAT(A46)
  ...
  SUBROUTINE define_output_variables()
    ...
    !Initialize
    outvarid(:) = outvaridtype('?','?',0,0,0,0,'?','?','?')   !min/max set by type default (0-big)
    ...
    outvarid(o_tobs)  = outvaridtype('temp','air temperature'   ,i_mean , i_notdef,i_basin,i_notdef,'deg'  ,'degree Celsius','deg',def_min)
    outvarid(o_crunT1)  = outvaridtype('coT1','computed T1 runoff',i_wmean, o_crun,i_notdef,i_class,'?','?','')
    outvarid(o_crunT2)  = outvaridtype('coT2','computed T2 runoff',i_wmean, o_crun,i_notdef,i_class,'deg' ,'degree Celsius','',def_min)
    ...
    outvarid(o_csoilT2) = outvaridtype('csT2','computed T2 soil'  ,i_wmean, o_soim,i_notdef,i_class,'deg','degree Celsius','',def_min)
    ...
    outvarid(o_reT2) = outvaridtype('reT2','recorded T2 outflow olake',i_wmean2, o_rout,i_notdef,i_notdef,'deg'  ,'degree Celsius','',def_min) 
    outvarid(o_snowdens) = outvaridtype('sden','computed snow density',i_wmean, o_snowdepth,i_notdef,i_class,'g/cm3','g/cm3','')
    outvarid(o_grwlevel) = outvaridtype('gwat','computed grw level',i_mean , i_notdef,i_land,i_class,'m' ,'m' ,'m',def_min)
    ...
    outvarid(30) = outvaridtype('qerr','outflow error',i_mean , i_notdef,i_notdef,i_notdef,'m3/s','m3/s','',def_min)
    outvarid(31) = outvaridtype('cobc','computed outflow before correction',i_mean , i_notdef,i_notdef,i_notdef,'m3/s','m3/s','',def_min)
    outvarid(32) = outvaridtype('wtmp','computed water temp' ,i_mean , i_notdef,i_notdef,i_notdef,'deg'  ,'degree Celsius','',def_min)
    outvarid(33) = outvaridtype('werr','waterstage error',i_mean , i_notdef,i_notdef,i_notdef,'m','m','',def_min)
    outvarid(34) = outvaridtype('cwbc','computed wcom before correction',i_mean , i_notdef,i_notdef,i_notdef,'m','m','',def_min)
    ...
    outvarid(o_soilfrost) = outvaridtype('sfst','computed soil frost' ,i_mean , i_notdef,i_land,i_class, 'cm'  ,'cm' ,'cm',def_min)
    outvarid(o_soiltmp) = outvaridtype('stmp','computed soil temp'  ,i_mean , i_notdef,i_land,i_class, 'deg' ,'degree Celcius','deg',def_min)
    ...
    outvarid(o_wcom) = outvaridtype('wcom','olake water stage',i_mean , i_notdef,i_notdef,i_notdef,'m','meter','',def_min)
    outvarid(o_rewstr) = outvaridtype('wstr','rec olake water st',i_mean , i_notdef,i_notdef,i_notdef,'m','meter','',def_min)
    ...
    outvarid(61) = outvaridtype('regw','recorded grw level',i_mean , i_notdef,i_notdef,i_notdef,'m','meter','',def_min)
    outvarid(63) = outvaridtype('ccT1','comp conc T1 olake',i_wmean, o_cout,i_notdef,i_notdef,'?','?','')
    outvarid(64) = outvaridtype('ccT2','comp conc T2 olake',i_wmean, o_cout,i_notdef,i_notdef,'deg'  ,'degree Celsius','',def_min)
    ...
    outvarid(100) = outvaridtype('acdf','accumulated volume error',i_sum , i_notdef,i_notdef,i_notdef,'mm','mm' ,'',def_min)
    ...
    outvarid(112) = outvaridtype('cinf','comp inflow to olake',i_mean , i_notdef,i_notdef,i_notdef,'m3/s' ,'m3/s','',def_min)
    outvarid(113) = outvaridtype('rinf','rec inflow to olake',i_mean , i_notdef,i_notdef,i_notdef,'m3/s' ,'m3/s','',def_min)
    ...
    outvarid(o_ctmp) = outvaridtype('ctmp','corrected air temperature'   ,i_mean , i_notdef,i_basin,i_class,'deg'  ,'degree Celsius','deg',def_min)
    outvarid(122) = outvaridtype('irel','irrigation evap losses'   ,i_sum , i_notdef,i_scale3,i_notdef,'m3'  ,'m3' ,'mm') 
    outvarid(o_coum) = outvaridtype('coum','comp outflow main',i_mean , i_notdef,i_notdef,i_notdef,'m3/s','m3/s','',def_min)
    ...
    outvarid(o_coub) = outvaridtype('coub','comp outflow branch',i_mean , i_notdef,i_notdef,i_notdef,'m3/s','m3/s','',def_min)
    ...
    outvarid(o_sltmp(1)) = outvaridtype('stm1','computed soillayer 1 temp'  ,i_mean,i_notdef,i_land,i_class, 'deg' ,'degree Celcius','deg',def_min)
    outvarid(o_sltmp(2)) = outvaridtype('stm2','computed soillayer 2 temp'  ,i_mean,i_notdef,i_notdef,i_class, 'deg' ,'degree Celcius','',def_min)
    outvarid(o_sltmp(3)) = outvaridtype('stm3','computed soillayer 3 temp'  ,i_mean,i_notdef,i_notdef,i_class, 'deg' ,'degree Celcius','',def_min)
    ...
    outvarid(172) = outvaridtype('olst','comp olake surface temp'        ,i_mean , i_notdef,i_notdef,i_notdef,'deg'  ,'degree Celsius','',def_min)
    outvarid(173) = outvaridtype('olut','comp olake upper temp'          ,i_mean , i_notdef,i_notdef,i_notdef,'deg'  ,'degree Celsius','',def_min)
    outvarid(174) = outvaridtype('ollt','comp olake lower temp'          ,i_mean , i_notdef,i_notdef,i_notdef,'deg'  ,'degree Celsius','',def_min)
    outvarid(175) = outvaridtype('olwt','comp olake mean temp'           ,i_mean , i_notdef,i_notdef,i_notdef,'deg'  ,'degree Celsius','',def_min) 
    outvarid(176) = outvaridtype('ilst','comp ilake surface temp'        ,i_mean , i_notdef,i_notdef,i_notdef,'deg'  ,'degree Celsius','',def_min)
    outvarid(177) = outvaridtype('ilwt','comp ilake mean temp'           ,i_mean , i_notdef,i_notdef,i_notdef,'deg'  ,'degree Celsius','',def_min)
    outvarid(178) = outvaridtype('lrst','comp local river surface temp'  ,i_mean , i_notdef,i_notdef,i_notdef,'deg'  ,'degree Celsius','',def_min)
    outvarid(179) = outvaridtype('lrwt','comp local river meantemp'      ,i_mean , i_notdef,i_notdef,i_notdef,'deg'  ,'degree Celsius','',def_min)
    outvarid(180) = outvaridtype('mrst','comp main river surface temp'   ,i_mean , i_notdef,i_notdef,i_notdef,'deg'  ,'degree Celsius','',def_min)
    outvarid(181) = outvaridtype('mrwt','comp main river mean temp'      ,i_mean , i_notdef,i_notdef,i_notdef,'deg'  ,'degree Celsius','',def_min)
    outvarid(182) = outvaridtype('rolt','rec. olake surface temp'         ,i_mean , i_notdef,i_notdef,i_notdef,'deg'  ,'degree Celsius','',def_min)
    outvarid(183) = outvaridtype('rilt','rec. ilake surface temp'         ,i_mean , i_notdef,i_notdef,i_notdef,'deg'  ,'degree Celsius','',def_min)
    outvarid(184) = outvaridtype('rmrt','rec. main river surface temp'    ,i_mean , i_notdef,i_notdef,i_notdef,'deg'  ,'degree Celsius','',def_min)
    outvarid(185) = outvaridtype('mrto','OLD comp main river mean temp'   ,i_mean , i_notdef,i_notdef,i_notdef,'deg'  ,'degree Celsius','',def_min)
    outvarid(186) = outvaridtype('lrto','OLD comp local river mean temp'  ,i_mean , i_notdef,i_notdef,i_notdef,'deg'  ,'degree Celsius','',def_min)
    outvarid(187) = outvaridtype('ilto','OLD comp ilake mean temp'        ,i_mean , i_notdef,i_notdef,i_notdef,'deg'  ,'degree Celsius','',def_min)
    outvarid(188) = outvaridtype('olto','OLD comp olake mean temp'        ,i_mean , i_notdef,i_notdef,i_notdef,'deg'  ,'degree Celsius','',def_min)
    outvarid(o_snowcover) = outvaridtype('cfsc','comp frac snowcover area'       ,i_mean , i_notdef,i_land,i_class,'-'  ,'fraction','-',0.,1.)
    outvarid(190) = outvaridtype('rfsc','rec frac snowcover area'        ,i_mean , i_notdef,i_notdef,i_notdef,'-'  ,'fraction','',0.,1.)
    ...
    outvarid(248) = outvaridtype('coic','comp olake ice cover',i_mean, i_notdef,i_notdef,i_notdef,'-','-','',0.,1.)
    outvarid(249) = outvaridtype('ciic','comp ilake ice cover',i_mean, i_notdef,i_notdef,i_notdef,'-','-','',0.,1.)
    outvarid(250) = outvaridtype('cmic','comp m river ice cov',i_mean, i_notdef,i_notdef,i_notdef,'-','-','',0.,1.)
    outvarid(251) = outvaridtype('clic','comp l river ice cov',i_mean, i_notdef,i_notdef,i_notdef,'-','-','',0.,1.)
    ...
    outvarid(o_cleanwcom) = outvaridtype('clwc','cleaned wcom',i_mean , i_notdef,i_notdef,i_notdef,'m','meter','',def_min)
    outvarid(o_cleanwstr) = outvaridtype('clws','cleaned wstr',i_mean , i_notdef,i_notdef,i_notdef,'m','meter','',def_min)
    outvarid(o_wcav) = outvaridtype('wcav','ave. comp waterstage', i_mean , i_notdef,i_notdef,i_notdef,'m','meter','',def_min)
    outvarid(283) = outvaridtype('wtm0','comp water temp >0' , i_mean , i_notdef,i_notdef,i_notdef,'deg'  ,'degree Celsius','',def_min)
    ...
    outvarid(o_ccAE) = outvaridtype('ccAE','comp AE outflow olake',i_wmean, o_cout,i_notdef,i_notdef,'mg/L' ,'mg Algae-N/L','')
    ...
    outvarid(o_rpwl) = outvaridtype('rpwl','m river flpl w level' ,i_mean , i_notdef,i_notdef,i_notdef,'m'  ,'m in FD-href','',def_min)
    outvarid(o_lpwl) = outvaridtype('lpwl','olake flpl w level'   ,i_mean , i_notdef,i_notdef,i_notdef,'m'  ,'m in FD-href','',def_min)
    outvarid(o_gmlt) = outvaridtype('gmlt','glacier melt'   ,i_mean , i_notdef,i_notdef,i_notdef,'mm'  ,'mm' ,'')
    outvarid(o_mrfa) = outvaridtype('mrfa','main river fractional area'   ,i_mean , i_notdef,i_notdef,i_notdef,'-'  ,'fraction of river area','',0.,1.)
    outvarid(o_lrfa) = outvaridtype('lrfa','local river fractional area'   ,i_mean , i_notdef,i_notdef,i_notdef,'-'  ,'fraction of river area','',0.,1.)
    ...
    outvarid(o_cleanwavg) = outvaridtype('clwa','cleaned wavg',i_mean , i_notdef,i_notdef,i_notdef,'m','meter','',def_min)
    ...
    outvarid(o_rnlss) = outvaridtype('nlSS','routing net load SS',i_sum, i_notdef,i_notdef,i_notdef,'kg' ,'kg SuspSed','',def_min)
    outvarid(o_rnlts) = outvaridtype('nlTS','routing net load TS',i_sum, i_notdef,i_notdef,i_notdef,'kg' ,'kg TotSuspSed','',def_min)
    outvarid(o_wstilake) = outvaridtype('wilk','ilake water stage',i_mean , i_notdef,i_notdef,i_notdef,'m','meter','',def_min)
    ...
    outvarid(o_wstiwet) = outvaridtype('wiwt','iwet water stage',i_mean , i_notdef,i_notdef,i_notdef,'m','meter','',def_min)
    outvarid(o_wstowet) = outvaridtype('wowt','owet water stage',i_mean , i_notdef,i_notdef,i_notdef,'m','meter','',def_min)
    ...
    outvarid(o_iwutcot2) = outvaridtype('wcT2','comp iwet conc T2 outflow',i_wmean, o_iwoutfl,i_notdef,i_notdef,'deg' ,'degree Celsius','',def_min)
    ...
    outvarid(o_iwincot2) = outvaridtype('wiT2','comp iwet conc T2 inflow',i_wmean, o_iwinfl,i_notdef,i_notdef,'deg' ,'degree Celsius','',def_min)
    ...
    outvarid(o_qcin) = outvaridtype('qcin','calculated (regional) lake inflow',i_mean, i_notdef,i_notdef,i_notdef,'m3/s','m3/s' ,'',def_min)
    outvarid(o_qrin) = outvaridtype('qrin','recorded (regional) lake inflow',i_mean, i_notdef,i_notdef,i_notdef,'m3/s','m3/s' ,'',def_min)
    
    outvarid(o_hged) = outvaridtype('hged','river depth (h geom)'     ,i_mean, i_notdef,i_notdef,i_notdef,'m' ,'m','')
    outvarid(408) = outvaridtype('corl','river water level'     ,i_mean, i_notdef,i_notdef,i_notdef,'m' ,'m','',def_min)
    outvarid(409) = outvaridtype('hgeu','river velo. (h geom)'     ,i_mean, i_notdef,i_notdef,i_notdef,'m/s' ,'m/s','')
    outvarid(410) = outvaridtype('rerl','rec river water level'    ,i_mean, i_notdef,i_notdef,i_notdef,'m' ,'m','',def_min)
    outvarid(o_crrl) = outvaridtype('crrl','river water level (locsys)'     ,i_mean, i_notdef,i_notdef,i_notdef,'?' ,'?','',def_min)
    
    !Snow heat content and snow temperatures
    outvarid(o_snht) = outvaridtype('snht','snow heat content'   ,i_mean , i_notdef,i_notdef,i_class,'MJ'  ,'megajoule','',def_min)
    outvarid(o_snte) = outvaridtype('snte','snow temperature'   ,i_mean , i_notdef,i_notdef,i_class,'deg'  ,'degree Celsius','',def_min)
    outvarid(o_snts) = outvaridtype('snts','snow surface temperature'   ,i_mean , i_notdef,i_notdef,i_class,'deg'  ,'degree Celsius','',def_min)
    outvarid(o_dtmp) = outvaridtype('dtmp','deep soil temp'   ,i_mean , i_notdef,i_notdef,i_class,'deg'  ,'degree Celsius','',def_min)
    outvarid(o_cmrp) = outvaridtype('cmrp','mriver ice porosity'   ,i_mean , i_notdef,i_notdef,i_notdef,'-','volfraction','',0.,1.)
    outvarid(o_colp) = outvaridtype('colp','olake ice porosity'   ,i_mean , i_notdef,i_notdef,i_notdef,'-','volfraction','',0.,1.)
    ...
    outvarid(o_c1T2) = outvaridtype('c1T2','comp T2 main flow',i_wmean, o_coum,i_notdef,i_notdef,'deg'  ,'degree Celsius','',def_min)
    ...
    outvarid(o_c1AE) = outvaridtype('c1AE','comp AE main flow',i_wmean, o_coum,i_notdef,i_notdef,'mg/L' ,'mg Algae-N/L','')
    ...
    outvarid(o_c2T2) = outvaridtype('c2T2','comp T2 branch flow',i_wmean, o_coub,i_notdef,i_notdef,'deg'  ,'degree Celsius','',def_min)
    ...
    outvarid(o_c2AE) = outvaridtype('c2AE','comp AE branch flow',i_wmean, o_coub,i_notdef,i_notdef,'mg/L' ,'mg Algae-N/L','')
    ...
    outvarid(o_c3T2) = outvaridtype('c3T2','comp T2 main flow',i_mean, i_notdef,i_notdef,i_notdef,'deg'  ,'degree Celsius','',def_min)
    ...
    outvarid(o_c3AE) = outvaridtype('c3AE','comp AE main flow',i_mean, i_notdef,i_notdef,i_notdef,'mg/L' ,'mg Algae-N/L','')
    outvarid(o_c3TS) = outvaridtype('c3TS','comp TS main flow',i_mean, i_notdef,i_notdef,i_notdef,'mg/L' ,'mg TotSuspSed/L','')
    outvarid(o_c4IN) = outvaridtype('c4IN','comp IN branch flow',i_mean, i_notdef,i_notdef,i_notdef,'ug/L','ug InorgN-N/L','')
    outvarid(o_c4ON) = outvaridtype('c4ON','comp ON branch flow',i_mean, i_notdef,i_notdef,i_notdef,'ug/L','ug OrgN-N/L','')
    outvarid(o_c4SP) = outvaridtype('c4SP','comp SP branch flow',i_mean, i_notdef,i_notdef,i_notdef,'ug/L','ug SRP-P/L','')
    outvarid(o_c4PP) = outvaridtype('c4PP','comp PP branch flow',i_mean, i_notdef,i_notdef,i_notdef,'ug/L','ug PartP-P/L','')
    outvarid(o_c4T1) = outvaridtype('c4T1','comp T1 branch flow',i_mean, i_notdef,i_notdef,i_notdef,'?','?','')
    outvarid(o_c4T2) = outvaridtype('c4T2','comp T2 branch flow',i_mean, i_notdef,i_notdef,i_notdef,'deg'  ,'degree Celsius','',def_min)
    outvarid(o_c4TN) = outvaridtype('c4TN','comp TN branch flow',i_mean, i_notdef,i_notdef,i_notdef,'ug/L' ,'ug Tot-N/L','')
    outvarid(o_c4TP) = outvaridtype('c4TP','comp TP branch flow',i_mean, i_notdef,i_notdef,i_notdef,'ug/L' ,'ug Tot-P/L','')
    outvarid(o_c4SS) = outvaridtype('c4SS','comp SS branch flow',i_mean, i_notdef,i_notdef,i_notdef,'mg/L','mg SuspSed/L','')
    outvarid(o_c4AE) = outvaridtype('c4AE','comp AE branch flow',i_mean, i_notdef,i_notdef,i_notdef,'mg/L' ,'mg Algae-N/L','')
    outvarid(o_c4TS) = outvaridtype('c4TS','comp TS branch flow',i_mean, i_notdef,i_notdef,i_notdef,'mg/L' ,'mg TotSuspSed/L','')
    ...
    outvarid(o_c5T2) = outvaridtype('c5T2','comp T2 in ilake',i_wmean, 145,i_notdef,i_notdef,'deg'  ,'degree Celsius','',def_min)
    ...
    outvarid(o_c5AE) = outvaridtype('c5AE','comp AE in ilake',i_wmean, 145,i_notdef,i_notdef,'mg/L' ,'mg Algae-N/L','')
    ...
    outvarid(o_c6T2) = outvaridtype('c6T2','comp T2 in olake',i_wmean, 146,i_notdef,i_notdef,'deg'  ,'degree Celsius','',def_min)
    ...
    outvarid(o_c6AE) = outvaridtype('c6AE','comp AE in olake',i_wmean, 146,i_notdef,i_notdef,'mg/L' ,'mg Algae-N/L','')
    ...
    outvarid(o_crnt) = outvaridtype('crnt','calculated net radiation'   ,i_mean , i_notdef,i_basin,i_class,'MJ/d'  ,'MJ/d','MJ/d',def_min)
    ...
    outvarid(o_fnca) = outvaridtype('fnca','fraction of non-contributing area',i_mean , 0,0,0,'-','-' ,'',0.,1.)
    outvarid(o_fcon) = outvaridtype('fcon','fraction of (ilake) connectivity',i_mean , 0,0,0,'-','-' ,'',0.,1.)
    ...
    outvarid(o_c7T2) = outvaridtype('c7T2','comp T2 lake outflow',i_wmean, o_coul,i_notdef,i_notdef,'deg'  ,'degree Celsius','',def_min)
    ...
    outvarid(o_c7AE) = outvaridtype('c7AE','comp AE lake outflow',i_wmean, o_coul,i_notdef,i_notdef,'mg/L' ,'mg Algae-N/L','')
    ...
    outvarid(o_c8T2) = outvaridtype('c8T2','comp T2 lake outflow',i_mean, i_notdef,i_notdef,i_notdef,'deg'  ,'degree Celsius','',def_min)
    ...
    outvarid(o_c8AE) = outvaridtype('c8AE','comp AE lake outflow',i_mean, i_notdef,i_notdef,i_notdef,'mg/L' ,'mg Algae-N/L','')
    outvarid(o_c8TS) = outvaridtype('c8TS','comp TS lake outflow',i_mean, i_notdef,i_notdef,i_notdef,'mg/L' ,'mg TotSuspSed/L','')

    outvarid(o_absr) = outvaridtype('absr','abstr f main river'  ,i_sum , i_notdef,i_scale3,i_notdef,'m3'  ,'m3' ,'mm')
    outvarid(o_absl) = outvaridtype('absl','abstr f olake'       ,i_sum , i_notdef,i_scale3,i_notdef,'m3'  ,'m3' ,'mm')
    outvarid(o_psoilDSi(1)) = outvaridtype('pSI1','pool dissolved Si soil1',i_mean , i_notdef,i_land,i_class,'kg/km2' ,'kg Si/km2','kg/km2')
    outvarid(o_psoilDSi(2)) = outvaridtype('pSI2','pool dissolved Si soil2',i_mean , i_notdef,0,i_class,'kg/km2' ,'kg Si/km2','')
    outvarid(o_psoilDSi(3)) = outvaridtype('pSI3','pool dissolved Si soil3',i_mean , i_notdef,0,i_class,'kg/km2' ,'kg Si/km2','')
    outvarid(o_csoilDSi) = outvaridtype('csSI','computed DSi soil'    ,i_wmean, o_soim,0,i_class,'mg/L','mg DSi/L','')
    outvarid(o_crunDSi) = outvaridtype('coSI','computed DSi runoff',i_wmean, o_crun,0,i_class,'mg/L' ,'mg DSi/L','')
    outvarid(o_ccDSi) = outvaridtype('ccDS','comp conc Diss. Si olake',i_wmean, o_cout,0,i_notdef,'mg/L','mg Diss-Si/L','')
    outvarid(o_ccASi) = outvaridtype('ccAS','comp conc Algea Si olake',i_wmean, o_cout,0,i_notdef,'mg/L','mg Alg-Si/L','')
    outvarid(o_ccSi) = outvaridtype('ccSi','comp conc total Si olake',i_wmean, o_cout,0,i_notdef,'mg/L','mg Si/L','')
    outvarid(o_clDSi) = outvaridtype('clDS','comp local conc Diss. Si',i_wmean, o_cloc,0,i_notdef,'mg/L','mg Diss-Si/L','')
    outvarid(o_clASi) = outvaridtype('clAS','comp local conc Algea Si',i_wmean, o_cloc,0,i_notdef,'mg/L','mg Alg-Si/L','')
    outvarid(o_clSi) = outvaridtype('clSi','comp local conc total Si',i_wmean, o_cloc,0,i_notdef,'mg/L','mg Si/L','')
    outvarid(o_reSi) = outvaridtype('reSi','rec Si outflow olake',i_wmean2, o_rout,0,i_notdef,'mg/L' ,'mg Si/L','')
    outvarid(o_psub) = outvaridtype('psub','precipitation', i_sum , 0,i_basin,i_class,'mm','mm' ,'mm')
    outvarid(o_esub) = outvaridtype('esub','evaporation', i_sum , 0,i_basin,i_class,'mm' ,'mm' ,'mm')
    outvarid(o_isub) = outvaridtype('isub','interception loss', i_sum , 0,i_basin,i_class,'mm' ,'mm' ,'mm')

    !Output variables for olake sediments
    outvarid(o_olsp) = outvaridtype('olsp','olake sediment SS pool',i_mean , 0,0,i_notdef,'kg/m2' ,'kg/m2','')
    outvarid(o_olas) = outvaridtype('olas','olake sedimenting TS',i_sum , 0,0,i_notdef,'kg' ,'kg','') !sediment added to lake sediment pool (SS+AE) (kg)
    outvarid(o_olan) = outvaridtype('olan','olake sedimenting TN',i_sum , 0,0,i_notdef,'kg' ,'kg','') !nitrogen added to lake sediment pool (ON) (kg)
    outvarid(o_olap) = outvaridtype('olap','olake sedimenting TP',i_sum , 0,0,i_notdef,'kg' ,'kg','') !phosphorus added to lake sediment pool (PP) (kg)
    outvarid(o_olac) = outvaridtype('olac','olake sedimenting OC',i_sum , 0,0,i_notdef,'kg' ,'kg','') !org.carbon added to lake sediment pool (OC) (kg)
    outvarid(o_olai) = outvaridtype('olai','olake sedimenting Si',i_sum , 0,0,i_notdef,'kg' ,'kg','') !sedimenting silica (ASi) (kg)
    outvarid(o_ola1) = outvaridtype('ola1','olake sedimenting T1',i_sum , 0,0,i_notdef,'?' ,'?','') !sediment added to lake sediment pool (T1)
    outvarid(o_olns) = outvaridtype('olns','olake TS change in pool',i_sum , 0,0,i_notdef,'kg' ,'kg','') ! #CB: net change in sediment in lake production pool (SS+AE) (kg)
    outvarid(o_olsa) = outvaridtype('olsa','olake sediment SS age',i_mean , 0,0,i_notdef,'ts' ,'ts','')
    outvarid(o_olsd) = outvaridtype('olsd','olake sediment density',i_mean , 0,0,i_notdef,'kg/m3' ,'kg/m3','') !Maybe not i_mean??
    outvarid(o_olss) = outvaridtype('olss','olake sediment SS+AE depth',i_mean , 0,0,i_notdef,'m' ,'m','')
    outvarid(o_olld) = outvaridtype('olld','olake lake depth',i_mean , 0,0,i_notdef,'m' ,'m','')
    outvarid(o_olsf) = outvaridtype('olsf','olake sediment flush',i_mean , 0,0,i_notdef,'T/F' ,'T/F','')
    outvarid(o_cmrq) = outvaridtype('cmrq','comp flow main river',i_mean , i_notdef,i_notdef,i_notdef,'m3/s' ,'m3/s','')
    outvarid(o_c9IN) = outvaridtype('c9IN','comp IN main river',i_wmean, o_cmrq,i_notdef,i_notdef,'ug/L','ug InorgN-N/L','')
    outvarid(o_c9ON) = outvaridtype('c9ON','comp ON main river',i_wmean, o_cmrq,i_notdef,i_notdef,'ug/L','ug OrgN-N/L','')
    outvarid(o_c9SP) = outvaridtype('c9SP','comp SP main river',i_wmean, o_cmrq,i_notdef,i_notdef,'ug/L','ug SRP-P/L','')
    outvarid(o_c9PP) = outvaridtype('c9PP','comp PP main river',i_wmean, o_cmrq,i_notdef,i_notdef,'ug/L','ug PartP-P/L','')
    outvarid(o_c9OC) = outvaridtype('c9OC','comp OC main river',i_wmean, o_cmrq,i_notdef,i_notdef,'mg/L' ,'mg Org-C/L','')
    outvarid(o_c9T1) = outvaridtype('c9T1','comp T1 main river',i_wmean, o_cmrq,i_notdef,i_notdef,'?','?','')
    outvarid(o_c9T2) = outvaridtype('c9T2','comp T2 main river',i_wmean, o_cmrq,i_notdef,i_notdef,'deg'  ,'degree Celsius','',def_min)
    outvarid(o_c9TN) = outvaridtype('c9TN','comp TN main river',i_wmean, o_cmrq,i_notdef,i_notdef,'ug/L' ,'ug Tot-N/L','')
    outvarid(o_c9TP) = outvaridtype('c9TP','comp TP main river',i_wmean, o_cmrq,i_notdef,i_notdef,'ug/L' ,'ug Tot-P/L','')
    outvarid(o_c9SS) = outvaridtype('c9SS','comp SS main river',i_wmean, o_cmrq,i_notdef,i_notdef,'mg/L','mg SuspSed/L','')
    outvarid(o_c9AE) = outvaridtype('c9AE','comp AE main river',i_wmean, o_cmrq,i_notdef,i_notdef,'mg/L' ,'mg Algae-N/L','')
    outvarid(o_c9TS) = outvaridtype('c9TS','comp TS main river',i_wmean, o_cmrq,i_notdef,i_notdef,'mg/L' ,'mg TotSuspSed/L','')
    outvarid(o_c9DS) = outvaridtype('c9DS','comp DS main river',i_wmean, o_cmrq,i_notdef,i_notdef,'mg/L' ,'mg Diss-Si/L','')
    outvarid(o_c9AS) = outvaridtype('c9AS','comp AS main river',i_wmean, o_cmrq,i_notdef,i_notdef,'mg/L' ,'mg Alg-Si/L','')
    outvarid(o_c9Si) = outvaridtype('c9Si','comp Si main river',i_wmean, o_cmrq,i_notdef,i_notdef,'mg/L' ,'mg Si/L','')

    outvarid(o_ssmx) = outvaridtype('ssmx','max SS conc.',i_mean, i_notdef,i_notdef,i_notdef,'mg/L','mg SuspSed/L','') !maximum sediment transport concentration for Bagnold equation
    outvarid(o_sdrs) = outvaridtype('sdrs','sed. resusp. mass',i_mean, i_notdef,i_notdef,i_notdef,'kg/d','kg SuspSed/d','') !amount of sedimentation/resuspension
  ...
  !>modparid structure: name, dependence type, index (defined in hypevar)
    ...
    modparid(n_gratk)  = modparidtype('gratk     ', m_gpar , m_grat1)
    modparid(n_gratp)  = modparidtype('gratp     ', m_gpar , m_grat2)
    ...
    modparid(n_sedexp)  = modparidtype('sedexp    ', m_gpar , m_sedexp)
    ...
    modparid(437) = modparidtype('weathrate ', m_spar , m_weathrate)   !silica weathering rate 
    modparid(438) = modparidtype('weathdep  ', m_gpar , m_weathdep)    !depth to which weathering occur
    modparid(439) = modparidtype('weathkt   ', m_gpar , m_weathkt)     !weathering temperature dependence parameter
    modparid(440) = modparidtype('wprodsi   ', m_gpar , m_wprodsi)     !Algal Si production rate
    modparid(441) = modparidtype('sedsi     ', m_gpar , m_sedsi)       !sedimentation of algal Si
    modparid(442) = modparidtype('sitmpexp  ', m_gpar , m_sitmpexp)    !exponent
    modparid(443) = modparidtype('plimsi    ', m_gpar , m_plimsi)      !P limitation for diatom growth
    modparid(444) = modparidtype('wprodsi   ', m_ldpar, m_ldwprodsi)
    modparid(445) = modparidtype('sedsi     ', m_ldpar, m_ldsedsi)
    modparid(446) = modparidtype('sedss     ', m_ldpar, m_ldsedss)
    modparid(n_iniSi) = modparidtype('inisi     ', m_gpar , m_iniSi)
    modparid(n_w0adjdays) = modparidtype('w0adjdays ', m_gpar , m_w0adjdays)
    modparid(449) = modparidtype('w0adjdays ', m_ldpar , m_ldw0adjdays)
    modparid(n_lseddens) = modparidtype('lseddens  ', m_gpar, m_lseddens)  !lake sediment density kg/m3
    modparid(n_erodmon) = modparidtype('erodmon   ', m_mpar, m_erodmon)
    modparid(n_eroddecay) = modparidtype('eroddecay', m_gpar, m_eroddecay)   !Eroded temporary pool decay (SS and PP) 
    modparid(n_qbank)  = modparidtype('qbank     ', m_gpar, m_qbank)   ! #CB qbank factor
    modparid(n_addsusp) = modparidtype('addsusp   ', m_gpar, m_addsusp)  ! #CB Bagnold equation additional sediment factor
    modparid(n_vpeak)  = modparidtype('vpeak     ', m_gpar, m_vpeak)   ! #CB Velocity peak across river factor
    modparid(n_sedch)  = modparidtype('suspch    ', m_gpar, m_sedch)   ! #CB Resuspension channel factor (reduction)
    modparid(n_suspconSS)  = modparidtype('suspconss ', m_gpar, m_suspconSS)   !Constant for max suspension concentration
    modparid(n_suspconPP)  = modparidtype('suspconpp ', m_gpar, m_suspconPP)   !Constant for max suspension concentration
    modparid(n_suspconT1)  = modparidtype('suspcont1 ', m_gpar, m_suspconT1)   !Constant for max suspension concentration
    modparid(n_suspexpSS)  = modparidtype('suspexpss ', m_gpar, m_suspexpSS)   !Exponent for max suspension concentration
    modparid(n_suspexpPP)  = modparidtype('suspexppp ', m_gpar, m_suspexpPP)   !Exponent for max suspension concentration
    modparid(n_suspexpT1)  = modparidtype('suspexpt1 ', m_gpar, m_suspexpT1)   !Exponent for max suspension concentration
    modparid(n_remdamdays) = modparidtype('remdamdays', m_gpar, m_remdamdays)  !Days for removing dam at removedam date
...
!------------------------------------------------------------
  ...
  !>Set atmospheric deposition for model if no AtmdepData.txt file is used.
  !>HYPE uses model paraemters to be compatible with olde versions.
  !-----------------------------------------------------------------------
  SUBROUTINE model_specified_atmospheric_deposition()
  
    USE HYPEVARIABLES, ONLY : m_drypp, &
                              m_wetsp, &
                              m_wetspload
  ...
  INTEGER i
    ...
    IF(.NOT.conduct%simP) RETURN

    !Wet deposition concentration of SP
    IF(genpar(m_wetsp)>0.)THEN
      deposition2(i_sp)%wetconc = genpar(m_wetsp)*1.E-3   !for all subbasins, mg/L
    ENDIF
      
    !Wet deposition load on water of SP, will be added as drydep SP
    IF(genpar(m_wetspload)>0.)THEN
      deposition2(i_sp)%dryload(i_water,:,1) = genpar(m_wetspload)
    ENDIF

    !Dry deposition load of PP
    DO i = 1,nluse
      IF(landpar(m_drypp,i)>0.)THEN
        deposition2(i_pp)%dryload(i,:,1) = landpar(m_drypp,i)
      ENDIF
    ENDDO
    
    RETURN
  END SUBROUTINE model_specified_atmospheric_deposition
...
!------------------------------------------------------------
  ...
  SUBROUTINE initiate_model_state(frozenstate,soilstate,aquiferstate,riverstate,lakestate,miscstate)
    ...
    USE NPC_SURFACEWATER_PROCESSES, ONLY : initiate_river_substance_state,     &
...
!      IF(slc_ilake>0) lakestate%water(1,i) = basin(i)%lakedepth(1)*1000.  !ilake water stage (mm)
    ...
    DO i = 1,nsub
      IF(ALLOCATED(lakedatapar))THEN
        riverstate%Qmean(1,i) = lakedatapar(lakedataparindex(i,1),m_ldqmean)* 0.001 * basin(i)%area / (365. * real_seconds_per_day)
        riverstate%Qmean(2,i) = lakedatapar(lakedataparindex(i,2),m_ldqmean)* 0.001 * upstreamarea(i) / (365. * real_seconds_per_day)
      ELSE
        riverstate%Qmean(1,i) = genpar(m_Qmean)* 0.001 * basin(i)%area / (365. * real_seconds_per_day)
        riverstate%Qmean(2,i) = genpar(m_Qmean)* 0.001 * upstreamarea(i) / (365. * real_seconds_per_day)
      ENDIF
    ENDDO
    ...
    CALL initiate_river_substance_state(conduct,riverstate)
    
    !Initialize lake concentrations
    CALL initiate_lake_substance_state(conduct,lakestate)
  ...
  SUBROUTINE initiate_model(frozenstate,soilstate,aquiferstate,riverstate,lakestate,miscstate)
    ...
    USE NPC_SURFACEWATER_PROCESSES, ONLY : sediment_pool_density
    ...
    !Set potential evaporation correction
    ...
    CALL calculate_riverlength(nsub,landarea,riverlength)
    DO i = 1,nsub
      deadriver(1,i) = genpar(m_deadl) * (basin(i)%area/1.0E6) * riverlength(1,i)   !(m3)
      deadriver(2,i) = genpar(m_deadm) * (upstreamarea(i)/1.0E6) * riverlength(2,i)
    ENDDO
    IF(.NOT. ALLOCATED(deadwidth))    ALLOCATE(deadwidth(2,nsub))
    IF(.NOT. ALLOCATED(ratingk))      ALLOCATE(ratingk(2,nsub))
    IF(.NOT. ALLOCATED(thresholddepth))    ALLOCATE(thresholddepth(2,nsub)) 
    DO i = 1,nsub
      deadwidth(1,i) = 10. * SQRT(genpar(m_deadl) * (basin(i)%area/1.0E6)*0.1)   !(m)  !width=10*depth, depth = sqrt(area/10), area = vol/len
      deadwidth(2,i) = 10. * SQRT(genpar(m_deadm) * (upstreamarea(i)/1.0E6)*0.1)   !(m) 
      thresholddepth(1:2,i) = basin(i)%lakedepth(1:2)   ![m] current lake depth initialised 
      IF(conduct%simS.AND.conduct%siltation)THEN
        DO itype = 1,2
          IF((lakestate%sedpool(i_ss,itype,i) + lakestate%sedpool(i_ae,itype,i))>0.)THEN
            density = lakestate%seddens(itype,i)  !initial state file
            IF(density>0.) thresholddepth(itype,i) = thresholddepth(itype,i) - (lakestate%sedpool(i_ss,itype,i) + lakestate%sedpool(i_ae,itype,i))/density
          ENDIF
        ENDDO
      ENDIF
    ENDDO
    ...
    IF(.NOT. ALLOCATED(riverrc))   ALLOCATE(riverrc(2,2,nsub))
    ...
    ELSE
      ...
      DO i = 1,nsub
        ...
        DO itype = 1,2          
          ...
          kt = part * totaltime                    !damptime, k/deltatt in equation q=(1/k)*S
          IF(kt>0.)THEN
!            riverrc(itype,i) = 1. - kt + kt * exp(-1./kt)  !recession coefficient in equation Q=r*S
            riverrc(1,itype,i) = 1. - kt + kt * exp(-1./kt)  !coefficient 1 in equation Q=r1*I+r2*S0 (or Q=r*S)
            riverrc(2,itype,i) = 1. -  exp(-1./kt)  !coefficient 2 in equation Q=r1*I+r2*S0
          ELSE
!            riverrc(itype,i) = 1.   !Safe for all lake subbasin
            riverrc(1,itype,i) = 1.   !Safe for all lake subbasin
            riverrc(2,itype,i) = 1.
          ENDIF
        ENDDO
      ENDDO
  ...
  SUBROUTINE reallocate_nclass_based_variables()
    USE ATMOSPHERIC_PROCESSES, ONLY : calculate_class_wind_transformation_factor
    USE SOIL_PROCESSES, ONLY : initiate_soil_water
    USE SURFACEWATER_PROCESSES, ONLY : calculate_landarea
    USE REGIONAL_GROUNDWATER_MODULE, ONLY : initiate_regional_groundwater_flow
   ...
   !                               cirrigation
    ...
    !Initialize internal landclass wetland (iwet).
    IF(slc_iwet>0)THEN
        IF(ALLOCATED(iwetnoninflow)) DEALLOCATE(iwetnoninflow)
        IF(.NOT.ALLOCATED(iwetnoninflow)) ALLOCATE(iwetnoninflow(nsub))
        !iwetcatch is fraction of basinarea
        landfraction = 1.
        landfraction = landfraction - classbasin(:,slc_iwet)%part
    ...
    ENDIF
    IF(ALLOCATED(outvarclassinfo)) DEALLOCATE(outvarclassinfo)
  ...
  SUBROUTINE model(frozenstate,soilstate,aquiferstate,riverstate,lakestate,miscstate)
    ...
    USE SURFACEWATER_PROCESSES, ONLY : add_precipitation_to_river, &
                                       ...
                                       ice_on_river
    USE NPC_SOIL_PROCESSES, ONLY : croprotation_soilpoolaverage
    ...
    REAL totalsurfaceflow,surfaceflow(2),icpevap,icpevap2
    ...
    REAL cprec(numsubstances)   !concentration of precipitation, class
    ...
    REAL atmdepload1(numsubstances),atmdepload2(numsubstances)  !wet dep and dry dep
    ...
    REAL aquiferabstractionflows(naquifers) !abstraction from aquifer
    ...
    REAL qinmm,qin,cin(numsubstances),helpq
    ...
    REAL riverboxinflow   !help for outflow calculation (m3)
    REAL evapr,cevaprT1
    REAL lakearea(nlaketypes)             !lake area (m2) for lake
    REAL flow1000m3ts         !olake outflow in 1000 m3/ts
    REAL river_maxSSconc(nrivertypes),river_sedresSS(nrivertypes)   !river SS maximum transport concentration and sedimentation/resuspension of SS
    REAL resusp_downstream(numsubstances) !resuspension of olake to be transported downstream
    REAL netroutload(numsubstances,nsub)   !net load of main river and olake (outflow-(local and upstream)inflows)
    REAL pooladd_ts,pooladd_tn,pooladd_tp,pooladd_oc,pooladd_si, poolnet_ts !sediment added to olake, net change in sediment (kg)
    REAL pooladd_t1
    ...
    REAL ldremi,lrremi,rsremi,rivervremi         !abstraction surface water for irrigation (m3)
    ...
    IF(modeloption(p_deepgroundwater)==2)THEN
      CALL calculate_current_flow_from_aquifer(naquifers,numsubstances,aquiferstate,aqoutside)   !prepare for adding regional groundwater to model
      IF(conductwb) wbflows(w_rgrwtoos,1:naquifers) = aqoutside
      CALL point_abstraction_from_aquifer(aquiferstate,aquiferabstractionflows)   !remove water from aquifer by abstraction PointSourceData
      IF(conductwb) wbflows(w_abstaq,1:naquifers) = aquiferabstractionflows
    ENDIF
    ...
    IF(conductxoms) CALL get_current_xoms(current_time%date,nsub)
    ...
    CALL calculate_subbasin_temperature(nsub,current_time%date%month,tempi)
    IF(ALLOCATED(tmini)) CALL calculate_subbasin_temperature(nsub,current_time%date%month,tmini)
    IF(ALLOCATED(tmaxi)) CALL calculate_subbasin_temperature(nsub,current_time%date%month,tmaxi)
    CALL calculate_subbasin_precipitation(nsub,tempi,preci,pcorricep)
    IF(calcSWRAD)THEN
      CALL calculate_extraterrestrial_radiation(nsub,current_time%dayno,radexti)
      CALL get_current_cloudiness(nsub,current_time%date%month,cloudi)
    ...
    DO i = 1,nsub
      ...
      pseudo_dayno = get_pseudo_dayno(current_time%dayno,basin(i)%latitude)
      daylength = 0.
      IF(modeloption(p_growthstart)==1)  &
        CALL calculate_daylength(current_time%dayno,basin(i)%latitude,daylength)
      ...
      DO j=1,nclass
        ...
        IF(a>0)THEN
          ...
          CALL set_class_precipitation_concentration_and_load(i,classdata(j)%luse,classdata(j)%vegtype,numsubstances, &
                   classarea,precorg(i),temp,prec,cprec,atmdepload1)

          !> \li Calculate soil processes            
          IF(classmodel(j)==0)THEN
            CALL soilmodel_0(i,j,classdata(j)%soil,classdata(j)%luse,basin(i)%subid,pseudo_dayno,classarea,prec,cprec,temp,  & 
          ...
          ELSEIF(j==slc_mriver)THEN
            IF(conduct%floodplain)THEN
              IF(floodindex(i)>0)THEN
                IF(flooding(floodindex(i))%fpfmr>0.)THEN
                  CALL soilmodel_4(i,j,classdata(j)%soil,classdata(j)%luse,basin(i)%subid,pseudo_dayno,classarea,prec,cprec,temp,  & 
                   daylength,tmin,tmax,swrad,radexti(i),netrad,actvap,satvap,wind, &
                   basinrrcscorr(i),  &
                   frozenstate,soilstate,miscstate,surfaceflow,csrunoff,crunoffd,    &
                   cropuptake,nitrif,denitrif,epot,gwat,frostdepth,smdef,evap,cevap,crunoff1,crunoff2,crunoff3,  &
          ...
          ELSEIF(classmodel(j)==glacier_model)THEN
            ...
            CALL soilmodel_3(i,j,classdata(j)%soil,classdata(j)%luse,basin(i)%subid,pseudo_dayno,classarea,prec,cprec,temp, & 
                  daylength,tmin,tmax,swrad,radexti(i),netrad,actvap,satvap,wind, &
                  basinrrcscorr(i),  &
                  frozenstate,soilstate,miscstate,surfaceflow,csrunoff,crunoffd,  &
                  cropuptake,nitrif,denitrif,epot,gwat,frostdepth,smdef,evap,cevap,crunoff1,crunoff2,crunoff3,  &
          ...
          ELSEIF(classmodel(j)==5)THEN
            CALL soilmodel_5(i,j,classdata(j)%soil,classdata(j)%luse,basin(i)%subid,classarea,prec,cprec,temp,  & 
                 tmin,tmax,swrad,radexti(i),netrad,actvap,satvap,wind, &
                 basinrrcscorr(i),  &
          ...
          IF(.NOT.floodplainclass)THEN  !Floodplain will be added in routing loop
            ...
            IF(outvarstatus(o_psub)) CALL calculate_class_outvar_add(o_psub,i,j,a,prec+icpevap2)
          ...
          IF(.NOT.wetlandclass)THEN   
            ...
            IF(SUM(outvarindex(267:274))>0)THEN
              IF(classdata(j)%vegtype==i_open.OR.classdata(j)%vegtype==i_forest)THEN      !Open field or forest landuse set
                ...
                IF(classdata(j)%vegtype==i_open) area_open   = area_open + aadj*snowplain_area_fraction
                IF(classdata(j)%vegtype==i_forest) area_forest = area_forest + aadj*snowplain_area_fraction
              ENDIF
            ENDIF
          ...
          IF(outvarstatus(o_esub)) CALL calculate_class_outvar_add(o_esub,i,j,aadj,evap+icpevap2)
          IF(outvarstatus(o_isub)) CALL calculate_class_outvar_add(o_isub,i,j,aadj,icpevap2)
          ...
          IF(.NOT.wetlandclass)THEN
            ...
            IF(numsubstances>0)THEN
              IF(outvarstatus(o_csoilIN).OR.outvarstatus(o_csoilT1).OR.outvarstatus(o_csoilOC).OR.outvarstatus(o_csoilT2).OR.outvarstatus(o_csoilDSi))THEN
                ...
                IF(conduct%simSi.AND.outvarstatus(o_csoilDSi)) CALL calculate_class_outvar_add_amount(o_csoilDSi,i,j,aadj,loadrf(i_dsi),concrf(i_dsi))             
            ...
            IF(conduct%simN)THEN
              ...
              DO isl=1,3
                ...
                IF(outvarstatus(o_psoilON(isl))) CALL calculate_class_outvar_add(o_psoilON(isl),i,j,aadj,soilstate%conc(i_on,isl,j,i)*soilstate%water(isl,j,i))
              ENDDO
            ENDIF
            ...
            IF(conduct%simSi)THEN
              DO isl=1,3  
                IF(outvarstatus(o_psoilDSi(isl))) CALL calculate_class_outvar_add(o_psoilDSi(isl),i,j,aadj,soilstate%conc(i_dsi,isl,j,i)*soilstate%water(isl,j,i))
              ENDDO
            ENDIF            
            IF(simulate%substance(i_t1))THEN
              DO isl=1,3
            ...
            IF(numsubstances>0)THEN
              IF(outvarstatus(o_crunT1).OR.outvarstatus(o_crunT2).OR.outvarstatus(o_crunIN).OR.outvarstatus(o_crunON).OR.outvarstatus(o_crunTN).OR.outvarstatus(o_crunSP).OR.outvarstatus(o_crunPP).OR.outvarstatus(o_crunTP).OR.outvarstatus(o_crunOC).OR.outvarstatus(o_crunSS).OR.outvarstatus(o_crunDSI))THEN
                ...
                IF(simulate%substance(i_dsi).AND.outvarstatus(o_crunDSI)) CALL calculate_class_outvar_add_amount(o_crunDSI,i,j,aadj,loadrf(i_dsi),concrf(i_dsi))
              ENDIF
            ENDIF
          ENDIF
      ...
      IF(current_time%dayno==360)THEN
      ...
      IF(modeloption(p_soilleakage)==0)THEN
        ...
        IF(outvarstatus(o_csoilDSi)) CALL calculate_class_outvar_finish(o_csoilDSi,i,SUM(soilwateri))  !dissolved Si conc soil (mg/L)
      ELSE
        ...
        CALL set_class_outvar_missing(o_csoilDSi,i)
      ENDIF
      DO isl = 1,3
        ...
        IF(modeloption(p_soilleakage)==0)THEN
          ...
          CALL calculate_class_outvar_finish(o_psoilDSi(isl),i,asum(isl))
        ELSE
          ...
          CALL set_class_outvar_missing(o_psoilDSi(isl),i)
        ENDIF
      ENDDO
      ...
      CALL calculate_class_outvar_finish(o_crunDSI,i,runoffi)  !DSi conc in runoff [mg/L] 
      ...
      IF(modeloption(p_soilleakage)==1)THEN
        ...
        IF(simulate%substance(i_dsi))THEN
          CALL calculate_class_outvar_initialize(o_crunDSI,i)
          CALL calculate_class_outvar_add(o_crunDSI,i,1,1.,crunofftsi(i_dsi))
          CALL calculate_class_outvar_finish(o_crunDSI,i,runofftsi)  !DSi conc in runoff [mg/L]    
        ENDIF      
      ENDIF
      ...
      localriverflow = qin
      clocalriverflow = cin
      CALL calculate_local_river(i,riverlength(itype,i),radexti(i),cloudi(i),precorg(i),sfdist,pcorricep(i), &
      ...
      concout = 0.
      a = 0.
      IF(slc_ilake>0) a = classbasin(i,slc_ilake)%part
      
      !Lake calculations
      IF(a>0)THEN      !local lake exist
        inflowpart = basin(i)%ilakecatch
        CALL calculate_local_lake_model(i,radexti(i),cloudi(i),precorg(i),sfdist,  &  !IN
                                        pcorricep(i),localriverflow*inflowpart,clocalriverflow,hypodepth(itype), & !IN
                                        eacti,area_water,prec_water,evap_water, &   !INOUT
                                        frozenstate,riverstate,lakestate,lakesurftemp, &  !INOUT
                                        lakeoutflow(itype),concout,lakewst(itype), & !OUT
                                        outvar,wbflows,Latmdep,Lpathway)  !output
      ...
      riverarea(itype) = 0.
      riverboxinflow = 0.   !total inflow to riverboxi
      ...
      IF(conduct%irrigation)THEN
        ...
        ENDIF
        riverboxinflow = riverboxinflow - rivervremi
      ENDIF
      ...
      IF(riverlength(itype,i)>0.)THEN
        ...
        riverboxinflow = riverboxinflow + transq * seconds_per_timestep
        ...
        IF(a>0)THEN   !river has area
          ...
          IF(simulate%substance(i_t2))THEN
            ...
            CALL set_class_precipitation_concentration_and_load(i,classdata(j)%luse,classdata(j)%vegtype,numsubstances, &
                   riverareakm2,precorg(i),temp,prec,cprec,atmdepload1,snowfall,rainfall,meanrivertemp,frozenstate%rivericecov(itype,i))
          ELSE
            CALL set_class_precipitation_concentration_and_load(i,classdata(j)%luse,classdata(j)%vegtype,numsubstances, &
                   riverareakm2,precorg(i),temp,prec,cprec,atmdepload1)
          ENDIF
          helpq = 0.
          IF(prec>0) CALL add_precipitation_to_river(i,itype,riverarea(itype),prec,cprec,helpq,riverstate)
          riverboxinflow = riverboxinflow + helpq
          CALL add_deposition_to_river_as_load(i,classdata(j)%luse,itype,classdata(j)%vegtype,riverareakm2,   &
              atmdepload2,riverstate)
          IF(conductload)THEN
          ...
          IF(epot>0.)THEN
            CALL calculate_river_evaporation(i,j,itype,numsubstances,riverarea(itype)*riverfracarea(itype),temp,epot,evapr,cevaprT1,helpq,riverstate)
            riverboxinflow = riverboxinflow - helpq
          ELSE
            evapr = 0.
          ENDIF
          ...
          IF(outvarstatus(o_esub)) CALL calculate_class_outvar_add(o_esub,i,j,a*(1.-fpfrac),evapr*riverfracarea(itype)+icpevap2)
          IF(outvarstatus(o_isub)) CALL calculate_class_outvar_add(o_isub,i,j,a*(1.-fpfrac),icpevap2)
        ...
        IF(modeloption(p_deepgroundwater)==2)THEN
          CALL calculate_river_groundwaterflow_removal(i,j,basin(i)%subid,numsubstances,riverstate,grwout2,helpq)
          IF(conductwb) wbflows(w_rgrwofmr,i) = grwout2
          riverboxinflow = riverboxinflow - helpq
          IF(fpfrac>0 .AND. a>0)THEN
            IF(genpar(m_optonoff).LE.0 .OR. &
               (genpar(m_optonoff)>1.9.AND.genpar(m_optonoff)<3.1))THEN
        ...
        CALL point_abstraction_from_main_river(i,itype,riverstate,nutrientloadflows(3),helpq)
        riverboxinflow = riverboxinflow - helpq
          
        !Calculate substances processes in river
        CALL substance_processes_in_river(i,itype,riverarea(itype),riverdepth,transq,riverQbank,       &
                 river_maxSSconc(itype),river_sedresSS(itype), &
                 riverstate)
        ...
        IF(fpfrac>0 .AND. a>0)THEN
          ...
          IF(genpar(m_optonoff)>0.9.AND.genpar(m_optonoff)<1.1)THEN
          ...
          ELSEIF(genpar(m_optonoff)>1.9.AND.genpar(m_optonoff)<2.1)THEN
            interflowpar(1) = genpar(m_opt3)   !flmr
            interflowpar(2) = genpar(m_opt4)   !flfp
          ELSEIF(genpar(m_optonoff)>2.9.AND.genpar(m_optonoff)<3.1)THEN
            interflowpar(4) = genpar(m_opt5)
            interflowpar(5) = genpar(m_opt8)
          ELSEIF(genpar(m_optonoff)>3.9.AND.genpar(m_optonoff)<4.1)THEN
            interflowpar(3) = genpar(m_opt7)
          ENDIF
          ...
          ENDIF
          riverboxinflow = riverboxinflow - qmrflood
        ELSE  !no floodplain
        ...
        dampq = 0.
        IF(modeloption(p_rivermodel)==0)THEN
          !dampq = riverrc(1,itype,i)*transq*seconds_per_timestep + &
          !        riverrc(2,itype,i)*(riverstate%water(itype,i) - deadriver(itype,i) - transq*seconds_per_timestep)
          dampq = riverrc(1,itype,i)*riverboxinflow + &
                  riverrc(2,itype,i)*(riverstate%water(itype,i) - deadriver(itype,i) - riverboxinflow)
          IF(dampq>riverstate%water(itype,i) - deadriver(itype,i)) dampq = riverstate%water(itype,i) - deadriver(itype,i)
        ELSEIF(modeloption(p_rivermodel)==1)THEN
          dampq = (riverrc(1,itype,i)*(riverstate%water(itype,i) - deadriver(itype,i)))
        ENDIF
...
!        IF(conductload) Lpathway(:,16) = dampc * dampq * 1.E-3  !Load at point O (downstream of main river)
        dampq = dampq / seconds_per_timestep    !Calculate dampq (m3/s) before Lpathway to avoid numerical differences toward main/branch-load which is calculated from q (m3/s)
        IF(conductload) Lpathway(:,16) = dampc * dampq * seconds_per_timestep * 1.E-3  !Load at point O (downstream of main river).
        IF(outvarindex(o_cmrr)>0) outvar(i,outvarindex(o_cmrr)) = swrad
        IF(outvarindex(o_cmrq)>0) outvar(i,outvarindex(o_cmrq)) = dampq
        IF(numsubstances>0)THEN
          IF(simulate%substance(i_ss).AND.outvarindex(o_c9SS)>0) outvar(i,outvarindex(o_c9SS)) = dampc(i_ss)
          IF(simulate%substance(i_ae).AND.outvarindex(o_c9AE)>0) outvar(i,outvarindex(o_c9AE)) = dampc(i_ae)
          IF(simulate%substance(i_ss).AND.simulate%substance(i_ae).AND.outvarindex(o_c9TS)>0) outvar(i,outvarindex(o_c9TS)) = dampc(i_ss)+dampc(i_ae)
          IF(simulate%substance(i_dsi).AND.outvarindex(o_c9DS)>0) outvar(i,outvarindex(o_c9DS)) = dampc(i_dsi)
          IF(simulate%substance(i_asi).AND.outvarindex(o_c9AS)>0) outvar(i,outvarindex(o_c9AS)) = dampc(i_asi)
          IF(simulate%substance(i_dsi).AND.simulate%substance(i_asi).AND.outvarindex(o_c9Si)>0) outvar(i,outvarindex(o_c9Si)) = dampc(i_dsi)+dampc(i_asi)
          IF(simulate%substance(i_t1).AND.outvarindex(o_c9T1)>0) outvar(i,outvarindex(o_c9T1)) = dampc(i_t1)
          IF(simulate%substance(i_t2).AND.outvarindex(o_c9T2)>0) outvar(i,outvarindex(o_c9T2)) = dampc(i_t2)
          IF(simulate%substance(i_in).AND.outvarindex(o_c9IN)>0) outvar(i,outvarindex(o_c9IN)) = dampc(i_in)*1000.
          IF(simulate%substance(i_on).AND.outvarindex(o_c9ON)>0) outvar(i,outvarindex(o_c9ON)) = dampc(i_on)*1000.
          IF(simulate%substance(i_in).AND.simulate%substance(i_on).AND.outvarindex(o_c9TN)>0) outvar(i,outvarindex(o_c9TN)) = (dampc(i_in)+dampc(i_on))*1000.
          IF(simulate%substance(i_sp).AND.outvarindex(o_c9SP)>0) outvar(i,outvarindex(o_c9SP)) = dampc(i_sp)*1000.
          IF(simulate%substance(i_pp).AND.outvarindex(o_c9PP)>0) outvar(i,outvarindex(o_c9PP)) = dampc(i_pp)*1000.
          IF(simulate%substance(i_sp).AND.simulate%substance(i_pp).AND.outvarindex(o_c9TP)>0) outvar(i,outvarindex(o_c9TP)) = (dampc(i_sp)+dampc(i_pp))*1000.
          IF(simulate%substance(i_oc).AND.outvarindex(o_c9OC)>0) outvar(i,outvarindex(o_c9OC)) = dampc(i_oc)
          IF(simulate%substance(i_ss).AND.outvarindex(o_ssmx)>0) outvar(i,outvarindex(o_ssmx)) = river_maxSSconc(2)
          IF(simulate%substance(i_ss).AND.outvarindex(o_sdrs)>0) outvar(i,outvarindex(o_sdrs)) = river_sedresSS(2)
        ENDIF
      ...
      !No outlet lake or wetland, outflow of subbasin = outflow of main river
      ...
      !Calculate diagnostic output of river water level, depth, velocity
      IF(riverlength(itype,i)>0.)THEN
        IF(aolake==0 .AND. aowet==0)THEN
          helpq = basinoutflow(itype)
        ELSE
          helpq = dampq
        ENDIF
        IF(outvarindex(o_hged)>0) outvar(i,outvarindex(o_hged)) = genpar(m_hygeomc)*helpq**genpar(m_hygeomf)
        IF(outvarindex(408)>0) outvar(i,outvarindex(408)) = river_water_level(itype,i,helpq,ice_on_river(itype,i,frozenstate),frozenstate)
        IF(outvarindex(o_crrl)>0)THEN
          IF(outvarindex(408)>0)THEN
            outvar(i,outvarindex(o_crrl)) = local_water_level(itype,i,outvar(i,outvarindex(408)))
          ELSE
            outvar(i,outvarindex(o_crrl)) = local_water_level(itype,i,river_water_level(itype,i,helpq,ice_on_river(itype,i,frozenstate),frozenstate))
          ENDIF
        ENDIF
        IF(outvarindex(409)>0) outvar(i,outvarindex(409)) = genpar(m_hygeomk)*helpq**genpar(m_hygeomm)
      ELSE
      ...
      IF(aolake>0)THEN
        ...
        atmdepload1 = 0.
        atmdepload2 = 0.
        IF(conductdamconstruction) CALL change_current_dam_status(i) !Change regulation parameters for dam being built or removed
        ...
        IF(simulate%substance(i_t2))THEN
          CALL set_class_precipitation_concentration_and_load(i,classdata(j)%luse,classdata(j)%vegtype,numsubstances,  &
                 lakeareakm2,precorg(i),temp,prec,cprec,atmdepload1,snowfall,rainfall,lakestate%uppertemp(itype,i), &
                          frozenstate%lakeicecov(itype,i))
        ELSE
          CALL set_class_precipitation_concentration_and_load(i,classdata(j)%luse,classdata(j)%vegtype,numsubstances,  &
                 lakeareakm2,precorg(i),temp,prec,cprec,atmdepload1)
        ENDIF

        !Add precipitation and atmospheric deposition to lake water
        IF(prec>0) CALL add_water(numsubstances,lakestate%water(itype,i),lakestate%conc(:,itype,i),prec,cprec)
        CALL add_deposition_to_lake_as_load(i,classdata(j)%luse,itype,classdata(j)%vegtype,lakeareakm2,   &
              atmdepload2,lakestate)
        ...
        IF(outvarstatus(o_icloss)) CALL calculate_class_outvar_add(o_icloss,i,j,a*(1.-fpfrac),pcorricep(i)+icpevap)
        IF(outvarstatus(o_esub)) CALL calculate_class_outvar_add(o_esub,i,j,a*(1.-fpfrac),evapl+icpevap2)
        IF(outvarstatus(o_isub)) CALL calculate_class_outvar_add(o_isub,i,j,a*(1.-fpfrac),icpevap2)
        ...
        CALL tracer_processes_in_lake(i,itype,lakearea(itype),pooladd_t1,lakestate)
        ...
        CALL substance_processes_in_lake(i,itype,lakearea(itype),    &
               lakestate,resusp_downstream,pooladd_ts,pooladd_tn,pooladd_tp,pooladd_oc,pooladd_si,poolnet_ts)
        ...
        !Calculate and remove outflows from outlet lake (not lakebasin), including updating by observations
        IF(.NOT.statuslb)THEN
          ...
          !If siltation, add to outflow
          IF(conduct%siltation)THEN
            IF(basinoutflow(itype)>0.)THEN
              bconcout = bconcout + (resusp_downstream*1000.)/(basinoutflow(itype)*seconds_per_timestep) !#CB add resuspension (mg/L for S); convert resuspension from kg to mg; convert flow from m3/s to m3/day; convert m3 to L
              IF(conductload) Lpathway(:,19) = Lpathway(:,19) + resusp_downstream
            ENDIF
          ENDIF
        ...
        ELSEIF(statuslb)THEN
          ...
          IF(lakebasin(lakebasinindex(i))%last)THEN
            ...
            CALL calculate_flow_for_lakebasin_lake(i,itype,looplakes,& !IN
                                                qini2,lakeareai2,hypodepthi2,olakewstoldi2,resusp_downstream,&  !IN
      ...
      !Calculate flow and processes of outlet wetland if present, including updating by observations
      ...
      IF(outvarindex(o_rewstr)>0.AND.ALLOCATED(wobsi))THEN
        IF(wobsindex(i)>0)THEN
          outvar(i,outvarindex(o_rewstr)) = wobsi(wobsindex(i))
        ELSE
          outvar(i,outvarindex(o_rewstr)) = missing_value !needed? better way?
        ENDIF
      ELSEIF(outvarindex(o_rewstr)>0)THEN
        outvar(i,outvarindex(o_rewstr)) = missing_value   !needed?
      ENDIF
      ...
      CALL set_outvar_xobs(o_reSi,i)   !recorded Si in outflow, mg/L
      ...
      !Output variables for ilake water stage
      IF(outvarindex(o_wstilake)>0.AND.slc_ilake>0)THEN
        IF(classbasin(i,slc_ilake)%part>0)THEN
          outvar(i,outvarindex(o_wstilake)) = lakestate%water(1,i)*1.E-3 - thresholddepth(1,i)
        ENDIF
      ENDIF
      ...
      ENDIF
      IF(simulate%substance(i_dsi))THEN
        IF(outvarindex(o_clDSi)>0) outvar(i,outvarindex(o_clDSi)) = cbasinoutflow(i_dsi,1)
        IF(outvarindex(o_clASi)>0) outvar(i,outvarindex(o_clASi)) = cbasinoutflow(i_dsi,1)
        IF(outvarindex(o_clSi)>0) outvar(i,outvarindex(o_clSi)) = cbasinoutflow(i_dsi,1) + cbasinoutflow(i_asi,1)
      ENDIF      
      IF(slc_ilake>0)THEN
        IF(classbasin(i,slc_ilake)%part>0)THEN
      ...
      !Output irrigation and (other) abstraction
      ...
      IF(outvarindex(o_absr)>0) outvar(i,outvarindex(o_absr)) = nutrientloadflows(3) !Abstraction main river (m3)
      IF(outvarindex(o_absl)>0) outvar(i,outvarindex(o_absl)) = nutrientloadflows(4) !Abstraction outlet lake (m3)
      ...
      IF(slc_olake>0)THEN
        IF(classbasin(i,slc_olake)%part>0)THEN
          ...
          IF(simulate%substance(i_on).AND.outvarindex(o_olan)>0) outvar(i,outvarindex(o_olan)) = pooladd_tn ! #CB: nitrogen added to lake sediment (ON) (kg)
          IF(simulate%substance(i_pp).AND.outvarindex(o_olap)>0) outvar(i,outvarindex(o_olap)) = pooladd_tp ! #CB: phosphorus added to lake sediment (PP) (kg)
          IF(simulate%substance(i_oc).AND.outvarindex(o_olac)>0) outvar(i,outvarindex(o_olac)) = pooladd_oc ! #CB: org. carbon added to lake sediment (OC) (kg)
          IF(simulate%substance(i_asi).AND.outvarindex(o_olai)>0) outvar(i,outvarindex(o_olai)) = pooladd_si !silica added to lake sediment (ASi) (kg)
          IF(simulate%substance(i_t1).AND.outvarindex(o_ola1)>0) outvar(i,outvarindex(o_ola1)) = pooladd_t1 ! T1 added to lake sediment
          IF(outvarindex(o_olld)>0) outvar(i,outvarindex(o_olld)) = thresholddepth(2,i)       !olake lake depth --- maybe this output should be moved out of the simulate%substance(i_ss) section
          IF(simulate%substance(i_ss))THEN
            ...
            IF(outvarindex(o_olas)>0) outvar(i,outvarindex(o_olas)) = pooladd_ts ! #CB: sediment added to lake sediment pool (SS+AE) (kg)
              IF(outvarindex(o_olns)>0) outvar(i,outvarindex(o_olns)) = poolnet_ts ! #CB: net change in sediment in lake sediment pool (SS+AE) (kg)
            ! #CB Export olake sediment variables           
            IF(conduct%siltation)THEN
              IF(outvarindex(o_olsp)>0) outvar(i,outvarindex(o_olsp)) = lakestate%sedpool(i_ss,2,i) !olake sediment pool (SS only)
              IF(outvarindex(o_olsa)>0) outvar(i,outvarindex(o_olsa)) = lakestate%sedage(2,i)  !olake sediment age  (SS only)
              IF(outvarindex(o_olsd)>0) outvar(i,outvarindex(o_olsd)) = lakestate%seddens(2,i)    !olake sediment density
              IF(outvarindex(o_olss)>0) outvar(i,outvarindex(o_olss)) = (lakestate%sedpool(i_ss,2,i) + lakestate%sedpool(i_ae,2,i)) / lakestate%seddens(2,i) !olake sediment depth (SS+AE)
              IF(outvarindex(o_olsf)>0) outvar(i,outvarindex(o_olsf)) = lakestate%sedflush(2,i)       !olake sediment flush status
            ENDIF
          ENDIF
        ENDIF
      ENDIF
      ...
      !>Set subbasin output variables (outvar) for print out dependent on outlet lake
          ...
          IF(slc_olake>0)THEN
            IF(lakearea(2)>0.)THEN
              ...
              IF(ALLOCATED(wobsindex))THEN
                IF(wobsindex(i)>0)THEN
                  IF(wobsi(wobsindex(i))/=missing_value)THEN
                    IF(outvarindex(o_cleanwstr)>0) outvar(i,outvarindex(o_cleanwstr)) = wobsi(wobsindex(i)) - w0ref       !recorded olake waterstage (cleaned from w0ref)
                    IF(outvarindex(33)>0) outvar(i,outvarindex(33)) = oldolakewst - wobsi(wobsindex(i)) !error in W
                  ENDIF
                ENDIF
              ENDIF
            ENDIF
        ...
        ENDIF
        IF(conduct%simSi)THEN
          IF(outvarindex(o_ccSi)>0) outvar(i,outvarindex(o_ccSi)) = cbasinoutflow(i_dsi,2) + cbasinoutflow(i_asi,2)
        ENDIF      
        IF(simulate%substance(i_dsi).AND.outvarindex(o_ccDSi)>0) outvar(i,outvarindex(o_ccDSi)) = cbasinoutflow(i_dsi,2)   !comp conc DSi outflow olake (mg/l)
        IF(simulate%substance(i_asi).AND.outvarindex(o_ccASi)>0) outvar(i,outvarindex(o_ccASi)) = cbasinoutflow(i_asi,2)   !comp conc ASi outflow olake (mg/l)
        ...
        DO i2=1,i
          IF(looplakes(i2))THEN
              ...
              IF(slc_olake>0)THEN
                IF(lakeareai2(i2)>0.)THEN
                  ...
                  IF(wcomaveri2(i2)/=missing_value)THEN
                    ...
                    ENDIF
                  ENDIF
                  IF(ALLOCATED(wobsi))THEN
                    IF(wobsindex(i2)>0)THEN
                      IF(wobsi(wobsindex(i2))/=missing_value)THEN
                        IF(outvarindex(o_cleanwstr)>0) outvar(i2,outvarindex(o_cleanwstr)) = wobsi(wobsindex(i2)) - w0ref       !recorded olake waterstage (cleaned from w0ref)
                        IF(outvarindex(33)>0) outvar(i2,outvarindex(33)) = oldolakewst - wobsi(wobsindex(i2)) !error in W
                      ENDIF
                    ENDIF
                  ENDIF
                ENDIF
            ...
            IF(mainflowi2(i2)>0. .OR. branchflowi2(i2)>0.)THEN
              ...
              ENDIF
              IF(conduct%simSi)THEN
                IF(outvarindex(o_ccDSi)>0) outvar(i2,outvarindex(o_ccDSi)) = cpositiveoutflow(i_dsi)
                IF(outvarindex(o_ccASi)>0) outvar(i2,outvarindex(o_ccASi)) = cpositiveoutflow(i_asi)
                IF(outvarindex(o_ccSi)>0) outvar(i2,outvarindex(o_ccSi)) = cpositiveoutflow(i_dsi) + cpositiveoutflow(i_asi)
              ENDIF      
            ENDIF
    ...
    IF(conductwb) CALL print_waterbalance_timestep(nsub,naquifers,current_time%date)
...
!created.
    ...
    REAL wthresh    !outflow threshold (mm)
      ...
      IF(conduct%floodplain.AND.modeloption(p_floodplain)==3)THEN
        IF(flowreduction<0.)THEN
          ...
          wthresh = thresholddepth(itype,i)*1000.
          IF(outflowmm<0.)THEN
      ...
      CALL remove_outflow_from_lake(i,itype,numsubstances,outflowmm,basin(i)%subid,thresholddepth(itype,i),hypodepth,wst,concout,lakestate) !remove water (affect T2)
  ...
  SUBROUTINE calculate_flow_for_lakebasin_lake(ilast,itype,looplakes,& !IN
    ...
    REAL, INTENT(IN)    :: resusp_downstream(numsubstances) !<flushing to downstream subbasin from lake last subbasin
    ...
    REAL, PARAMETER :: limitlevel = 0.02   !mm, lowest netflow between lakebasin to avoid numerical problem
      ...
      !H�r finns ingen kontroll att det finns vatten i sj�n tillr�ckligt f�r outflowm3s (= all clboutflow)
...
!            wstlake = (newwholelakewst + basin(i2)%lakedepth(2))/basin(i2)%lakedepth(2)
            wstlake = (newwholelakewst + thresholddepth(2,i2))/thresholddepth(2,i2)
      ...
      DO i2 = 1,ilast
        IF(looplakes(i2))THEN
          wstlake = lakestate%water(itype,i2)*1.E-3 - thresholddepth(2,i2)  !m  !all lakebasins have the same threshold
          ...
          IF(lakestate%water(itype,i2)<0.)THEN  !Negative volume in lake is not good
            IF(conductwarning) WRITE(6,*) 'WARNING: negative volume of lakebasin in subbasin', basin(i2)%subid, 'water depth (m)',wstlake
          ...
          ELSE
            ...
            IF(wstlake-outflowm < -thresholddepth(2,i2))THEN
              IF(conductwarning) WRITE(6,*) 'WARNING: Water level below bottom of lake basin in subbasin', basin(i2)%subid
              IF(conductwarning) WRITE(6,*) 'WARNING: Limited outflow to available water'
              outflowm = wstlake + thresholddepth(2,i2)
            ENDIF
          ENDIF
      ...
      outflowsimi2(ilast) = clboutflow(ilast,1) + clboutflow(ilast,2) !No negative cout! Det �r �nd� denna jag anv�nder i r�kningarna!
            ...
            DO isource=1,ilast     !Loop over all lakebasins next to the i2 one
              IF(looplakes(isource))THEN
                ...
                IF(wateravail>currentflow)THEN    !I think this has to be done in mm for accuracy?
                  ...
                  IF(numsubstances>0)THEN
                    CALL remove_outflow_from_lake(isource,2,numsubstances,outflowmm,basin(isource)%subid,thresholddepth(2,isource),hypodepthi2(isource),lakestate%water(itype,isource),conccurrentflow,lakestate) !remove water and set conc of flow
            ...
            IF(isbout(i2))THEN
              ...
              IF(clboutflow2(i2,2)>0)THEN
                ...
                IF(wateravail>currentflow)THEN    !calculate in mm here instead!?
                  ...
                  IF(numsubstances>0)THEN
                    CALL remove_outflow_from_lake(i2,itype,numsubstances,outflowmm,basin(i2)%subid,thresholddepth(2,i2),hypodepthi2(i2),lakestate%water(itype,i2),conccurrentflow,lakestate) !remove water and set conc of flow
                ...
                ELSEIF(wateravail>0.)THEN
                  ...
                  IF(numsubstances>0)THEN
                    CALL remove_outflow_from_lake(i2,itype,numsubstances,outflowmm,basin(i2)%subid,thresholddepth(2,i2),hypodepthi2(i2),lakestate%water(itype,i2),conccurrentflow,lakestate) !set conc of flow
                    CALL add_water(numsubstances,helpflowi2(i2),conclboutflow(:,i2),currentflow/seconds_per_timestep,conccurrentflow)
                  ENDIF
      ...
      DO i2 = 1,ilast-1
        IF(looplakes(i2))THEN
          lakeoutflowi2(i2) = outflowsimi2(i2) !Kan vara summan av pos branch och neg internt fl�de
      ...
      IF(conduct%siltation)THEN
        IF(lakeoutflowi2(ilast)>0.)THEN
          clakeoutflowi2(:,ilast) = clakeoutflowi2(:,ilast) + (resusp_downstream*1000.)/(lakeoutflowi2(ilast)*seconds_per_timestep) !#CB add resuspension (mg/L for S); convert resuspension from kg to mg; convert flow from m3/s to m3/day; convert m3 to L
          IF(conductload) Lpathwayi2(:,19,ilast) = Lpathwayi2(:,19,ilast) + resusp_downstream
        ENDIF
      ENDIF
  ...
  SUBROUTINE calculate_internal_wetland(i,radext,cloud,added_water,q,conc,frozenstate,soilstate, &
    ...
    USE SURFACEWATER_PROCESSES, ONLY : get_wetland_threshold, &
      ...
      REAL temp,prec,tmin,tmax,swrad,rhmin,actvap,satvap,icpevap,icpevap2,netrad,wind,swpot  !class forcing
      ...
      IF(conduct%simN)THEN
        IF(outvarindex(o_iwincoin)>0) localoutvar(i,outvarindex(o_iwincoin)) = conc(i_in) * 1000. !ug/L
        IF(outvarindex(o_iwincoon)>0) localoutvar(i,outvarindex(o_iwincoon)) = conc(i_on) * 1000.
        IF(outvarindex(o_iwutcoin)>0) localoutvar(i,outvarindex(o_iwutcoin)) = cwetoutflow(i_in) * 1000.
        IF(outvarindex(o_iwutcoon)>0) localoutvar(i,outvarindex(o_iwutcoon)) = cwetoutflow(i_on) * 1000.
      ENDIF
      IF(conduct%simP)THEN
        IF(outvarindex(o_iwincosp)>0) localoutvar(i,outvarindex(o_iwincosp)) = conc(i_sp) * 1000.  !ug/L
        IF(outvarindex(o_iwincopp)>0) localoutvar(i,outvarindex(o_iwincopp)) = conc(i_pp) * 1000.
        IF(outvarindex(o_iwutcosp)>0) localoutvar(i,outvarindex(o_iwutcosp)) = cwetoutflow(i_sp) * 1000.
        IF(outvarindex(o_iwutcopp)>0) localoutvar(i,outvarindex(o_iwutcopp)) = cwetoutflow(i_pp) * 1000.
      ENDIF
      IF(conduct%simS)THEN
  ...
  !-------------------------------------------------------------------------------------
  SUBROUTINE calculate_local_river(i,lriverlen,radext,cloud,pobs,sfdist,localpcorricep, &  !IN
                                  qriver,criver, &  !INOUT
    ...
    !Argument declarations
      ...
      REAL atmdepload1(numsubstances),atmdepload2(numsubstances)  !wet dep (including temp variable used to add dep as load to dep with rain) and dry dep
      REAL rload(numsubstances)   !rural load, needed if localLruralb not allocated, numbsubstances=0 
      REAL temp,prec,tmin,tmax,swrad,rhmin,actvap,satvap,icpevap,icpevap2,netrad,wind,swpot  !class forcing
      REAL cprec(numsubstances)   !concentration of precipitation for this class
      REAL snowfall, rainfall  !Precipiation as snow, rain (mm)
      ...
      REAL maxSSconc_lr !local river SS maximum transport concentration (mg/L)
      REAL sedresSS_lr  !local river sedimentation/resuspension of SS (kg/timestep)
      REAL riverboxinflow, helpq   !help for outflow calculation (m3)
      ...
      IF(lriverlen>0.)THEN
        ...
        IF(a>0)THEN
          ...
          IF(simulate%substance(i_t2))THEN
            ...
            CALL set_class_precipitation_concentration_and_load(i,classdata(j)%luse,classdata(j)%vegtype,numsubstances, &
                   riverareakm2,pobs,temp,prec,cprec,atmdepload1,snowfall,rainfall,meanrivertemp,frozenstate%rivericecov(itype,i))
          ELSE
            CALL set_class_precipitation_concentration_and_load(i,classdata(j)%luse,classdata(j)%vegtype,numsubstances, &
                   riverareakm2,pobs,temp,prec,cprec,atmdepload1)
          ENDIF
          IF(prec>0) CALL add_precipitation_to_river(i,itype,lriverarea,prec,cprec,helpq,riverstate)
          riverboxinflow = riverboxinflow + helpq
          CALL add_deposition_to_river_as_load(i,classdata(j)%luse,itype,classdata(j)%vegtype,riverareakm2,   &
              atmdepload2,riverstate)
          IF(conductload)THEN
          ...
          IF(conduct%lakeriverice)  epot = epot * (1. - frozenstate%rivericecov(itype,i))
          IF(epot>0.)THEN
            CALL calculate_river_evaporation(i,j,itype,numsubstances,lriverarea*lrfracarea,temp,epot,evapr,cevaprT1,helpq,riverstate)
            riverboxinflow = riverboxinflow - helpq
          ELSE
          ...
          IF(outvarstatus(o_esub)) CALL calculate_class_outvar_add(o_esub,i,j,a,evapr*lrfracarea+icpevap2)
          IF(outvarstatus(o_isub)) CALL calculate_class_outvar_add(o_isub,i,j,a,icpevap2)
        ...
        CALL substance_processes_in_river(i,itype,lriverarea,riverdepth,transq,riverQbank,maxSSconc_lr,sedresSS_lr,riverstate)
        ...
        !dampq = riverrc(1,itype,i)*(riverstate%water(itype,i) - deadriver(itype,i))   !m3/ts
        dampq = 0.
        IF(modeloption(p_rivermodel)==0)THEN
          !dampq = riverrc(1,itype,i)*transq*seconds_per_timestep + &
          !        riverrc(2,itype,i)*(riverstate%water(itype,i) - deadriver(itype,i) - transq*seconds_per_timestep)
        ...
        IF(conductload) localLpathway(:,8) = criver * qriver *seconds_per_timestep * 1.E-3   !Load at point C (downstream of local river), point H
  ...
  !>\brief Almost all calculations related to local internal lake.
  !>
  !>Calculate flow into local lake and resulting outflow. Calculates processes 
  !>related to evaporation, ice, nutrients, and other substances. Calculate 
  !>special output, e.g. for loads and waterbalance.
  !>Note that abstraction for irrigation is done outside (before) this subroutine.
  !>
  !>\b Reference ModelDescription Chapter Rivers and lakes (Local lake (ilake))
  !-------------------------------------------------------------------------------------
  SUBROUTINE calculate_local_lake_model(i,radext,cloud,pobs,sfdist,localpcorricep, &  !IN
                                  qin,cin,hypodep, &  !IN
                                  localeacti,localarea_water,localprec_water,localevap_water, &  !INOUT
                                  frozenstate,riverstate,lakestate, &   !INOUT
                                  surftemp,outflow,coutflow,outwst, & !OUT
                                  localoutvar,localwbflows,localLatmdep,localLpathway) !INOUT
    ...
    USE MODVAR, ONLY : basin, &
                       ...
                       simulate, &
                       slc_ilake
    USE HYPEVARIABLES, ONLY : m_wetspload,m_drypp, &
                              o_ctmp,o_cprc,o_psim,o_rainfall,o_snowfall, &
                              o_epot,o_evap,o_evpt,o_icloss,o_cevapT1, &
                              o_fnca,o_fcon,o_psub,o_esub,o_isub, &
    ...
    USE ATMOSPHERIC_PROCESSES, ONLY : calculate_class_atmospheric_forcing, & 
                                      ...
                                      calculate_potential_evaporation,          &
                                      set_class_precipitation_concentration_and_load
    USE SURFACEWATER_PROCESSES, ONLY : calculate_actual_lake_evaporation, &
                                       ...
                                       ice_processes_in_lake, &
                                       translation_in_river
    USE NPC_SURFACEWATER_PROCESSES, ONLY : add_deposition_to_lake_as_load, &
                                           substance_processes_in_lake
    USE TRACER_PROCESSES, ONLY : add_tracer_point_source_to_lake,  &
                                 tracer_processes_in_lake
    ...
    IMPLICIT NONE
      ...
      REAL, INTENT(IN) :: qin  !<flow into lake from local river (m3/s)
      REAL, INTENT(IN) :: cin(numsubstances)  !<concentration of flow in local river
      REAL, INTENT(IN) :: hypodep  !< local lake hypolimnion depth (m)
      REAL, INTENT(INOUT) :: localeacti   !<accumulation of actual evapotranspiration (mm)
      REAL, INTENT(INOUT) :: localarea_water   !<accumulation of water area fraction (-)
      REAL, INTENT(INOUT) :: localprec_water   !<accumulation of precipitation on water (mm)
      REAL, INTENT(INOUT) :: localevap_water   !<accumulation of actual evapotranspiration on water (mm)
      ...
      REAL, INTENT(INOUT) :: surftemp(2)   !lake surface temperature (local and main, only local used here)
      REAL, INTENT(OUT) :: outflow  !<flow from lake  (m3/s)
      REAL, INTENT(OUT) :: coutflow(numsubstances)  !<concentration of flow from lake
      REAL, INTENT(OUT) :: outwst  !<lake water (mm)
      REAL, INTENT(INOUT),ALLOCATABLE :: localoutvar(:,:)    !<outvar; output for print out
      REAL, INTENT(INOUT),ALLOCATABLE :: localwbflows(:,:)   !<wbflows; flow for water balance output (m3/ts)
      REAL, INTENT(INOUT),ALLOCATABLE :: localLatmdep(:,:,:) !<Latmdep; load for print out
      REAL, INTENT(INOUT),ALLOCATABLE :: localLpathway(:,:)  !<Lpathway; load for print out
      ...
      !Local variables
      INTEGER j,k   !index of ilake class,substance
      INTEGER freezeupday,breakupday
      REAL a   !area fraction of local lake
      REAL lakearea,lakeareakm2  !area of local lake (m2, km2)
      REAL qunitfactor
      REAL atmdepload1(numsubstances),atmdepload2(numsubstances)  !wet dep (including temp variable used to add dep as load to dep with rain) and dry dep
      REAL temp,prec,tmin,tmax,swrad,rhmin,actvap,satvap,icpevap,icpevap2,netrad,wind,swpot  !class forcing
      REAL cprec(numsubstances)   !concentration of precipitation for this class
      ...
      REAL epot,epotsnow   !Potential evaporation
      REAL evapl,cevapl(numsubstances)   !Evaporation
      REAL qinloc,cinloc(numsubstances), qinmm   !Inflow to lake (from river or net inflow)
      REAL freezeuparea  !freezeuparea for lake
      REAL pein,fnca,fcon   !ilake connectivity variables
      REAL resuspdown(numsubstances) !resuspension of lake to be transported downstream (kg)
      REAL pooladd_ts,pooladd_tn,pooladd_tp,pooladd_oc,pooladd_si,pooladd_t1,poolnet_ts !sediment added to ilake, net change in sediment (kg)
      ...
      atmdepload1 = 0.
      atmdepload2 = 0.
      fnca = 0.     !fraction of non-contributing sub-basin area
      fcon = 1.     !fraction of ilake connectivity
      qinloc = qin  !local variable will change
      cinloc = cin
      ...
      IF(simulate%substance(i_t2))THEN
        CALL set_class_precipitation_concentration_and_load(i,classdata(j)%luse,classdata(j)%vegtype,numsubstances,  &
                lakeareakm2,pobs,temp,prec,cprec,atmdepload1,snowfall,rainfall,lakestate%uppertemp(itype,i), &
                        frozenstate%lakeicecov(itype,i))
      ELSE
        CALL set_class_precipitation_concentration_and_load(i,classdata(j)%luse,classdata(j)%vegtype,numsubstances,  &
                lakeareakm2,pobs,temp,prec,cprec,atmdepload1)
      ENDIF
      ...
      IF(outvarstatus(o_evpt)) CALL calculate_class_outvar_add(o_evpt,i,j,a,evapl+localpcorricep+icpevap)
      IF(outvarstatus(o_icloss)) CALL calculate_class_outvar_add(o_icloss,i,j,a,localpcorricep+icpevap)
      IF(outvarstatus(o_esub)) CALL calculate_class_outvar_add(o_esub,i,j,a,evapl+icpevap2)
      IF(outvarstatus(o_isub)) CALL calculate_class_outvar_add(o_isub,i,j,a,icpevap2)
      localeacti = localeacti + evapl*a
      ...
      localarea_water = localarea_water + a
      localprec_water = localprec_water + (rainfall+snowfall)*a
      localevap_water = localevap_water + evapl * a
        
      !Set water balance output
      IF(conductwb)THEN
        localwbflows(w_pilake,i) = prec * lakearea*1.E-3     !m3
        localwbflows(w_eilake,i) = evapl * lakearea*1.E-3
      ENDIF

      !>Calculate and add inflow (including point source) to local lake
      IF(conduct%simT1.OR.conduct%simT2) CALL add_tracer_point_source_to_lake(i,itype,qinloc,cinloc)
      qinmm = qinloc * qunitfactor        ![mm/timestep] !moved down, erronous before add_tracer_point_source
      CALL add_water(numsubstances,lakestate%water(itype,i),lakestate%conc(:,itype,i),qinmm,cinloc)
      IF(conductload)THEN
        localLpathway(:,10) = cinloc * qinloc * seconds_per_timestep * 1.E-3  !Load at point J, local flow to ilake (kg/timestep)
        localLpathway(:,9) = localLpathway(:,8) - localLpathway(:,10)             !Load at point I, bypassed local flow
        DO k= 1,numsubstances
          IF(localLpathway(k,9)<0.) localLpathway(k,9) = 0.   !Numerical issues gave minus zero in output
        ENDDO
      ENDIF
      IF(conductwb .AND. qinloc>0)THEN
        localwbflows(w_irtoil,i) = qinloc * seconds_per_timestep
        localwbflows(w_irtomr,i) = localwbflows(w_irtomr,i) - localwbflows(w_irtoil,i)
      ENDIF
      qinloc = qinloc + (prec-evapl)/qunitfactor    !All added water (net), to be used for outflow calculation

      !>Calculate lake tracer (T1), temperature (T2) and ice processes
      CALL tracer_processes_in_lake(i,itype,lakearea,pooladd_t1,lakestate)
      IF(simulate%substance(i_t2)) CALL T2_processes_in_lake(i,itype,temp,swrad,surftemp, &
                              lakearea,hypodep,frozenstate,lakestate, &
                              freezeupday,freezeuparea)
      IF(conduct%lakeriverice) CALL ice_processes_in_lake(i,itype,classdata(j)%luse,snowfall,temp, & 
                                      surftemp,swrad,frozenstate,lakestate, & 
                                      freezeupday,breakupday,hypodep,freezeuparea) 
      IF(modeloption(p_swtemperature)==1)  CALL set_water_temperature(itype*2,i,riverstate,lakestate)
        
      !>Calculate substances processes in lakes
      CALL substance_processes_in_lake(i,itype,lakearea,lakestate,resuspdown,pooladd_ts,pooladd_tn,pooladd_tp,pooladd_oc,pooladd_si,poolnet_ts)
          
      !>Calculate and remove outflow from lake
      IF(modeloption(p_connectivity)>0 .AND. basin(i)%lakesection>0)THEN
        !ilake section connectivity model -> note that qin is separated into runoff and P+E
        pein = (prec-evapl)/qunitfactor
        CALL calculate_ilakesection_outflow(i,basin(i)%subid,numsubstances,qinloc-pein,pein,    &
                   lakearea,qunitfactor,outflow,coutflow,  &
                   localLpathway,localwbflows,outwst,fnca,fcon,lakestate)
      ELSE 
        CALL calculate_ilake_outflow(i,basin(i)%subid,numsubstances,qinloc,    &
                lakearea,qunitfactor,outflow,coutflow,  &
                localLpathway,localwbflows,outwst,lakestate)
      ENDIF

      !If siltation, add to outflow
      IF(conduct%siltation)THEN
        IF(outflow>0.)THEN
          coutflow = coutflow + (resuspdown*1000.)/(outflow*seconds_per_timestep) !#CB add resuspension (mg/L for S); convert resuspension from kg to mg; convert flow from m3/s to m3/day; convert m3 to L
          IF(conductload) localLpathway(:,11) = localLpathway(:,11) + resuspdown
        ENDIF
      ENDIF
      
      !Ilake connectivity outputs
      IF(outvarindex(o_fnca)>0) localoutvar(i,outvarindex(o_fnca)) = fnca 
      IF(outvarindex(o_fcon)>0) localoutvar(i,outvarindex(o_fcon)) = fcon 
 
    END SUBROUTINE calculate_local_lake_model

...
SUBROUTINE run_model_crit(npar,mpar,par,frozenstate,soilstate,aquiferstate,riverstate,lakestate,miscstate,criterion,status)
  ...
  USE MODVAR, ONLY : nsub,current_time
...
SUBROUTINE run_model_simout(npar,mpar,par,iens,runens,allens,frozenstate,soilstate,aquiferstate, &
  ...
  USE MODVAR, ONLY : nsub,naquifers,current_time
...
!DE-MC parameters
     ...
     IF(npop.LT.npar)THEN
       IF(conductwarning) WRITE(6,*)'WARNING[DEMC]: The number of populations',npop,'is smaller than number of calibrated parameters. You might consider to increase the DEMC_npop parameter in optpar.txt'
     ENDIF
     IF(gamma.GT.0.9)THEN
       IF(conductwarning) WRITE(6,*)'WARNING[DEMC]: gamma = gamma_scale * 2.38/sqrt(2*npar) = ',gamma,' is larger than 0.9, you might consider decreasing the gamma_scale in optpar.txt'
     ENDIF
...
! Start printing log file
      OPEN(UNIT = fileunit_callog, FILE = TRIM(resdir)//filename_callog, STATUS = 'replace',ACTION='write',ERR=900)   ! Replace calibration log file
    ...
    !Error handling
900 CONTINUE
    WRITE(6,*) 'ERROR: Open file: '//TRIM(resdir)//filename_callog//' for writing.'
    STOP 1
...
SUBROUTINE param_scanning(frozenstate,soilstate,aquiferstate,riverstate,lakestate,miscstate)
  ...
  OPEN(UNIT = fileunit_callog, FILE = TRIM(resdir)//filename_callog, STATUS = 'replace',ACTION='write',ERR=900) !Start calibration log file
...
SUBROUTINE quasiNewton_algorithm(critLastVect,parLastTable,frozenstate,soilstate,aquiferstate,riverstate,lakestate,miscstate)
  ...
  DO WHILE(gradNorm > optim%QN_flatTol)
     ...
     CALL direction_multiplier(par,parMin,parMax,parPrecis,direction,frozenstate,soilstate,aquiferstate,riverstate,lakestate,miscstate,lambdaBest,critBest,lastCritBest)
...
SUBROUTINE load_qNstarting_vector(parMin, parMax, par)
  ...
  DO lineCounter = 1, numoptimpar
    ...
    IF(par(lineCounter) < parMin(lineCounter) .OR. par(lineCounter) > parMax(lineCounter)) THEN
      WRITE(6,*) 'ERROR: Problem in qNstartpar: starting parameter value outside optimization boundaries'
      WRITE(6,*) 'ERROR: File ',TRIM(filename)
      STOP 1
    ENDIF
  ENDDO
...
SUBROUTINE direction_multiplier(par,parMin,parMax,parPrecis,direction,  &
  ...
  CALL linesearch_HYSS(0.0,maxLambda,par,parPrecis,direction,frozenstate,soilstate,aquiferstate,riverstate,lakestate,miscstate,lambdaBest,critBest,0.0,prevCritBest)

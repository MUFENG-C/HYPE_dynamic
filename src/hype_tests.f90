...
MODULE MODEL_TEST_ROUTINES
  ...
  !The module procedures also use MODVAR, WORLDVAR, HYPE_INDATA and COMPOUT, 
  !and more from READWRITE_ROUTINES and HYPEVARIABLES.
  ...
  INTEGER, PARAMETER :: t_sedresusp                = 29
  ! This is the total number of processes
  INTEGER, PARAMETER :: nof_test_processes         = 29

  !Parameter used as index for the external tests performed elsewhere
  INTEGER, PARAMETER :: e_lakedata                 = 1  !
  INTEGER, PARAMETER :: e_geodata                  = 2  !
  INTEGER, PARAMETER :: e_damdata                  = 3  !
  INTEGER, PARAMETER :: e_flooddata                = 4  !
  INTEGER, PARAMETER :: e_mgmtdata                 = 5  !
  INTEGER, PARAMETER :: e_forckey                  = 6  !
  INTEGER, PARAMETER :: e_par                      = 7  !
  INTEGER, PARAMETER :: e_update                   = 8  !
  INTEGER, PARAMETER :: e_model_base_conf_ok       = 9  !
  INTEGER, PARAMETER :: e_branchdata               = 10 !
  INTEGER, PARAMETER :: e_aquiferdata              = 11 !
  INTEGER, PARAMETER :: e_glacierdata              = 12 !
  INTEGER, PARAMETER :: e_pointsourcedata          = 13 !
  INTEGER, PARAMETER :: e_leakagedata              = 14 !
  INTEGER, PARAMETER :: e_cropdata                 = 15 !
  INTEGER, PARAMETER :: e_info_file                = 16 !
  INTEGER, PARAMETER :: e_set_outvar_for_var       = 17 !
  INTEGER, PARAMETER :: e_outregions               = 18 !
  INTEGER, PARAMETER :: e_otest                    = 19 !
  INTEGER, PARAMETER :: e_output_range_violation   = 20 !
  INTEGER, PARAMETER :: e_rrcdata                  = 21 !
  INTEGER, PARAMETER :: e_atmdepdata               = 22  !
  
  ! This is the last number of external tests
  INTEGER, PARAMETER :: nof_external_tests         = 22
  ...
  'ext.par','ext.update','ext.model_base_conf_ok', &
  'ext.branchdata','ext.aquiferdata','ext.glacierdata','ext.pointsourcedata', &
  'ext.leakagedata','ext.cropdata','ext.info_file','ext.set_outvar_for_var', &
  'ext.outregions','ext.otest','ext.output_range_violation', &
  'ext.riverratingcurvedata','ext.atmdepdata'])
...
! This is the maximum number of test cases; modparid + extra for test of generic, input and forcing
  INTEGER, PARAMETER :: max_testcases             = n_max_par + 186
  ...
  PARAMETER(testing_names=[CHARACTER(LEN=32) :: 'tobs', 'tminobs', 'tmaxobs', 'swobs', &
      ...
      'tobselev', 'forcings_stationid', 'number_of_neg_or_miss', 'consistent_forcing', &
      'no_neg_value', 'time period', 'irrigation_subid', 'lakeriverice_class', &
      ...
      'basinregion','nofprintsteps','consistent_observations','branch defined','lrwet area', &
      'mrwet area','lrwet depth','mrwet depth','lrwet_part','mrwet_part','lrwet_inflow', &
      'mrwet_inflow','iwet area','owet area','fewsignfig','regvol', &
      'links lakebasin internal', 'links lb extra outlets','no wind directions', &
      'uwobs','vwobs','mrivrc1','mrivrc2','mrivrc3','mrivrc4','mrivrc5', &
      'outregionid','outregion_subid','ricethpo','rural_landarea','PSD_exist','PST_exist', &
      ...
      'irrigation_gwpart','irrigation_swpart','irrigation_local_eff','irrigation_regional_eff', &
      'irr_regsource','irrdam_regsource','irrdam_localsource','irrigation_demandtype', &
      'watertr_subid','watertr_reciever','watertr_no_dwtr','numsubst','deltaw0', &
      'clay','sand','silt','claysandsilt','flushing_interval','flush value s', &
      'flush value m','dambuild_before_removed','dambuild_before_removed', &
      'regest_rivvel','regest_damp'])
  ...
  TYPE TESTCASETYPE
    ...
    INTEGER           :: tolerance             !<parameter for specific tolerance 
  ...
  PUBLIC :: stop_simulation_and_finalize_tests, &   !subroutines
            setup_for_hype_tests, &
            finalize_hype_tests, &
            run_hype_observation_tests, &
            run_hype_tests, &
            propagate_external_msg, &
            e_lakedata,e_geodata,e_rrcdata,e_damdata, &
            e_flooddata,e_mgmtdata,e_forckey,e_par, &
            e_update,e_model_base_conf_ok,e_branchdata, &
            e_aquiferdata,e_glacierdata,e_pointsourcedata, &
            e_leakagedata,e_cropdata,e_atmdepdata,e_info_file, &
            e_set_outvar_for_var,e_outregions, &
            e_otest,e_output_range_violation, &
  ...
  SUBROUTINE stop_simulation_and_finalize_tests(errnum, errtext)
    ...
    CHARACTER(LEN=*), OPTIONAL, INTENT(IN) :: errtext !< Error text 
    ...
    status = 0
    WRITE(funit_used,*) '---------------- FROM FILE LOADING ----------------------------'
    CALL check_external_tests(status)

    ! be sure to not stop with error number 3 (which onoff_option=3 does)
    ! but stop with the input errnum instead!
    !IF(onoff_option>=3) onoff_option = 2
    IF(onoff_option==3) onoff_option = 2
    
    !Close file, clear tests and inform that tests ended
    CALL finalize_hype_tests()

    ! annoying select case due to restrictions in fortran to send a variable to STOP statement
    SELECT CASE(errnum)
      CASE(0)
        STOP 0
      CASE(1)
  ...
  !>Setup for tests by setting flags according to given input in info-file 
  !>to perform chosen test options and opens file for output of test results.
  !----------------------------------------------------------------------------
  SUBROUTINE setup_for_hype_tests(funit,fname,onoff,level)
    ...
    IF (PRESENT(onoff)) THEN
      onoff_option = onoff
      IF (onoff_option == 1) THEN   !all test stop when error
        error_halt = .TRUE.
      ELSEIF (onoff_option == 2) THEN   !all test do not stop for error
        error_halt = .FALSE.
      ELSEIF (onoff_option == 3) THEN   !all test, stop with exit 3 always
        !onoff_option = 3
        error_halt = .FALSE.
      ELSEIF (onoff_option == 4) THEN   !only observation tests stop when error
        error_halt = .TRUE.
      ELSEIF (onoff_option == 5) THEN   !only observation tests do not stop for error
        error_halt = .FALSE.
      ELSEIF (onoff_option > 5) THEN    !set to 1
        onoff_option = 1
        error_halt = .TRUE. 
      ENDIF
    END IF
    ...
    !Error handling
700 WRITE(6,*) 'Error: creating the hype tests file ',TRIM(fname)
    funit_used = 0
  ...
  SUBROUTINE propagate_external_msg(external_id,msg_type,msg,vdata)
    ...
    IF(PRESENT(vdata))THEN
      IF(SIZE(vdata) > 0) THEN
        IF (ALLOCATED(external_tests(external_id)%ndata(i)%vdata)) THEN
          DEALLOCATE(external_tests(external_id)%ndata(i)%vdata)
        END IF
        ALLOCATE(external_tests(external_id)%ndata(i)%vdata(SIZE(vdata)))
        external_tests(external_id)%ndata(i)%vdata = vdata
      END IF
    END IF
    ...
    DO i = 1, nof_external_tests
      IF (external_tests(i)%propagated) THEN
        ...
        DO j = 1, external_tests(i)%index_ndata
          ...
          IF (ALLOCATED(external_tests(i)%ndata(j)%vdata) .AND. printout_level > 1) THEN
            WRITE(funit_used,*) 'column code:', external_tests(i)%ndata(j)%vdata
          END IF
        END DO
      END IF
    END DO
  ...
  USE MODVAR, ONLY : model_version_string
  USE READWRITE_ROUTINES, ONLY : write_comment_with_metadata,write_commentline_to_file
    ...
    CALL write_comment_with_metadata(commentline,model=model_version_string,comment='Test log file')
    CALL write_commentline_to_file(funit_used,commentline)
    WRITE(funit_used,*,ERR=300) 'indatacheckonoff', onoff_option
    WRITE(funit_used,*,ERR=300) 'indatachecklevel', printout_level
    WRITE(funit_used,*,ERR=300) '---------------------------------------------------------------'
    WRITE(funit_used,*,ERR=300) '---------------- OBSERVATIONS ---------------------------------'
    CALL check_observations(status)
    WRITE(funit_used,*,ERR=300) '-------------------- STATUS -----------------------------------'
    WRITE(funit_used,*,ERR=300) '-- Observations --', status
    WRITE(funit_used,*,ERR=300) '---------------------------------------------------------------'
	...
	!Error handling
300 WRITE(6,*) 'Error: writing to hype tests funit',funit_used
    status = 1
    RETURN
  ...
  SUBROUTINE run_hype_tests(status)
    ...
    !IF (onoff_option > 0) THEN
    IF (onoff_option > 0 .AND. onoff_option < 4) THEN   !4 is only obs tests
    ...
    WRITE(funit_used,*) '-------------- Sediment ---------------------------------------'
    WRITE(funit_used,*) '-- Sources --'
    WRITE(funit_used,*) '-- Rivers and lakes --'
    CALL check_sediment_sedresusp(status)
    CALL check_siltation(status)

    WRITE(funit_used,*) '---------------------------------------------------------------'
    WRITE(funit_used,*) '-------------------- STATUS -----------------------------------'
    WRITE(funit_used,*) '-- Other input --', status
    WRITE(funit_used,*) '---------------------------------------------------------------'
    WRITE(funit_used,*) 
	...
	!Error handling
301 WRITE(6,*) 'Error: writing to hype tests funit', funit_used
    status = 1
    RETURN
...
!---------------------------------------------
! Private subroutines for HYPE specific tests
!---------------------------------------------
! Subroutines for checking observations
!---------------------------------------
  ...
  SUBROUTINE check_observations(status)
    ...
    INTEGER, PARAMETER :: count_miss_also = 0
    INTEGER, PARAMETER :: skip_miss = 1
    ...
    INTEGER iforc,s,i,j
    INTEGER obsindex(basemodel%nsub) !Index of observation stations
    INTEGER nobscol                  !Number of columns with observations in file
    INTEGER numneg(basemodel%nsub)
    INTEGER ncomrow                  !Number of comment rows
    ...
    !> checking station ids, timeperiods, consistency and missing or other negative illegal values if file is present
...
!        filepath = TRIM(modeldir)//TRIM(filename)
        ...
        IF (.NOT.fileexist) THEN
          ret2 = .FALSE.
          CALL add_generic_result(.FALSE.,a_file_exist,t_observation,'file found')
        ELSE
          CALL add_generic_result(.TRUE.,a_file_exist,t_observation,'file found')
          CALL check_input(ret2,REAL(forcingdata(iforc)%stationid),gt_zero,a_forcings_stationid,t_observation)
!          CALL check_input(ret2,REAL(forcingdata(iforc)%stationid),gt_zero,a_forcings_stationid,t_observation,force=.TRUE.)
          CALL count_comment_rows(fileunit_temp,filepath,ncomrow,s) 
          CALL check_file_station_id_order(fileunit_temp,filepath,basemodel%nsub,ncomrow,forcingdata(iforc)%stationid,obsindex,nobscol,s)
          IF (s .NE. 0) THEN
            ret2 = .FALSE.
            CALL add_generic_result(.FALSE.,a_consistent_forcing,t_observation,'consistency, model and '//TRIM(forcingdata(iforc)%filename))
          ELSE
            CALL add_generic_result(.TRUE.,a_consistent_forcing,t_observation,'consistency, model and '//TRIM(forcingdata(iforc)%filename))
          END IF
          CALL check_obs_timeperiod(fileunit_temp,filepath,ncomrow+1,i_t,bdate,sdate,fbdate,fedate,notimefound,s)
          IF (s .NE. 0) THEN
            ret2 = .FALSE.
            CALL add_generic_result(.FALSE.,a_timeperiod_forcing,t_observation,testing_names(a_timeperiod_forcing))
          ELSE
            CALL add_generic_result(.TRUE.,a_timeperiod_forcing,t_observation,testing_names(a_timeperiod_forcing))
          END IF
          IF (iforc==i_pobs.OR.iforc==i_rhobs.OR.iforc==i_sfobs.OR.iforc==i_swobs.OR.iforc==i_uobs) THEN
            !Count all negative values, including missing!
            CALL count_data_negative_in_file(filepath,notimefound,nobscol,ncomrow+1,basemodel%nsub,obsindex,count_miss_also,numneg,s)
          ELSE
            !For T,Tmin,Tmax count only for missing values!
            CALL count_data_missing_in_file(filepath,notimefound,nobscol,ncomrow+1,basemodel%nsub,obsindex,numneg,s)
          END IF
          !Check if any missing (or other illegal negative) values found
!          CALL check_input(ret2,REAL(numneg),eq_zero,a_iforc_no_neg_or_no_miss,t_observation,force=.TRUE.)
          CALL check_input(ret2,REAL(numneg),eq_zero,a_iforc_no_neg_or_no_miss,t_observation)
        END IF
        !Be sure to print and clear tests after each cycle in the loop, but also force test is needed
        IF (.NOT. ret2) THEN
          ret = .FALSE.
        END IF
        IF (.NOT. ret) THEN
    ...
    IF (fileexist) THEN
      ...
      ncomrow = 0
      CALL count_comment_rows(fileunit_temp,filepath,ncomrow,s) 
      CALL check_station(fileunit_temp,filepath,basemodel%nsub,ncomrow,basin(:)%subid,nobscol,obsindex,s)
      IF(s .NE. 0) THEN
        CALL add_generic_result(.FALSE.,a_consistent_forcing,t_observation,'consistency, model and Qobs')
      ELSE
        CALL add_generic_result(.TRUE.,a_consistent_forcing,t_observation,'consistency, model and Qobs')
      ENDIF
      IF (s == 0) THEN
        CALL check_obs_timeperiod(fileunit_temp,filepath,ncomrow+1,i_t,bdate,sdate,fbdate,fedate,notimefound,s)
        IF (s == 1) THEN
        ...
        ELSE
          CALL add_generic_result(.TRUE.,a_timeperiod_forcing,t_observation,testing_names(a_timeperiod_forcing))
        END IF
        CALL count_data_negative_in_file(filepath,notimefound,nobscol,ncomrow+1,basemodel%nsub,obsindex,skip_miss,numneg,s) !Skip missing values
        IF (s == 1) THEN
          ret2 = .FALSE.
        END IF
!        CALL check_input(ret2,REAL(numneg),eq_zero,a_no_neg_value,t_observation,force=.TRUE.)
    ...
    xoms_loop : DO i = 1, max_xoms
      DO j = 1,2
        IF(conductxoms) THEN
          ...
          IF (fileexist) THEN
            !Check stations
            CALL check_station(fileunit_temp,filepath,basemodel%nsub,1,basin(:)%subid,nobscol,obsindex,s)
            IF (s .NE. 0) THEN
              CALL add_generic_result(.FALSE.,a_consistent_observation,t_observation,'consistency, model and '//TRIM(xoms_filename(i,j)))
            ELSE
              CALL add_generic_result(.TRUE.,a_consistent_observation,t_observation,'consistency, model and '//TRIM(xoms_filename(i,j)))
            END IF
            IF (s == 0) THEN
              !Check time period
              ...
              ELSE
                CALL add_generic_result(.TRUE.,a_timeperiod_forcing,t_observation,testing_names(a_timeperiod_forcing))
              END IF
              !CALL count_data_negative_in_file(filepath,notimefound,nobscol,2,basemodel%nsub,obsindex,skip_miss,numneg,s)  !Xoms not limited to positive values
              !IF (s == 1) THEN
              !  ret2 = .FALSE.
              !END IF
              !!Check data
    ...
    !> @test Check Wobs:
    !> checking stations and timeperiods if file is present
    ret = .TRUE.
    filepath = TRIM(forcingdir)//filename_Wobs
    INQUIRE(FILE=filepath,EXIST=fileexist)
    ret2 = .TRUE.
    IF (fileexist) THEN
      !Check stations in Wobs, but allow stations not present in GeoData
      ncomrow = 0
      CALL count_comment_rows(fileunit_temp,filepath,ncomrow,s) 
      CALL check_station(fileunit_temp,filepath,basemodel%nsub,ncomrow,basin(:)%subid,nobscol,obsindex,s)
      IF(s .NE. 0) THEN
        CALL add_generic_result(.FALSE.,a_consistent_forcing,t_observation,'consistency, model and Wobs')
      ELSE
        CALL add_generic_result(.TRUE.,a_consistent_forcing,t_observation,'consistency, model and Wobs')
      ENDIF
      IF (s == 0) THEN
        !Check time period
        CALL check_obs_timeperiod(fileunit_temp,filepath,ncomrow+1,i_t,bdate,sdate,fbdate,fedate,notimefound,s)
    ...
    IF (fileexist) THEN
      ncomrow = 0
      CALL count_comment_rows(fileunit_temp,filepath,ncomrow,s) 
      CALL count_data_cols(fileunit_temp,filepath,ncomrow,nobscol,s)
      IF (s == 0 .AND. nobscol > 0) THEN
        CALL check_xobs_validity(fileunit_temp,filepath,ncomrow,nobscol-1,s)
        IF (s > 0) THEN
          CALL add_generic_result(.FALSE.,a_consistent_observation,t_observation,testing_names(a_consistent_observation))
        ELSE
          CALL add_generic_result(.TRUE.,a_consistent_observation,t_observation,testing_names(a_consistent_observation))
        END IF
      END IF
      IF (s == 0) THEN
        CALL check_obs_timeperiod(fileunit_temp,filepath,ncomrow+2,i_t,bdate,sdate,fbdate,fedate,notimefound,s)
        IF (s == 1) THEN
          ret2 = .FALSE.
          CALL add_generic_result(.FALSE.,a_timeperiod_forcing,t_observation,testing_names(a_timeperiod_forcing))
        ELSE
  ...
  !------------------------------------------------------------
  SUBROUTINE count_data_negative_in_file(filepath,notimefound,ncols,nskip,ns,oindex,miss,numneg,negfound)
    ...
    INTEGER, INTENT(IN)  :: miss         !<Flag for including missing values, 0=count, 1=ignore
    ...
    DO
      y = missing_value
      IF(readmatlab)THEN
        READ(fileunit_temp,*,END=900,ERR=901) d2,y
      ELSEIF(timeform==0)THEN
        READ(fileunit_temp,*,END=900,ERR=901) d2,y
      ELSEIF(timeform==1)THEN
        READ(fileunit_temp,*,END=900,ERR=901) d2,d3,y    
      ENDIF
      !>Count negative data
      DO i = 1,ncols
        IF(miss==0)THEN
...
901 CONTINUE
    WRITE(6,*) 'ERROR: reading observation data file: '//TRIM(filepath)
    STOP 1

  END SUBROUTINE count_data_negative_in_file

  !>Check and count for missing data in file. Used for forcing data files; 
  !>Tobs.txt, TMINobs.txt and TMAXobs.txt
  !-----------------------------------------------------------------------
  SUBROUTINE count_data_missing_in_file(filepath,notimefound,ncols,nskip,ns,oindex,nummiss,missfound)
...
900 IF(SUM(miss)/=0.)THEN
      DO i = 1,ns
        ...
        ENDIF
      ENDDO
      IF(SUM(nummiss)/=0.) missfound = 1
    ENDIF

    CLOSE(fileunit_temp)

  END SUBROUTINE count_data_missing_in_file
  ...
  SUBROUTINE check_xobs_validity(funit,infile,nskip,ncols,status)
    ...
    INTEGER, INTENT(IN)  :: nskip             !<Total number of comment rows
    ...
    DO i = 1,nskip
      READ(funit,*,ERR=201)                       !Skip comment line
    ENDDO
...
!----------------------------------
! Subroutines for checking generic
!----------------------------------
  ...
  SUBROUTINE check_info_generic(status)
    ...
    USE WORLDVAR, ONLY: ndt,dtskip,bdate,sdate,outstartdate,output,noutput,&
                        weightsub,subweightcrit

    USE MODVAR, ONLY : outvarid,nsub,numsubstances, &
                       i_in,i_on,i_sp,i_pp,i_oc,i_ss,i_ae,i_t1,i_t2,i_dsi,i_asi
    ...
    !> @test Check simulation period: positive number of time steps, output time steps and proper dates.
    ret = ndt > 0
    CALL add_generic_result(ret,a_noftimesteps,t_info_generic,'nof timesteps')
    IF (.NOT. ret) THEN
        status = status + 1
    END IF
    ret = dtskip < ndt
    CALL add_generic_result(ret,a_nofprintsteps,t_info_generic,'nof print steps')
    IF (.NOT. ret) THEN
        status = status + 1
    END IF
    ret = bdate%day >= 1 .AND. bdate%day <= 31
    ret = ret .AND. (bdate%month >= 1 .AND. bdate%month <= 12)
    CALL add_generic_result(ret,a_bdaterange,t_info_generic,'bdate range')
    IF (.NOT. ret) THEN
        status = status + 1
    END IF
    ret = sdate%day >= 1 .AND. sdate%day <= 31
    ret = ret .AND. (sdate%month >= 1 .OR. sdate%month <= 12)
    CALL add_generic_result(ret,a_edaterange,t_info_generic,'edate range')
    IF (.NOT. ret) THEN
        status = status + 1
    END IF
    ret = outstartdate%day >= 1 .AND. outstartdate%day <= 31
    ret = ret .AND. (outstartdate%month >= 1 .OR. outstartdate%month <= 12)
    CALL add_generic_result(ret,a_cdaterange,t_info_generic,'cdate range')
    IF (.NOT. ret) THEN
        status = status + 1
    END IF
    ...
    DO io = 1, noutput
      ...
      ENDIF
    ENDDO
    CALL add_generic_result(ret,a_timemapduplicates,t_info_generic,'no duplicated time/mapoutputs')
    IF (.NOT. ret) THEN
        status = status + 1
    END IF
    ...
    DO io = 1, noutput
      ...
      ENDIF
    ENDDO
    CALL add_generic_result(ret,a_signfig,t_info_generic,'enough signfigures')
    IF(.NOT. ret)THEN
      status = status + 1
    ENDIF
    ...
    IF(weightsub)THEN
      x = SUM(subweightcrit)
      IF(x==nsub) ret = .FALSE.
      CALL add_generic_result(ret,a_weightsub,t_info_generic,'subweightcrit')
      IF(.NOT. ret)THEN
        status = status + 1
      ENDIF
    ENDIF

    !> @test Check substances for duplicates:
    ret = .TRUE.
    s = 0
    IF(i_in>0) s=s+1
    IF(i_on>0) s=s+1
    ...
    IF(i_asi>0) s=s+1
    ret = (s==numsubstances)
    CALL add_generic_result(ret,a_numsubst,t_info_generic,'inconsistency in substances set')
  ...
  SUBROUTINE check_geodata_generic(status)
    ...
    USE MODVAR, ONLY : basin,classbasin,max_subid,slc_olake,slc_ilake, &
                       load,path,branchdata,nclass,nsub
!    USE WORLDVAR, ONLY : basemodel
    ...
    CALL add_generic_result(ret,a_basinarea,t_geodata_generic,'some basin areas gt zero')
    IF (.NOT. ret) THEN
        status = status + 1
    END IF
    ...
    CALL add_generic_result(ret,a_basinslcfrac,t_geodata_generic,'basin area zero')
    IF (.NOT. ret) THEN
        status = status + 1
    END IF
    ...
    CALL add_generic_result(ret,a_subidrange,t_geodata_generic,'subid range')
    IF (.NOT. ret) THEN
        status = status + 1
    END IF
    ...
    CALL add_generic_result(ret,a_basinslope,t_geodata_generic,'basin slope')
    IF (.NOT. ret) THEN
        status = status + 1
    END IF
    ...
    CALL add_generic_result(ret,a_basinrivlen,t_geodata_generic,'basin rivlen')
    IF (.NOT. ret) THEN
        status = status + 1
    END IF
    ...
    CALL add_generic_result(ret,a_basinpospart,t_geodata_generic,'slc fractions positive')
    IF (.NOT. ret) THEN
        status = status + 1
    END IF
    ...
    DO j = 1, nclass
      ...
      ret = ret.AND.ret2
    ENDDO
    CALL add_generic_result(ret,a_scr_part,t_geodata_generic,'basin part2cr')
    IF(.NOT. ret) status = status + 1
    ...
    IF (slc_olake > 0) THEN
        ...
        CALL add_generic_result(ret,a_basinodepth,t_geodata_generic,'basin olake depth')
        IF (.NOT. ret) THEN
            status = status + 1
        END IF
    END IF
    IF (slc_ilake > 0) THEN
        ...
        CALL add_generic_result(ret,a_basinidepth,t_geodata_generic,'basin ilake depth')
        IF (.NOT. ret) THEN
            status = status + 1
        END IF
    END IF
    ...
    CALL add_generic_result(ret,a_slcforolake,t_geodata_generic,'no abstraction without olake')
    IF (.NOT. ret) THEN
      status = status + 1
    END IF
    ...
    IF(slc_olake>0)THEN
      ...
      CALL add_generic_result(ret,a_abstrwitholake,t_geodata_generic,'abstraction with olake')
      IF (.NOT. ret) THEN
        status = status + 1
      END IF
    ENDIF
...
!      DO i = 1,basemodel%nsub
      ...
      ENDDO
      ret = (s == 0)
      CALL add_generic_result(ret,a_linkmain,t_geodata_generic,'linking main branch')
      IF(.NOT. ret) status = status + 1
    ENDIF
...
!        DO k = 1,basemodel%nsub
      ...
      END DO
      ret = (s == 0)
      CALL add_generic_result(ret,a_linksecondary,t_geodata_generic,'linking secondary branch')
      IF (.NOT. ret) THEN
        status = status + 1
      END IF
    END IF
...
!    USE WORLDVAR, ONLY : atmdepvegreset
    ...
    ret = (nclass > 0)
    CALL add_generic_result(ret,a_nclass,t_geoclass_generic,'nclass gt zero')
    IF (.NOT. ret) THEN
        status = status + 1
    END IF
    ...
    !!> @test Check vegetation type for atmospheric deposition and snow output; 1-3 and 3=water class
    !ret = .NOT. atmdepvegreset
    !DO j=1,nclass
    !  IF(classdata(j)%vegtype>3.OR.classdata(j)%vegtype<1) ret = .FALSE.
    !  IF(classmodel(j)==1 .OR. classmodel(j)==2) THEN
    !    ret = ret .AND. classdata(j)%vegtype==3   !lake
    !  ELSEIF(classmodel(j)==11 .OR. classmodel(j)==12) THEN
    !    ret = ret .AND. classdata(j)%vegtype==3   !river
    !  ELSEIF(classmodel(j)==13 .OR. classmodel(j)==14) THEN
    !    ret = ret .AND. classdata(j)%vegtype==3   !wetland
    !  END IF
    !ENDDO
    !CALL add_generic_result(ret,a_vegset,t_geoclass_generic,testing_names(a_vegset))
    !IF (.NOT. ret) THEN
    !  status = status + 1
    !ENDIF
    ...
    IF(ALLOCATED(classmodel)) THEN
      ...
      CALL add_generic_result(ret,a_multiple_classes,t_geoclass_generic,'no multiple classes, ilake,olake,mriver,lriver,glacier,iwet,owet')
      IF (.NOT. ret) THEN
          status = status + 1
      END IF
    END IF
    ...
    ret2 = .TRUE.
    ret3 = .TRUE.
    CALL add_generic_result(ret2,a_inc_soildepth,t_geoclass_generic,'increasing soildepth')
    DO i=1,maxsoillayers
        ...
        IF (i<maxsoillayers) THEN
            ...
            ret2 = ret2 .AND. (MINVAL(classdata(:)%soildepth(i+1)-classdata(:)%soildepth(i)) >= 0.0)
            CALL add_generic_result(ret2,a_inc_soildepth,t_geoclass_generic,'increasing soildepth')
            ...
            IF (i>1 .AND. ret2) THEN
              ...
              IF (.NOT. ret2) THEN
                ret3 = .FALSE.
                CALL add_generic_result(ret3,a_inc_soildepth,t_geoclass_generic,'increasing soildepth')
              END IF
            END IF
        END IF
    END DO
  ...
  !> @brief Checks input data in file MgmtData
  !--------------------------------------------------------------------
  SUBROUTINE check_mgmtdata_generic(status)
    ...
    LOGICAL ret,ret2,ret3
    INTEGER i,j,nirrsys
    INTEGER irr_status,wtr_status

    !If irrigation in MgmtData, check input
    ret = .TRUE.
    irr_status = 0
    IF(ALLOCATED(irrigationsystem)) THEN
      !> @test Check that input data are given for all irrigated subbasins (subids).
      ret = ALL(irrigationsystem(:)%subid > 0)
      CALL add_generic_result(ret,a_irrigation_subid,t_mgmtdata_generic,'mgmtdata has subids defined')

      !> @test Check that input data are given for all irrigated subbasins (gw_part,local_eff).
      CALL check_input(ret,irrigationsystem(:)%gw_part,ge_zero_and_le_one,a_irrigation_gwpart,t_mgmtdata_generic)
      CALL check_input(ret,irrigationsystem(:)%sw_part,ge_zero_and_le_one,a_irrigation_swpart,t_mgmtdata_generic)

      !Do not work for rows with a regsrc that has no irr fields! 
      !These two tests should be done together with check of any irrigation fields in check_irrigation instead. For later...
      !And also irrdam for regsrc further below.
      !CALL check_input(ret,irrigationsystem(:)%local_eff,gt_zero_and_le_one,a_irrigation_loceff,t_mgmtdata_generic) !Do not work for rows with a regsrc that has no irr fields!
      !
      !!> @test Check that input data are given for all irrigated subbasins (demandtype). 
      !ret2 = .TRUE.
      !nirrsys = SIZE(irrigationsystem)
      !DO i=1,nirrsys
      !  IF(irrigationsystem(i)%demandtype<=0 .OR. irrigationsystem(i)%demandtype>3) ret2 =.FALSE.
      !ENDDO
      !CALL add_generic_result(ret2,0,a_irrigation_type,t_mgmtdata_generic,'mgmtdata demandtype')!Do not work for rows with a regsrc that has no irr fields!
      !ret = ret.AND.ret2
     

      !> @test Check that input data (region_eff) are given for all subbasins irrigated by regional source.
      ret2 = .TRUE.
      nirrsys = SIZE(irrigationsystem)
      DO i=1,nirrsys
        IF(irrigationsystem(i)%regsourceid>0)THEN
          IF(irrigationsystem(i)%reg_eff<=0. .OR. irrigationsystem(i)%reg_eff>1.) ret2 =.FALSE.
        ENDIF
      ENDDO
      CALL add_generic_result(ret2,a_irrigation_regeff,t_mgmtdata_generic,'reg source has eff defined')
      ret = ret.AND.ret2
      
      !> @test Check that a defined regional source is in the model set-up
      ret2 = .TRUE.
      DO i = 1,nirrsys
        IF(irrigationsystem(i)%regsourceid>0)THEN
          ret2 = .FALSE.  !not found yet
          DO j = 1,nirrsys
            IF(irrigationsystem(i)%regsourceid==irrigationsystem(j)%subid)THEN
              ret2 = .TRUE. !found
              EXIT
            ENDIF
          ENDDO
          IF(.NOT.ret2) EXIT  !not found, failed check!
        ENDIF
      ENDDO
      CALL add_generic_result(ret2,a_irr_regsrc,t_mgmtdata_generic,'reg source in model domain')
      ret = ret.AND.ret2
      
      !> @test Check that a source from a dam has a lake defined.
      !If not this will not cause problem for HYPE, but MgmtData have wrong data.
      ret2 = .TRUE.
      ret3 = .TRUE.
      DO i = 1,nirrsys
        IF(irrigationsystem(i)%dam)THEN
          DO j = 1, nsub
            IF(irrigationsystem(i)%subid==basin(j)%subid)THEN
!              IF(.NOT. (classbasin(j,slc_olake)%part>0.)) ret2 = .FALSE.
              IF(.NOT. (classbasin(j,slc_ilake)%part>0.) .AND. .NOT. (classbasin(j,slc_olake)%part>0.))THEN
                ret3 = .FALSE.
                EXIT
              ENDIF
!              IF(.NOT.ret2) EXIT
            ENDIF
          ENDDO
!          IF(.NOT.ret2 .AND. .NOT.ret3) EXIT
          IF(.NOT.ret3) EXIT
        ENDIF
      ENDDO
     ! CALL add_generic_result(ret2,0,a_irrdam_reg,t_mgmtdata_generic,'reg source has outlet lake') !This do not work, have not checked that subid is a regsrc! Move to t_irrigation with above
      CALL add_generic_result(ret3,a_irrdam_loc,t_mgmtdata_generic,'local source from lake')
      ret = ret.AND.ret2.AND.ret3

      IF (.NOT. ret) irr_status = irr_status + 1
      !ret = (irr_status == 0)
      CALL print_testresult_logical('Irrigation data in MgmtData.txt',ret,suffix='')
    ...
    IF(ALLOCATED(watertransfer))THEN
      !> @test Check that input data are given for all subbasins (subids,reciever).
      ret = ALL(watertransfer(:)%srcsubid > 0)
      CALL add_generic_result(ret,a_watertr_subid,t_mgmtdata_generic,'mgmtdata has subids defined')
      
      !Move these to special water transfer check were presence of dwtr can be checked!
      !ret2 = ALL(watertransfer(:)%rcvindex > 0)
      !CALL add_generic_result(ret2,0,a_watertr_rcv,t_mgmtdata_generic,'all recievers in model')  !Ok not to have, maybe check but not include in fail?
      !!ret = ret.AND.ret2
      !ret2 = ALL(watertransfer(:)%flow > 0)
      !CALL add_generic_result(ret2,0,a_watertr_no_dwtr,t_mgmtdata_generic,'all water transfer constant')  !This is just info, but will be counted for output failures

      IF(.NOT.ret) wtr_status = wtr_status + 1
      CALL print_testresult_logical('Water transfer data in MgmtData.txt',ret,suffix='')
    ENDIF
    
    CALL print_test_process(t_mgmtdata_generic)

    status = status + irr_status + wtr_status
  ...
  SUBROUTINE check_outregion_generic(status)
    ...
    IF(num>0)THEN
      ...
      status = status + 1
    ENDIF
    CALL add_generic_result(ret,a_outregionid,t_outregion_generic,'outregion ids are unique')
    ...
    IF(num>0)THEN
      ...
      status = status + 1
    ENDIF
    CALL add_generic_result(ret,a_outregionsubid,t_outregion_generic,'outregion subids exist')
  ...
  SUBROUTINE check_lakedata_generic(status)
    ...
    USE MODVAR, ONLY : elake,lake,path, &
    ...
    !!!> @test Check that lakes in LakeData have rating curve parameters set, but not necessary so 0 allowed (maybe missing also?)
    !!Checked lake by lake under lakes instead!
    ...
    IF(ALLOCATED(lakebasinindex))THEN
        ...
        IF(lakebasinindex(i)>0)THEN
          IF(.NOT.lakebasin(lakebasinindex(i))%last)THEN
            IF(path(i)%main<=0)THEN
              s = s + 1
            ELSEIF(lakebasinindex(path(i)%main)<=0)THEN
              s = s + 1
            ELSEIF(lakebasin(lakebasinindex(i))%ilk/=lakebasin(lakebasinindex(path(i)%main))%ilk)THEN
              s = s + 1
            ENDIF
          ENDIF
        ENDIF
      ENDDO
      ret = (s == 0)
      CALL add_generic_result(ret,a_linklbinternal,t_lakedata_generic,testing_names(a_linklbinternal))
      IF(.NOT. ret) status = status + 1
    ENDIF
...
!            DO k = 1,basemodel%nsub
        ...
        ENDDO
        ret = (s == 0)
        CALL add_generic_result(ret,a_linklbsecondout,t_lakedata_generic,testing_names(a_linklbsecondout))
        IF(.NOT. ret) status = status + 1
      ENDIF
    ENDIF

    !> @test Check dam build before removed
    ret = .TRUE.
    IF(ALLOCATED(lake))THEN
      DO i = 1,SIZE(lake(:))
        IF(lake(i)%btdate(1)/=DateType(0,0,0,0,0))THEN
          IF(lake(i)%btdate(2)/=DateType(0,0,0,0,0))THEN
            IF(lake(i)%btdate(1).GE.lake(i)%btdate(2)) ret = .FALSE.
          ENDIF
        ENDIF
      ENDDO
    ENDIF
    IF(ALLOCATED(elake))THEN
      DO i = 1,SIZE(elake(:))
        IF(elake(i)%btdate(1)/=DateType(0,0,0,0,0))THEN
          IF(elake(i)%btdate(2)/=DateType(0,0,0,0,0))THEN
            IF(elake(i)%btdate(1).GE.elake(i)%btdate(2)) ret = .FALSE.
          ENDIF
        ENDIF
      ENDDO
    ENDIF
    CALL add_generic_result(ret,a_lddambuild,t_lakedata_generic,testing_names(a_lddambuild))
    IF(.NOT. ret) status = status + 1
  ...
  SUBROUTINE check_damdata_generic(status)
    ...
    IF (ALLOCATED(dam)) THEN
      ...
      DO i=1,SIZE(dam)
        ...
        ENDIF
      ENDDO
      CALL add_generic_result(ret2,a_damqprod,t_damdata_generic,testing_names(a_damqprod))
      ret = ret.AND.ret2
      ...
      IF(.NOT.ret2)THEN
        ...
        CALL add_generic_result(ret2,a_damlimprod,t_damdata_generic,testing_names(a_damlimprod))
      ENDIF
      ret = ret.AND.ret2
        
      !!> @test Check rating curve set, but it is not necessary so allow also 0. Should missing value be allowed?
      !Checked dam by dam under lakes instead.
      !ret2 = .TRUE.
      !CALL check_input(ret2,dam(:)%rate,ge_zero,a_damrate,t_damdata_generic)
      !CALL check_input(ret2,dam(:)%exp,ge_zero,a_damexp,t_damdata_generic)
      !ret = ret.AND.ret2

      !> @test Check dam build before removed
      ret2 = .TRUE.
      DO i = 1,SIZE(dam)
        IF(dam(i)%btdate(1)/=DateType(0,0,0,0,0))THEN
          IF(dam(i)%btdate(2)/=DateType(0,0,0,0,0))THEN
            IF(dam(i)%btdate(1).GE.dam(i)%btdate(2)) ret2 = .FALSE.
          ENDIF
        ENDIF
      ENDDO
      CALL add_generic_result(ret2,a_dddambuild,t_damdata_generic,testing_names(a_dddambuild))
      ret = ret.AND.ret2
  ...
  SUBROUTINE check_generic(status)
    ...
    DO i = 1, num_modelprocess
        ...
        END IF
    END DO
    CALL add_generic_result(ret,a_modeloptionrange,t_generic,'modeloption range')
    ...
    ELSE
        ...
        status = status + 1
    END IF
    CALL add_generic_result(ret,a_snalbrange,t_generic,'snalbminmax range')
    ...
    ret = .TRUE.
    CALL add_generic_result(ret,a_wcwpwcfcwcep,t_generic,'wcwp+wcfc+wcep')
    DO i = 1, nsoil
      DO j = 1, maxsoillayers
        !wcfc
        ...
        IF (sum_val <= 0 .OR. sum_val > 1) THEN
          ...
          r(j) = ret
          s(j) = s(j) + 1
          CALL add_generic_result(r(j),a_wcwpwcfcwcep,t_generic,'wcwp+wcfc+wcep')
        END IF
      END DO
    END DO
    ...
    IF (conduct%irrigation .OR. conduct%simN .OR. conduct%simP .OR. conduct%simC .OR. (conduct%simS .AND. modeloption(p_erosion)==0)) THEN
      ...
      CALL add_generic_result(ret,a_basinregion,t_generic,testing_names(a_basinregion))
      IF (.NOT. ret) THEN
        status = status + 1
      END IF
...
!--------------------------------
! Subroutines for checking water
!--------------------------------
  ...
  SUBROUTINE check_evaporation(status)
    ...
    USE MODVAR, ONLY : p_petmodel,petmodelnames,maxmodelsinoption,modeloptionname
    ...
    IF(num_petmodels_used==0) num_petmodels_used = 1  !add default petmodel (=0) if no found
    ...
    DO i = 1, num_petmodels_used
      ...
      IF (ret) THEN
        s = s - 1
      END IF
      CALL print_testresult_logical(petmodelnames(petmodels_used(i)),ret,prefix=TRIM(modeloptionname(p_petmodel))//': ')
    END DO
  ...
  SUBROUTINE log_petmodels_used(funit,petmodels_used,num_petmodels_used)
    ...
    IF(num_petmodels_used==0)RETURN
  ...
  LOGICAL FUNCTION valid_petmodel_options(current_petmodel)
    ...
    SELECT CASE(current_petmodel)
      CASE(0) 
        ...
        CALL check_forcing_exist(valid_petmodel_options,i_tobs,a_tobs,t_evaporation)
      CASE(1)
      ...
      CASE(2)
        ...
        IF(tmp_kc_result(1))THEN
          CALL uncheck_testcase(n_kc(1),t_evaporation)
          valid_petmodel_options = valid_petmodel_options .AND. .TRUE.
        ELSEIF(tmp_kc_result(2))THEN
          CALL uncheck_testcase(n_kc(2),t_evaporation)
          valid_petmodel_options = valid_petmodel_options .AND. .TRUE.
        ELSE
        ...
        CALL add_generic_result(r,a_latitude_defined,t_evaporation,'latitude defined')
      ...
      CASE(3)
        ...
        IF(tmp_kc_result(1))THEN
          ...
          valid_petmodel_options = valid_petmodel_options .AND. .TRUE.
        ELSEIF(tmp_kc_result(2))THEN
          CALL uncheck_testcase(n_kc(3),t_evaporation)
          valid_petmodel_options = valid_petmodel_options .AND. .TRUE.
        ELSE
          valid_petmodel_options = .FALSE.
        ENDIF
        ...
        CALL check_forcing_exist(tmp_result(1),i_swobs,a_swobs,t_evaporation)
        CALL check_forcing_exist(tmp_result(2),i_tminobs,a_tminobs,t_evaporation)
        CALL check_forcing_exist(tmp_result(3),i_tmaxobs,a_tmaxobs,t_evaporation)
        IF (tmp_result(1)) THEN
          CALL uncheck_testcase(a_tminobs,t_evaporation)
          CALL uncheck_testcase(a_tmaxobs,t_evaporation)
          valid_petmodel_options = valid_petmodel_options .AND. .TRUE.
        ELSE IF (tmp_result(2) .AND. tmp_result(3)) THEN
          CALL uncheck_testcase(a_swobs,t_evaporation)
          valid_petmodel_options = valid_petmodel_options .AND. .TRUE.
        ELSE
          valid_petmodel_options = .FALSE.
        END IF
      CASE(4)
        ...
        IF(tmp_kc_result(1))THEN
          ...
          valid_petmodel_options = valid_petmodel_options .AND. .TRUE.
        ELSEIF(tmp_kc_result(2))THEN
          CALL uncheck_testcase(n_kc(4),t_evaporation)
          valid_petmodel_options = valid_petmodel_options .AND. .TRUE.
        ELSE
          valid_petmodel_options = .FALSE.
        ENDIF
        ...
        CALL check_forcing_exist(valid_petmodel_options,i_tobs,a_tobs,t_evaporation)
        CALL check_forcing_exist(valid_petmodel_options,i_tminobs,a_tminobs,t_evaporation)
        CALL check_forcing_exist(valid_petmodel_options,i_tmaxobs,a_tmaxobs,t_evaporation)
        !Depending on forcing data, different cases are ok
        !(rhobs AND swobs) OR krs != 0
        CALL check_forcing_exist(tmp_result(1),i_rhobs,a_rhobs,t_evaporation)
        CALL check_forcing_exist(tmp_result(2),i_swobs,a_swobs,t_evaporation)
        IF (tmp_result(1) .AND. tmp_result(2)) THEN
          valid_petmodel_options = valid_petmodel_options .AND. .TRUE.
        ELSE
          CALL uncheck_testcase(a_rhobs,t_evaporation)
          CALL uncheck_testcase(a_swobs,t_evaporation)
          CALL check_param(valid_petmodel_options,ne_zero,n_krs,t_evaporation)
        END IF
      CASE(5)
        ...
        IF(tmp_kc_result(1))THEN
          ...
          valid_petmodel_options = valid_petmodel_options .AND. .TRUE.
        ELSEIF(tmp_kc_result(2))THEN
          CALL uncheck_testcase(n_kc(5),t_evaporation)
          valid_petmodel_options = valid_petmodel_options .AND. .TRUE.
        ELSE
          valid_petmodel_options = .FALSE.
        ENDIF
        ...
        CALL check_forcing_exist(tmp_result(3),i_uobs,a_uobs,t_evaporation)
        ...
        ELSE
          CALL uncheck_testcase(a_uobs,t_evaporation)
  ...
  LOGICAL FUNCTION check_actual_soil_evapotranspiration()
    ...
    ELSE
      CALL uncheck_testcase(n_tredA,t_evaporation)
      tmp_logical = .TRUE.
    END IF
    ...
    IF(simulate%substance(i_in))THEN
      ...
      IF(ALLOCATED(deposition%indryload))THEN
        DO j = 1,SIZE(deposition%indryload(:,:),2)
          ...
          ret = ret.AND.ret2
        ENDDO
        CALL add_generic_result(ret,a_basinindep2,t_atmdep,testing_names(a_basinindep2))
      ENDIF
      IF(ALLOCATED(deposition%inloadwater))THEN
        DO j = 1,12 !month
          ...
          ret = ret.AND.ret2
        ENDDO
        CALL add_generic_result(ret,a_basinindep3,t_atmdep,testing_names(a_basinindep3))
      ENDIF
    ENDIF
  ...
  SUBROUTINE check_soil_water(status)
    ...
    USE MODVAR, ONLY : modeloption,modeloptionname,p_deepgroundwater,p_frozensoil,p_surfacerunoff,p_infiltration, &
    ...
    CALL print_testresult_logical(infiltrationmodelnames(modeloption(p_infiltration)),ret,prefix=TRIM(modeloptionname(p_infiltration))//': ')
    IF (.NOT. ret) THEN
      status = status + 1
    END IF
    ...
    CALL print_testresult_logical(surfacerunoffmodelnames(modeloption(p_surfacerunoff)),ret,prefix=TRIM(modeloptionname(p_surfacerunoff))//': ')
    IF (.NOT. ret) THEN
      status = status + 1
    END IF
    !> @test Check and test frozen soil model options: specifics elsewhere
    ret = valid_frozensoil_options(modeloption(p_frozensoil))
    CALL print_testresult_logical(frozensoilmodelnames(modeloption(p_frozensoil)),ret,prefix=TRIM(modeloptionname(p_frozensoil))//': ')
    IF (.NOT. ret) THEN
      status = status + 1
    END IF
  ...
  LOGICAL FUNCTION valid_infiltration_options(current_infiltrationmodel)
    ...
    CASE(1,3)
      ...
      DO i = 1, nsoil
        ...
        END IF
      END DO
      CALL add_generic_result(r,a_sum_macratesrrate2,t_soil_water,'macrate srrate sum pos')
  ...
  LOGICAL FUNCTION valid_surfacerunoff_options(current_surfacerunoffmodel)
    ...
    CASE(0)
      ...
      DO i = 1, nsoil
        ...
        END IF
      END DO
      CALL add_generic_result(r,a_sum_macratesrrate,t_soil_water,'macrate srrate sum')
    CASE(1)
  ...
  SUBROUTINE check_snow(status)
    ...
    USE MODVAR, ONLY : modeloption,modeloptionname,p_snowfall,p_snowmelt,p_snowevap,p_snowdensity, &
    ...
    CALL print_testresult_logical(snowfallmodelnames(modeloption(p_snowfall)),ret,prefix=TRIM(modeloptionname(p_snowfall))//': ')
    IF (.NOT. ret) THEN
      status = status + 1
    END IF
    ...
    CALL print_testresult_logical(snowfalldistmodelnames(modeloption(p_snowfalldist)),ret,prefix=TRIM(modeloptionname(p_snowfalldist))//': ')
    IF (.NOT. ret) THEN
      status = status + 1
    END IF
    ...
    CALL print_testresult_logical(snowmeltmodelnames(modeloption(p_snowmelt)),ret,prefix=TRIM(modeloptionname(p_snowmelt))//': ')
    IF (.NOT. ret) THEN
      status = status + 1
    END IF
    ...
    CALL print_testresult_logical(snowevapmodelnames(modeloption(p_snowevap)),ret,prefix=TRIM(modeloptionname(p_snowevap))//': ')
    IF (.NOT. ret) THEN
      status = status + 1
    END IF
    ...
    CALL print_testresult_logical(snowdensitymodelnames(modeloption(p_snowdensity)),ret,prefix=TRIM(modeloptionname(p_snowdensity))//': ')
    IF (.NOT. ret) THEN
      status = status + 1
    END IF
    ...
    CALL print_testresult_logical(snowheatmodelnames(modeloption(p_snowheat)),ret,prefix=TRIM(modeloptionname(p_snowheat))//': ')
    IF (.NOT. ret) THEN
      status = status + 1
    END IF
  ...
  LOGICAL FUNCTION valid_snowfall_options(current_snowfallmodel)
    ...
    SELECT CASE(current_snowfallmodel)
      ...
      CASE(1) !Fraction of precipitation as snowfall is given in input data
        !sfobs
        CALL check_forcing_exist(valid_snowfall_options,i_sfobs,a_sfobs,t_snow_routines)
      CASE DEFAULT
  ...
  LOGICAL FUNCTION valid_snowfalldistribution_options(current_snowfalldistmodel)
    ...
    SELECT CASE(current_snowfalldistmodel)
      ...
      CASE(1) !Linear WSF function
        ...
        CALL add_generic_result(ret,a_winddir,t_snow_routines,'no wind directions')
        ...
        CALL check_forcing_exist(valid_snowfalldistribution_options,i_uwobs,a_uwobs,t_snow_routines)
        CALL check_forcing_exist(valid_snowfalldistribution_options,i_vwobs,a_vwobs,t_snow_routines)
  ...
  LOGICAL FUNCTION valid_snowmelt_options(current_snowmeltmodel)
    ...
    SELECT CASE(current_snowmeltmodel)
      ...
      CASE(2) 
        ...
        tmp_logical = .TRUE.
        CALL check_forcing_exist(tmp_logical,i_swobs,a_swobs,t_snow_routines)
        IF (.NOT. tmp_logical) THEN
          tmp_logical = .TRUE.
          CALL uncheck_testcase(a_swobs,t_snow_routines)
          CALL check_forcing_exist(tmp_logical,i_tminobs,a_tminobs,t_snow_routines)
          CALL check_forcing_exist(tmp_logical,i_tmaxobs,a_tmaxobs,t_snow_routines)
          IF (.NOT. tmp_logical) THEN
            tmp_logical = .TRUE.
            CALL uncheck_testcase(a_tminobs,t_snow_routines)
            CALL uncheck_testcase(a_tmaxobs,t_snow_routines)
  ...
  SUBROUTINE check_glaciers(status)
    ...
    USE MODVAR, ONLY : conduct,modeloption,modeloptionname,p_glacierini,glacierinimodelnames, &
    ...
    CALL print_testresult_logical(glacierinimodelnames(modeloption(p_glacierini)),ret,prefix=TRIM(modeloptionname(p_glacierini))//': ')
    IF (.NOT. ret) THEN
      status = status + 1
    END IF
  ...
  SUBROUTINE check_rivers(status)
    ...
    USE MODVAR, ONLY : conductregest,genpar,rrcdata, &
                       modeloption,modeloptionname,nsub, &
                       p_lakeriverice,p_rivermodel, &
                       lakerivericemodelnames,rivermodelnames
    USE HYPEVARIABLES, ONLY : m_damp,m_rivvel,n_damp,n_rivv,n_hygeomc,n_hygeomf,n_hygeomm,n_hygeomg
    ...
    LOGICAL ret
    INTEGER i
    REAL localrivvel(nsub),localdamp(nsub)

    !> @test Check river flow model options: specifics elsewhere
    ret = .TRUE.
    IF(.NOT.conductregest)THEN
      CALL check_param(ret,ge_zero_and_le_one,n_damp,t_rivers)
      CALL check_param(ret,gt_zero,n_rivv,t_rivers)
    ELSE
      localrivvel = genpar(m_rivvel)
      localdamp = genpar(m_damp)
      DO i = 1,nsub
        CALL set_regest_parameter(i,n_rivv,localrivvel(i))
        CALL set_regest_parameter(i,n_damp,localdamp(i))
      ENDDO
      CALL check_input(ret,localrivvel,gt_zero,a_rivvel,t_rivers)
      CALL check_input(ret,localdamp,ge_zero_and_le_one,a_damp,t_rivers)
    ENDIF
    CALL print_testresult_logical(rivermodelnames(modeloption(p_rivermodel)),ret,prefix=TRIM(modeloptionname(p_rivermodel))//': ')
    IF (.NOT. ret) THEN
      status = status + 1
    ENDIF
    ...
    CALL print_testresult_logical(lakerivericemodelnames(modeloption(p_lakeriverice)),ret,prefix=TRIM(modeloptionname(p_lakeriverice))//': ')
    IF (.NOT. ret) THEN
      status = status + 1
    ENDIF

    !> @test Check parameters and input data for hydraulic geometry. 
    !>Parameters hygeomf, hygeomm, hygeomc, hygeomg positive.
    ret = .TRUE.
    ...
    !> @test Check river rating curve data. Coefficients of rating curve positive.
    ret = .TRUE.
    IF(ALLOCATED(rrcdata))THEN
      CALL check_valid_river_rating_curve_data(ret)
  ...
  SUBROUTINE check_valid_river_rating_curve_data(ret)
    ...
    USE MODVAR, ONLY : rrcdata,nsub  

    !Argument declaration
    LOGICAL, INTENT(INOUT) :: ret
  
    !Local variables
    LOGICAL ret1,ret2,ret3,ret4,ret5
    INTEGER i,isec,nsec
  
    IF(.NOT.ALLOCATED(rrcdata))THEN
      CALL add_generic_result(.TRUE.,a_rrc4,t_rivers,'no rrcdata')
      RETURN
    ENDIF
    ...
    ret5 = .TRUE.
    DO i = 1,nsub
      IF(rrcdata(i)%typeid==0)CYCLE
      IF(rrcdata(i)%typeid==1)THEN
      !> @test Check positive rating curve coefficients (rate,exp)
        ret1 = ret1 .AND. (rrcdata(i)%single(1)%a>0.) .AND. (rrcdata(i)%single(1)%b>0.)
      ENDIF
      IF(rrcdata(i)%typeid==2)THEN
      !> @test Check positive rating curve coefficients (rate,exp)
        ret2 = ret2 .AND. (rrcdata(i)%ice(1)%qpar(1)%k>0.) .AND. (rrcdata(i)%ice(1)%qpar(1)%p>0.)
        ret2 = ret2 .AND. (rrcdata(i)%ice(1)%qpar(2)%k>0.) .AND. (rrcdata(i)%ice(1)%qpar(2)%p>0.)
      ENDIF
      !IF(rrcdata(i)%typeid==3)THEN
      !!> @test Check positive rating curve coefficients (rate,exp)
      !  ret3 = ret3 .AND. (rrcdata(i)%ice(1)%qpar(1)%k>0.) .AND. (rrcdata(i)%ice(1)%qpar(1)%p>0.)
      !ENDIF
      IF(rrcdata(i)%typeid==3)THEN
      !> @test Check positive rating curve coefficients (rate,exp) and consistent sections
        nsec = SIZE(rrcdata(i)%sector)
        IF(nsec>1)THEN
          DO isec=1,nsec
            ret4 = ret4 .AND. (rrcdata(i)%sector(isec)%wpar(1)%a>0.) .AND. (rrcdata(i)%sector(isec)%wpar(1)%b>0.)
            IF(isec<nsec) ret4 = ret4 .AND. ((rrcdata(i)%sector(isec+1)%qmax-rrcdata(i)%sector(isec)%qmax)>0.)
          ENDDO
        ELSE
          ret4 = .FALSE.
        ENDIF
      ENDIF
      IF(rrcdata(i)%typeid>5) ret5 = .FALSE.
    ENDDO
    CALL add_generic_result(ret1,a_rrc1,t_rivers,'RRC type 1')
    CALL add_generic_result(ret2,a_rrc2,t_rivers,'RRC type 2 parameters')
    !CALL add_generic_result(ret3,0,a_rrc3,t_rivers,'RRC type 3 parameters')
    CALL add_generic_result(ret4,a_rrc4,t_rivers,'RRC type 3 parameters')
    CALL add_generic_result(ret5,a_rrc5,t_rivers,'RRC type unknown')
      
    ret = ret.AND.ret1.AND.ret2.AND.ret3.AND.ret4

  END SUBROUTINE check_valid_river_rating_curve_data
  ...
  LOGICAL FUNCTION valid_lakeriverice_options(current_lakerivericemodel)
    ...
    CASE(2) 
      ...
      ELSEIF (.NOT. tmp_result(1)) THEN
        CALL uncheck_testcase(n_tcflake,t_rivers)
      ELSEIF (.NOT. tmp_result(2)) THEN
        CALL uncheck_testcase(n_scflake,t_rivers)
      ...
      IF ((.NOT. tmp_result(1)) .AND. (.NOT. tmp_result(2))) THEN
        valid_lakeriverice_options = .FALSE.
      ELSEIF (.NOT. tmp_result(1)) THEN
        CALL uncheck_testcase(n_tcfriver,t_rivers)
      ELSEIF (.NOT. tmp_result(2)) THEN
        CALL uncheck_testcase(n_scfriver,t_rivers)
      ...
      !CALL check_param(valid_lakeriverice_options,ge_zero,n_stbcorr2,t_rivers) !was neg. in par.txt, ok?
    ...
    IF (current_lakerivericemodel > 0) THEN
      ...
      CALL add_generic_result(ret,a_lakeriverice_class,t_rivers,'lakeriverice class exists')
    ...
    IF(ret)THEN
      ...
      CALL add_generic_result(ret,a_lakeriverice_thpo,t_rivers,'ricethpo in range')
      IF (.NOT. ret) valid_lakeriverice_options = .FALSE.
    ELSE
      ret = .TRUE.
  ...
  SUBROUTINE check_lakes(status)
    ...
    USE HYPEVARIABLES, ONLY : n_gratk,n_gratp
    USE MODVAR, ONLY : branchindex, &
...
!    USE WORLDVAR, ONLY : basemodel
    ...
    CALL print_testresult_logical(swtemperaturemodelnames(modeloption(p_swtemperature)),ret,prefix=TRIM(modeloptionname(p_swtemperature))//': ')
    IF(.NOT. ret) THEN
      status = status + 1
    END IF
    ...
    CALL print_testresult_logical(connectivitymodelnames(modeloption(p_connectivity)),ret,prefix=TRIM(modeloptionname(p_connectivity))//': ')
    IF(.NOT. ret) THEN
      status = status + 1
    END IF
...
!      DO i = 1, basemodel%nsub
      DO i = 1, nsub
        ...
        ENDIF
      ENDDO
      CALL add_generic_result(ret,a_branchdefined,t_lakes,testing_names(a_branchdefined))
    ...
    !> @test Check that lakes and dams have rating curve set (if needed)
    CALL check_rating_curves(status)

    !> @test Check that lakes and dams have larger depth than regulation depth
    CALL check_regvol(status)
  ...
  LOGICAL FUNCTION valid_swtemperature_options(current_swtemperaturemodel)
    ...
    SELECT CASE(current_swtemperaturemodel)
      ...
      CASE(1)
        !i_t2>0            TRUE                 --> OK
        CALL add_generic_result(conduct%simT2,a_t2sim,t_lakes,testing_names(a_t2sim))
  ...
  LOGICAL FUNCTION valid_connectivity_options(current_connectivitymodel)
    ...
    USE HYPEVARIABLES, ONLY : n_laketemp
    USE MODVAR, ONLY : basin,lakesectiondata,nsub
!    USE WORLDVAR, ONLY : basemodel
...
!        DO i=1,basemodel%nsub
        DO i=1,nsub
          ...
          ENDIF
        ENDDO
        CALL add_generic_result(ret3,a_sum_lksficatch,t_lakes,testing_names(a_sum_lksficatch))
        CALL add_generic_result(ret4,a_sum_lksfarea,t_lakes,testing_names(a_sum_lksfarea))
  ...
  SUBROUTINE check_rating_curves(status)
    ...
    !Local variables
    LOGICAL ret,ret1,ret5,ret7
    INTEGER i

    !>Check that lakes and dams have rating curve set (if needed, i.e. if wmin=missing)
    ret = .TRUE.
    CALL check_param(ret,gt_zero,n_gratk,t_lakes)
    CALL check_param(ret,gt_zero,n_gratp,t_lakes)
!    CALL print_testresult_logical('general rating curves for lakes and dams',ret,suffix=' set')
    IF(.NOT.ret)THEN
      !Need to check all subbasin if they have a lake that is dependent on general rating curve
      !Check for local lakes
      ret1 = .TRUE.
      IF(slc_ilake>0)THEN
        DO i = 1,nsub
          IF(classbasin(i,slc_ilake)%part>0.) ret1 = .FALSE.
          IF(.NOT.ret1) EXIT
        ENDDO
      ENDIF
      IF(ret1) CALL print_testresult_logical('no local lakes that need rating curve',ret1,suffix='')
      IF(.NOT.ret1) CALL print_testresult_logical('general rating curves for local lakes',ret1,suffix=' set')
      
      !Check for outlet lakes in LakeData, DamData or using general parameters
      ret5 = .TRUE.
      ret7 = .FALSE.
      IF(slc_olake>0)THEN
        DO i = 1,nsub
          IF(classbasin(i,slc_olake)%part>0.)THEN
            ...
            IF(ALLOCATED(lakeindex))THEN
              IF(lakeindex(i)>0)THEN
                IF(lake(lakeindex(i))%rate>0. .AND. lake(lakeindex(i))%exp>0.)THEN
                  CYCLE
                ELSEIF(lake(lakeindex(i))%wmin/=missing_value)THEN
                  CYCLE
                ENDIF
              ENDIF
            ENDIF
            IF(ALLOCATED(damindex))THEN
              IF(damindex(i)>0)THEN
                IF(dam(damindex(i))%rate>0. .AND. dam(damindex(i))%exp>0.)THEN
                  CYCLE
                ELSEIF(dam(damindex(i))%wmin/=missing_value)THEN
                  CYCLE
                ENDIF
              ENDIF
            ENDIF
            !IF(ALLOCATED(lakebasinindex))THEN    !No need test elake here, because already checked in reading of LakeData
            !  IF(lakebasinindex(i)>0)THEN
            !    IF(elake(lakebasinindex(i))%wmin/=missing_value)THEN
            !      CYCLE
            !    ELSE
            !      ret6=.TRUE.
            !      DO j=1,elake(lakebasinindex(i))%noutlet
            !        IF(elake(lakebasinindex(i))%outlet(j)%rate>0 .AND. elake(lakebasinindex(i))%outlet(j)%rate>0)THEN
            !          CYCLE
            !        ELSE
            !          ret6 = .FALSE.
            !        ENDIF
        ...
        ENDDO
      ENDIF
      IF(ret7) CALL print_testresult_logical('rating curves for outlet lakes',ret5,suffix=' set')
      IF(.NOT.ret7) CALL print_testresult_logical('no outlet lakes in model that need rating curves',ret5,suffix='')
    ENDIF

    IF(ret)THEN
      CALL print_testresult_logical('rating curves for lakes and dams',ret,suffix=' set')
    ELSE
      ret = ret1 .AND. ret5
      IF(.NOT.ret) status = status + 1
    ENDIF

  END SUBROUTINE check_rating_curves
  ...
  SUBROUTINE check_regvol(status)
    ...
    LOGICAL ret,ret2
    INTEGER i
    REAL wmin,outw0rel,out2wmin

    !Collect regulation lower depth and check against initial lake depth
    ret = .TRUE.
    ret2 = .TRUE.
    IF(slc_olake>0)THEN
      DO i = 1,nsub
        IF(classbasin(i,slc_olake)%part>0.)THEN
          !Get lake parameter values: wmin, deltaw0
          wmin = missing_value
          outw0rel = 0.
          out2wmin = missing_value
          IF(ALLOCATED(lakeindex))THEN
            IF(lakeindex(i)>0)THEN
              wmin = lake(lakeindex(i))%wmin
              outw0rel = lake(lakeindex(i))%deltaw0
            ENDIF
          ENDIF
          IF(ALLOCATED(lakeout2index))THEN  !Second outlet of lake/dam
            IF(lakeout2index(i)>0)THEN
              out2wmin = lake(lakeout2index(i))%wmin            !wmin of outlet 2
              outw0rel = lake(lakeout2index(i))%w0ref            !w0 of outlet 2 relative to outlet 1 (w0ref)
            ENDIF
          ENDIF
          IF(ALLOCATED(lakebasinindex))THEN
            IF(lakebasinindex(i)>0)THEN
              wmin = elake(lakebasin(lakebasinindex(i))%ilk)%wmin
              outw0rel = elake(lakebasin(lakebasinindex(i))%ilk)%outlet(1)%deltaw0
            ENDIF
          ENDIF
          IF(ALLOCATED(damindex))THEN
            IF(damindex(i)>0)THEN
              wmin = dam(damindex(i))%wmin
            ENDIF
          ENDIF

          !Check regulation volume and varying threshold against lake depth
          IF(wmin>missing_value)THEN
            IF(basin(i)%lakedepth(2)<=-wmin) ret = .FALSE.
          ENDIF
          IF(out2wmin>missing_value)THEN
            IF(basin(i)%lakedepth(2)<=-out2wmin) ret = .FALSE.
          ENDIF
          IF(basin(i)%lakedepth(2)<=-outw0rel) ret2 = .FALSE.
        ENDIF
      ENDDO
    ENDIF
    CALL add_generic_result(ret,a_regvol1,t_lakes,testing_names(a_regvol1))   !regvol<lakedepth
    CALL add_generic_result(ret2,a_regvol2,t_lakes,testing_names(a_regvol2))  !deltaw0/out2w0ref<lakedepth
    ret = ret.AND.ret2
    CALL print_testresult_logical('regvol less than lake volume',ret,suffix='')
    IF(.NOT.ret) status = status + 1

  END SUBROUTINE check_regvol
  ...
  SUBROUTINE check_floodplains(status)
    ...
    USE MODVAR, ONLY : modeloption,modeloptionname,p_floodplain, &
    ...
    CALL print_testresult_logical(floodplainmodelnames(modeloption(p_floodplain)),ret,prefix=TRIM(modeloptionname(p_floodplain))//': ')
    IF (.NOT. ret) THEN
      status = status + 1
    END IF
  ...
  SUBROUTINE check_river_wetlands(status)
    ...
    USE MODVAR, ONLY : modeloption,modeloptionname,p_wetland, &
                       wetlandmodelnames, &
                       conduct,wetland,classbasin, &
                       slc_iwet,slc_owet,nsub
!    USE WORLDVAR, ONLY : basemodel
    ...
    !Local variables
    INTEGER i,j1,j2,wetmask(nsub,2)
    LOGICAL ret
...
!        DO i = 1,basemodel%nsub
    ...
    ELSEIF(modeloption(p_wetland)==2)THEN
      ...
      IF (.NOT. ret) status = status + 1
    ENDIF
    CALL print_testresult_logical(wetlandmodelnames(modeloption(p_wetland)),ret,prefix=TRIM(modeloptionname(p_wetland))//': ')
  ...
  !    !IF(cropiindex==0)THEN   !This test should be moved here! And the one in irrigation can go STOP 1.
  ...
  !    !ENDIF
  !    
  !    !Check which subids has irrigation fields.
  !    !Then check if these have region_eff,demandtype.
  !    !Check irrdam&olake for reg srcs
  !
  !    !> @test Check irrigation parameters...
  !    
  !    IF (.NOT. ret) THEN
  !      status = status + 1
  !    ENDIF
  !  ENDIF
  !
  !  CALL print_testresult_logical('irrigation',ret,suffix='')
  !  CALL print_test_process(t_irrigation)
  !
  !
  !  RETURN
  !
  ...
  SUBROUTINE check_pointsourcedata(status)
    ...
    IF(readpstime)THEN
      IF(.NOT.fileexist)THEN
        CALL add_generic_result(.FALSE.,a_consistent_pstimeinfo,t_pointsource,'PointSourceData file for PS timeserie')
        ret = .FALSE.
      ELSE
        CALL add_generic_result(.TRUE.,a_consistent_pstimeinfo,t_pointsource,'PointSourceData file for PS timeseries')
        ...
        ENDDO
        IF(nmiss .NE. 0) THEN
          CALL add_generic_result(.FALSE.,a_missing_pstime_forcing,t_pointsource,'all PS timeseries present')
          ret = .FALSE.
        ELSE
          CALL add_generic_result(.TRUE.,a_missing_pstime_forcing,t_pointsource,'all PS timeseries present')
        ENDIF
      ENDIF
      IF (.NOT. ret) THEN
        status = status + 1
  ...
  SUBROUTINE check_general_deep(status)
    ...
    USE MODVAR, ONLY : deepgroundwatermodelnames,modeloption,modeloptionname,p_deepgroundwater
    ...
    !> @test Check and test groundwater model options: specifics elsewhere
    ret = valid_deepgroundwater_options(modeloption(p_deepgroundwater))
    CALL print_testresult_logical(deepgroundwatermodelnames(modeloption(p_deepgroundwater)),ret,prefix=TRIM(modeloptionname(p_deepgroundwater))//': ')
    IF (.NOT. ret) THEN
      status = status + 1
    END IF
  ...
  SUBROUTINE check_aquifers(status)
    ...
    IF(ALLOCATED(aquifer)) THEN
      ...
      ENDDO
      CALL add_generic_result(ret,a_aqpassivedepth,t_aquifers,'passive depth consistent')
      IF(conduct%simN) CALL add_generic_result(ret2,a_aqtopdepth,t_aquifers,'top depth consistent')
...
!-------------------------------------
! Subroutines for checking substances
!-------------------------------------
  ...
  SUBROUTINE check_nutrient_sources(status)
    ...
    USE MODVAR, ONLY : conduct,conductbasinlocsoil,cropdata, &
                       genpar,load,modeloption,p_soilleakage, &
                       i_in,i_on,i_sp,i_pp,numsubstances,nsub
    USE HYPEVARIABLES, ONLY : n_fertdays, m_locsoil
    ...
    !> @test Check local rural household source: flow not negative, landarea present, nutrient concentration not negative (N and P)
    ret2 = .TRUE.
    IF(ALLOCATED(load))THEN
      CALL check_input(ret2,load(:)%volloc,ge_zero,a_volloc,t_nut_source)
      ret3 = .TRUE.
      CALL calculate_landarea(nsub,locallandarea)
      DO i = 1,nsub
        IF(locallandarea(i)<=0.)THEN
          IF(load(i)%volloc>0.)THEN
            IF(conductbasinlocsoil .AND. load(i)%locsoil>0.)THEN
              ret3 = .FALSE.  !rural added to non existent land
              EXIT
            ELSEIF(.NOT.conductbasinlocsoil .AND. genpar(m_locsoil)>0.)THEN
              ret3 = .FALSE.  !rural added to non existent land
              EXIT
            ENDIF
          ENDIF
        ENDIF
      ENDDO
      CALL add_generic_result(ret3,a_rurlocarea,t_nut_source,'rural landarea')
      ret2 = ret2.AND.ret3
      IF (conduct%simN .OR. conduct%simP) THEN
        minvalue = 0.
        ret3 = .TRUE.
        ...
        ENDIF  
        CALL add_generic_result(ret3,a_locconcnp,t_nut_source,'locconc NP')
        ret2 = ret2 .AND. ret3
      ENDIF
  ...
  SUBROUTINE check_veg_soil_surface(status)
    ...
    USE MODVAR, ONLY : modeloption,modeloptionname,p_growthstart,growthstartmodelnames, &
    ...
    IF(modeloption(p_soilleakage)==0)THEN
      IF (conduct%simN .OR. conduct%simP) THEN
          ...
          CALL print_testresult_logical(growthstartmodelnames(modeloption(p_growthstart)),ret,prefix=TRIM(modeloptionname(p_growthstart))//': ')
          IF (.NOT. ret) THEN
              status = status + 1
          END IF
      ...
      IF (conduct%simP .OR. conduct%simS) THEN
          ...
          IF(ret)THEN
            ...
            CALL print_testresult_logical(erosionmodelnames(modeloption(p_erosion)),ret,prefix=TRIM(modeloptionname(p_erosion))//': ')
            IF (.NOT. ret) THEN
                status = status + 1
            END IF
  ...
  LOGICAL FUNCTION valid_growthstart_options(current_growthstartmodel)
    ...
    CASE(1)
      ...
      IF(timesteps_per_day/=1)THEN
        r = .FALSE.
        CALL add_generic_result(r,a_dailytimestep,t_veg_soil_surface,'daily time step')
    ...
    r = (MINVAL(basin(:)%latitude) /= 0.0) .OR. (MAXVAL(basin(:)%latitude) /= 0.0)
    CALL add_generic_result(r,a_latitude_defined,t_veg_soil_surface,'latitude defined')
    valid_growthstart_options = valid_growthstart_options .AND. r
  ...
  LOGICAL FUNCTION valid_erosion_options(current_erosionmodel)
    ...
    USE HYPEVARIABLES, ONLY : n_soilcoh,n_soilerod,n_sreroexp, &
                              ...
                              n_incorr,n_oncorr,n_phoscorr,n_erodmon
    ...
    CASE(1)
      ...
      CALL check_param(valid_erosion_options,ge_minus_one,n_erodmon,t_veg_soil_surface)
  ...
  !> Check if erosion is simulated, parameter values can be used to turn 
  !>it off. If on more tests are needed
  !--------------------------------------------------------------------
  LOGICAL FUNCTION check_erosion_on(current_erosionmodel)
    ...
    SELECT CASE(current_erosionmodel)
      CASE(0)
        ...
        ENDIF
        CALL add_generic_result(ret,a_erosionon,t_veg_soil_surface,'erosion_on')
  ...
  LOGICAL FUNCTION check_plant_uptake()
    ...
    DO j = 1, nclass
      ...
      ENDIF
    ENDDO
    CALL add_generic_result(ret,a_scr_exist,t_veg_soil_surface,'a_scr_exist')
...
!              ret = .TRUE.  !ok
      ...
      ENDIF
    ENDDO
    CALL add_generic_result(ret,a_wcwp_gt_zero,t_veg_soil_surface,'wcwp greater than zero')
    check_plant_uptake = check_plant_uptake.AND.ret
    ...
    IF(modeloption(p_soilleakage)>0)THEN
      IF (numsubstances>0) THEN
        ...
        DO i = 1, numsubstances
          ...
          ENDDO
        ENDDO
        CALL add_generic_result(ret,a_soilleakage,t_soilleakage,'leakage conc')
  ...
  SUBROUTINE check_sediment_sedresusp(status)
    ...
    USE MODVAR, ONLY : conduct, &
                       modeloption,modeloptionname,p_sedresusp,sedresuspmodelnames
    ...
    IF(conduct%simS)THEN
      ...
      !> @test Check sedresusp model valid
      ret = valid_sedresusp_model(modeloption(p_sedresusp))
      CALL print_testresult_logical(sedresuspmodelnames(modeloption(p_sedresusp)),ret,prefix=TRIM(modeloptionname(p_sedresusp))//': ')
 
      CALL print_test_process(t_sedresusp)
  ...
  END SUBROUTINE check_sediment_sedresusp

  !>Check and test sediment/resuspension models for valid options
  !--------------------------------------------------------------------
  LOGICAL FUNCTION valid_sedresusp_model(sedrespmodel)

    USE HYPEVARIABLES, ONLY : n_sedexp,n_qbank,n_hygeomg,n_hygeomm,n_vpeak, &
                              n_suspconSS,n_suspexpSS,n_sedch,n_addsusp

  	!Argument declaration
    INTEGER, INTENT(IN)  :: sedrespmodel  !<sediment/resuspension model to be tested for valid options

    valid_sedresusp_model = .TRUE.
    SELECT CASE(sedrespmodel)
      CASE(0) !original HYPE method
        CALL check_param(valid_sedresusp_model,ge_zero,n_sedexp,t_sedresusp)
      CASE(1) !flow dependent with parameter
        CALL check_param(valid_sedresusp_model,gt_zero,n_qbank,t_sedresusp)
        CALL check_param(valid_sedresusp_model,ge_zero,n_sedexp,t_sedresusp)
      CASE(2) !simplified Bagnold Equation
        CALL check_param(valid_sedresusp_model,gt_zero,n_hygeomg,t_sedresusp)
        CALL check_param(valid_sedresusp_model,ge_zero,n_hygeomm,t_sedresusp)
        CALL check_param(valid_sedresusp_model,gt_zero,n_vpeak,t_sedresusp)
        CALL check_param(valid_sedresusp_model,gt_zero,n_suspconSS,t_sedresusp)
        CALL check_param(valid_sedresusp_model,ge_zero,n_suspexpSS,t_sedresusp)
        CALL check_param(valid_sedresusp_model,ge_zero_and_le_one,n_sedch,t_sedresusp)
        CALL check_param(valid_sedresusp_model,ge_zero_and_le_one,n_addsusp,t_sedresusp)
      CASE DEFAULT
        valid_sedresusp_model = .FALSE.
    END SELECT
    
  END FUNCTION valid_sedresusp_model
  
  !>Check and test sediment siltation and flushing
  !--------------------------------------------------------------------
  SUBROUTINE check_siltation(status)
    
    USE MODVAR, ONLY : conduct,elake,lake,lakebasin,lakebasinindex,lakeindex,nsub, &
                       modeloption,modeloptionname,p_siltation,siltationmodelnames
    USE WORLDVAR, ONLY : basemodel
    ...
    IF(conduct%simS)THEN
      ...
      !> @test Check siltation model valid
      ret = valid_siltation_model(modeloption(p_siltation))
      CALL print_testresult_logical(siltationmodelnames(modeloption(p_siltation)),ret,prefix=TRIM(modeloptionname(p_siltation))//': ')
      
      !> @test Check that lakes in LakeData has increasing flushing interval (simple lakes)
      !> @test Check that lakes in LakeData has correct mode (simple lakes)
      IF(ALLOCATED(lakeindex))THEN  !Ordinary lake
        s = 0
        CALL add_generic_result(.TRUE.,a_flushing2,t_siltation,'flushing values simple')
        DO i = 1, basemodel%nsub
          IF(lakeindex(i)>0)THEN
            IF(lake(lakeindex(i))%sedmgmt%mode==1) THEN !interval based on capacity 
              IF(lake(lakeindex(i))%sedmgmt%start>=lake(lakeindex(i))%sedmgmt%sstop) &
              s = s + 1
            ENDIF
            IF(lake(lakeindex(i))%sedmgmt%mode==1)THEN !interval based on capacity 
              IF(lake(lakeindex(i))%sedmgmt%start>=0.AND.lake(lakeindex(i))%sedmgmt%start<=1.AND.lake(lakeindex(i))%sedmgmt%sstop>=0.AND.lake(lakeindex(i))%sedmgmt%sstop<=1)THEN
              ELSE
                CALL add_generic_result(.FALSE.,a_flushing2,t_siltation,'flushing values simple')
              ENDIF
            ELSEIF(lake(lakeindex(i))%sedmgmt%mode==2) THEN !interval based on dayno
              IF(lake(lakeindex(i))%sedmgmt%start>=1.AND.lake(lakeindex(i))%sedmgmt%start<=365.AND.lake(lakeindex(i))%sedmgmt%sstop>=1.AND.lake(lakeindex(i))%sedmgmt%sstop<=365)THEN
              ELSE
                CALL add_generic_result(.FALSE.,a_flushing2,t_siltation,'flushing values simple')
              ENDIF
            ENDIF
        ...
        ENDDO
        ret = (s == 0)
        CALL add_generic_result(ret,a_flushing,t_siltation,'flushing interval simple')
        IF(.NOT. ret) status = status + 1
      ENDIF
    
      !> @test Check multi-basin lake flushing interval is increasing (except for dayno which may be around new year).
      !> @test Check that lakes in LakeData has correct mode (multi-basin lakes)
      IF(ALLOCATED(lakebasinindex))THEN
        s = 0
        CALL add_generic_result(.TRUE.,a_flushing3,t_siltation,'flushing values multi')
        DO i = 1,nsub
          IF(lakebasinindex(i)>0)THEN
            IF(lakebasin(lakebasinindex(i))%last)THEN
              IF(elake(lakebasin(lakebasinindex(i))%ilk)%sedmgmt%mode==1) THEN !interval based on capacity
                IF(elake(lakebasin(lakebasinindex(i))%ilk)%sedmgmt%start>=elake(lakebasin(lakebasinindex(i))%ilk)%sedmgmt%sstop) &
                s = s + 1
              ENDIF
              IF(elake(lakebasin(lakebasinindex(i))%ilk)%sedmgmt%mode==1)THEN !interval based on capacity 
                IF(elake(lakebasin(lakebasinindex(i))%ilk)%sedmgmt%start>=0.AND.elake(lakebasin(lakebasinindex(i))%ilk)%sedmgmt%start<=1.AND.elake(lakebasin(lakebasinindex(i))%ilk)%sedmgmt%sstop>=0.AND.elake(lakebasin(lakebasinindex(i))%ilk)%sedmgmt%sstop<=1)THEN
                ELSE
                  CALL add_generic_result(.FALSE.,a_flushing3,t_siltation,'flushing values multi')
                ENDIF
              ELSEIF(elake(lakebasin(lakebasinindex(i))%ilk)%sedmgmt%mode==2) THEN !interval based on dayno
                IF(elake(lakebasin(lakebasinindex(i))%ilk)%sedmgmt%start>=1.AND.elake(lakebasin(lakebasinindex(i))%ilk)%sedmgmt%start<=365.AND.elake(lakebasin(lakebasinindex(i))%ilk)%sedmgmt%sstop>=1.AND.elake(lakebasin(lakebasinindex(i))%ilk)%sedmgmt%sstop<=365)THEN
                ELSE
                  CALL add_generic_result(.FALSE.,a_flushing3,t_siltation,'flushing values multi')
                ENDIF
              ENDIF
        ...
        ENDDO
        ret = (s == 0)
        CALL add_generic_result(ret,a_flushing,t_siltation,'flushing interval multi')
        IF(.NOT. ret) status = status + 1
      ENDIF
  
      CALL print_test_process(t_siltation)
  ...
  END SUBROUTINE check_siltation
  ...
  LOGICAL FUNCTION valid_siltation_model(siltationmodel)
    ...
    USE MODVAR, ONLY : basin,nsub
    USE HYPEVARIABLES, ONLY : n_lseddens

  	!Argument declaration
    INTEGER, INTENT(IN)  :: siltationmodel  !<siltation model to be tested for valid options
    ...
    valid_siltation_model = .TRUE.
    ret = .TRUE.
    SELECT CASE(siltationmodel)
      CASE(0) !off (lake/reservoir does not affect hydrology)
      CASE(1) !no compaction + general density
        CALL check_param(valid_siltation_model,gt_zero,n_lseddens,t_siltation)
      CASE(2,3) !no compactation/compaction + density from soil fraction & reservoir operation
        !check sm_resop   1-4, but only if olake present
        !> @test Check that for all subbasins the fractions of clay, 
        !> sand and silt are between zero and one
        CALL check_input(ret,basin(:)%clay,ge_zero_and_le_one,a_clay,t_siltation)
        CALL check_input(ret,basin(:)%sand,ge_zero_and_le_one,a_sand,t_siltation)
        CALL check_input(ret,basin(:)%silt,ge_zero_and_le_one,a_silt,t_siltation)
        !> @test Check that for all subbasins the sum of fractions of clay, 
        !> sand and silt are one (approximately)
        ret2 = .TRUE.
        CALL add_generic_result(ret,a_claysandsilt,t_siltation,'clay+sand+silt')
        DO i = 1, nsub
          sum_val = basin(i)%clay + basin(i)%sand + basin(i)%silt
          IF (sum_val < 0.99 .OR. sum_val > 1.) THEN
            ret2 = .FALSE.
            CALL add_generic_result(ret2,a_claysandsilt,t_siltation,'clay+sand+silt')
          ENDIF
        ENDDO
        ret = ret.AND.ret2
        IF(.NOT.ret) valid_siltation_model = .FALSE.
      CASE DEFAULT
        valid_siltation_model = .FALSE.
    END SELECT
    
  END FUNCTION valid_siltation_model
  ...
  SUBROUTINE check_file_existence(io_result,filename,test_id,test_process)
    ...
    CALL add_generic_result(fileexist,test_id,test_process,TRIM(filename)//' exist')
    io_result = io_result .AND. fileexist

  END SUBROUTINE check_file_existence

!---------------------------------------------------
! Subroutines for handling test cases of four kinds
!---------------------------------------------------
  
  !>If the test is forced or not tested before, check the model parameter
  !>values against chosen tolerance and add the test case to the test process.
  !>Return the logical AND result.
  !--------------------------------------------------------------------
  SUBROUTINE check_param(io_result,tolerance,test_id,test_process,force)

    USE MODVAR, ONLY : modparid,m_gpar,m_lpar,m_mpar,m_rpar,m_spar,  &
                       genpar,landpar,monthpar,regpar,soilpar
    ...
    CASE(m_mpar)
      ptr => monthpar(modparid(test_id)%parno,:)
    CASE(m_gpar)
    ...
    !Check parameter values, add result to all test cases
    IF ((.NOT. all_test_cases(test_id)%tested) .OR. (test_forced)) THEN
      all_test_cases(test_id)%passed = real_check_tolerance(ptr,tolerance)
    ...
    !Add test case to test process
    CALL add_testcase(test_id, test_process)
  ...
  !>If the test is forced or not tested before, check the array input
  !>values against chosen tolerance and add the test case to the test process.
  !>Return the logical AND result.
  ...
  SUBROUTINE check_input(io_result,vector,tolerance,test_id,test_process,force)
    ...
    !Check input values, add result to all test cases
    IF ((.NOT. all_test_cases(test_id)%tested) .OR. (test_forced)) THEN
      all_test_cases(test_id)%passed = real_check_tolerance(vector,tolerance)
      all_test_cases(test_id)%tested = .TRUE.
  ...
  !>Check forcing data present (file exist) and add the test case to the test process.
  !>Return the logical AND result.
  !--------------------------------------------------------------------
  ...
  END SUBROUTINE check_forcing_exist
  ...
  SUBROUTINE add_generic_result(test_result,test_id,test_process,name)
    ...
    !Add result of generic test to all test cases
  ...
  !>The test cases to be printed as a logical unit are defined in the code.
  !>Test cases forming a group for a logical unit is also defined in the code.
  !--------------------------------------------------------------------
...
!----------------------------------------
!Subroutines for handling test processes
!----------------------------------------
  ...
  SUBROUTINE add_testcase(test_id, test_process)
    ...
    !Argument declaration
    INTEGER, INTENT(IN) :: test_id       !<current test case ID
    INTEGER, INTENT(IN) :: test_process  !<current test process
    
    !Local variables
    INTEGER i

    DO i = 1, max_testcases_per_process
    ...
    WRITE(6,*) 'ERROR: Size exceeded, could not add the test', test_id, ', test will not be counted for!'

  END SUBROUTINE add_testcase

  !>Clear test processes from test cases
  !>Remove all test cases from a given test process and reset these test cases
  !>OR remove all test cases from all test processes and all external test
  !--------------------------------------------------------------------
  SUBROUTINE clear_tests(test_process)

    !Argument declaration
    INTEGER, OPTIONAL, INTENT(IN) :: test_process  !<Clear test cases for this process only, if specified
    ...
    IF (PRESENT(test_process)) THEN
      DO j = 1, max_testcases_per_process
        IF(tests_c2_process(j,test_process)%test_id>0) all_test_cases(tests_c2_process(j,test_process)%test_id)%tested = .FALSE.
    ...
    DO i = 1, nof_external_tests
      ...
      DO j = 1, SIZE(external_tests(i)%ndata)
        ...
        IF (ALLOCATED(external_tests(i)%ndata(j)%vdata)) DEALLOCATE(external_tests(i)%ndata(j)%vdata)
      ENDDO
    ENDDO
  ...
  SUBROUTINE print_test_process(process)
    ...
    !Argument declaration
    INTEGER, INTENT(IN) :: process  !<Current process

    !Local variables
    INTEGER t,i,failures
    CHARACTER(LEN=32) tolerance_name

    !Print out basic information about the included test cases
    IF (printout_level > 0) THEN
      ...
      DO i = 1, max_testcases_per_process
        IF (tests_c2_process(i,process)%test_id .GT. 0) THEN
          ...
          ' passed:',all_test_cases(tests_c2_process(i,process)%test_id)%passed,TRIM(tolerance_name)
          IF (.NOT. all_test_cases(tests_c2_process(i,process)%test_id)%passed) THEN
            failures = failures + 1;
          ENDIF
        ENDIF
      ENDDO

      !Extended the printouts of test cases with further information
  ...
  SUBROUTINE uncheck_testcase(test_id,test_process)
    ...
    remove_index = -1
    substitute_index = 0
    TC_LOOP : DO i = 1, max_testcases_per_process
      IF (tests_c2_process(i,test_process)%test_id .EQ. test_id) THEN
        remove_index = i
      ELSEIF (tests_c2_process(i,test_process)%test_id .GT. 0) THEN
        substitute_index = i
      ELSE
        EXIT TC_LOOP
      ENDIF
  ...
  END SUBROUTINE uncheck_testcase

!--------------------------------------------------------
!Subroutines for printing extended test case information
!--------------------------------------------------------
  
  !>Print extended information for a test case. 
...
!---------------------------------
!Subroutines for tolerance checks
!---------------------------------
  ...
  LOGICAL FUNCTION real_check_tolerance(vector,tolerance)
    ...
    CASE(gt_minus_one_and_lt_plus_one)
      ...
      needed_retval = .TRUE.
      needed_status = SIZE(vector)
    CASE(ge_minus_one_and_le_plus_one)
      aimed_value = 0.0
      tolerance_min = -1.0
      tolerance_max = 1.0
      needed_retval = .TRUE.

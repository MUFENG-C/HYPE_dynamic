...
USE MODVAR, ONLY : def_min,def_max, &
                   max_substances, &
                   STATECONFIGURATIONTYPE,STATEDIMENSIONTYPE
!------------------------------------------------------------------------------
!Used by modules
!------------------------------------------------------------------------------
!"main"
...
!> \brief Type for state variables related to lake
  TYPE LAKESTATETYPE
    ...
    REAL,ALLOCATABLE :: sedpool(:,:,:)  !<sediment in lake (kg) (substance,laketype,subbasin)
    REAL,ALLOCATABLE :: sedage(:,:)     !<age of sediment in lake since flushing (timesteps) (laketype,subbasin)
    REAL,ALLOCATABLE :: seddens(:,:)    !<density of sediment in lake (timesteps) (laketype,subbasin)
    INTEGER,ALLOCATABLE :: sedflush(:,:) !<lake sediment flush on/off (laketype,subbasin)
...
!> \brief Type for model state information 
    TYPE STATEINFOTYPE
      ...
      REAL,ALLOCATABLE :: limits(:,:) !minimum and maximum values
!      REAL    :: limits(max_substances,2) !minimum and maximum values
      !INTEGER :: limittype         !maybe better??
  ...
  PUBLIC partly_allocate_model_states
  PUBLIC partly_deallocate_model_states
...
CONTAINS
  ...
  SUBROUTINE set_stateinfo(n,dim,stateinfo,frozenstate,soilstate,aquiferstate,riverstate,lakestate,miscstate)
    ...
    INTEGER nlimits !Size of limit array
    REAL,ALLOCATABLE :: no_limits(:,:)         !Limits for variables not limited 
    REAL,ALLOCATABLE :: positive_limits(:,:)   !Limits for variables limited to positive values and zero
    REAL,ALLOCATABLE :: substance_limits(:,:)  !Limits for substances, (different within state variable)
    REAL,ALLOCATABLE :: unity_limits(:,:)      !Limits for variables limited between zero and one
    ...
    INTEGER, PARAMETER :: nlakestates = 13
    ...
    nlimits = MAX(1,ns)
    ALLOCATE(no_limits(nlimits,2))
    no_limits(:,1) = def_min
    no_limits(:,2) = def_max
    ALLOCATE(positive_limits(nlimits,2))
    positive_limits(:,1) = 0.
    positive_limits(:,2) = def_max
    ALLOCATE(unity_limits(nlimits,2))
    unity_limits(:,1) = 0.
    unity_limits(:,2) = 1.
    ALLOCATE(substance_limits(nlimits,2))
    IF(ns>0)THEN
      substance_limits = dim%substancelimits
    ELSE
      substance_limits = no_limits  !not necessary?
    ENDIF
    ...
    DO i = 1,nstates
      IF(.NOT.ALLOCATED(stateinfo(i)%limits)) ALLOCATE(stateinfo(i)%limits(nlimits,2))
      stateinfo(i)%limits = positive_limits    !default
    ENDDO
    ...
    IF(ALLOCATED(frozenstate%csnow))THEN
      ...
      stateinfo(2)%limits = substance_limits
    ENDIF
    ...
    IF(ALLOCATED(frozenstate%snowcov))THEN
      ...
      stateinfo(5)%limits = unity_limits
    ENDIF
    ...
    IF(ALLOCATED(frozenstate%rivericecov))THEN
      ...
      stateinfo(13)%limits = unity_limits
    ENDIF
    IF(ALLOCATED(frozenstate%rivericepor))THEN
      ...
      stateinfo(14)%limits = unity_limits
    ENDIF
    ...
    IF(ALLOCATED(frozenstate%lakeicecov))THEN
      ...
      stateinfo(20)%limits = unity_limits
    ENDIF
    IF(ALLOCATED(frozenstate%lakeicepor))THEN
      ...
      stateinfo(21)%limits = unity_limits
    ENDIF
    ...
    IF(ALLOCATED(soilstate%temp))THEN
      ...
      stateinfo(iadd+2)%limits = no_limits
    ENDIF
    IF(ALLOCATED(soilstate%deeptemp))THEN
      ...
      stateinfo(iadd+3)%limits = no_limits
    ENDIF
    IF(ALLOCATED(soilstate%conc))THEN
      ...
      stateinfo(iadd+4)%limits = substance_limits
    ENDIF
    ...
    IF(ALLOCATED(soilstate%oldgrw))THEN
      ...
      stateinfo(iadd+14)%limits = no_limits
    ENDIF
    IF(ALLOCATED(soilstate%icelens))THEN
      ...
      stateinfo(iadd+15)%limits = unity_limits    !Actually 0 or 1
    ENDIF
    IF(ALLOCATED(soilstate%partT1))THEN
      ...
      !stateinfo(iadd+16)%limits = no_limits   !Could negative O18 be absorbed?
    ENDIF
    IF(ALLOCATED(soilstate%surface))THEN
      ...
      !stateinfo(iadd+17)%limits = substance_limits !This is substance load (soil leakage model 2), so probably only used for positive values
    ENDIF
    ...
    IF(ALLOCATED(aquiferstate%conc))THEN
      ...
      stateinfo(iadd+2)%limits = substance_limits
    ENDIF
    ...
    IF(ALLOCATED(aquiferstate%cnextoutflow))THEN
      ...
      stateinfo(iadd+6)%limits = substance_limits
    ENDIF
    ...
    IF(ALLOCATED(riverstate%conc))THEN
      ...
      stateinfo(iadd+3)%limits = substance_limits
    ENDIF
    ...
    IF(ALLOCATED(riverstate%temp10))THEN
      ...
      stateinfo(iadd+5)%limits = no_limits
    ENDIF
    IF(ALLOCATED(riverstate%temp20))THEN
      stateinfo(iadd+6)%allok = .TRUE.
      ALLOCATE(stateinfo(iadd+6)%svpoint%d2(stateinfo(iadd+6)%dims(1),stateinfo(iadd+6)%dims(2)))
      stateinfo(iadd+6)%limits = no_limits
    ENDIF
    ...
    IF(ALLOCATED(riverstate%cqueue))THEN    !0:ml+1-1
      ...
      stateinfo(iadd+9)%limits = substance_limits
    ENDIF
    IF(ALLOCATED(riverstate%cwetland))THEN
      stateinfo(iadd+10)%allok = .TRUE.
      ALLOCATE(stateinfo(iadd+10)%svpoint%d3(stateinfo(iadd+10)%dims(1),stateinfo(iadd+10)%dims(2),stateinfo(iadd+10)%dims(3)))
      stateinfo(iadd+10)%limits = substance_limits
    ENDIF
    ...
    IF(ALLOCATED(riverstate%T1sed))THEN
      stateinfo(iadd+14)%allok = .TRUE.
      ALLOCATE(stateinfo(iadd+14)%svpoint%d2(stateinfo(iadd+14)%dims(1),stateinfo(iadd+14)%dims(2)))
      !stateinfo(iadd+14)%limits = no_limits    !Could negative O18 be sedimented?
    ENDIF
    ...
    stateinfo(iadd+10)%svname = 'sedpool'
    stateinfo(iadd+11)%svname = 'sedage'
    stateinfo(iadd+12)%svname = 'seddens'
    stateinfo(iadd+13)%svname = 'sedflush'
    stateinfo(iadd+1:iadd+2)%ndim = 2
    stateinfo(iadd+3)%ndim = 3
    stateinfo(iadd+4:iadd+9)%ndim = 2
    stateinfo(iadd+10)%ndim = 3
    stateinfo(iadd+11:iadd+13)%ndim = 2
    ...
    stateinfo(iadd+10)%dims(1:3) = (/ns,nl,n/)
    stateinfo(iadd+11)%dims(1:2) = (/nl,n/)
    stateinfo(iadd+12)%dims(1:2) = (/nl,n/)
    stateinfo(iadd+13)%dims(1:2) = (/nl,n/)
    ...
    IF(ALLOCATED(lakestate%uppertemp))THEN
      stateinfo(iadd+7)%allok = .TRUE.
      ALLOCATE(stateinfo(iadd+7)%svpoint%d2(stateinfo(iadd+7)%dims(1),stateinfo(iadd+7)%dims(2)))
      stateinfo(iadd+7)%limits = no_limits
    ENDIF
    IF(ALLOCATED(lakestate%lowertemp))THEN
      ...
      stateinfo(iadd+8)%limits = no_limits
    ENDIF
    IF(ALLOCATED(lakestate%volfrac))THEN
      stateinfo(iadd+9)%allok = .TRUE.
      ALLOCATE(stateinfo(iadd+9)%svpoint%d2(stateinfo(iadd+9)%dims(1),stateinfo(iadd+9)%dims(2)))
      stateinfo(iadd+9)%limits = unity_limits
    ENDIF
    IF(ALLOCATED(lakestate%sedpool))THEN
      stateinfo(iadd+10)%allok = .TRUE.
      ALLOCATE(stateinfo(iadd+10)%svpoint%d3(stateinfo(iadd+10)%dims(1),stateinfo(iadd+10)%dims(2),stateinfo(iadd+10)%dims(3)))
      !stateinfo(iadd+10)%limits = substance_limits   !Load should be positive, but maybe not O18 or T2.
    ENDIF
    IF(ALLOCATED(lakestate%sedage))THEN
      stateinfo(iadd+11)%allok = .TRUE.
      ALLOCATE(stateinfo(iadd+11)%svpoint%d2(stateinfo(iadd+11)%dims(1),stateinfo(iadd+11)%dims(2)))
    ENDIF
    IF(ALLOCATED(lakestate%seddens))THEN
      stateinfo(iadd+12)%allok = .TRUE.
      ALLOCATE(stateinfo(iadd+12)%svpoint%d2(stateinfo(iadd+12)%dims(1),stateinfo(iadd+12)%dims(2)))
    ENDIF
    IF(ALLOCATED(lakestate%sedflush))THEN
      stateinfo(iadd+13)%allok = .TRUE.
      ALLOCATE(stateinfo(iadd+13)%svpoint%d2i(stateinfo(iadd+13)%dims(1),stateinfo(iadd+13)%dims(2)))
      stateinfo(iadd+13)%limits = unity_limits  !Actually 0 or 1
    ENDIF
    ...
    stateinfo(iadd+10)%svpoint%d3 => lakestate%sedpool
    stateinfo(iadd+11)%svpoint%d2 => lakestate%sedage
    stateinfo(iadd+12)%svpoint%d2 => lakestate%seddens
    stateinfo(iadd+13)%svpoint%d2i => lakestate%sedflush
    ...
    IF(ALLOCATED(miscstate%temp5))THEN
      stateinfo(iadd+1)%allok = .TRUE.
      ALLOCATE(stateinfo(iadd+1)%svpoint%d1(stateinfo(iadd+1)%dims(1)))
      stateinfo(iadd+1)%limits = no_limits
    ENDIF
    ...
    IF(ALLOCATED(miscstate%temp30))THEN
      ...
      stateinfo(iadd+4)%limits = no_limits
    ENDIF
    ...
    IF(ALLOCATED(miscstate%cfloodwater))THEN
      stateinfo(iadd+11)%allok = .TRUE.
      ALLOCATE(stateinfo(iadd+11)%svpoint%d3(stateinfo(iadd+11)%dims(1),stateinfo(iadd+11)%dims(2),stateinfo(iadd+11)%dims(3)))
      stateinfo(iadd+11)%limits = substance_limits
    ENDIF
    IF(ALLOCATED(miscstate%partT1sf))THEN
      stateinfo(iadd+12)%allok = .TRUE.
      ALLOCATE(stateinfo(iadd+12)%svpoint%d2(stateinfo(iadd+12)%dims(1),stateinfo(iadd+12)%dims(2)))
      !stateinfo(iadd+12)%limits = no_limits   !O18 could be negative?
    ENDIF
    ...
    IF(ALLOCATED(miscstate%cnexttransfer))THEN
      stateinfo(iadd+14)%allok = .TRUE.
      ALLOCATE(stateinfo(iadd+14)%svpoint%d2(stateinfo(iadd+14)%dims(1),stateinfo(iadd+14)%dims(2)))
      stateinfo(iadd+14)%limits = substance_limits
    ENDIF
    ...
    !Finish
    DEALLOCATE(no_limits)
    DEALLOCATE(positive_limits)
    DEALLOCATE(unity_limits)
    DEALLOCATE(substance_limits)
  ...
  SUBROUTINE allocate_model_states(n,dim,config, &
    ...
    CALL allocate_riverstate_variables(n,dim%substance,dim%river,dim%riverqueue,dim%timestep,config%simN,config%simP,config%simC,config%simS,config%simSi,config%simT1,config%qbank,config%riverwetland,riverstate)
    CALL allocate_lakestate_variables(n,dim,config,lakestate)
  ...
  SUBROUTINE partly_allocate_model_states(n,dim,config,frozenstate,soilstate,miscstate)
...
END SUBROUTINE partly_allocate_model_states
  ...
  SUBROUTINE partly_deallocate_model_states(frozenstate,soilstate,miscstate)
  ...
  END SUBROUTINE partly_deallocate_model_states
  ...
  SUBROUTINE initiate_state_zero(dim,config,   &
    ...
    CALL initiate_riverstate_zero(dim%substance,config%simP,config%simN.OR.config%simP.OR.config%simC.OR.config%simS.OR.config%simSi,config%simS,config%simT1,config%riverwetland,riverstate)
    CALL initiate_lakestate_zero(dim%substance,config,lakestate)
  ...
  SUBROUTINE allocate_riverstate_variables(n,ns,nr,ml,mt,isN,isP,isC,isS,isSi,ist1,isQb,iswetl,riverstate)
    ...
    LOGICAL,INTENT(IN) :: isC     !<Status of organic carbon simulation
    LOGICAL,INTENT(IN) :: isS     !<Status of suspended sediment simulation
    LOGICAL,INTENT(IN) :: isSi    !<Status of silica simulation   
    ...
    ENDIF
    IF(isN.OR.isP.OR.isC.OR.isS.OR.isSi)THEN
  ...
  SUBROUTINE initiate_riverstate_submodel(ns,n,config,indexarray,  &
    ...
    ENDIF
    IF(config%simN.OR.config%simP.OR.config%simC.OR.config%simS.OR.config%simSi)THEN
  ...
  SUBROUTINE initiate_riverstate_zero(ns,isP,isNPCSS,isS,ist1,iswetl,riverstate)
    ...
    INTEGER,INTENT(IN) :: ns      !<number of substances
    LOGICAL,INTENT(IN) :: isP     !<Status of phosphorus simulation
    LOGICAL,INTENT(IN) :: isNPCSS !<Status of N, P, C, S or Si simulation
    LOGICAL,INTENT(IN) :: isS     !<Status of suspended sediment simulation
    LOGICAL,INTENT(IN) :: ist1    !<Status of T1 simulation
    LOGICAL,INTENT(IN) :: iswetl  !<Status of wetland simulation
    TYPE(riverstatetype),INTENT(INOUT) :: riverstate  !<River states
...
!    ENDIF
  ...
  SUBROUTINE allocate_lakestate_variables(n,dim,config,lakestate)
    ...
    INTEGER :: ns      !number of substances
    INTEGER :: nl      !number of lake types
    INTEGER :: nlks    !number of lake type 1 sections
    ns = dim%substance
    nl = dim%lake
    nlks = dim%lakesection
...
!    IF(isconn .AND. nlks>0)THEN
    ...
    ENDIF
    IF(config%simS)THEN
      IF(.NOT.ALLOCATED(lakestate%sedpool))ALLOCATE(lakestate%sedpool(ns,nl,n))
      IF(.NOT.ALLOCATED(lakestate%sedage))ALLOCATE(lakestate%sedage(nl,n))
      IF(.NOT.ALLOCATED(lakestate%seddens))ALLOCATE(lakestate%seddens(nl,n))
      IF(.NOT.ALLOCATED(lakestate%sedflush))ALLOCATE(lakestate%sedflush(nl,n))
    ENDIF
  ...
  SUBROUTINE deallocate_lakestate_variables(lakestate)
    ...
    IF(ALLOCATED(lakestate%sedpool))   DEALLOCATE(lakestate%sedpool)
    IF(ALLOCATED(lakestate%sedage))    DEALLOCATE(lakestate%sedage)
    IF(ALLOCATED(lakestate%seddens))   DEALLOCATE(lakestate%seddens)
    IF(ALLOCATED(lakestate%sedflush))   DEALLOCATE(lakestate%sedflush)
  ...
  SUBROUTINE initiate_lakestate_submodel(ns,n,config,indexarray,lakestate,lakestate2)
    ...
    ENDIF
    IF(config%simS)THEN
      lakestate%sedpool(:,:,:) = lakestate2%sedpool(:,:,indexarray(:))
      lakestate%sedage(:,:) = lakestate2%sedage(:,indexarray(:))
      lakestate%seddens(:,:) = lakestate2%seddens(:,indexarray(:))
      lakestate%sedflush(:,:) = lakestate2%sedflush(:,indexarray(:))
    ENDIF
...
!------------------------------------
  ...
  SUBROUTINE set_riverstate_variables_from_array(n,nsfil,size,config, &
    ...
    IF(config%simN.OR.config%simP.OR.config%simC.OR.config%simS.OR.config%simSi)THEN
      ...
      DO i = 1,n
        ...
        ENDDO
      ENDDO
    ELSEIF(statefile%simN.OR.statefile%simP.OR.statefile%simC.OR.statefile%simS.OR.statefile%simSi)THEN
      iarr = iarr + n*nr*3
    ENDIF  
    IF(config%qbank)THEN
      DO i = 1,n
  ...
  SUBROUTINE get_lakestate_variables_arraysize(n,ns,size,config,dim)
    ...
    ENDIF  
    IF(config%simS)THEN
      dim = dim + ns*n*nl !sedpool
      dim = dim + n*nl    !sedage
      dim = dim + n*nl    !seddens
      dim = dim + n*nl    !sedflush
    ENDIF
...
!    IF(nlks>0 .AND. config%connectivity)THEN
    ...
    IF(config%simS)THEN
      DO i = 1,n
        DO j = 1,nl
          DO k = 1,ns
            iarr = iarr + 1
            IF(iarr>=iarrfirst.AND.iarr<=iarrlast) array(iarr-arrshift) = lakestate%sedpool(k,j,i)
          ENDDO
        ENDDO
      ENDDO
      DO i = 1,n
        DO j = 1,nl
          iarr = iarr + 1
          IF(iarr>=iarrfirst.AND.iarr<=iarrlast) array(iarr-arrshift) = lakestate%sedage(j,i)
        ENDDO
      ENDDO
      DO i = 1,n
        DO j = 1,nl
          iarr = iarr + 1
          IF(iarr>=iarrfirst.AND.iarr<=iarrlast) array(iarr-arrshift) = lakestate%seddens(j,i)
        ENDDO
      ENDDO
      DO i = 1,n
        DO j = 1,nl
          iarr = iarr + 1
          IF(iarr>=iarrfirst.AND.iarr<=iarrlast) array(iarr-arrshift) = REAL(lakestate%sedflush(j,i))
        ENDDO
      ENDDO
    ENDIF  
  ...
  SUBROUTINE set_lakestate_variables_from_array(n,nsfil,size,config,lakestate,iarrfirst,iarrlast,dim,array)
    ...
    ENDIF  
    IF(ns==0.AND.config%simN.OR.config%simP.OR.config%simC.OR.config%simS.OR.config%simSi)THEN
      iarr = iarr + n*nl*3
    ELSEIF(config%simN.OR.config%simP.OR.config%simC.OR.config%simS.OR.config%simSi)THEN
      DO i = 1,n
        DO j = 1,nl
          iarr = iarr + 1
...
!    IF(nlks>0 .AND. config%connectivity)THEN
    IF(config%connectivity)THEN
      ...
      ENDDO
    ENDIF
    IF(ns==0.AND.config%simS)THEN
      iarr = iarr + n*nl*3 + n*nl*ns
    ELSEIF(config%simS)THEN
      DO i = 1,n
        DO j = 1,nl
          DO k = 1,ns
            iarr = iarr + 1
            IF(iarr>=iarrfirst.AND.iarr<=iarrlast) lakestate%sedpool(k,j,i) = array(iarr-arrshift)
          ENDDO
        ENDDO
      ENDDO
      DO i = 1,n
        DO j = 1,nl
          iarr = iarr + 1
          IF(iarr>=iarrfirst.AND.iarr<=iarrlast) lakestate%sedage(j,i) = array(iarr-arrshift)
        ENDDO
      ENDDO
      DO i = 1,n
        DO j = 1,nl
          iarr = iarr + 1
          IF(iarr>=iarrfirst.AND.iarr<=iarrlast) lakestate%seddens(j,i) = array(iarr-arrshift)
        ENDDO
      ENDDO
      DO i = 1,n
        DO j = 1,nl
          iarr = iarr + 1
          IF(iarr>=iarrfirst.AND.iarr<=iarrlast) lakestate%sedflush(j,i) = INT(array(iarr-arrshift))
        ENDDO
      ENDDO
    ENDIF  

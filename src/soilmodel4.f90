...
MODULE FLOODPLAIN_SOILMODEL
  ...
  USE HYPEVARIABLES, ONLY : wpmm,fcmm,epmm,pwmm, &
       ...
       m_sswcorr,m_immdep,m_iwdfrac, &
       m_wdpar,m_ripz,m_rips,m_ripe, & 
       m_opt6,m_opt7,m_optonoff,  &
       soilmem,m_deepmem, &
       epotdist
...
!    IF(modeloption(p_soilleakage)==0)THEN
    IF(modeloption(p_soilleakage)/=1)THEN
      ...
      CALL add_dry_deposition_to_landclass(i,j,iluse,classdata(j)%vegtype, &
         classfparea*1.E-6,atmdepload,frozenstate,soilstate)

...
!>1-instant transport to outlet of same subbasin or soil of subbasin
...
!Copyright 2012-2017,2019-2022 SMHI
!
...
!    IF(cb_olake>0 .OR. cb_ilake>0 .OR. cb_lriver>0 .OR. cb_mriver>0)THEN 
    IF(cb_olake>0 .OR. cb_ilake>0 .OR. cb_lriver>0 .OR. cb_mriver>0 .OR. cb_iwet>0 .OR. cb_owet>0)THEN 
       lpart = 0.
  ...
  !--------------------------------------------------------------------------
  SUBROUTINE calculate_river_groundwaterflow_removal(i,j,subid,n,riverstate,flow,dampflow)
   ...
   USE MODVAR, ONLY : basin, &
    ...
    REAL, INTENT(OUT) :: dampflow     !<calculated regional groundwater flow from riverboxi (m3/timestep)
      ...
      IF(flow>0.)THEN
        ...
        IF(fractionw>0)THEN
          ...
          IF(outflow<riverstate%water(itype,i))THEN
            ...
            dampflow = outflow
          ELSE
            dampflow = riverstate%water(itype,i)
...
!    aqoutflow = outpar * aquiferstate%water(ia)
  ...
  SUBROUTINE add_regional_groundwater_flow_to_olake(i,itype,numsubst,   &
    ...
    REAL, INTENT(OUT)   :: grwload(numsubst) !<load added (kg/timestep, U/timestep)

...
!Copyright 2011-2013,2015,2017-2022 SMHI
!
...
100 WRITE(6,*) 'ERROR: Converting string to datetype: ',TRIM(datestr)
  ...
  !>\brief Print integer ids to string with blanks in between
  !----------------------------------------------------------------------
  SUBROUTINE write_integer_array_to_string(n,id,outtxt)

    !Argument declarations
    INTEGER, INTENT(IN) :: n       !<number of data
    INTEGER, INTENT(IN) :: id(n)   !<id data
    CHARACTER(LEN=*),INTENT(OUT) :: outtxt    !<output string
    ...
    DO j = 1,n
      ...
      lout = lout+lt
      IF(j < n) THEN
        outtxt(lout+1:lout+1) = ' '
        lout = lout+1
      ENDIF
    ENDDO

  END SUBROUTINE write_integer_array_to_string
